...
1. Download the runtime for your OS (extracts into `$CRABFORK_STATE_DIR/tools/sherpa-onnx-tts/runtime`, default `~/.crabfork/tools/sherpa-onnx-tts/runtime`)
2. Download a voice model (extracts into `$CRABFORK_STATE_DIR/tools/sherpa-onnx-tts/models`, default `~/.crabfork/tools/sherpa-onnx-tts/models`)
...
Then write those resolved paths into the active Crabfork config file (`$CRABFORK_CONFIG_PATH`, default `~/.crabfork/crabfork.json`):
