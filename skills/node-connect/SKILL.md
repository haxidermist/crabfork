---
name: node-connect
description: Diagnose Crabfork node connection and pairing failures for Android, iOS, and macOS companion apps. Use when QR/setup code/manual connect fails, local Wi-Fi works but VPS/tailnet does not, or errors mention pairing required, unauthorized, bootstrap token invalid or expired, gateway.bind, gateway.remote.url, Tailscale, or plugins.entries.device-pair.config.publicUrl.
---

# Node Connect

Goal: find the one real route from node -> gateway, verify Crabfork is advertising that route, then fix pairing/auth.
...
- whether `crabfork devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `crabfork qr --json`. It uses the same setup-code payload Android scans.

```bash
crabfork config get gateway.mode
crabfork config get gateway.bind
crabfork config get gateway.tailscale.mode
crabfork config get gateway.remote.url
crabfork config get gateway.auth.mode
crabfork config get gateway.auth.allowTailscale
crabfork config get plugins.entries.device-pair.config.publicUrl
crabfork qr --json
crabfork devices list
crabfork nodes status
```

If this Crabfork instance is pointed at a remote gateway, also run:

```bash
crabfork qr --remote --json
```
...
`crabfork qr --json` success means:
...
If `crabfork qr --json` says `Gateway is only bound to loopback`:
...
```bash
crabfork devices list
crabfork devices approve --latest
```
...
- `crabfork devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run crabfork qr again, rescan, then approve the pending device pairing.`
