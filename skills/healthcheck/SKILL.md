---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Crabfork deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Crabfork cron scheduling for periodic checks, or version status checks on a machine running Crabfork (laptop, workstation, Pi, VPS).
---

# Crabfork Host Hardening

## Overview

Assess and harden the host running Crabfork, then align it to a user-defined risk tolerance without breaking access. Use Crabfork security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Crabfork changes the host firewall, SSH, or OS updates; it does not.
...
5. Crabfork gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Crabfork can access sensitive data.
...
### 2) Run Crabfork security audits (read-only)

As part of the default read-only checks, run `crabfork security audit --deep`. Only offer alternatives if the user requests them:

1. `crabfork security audit` (faster, non-probing)
2. `crabfork security audit --json` (structured output)

Offer to apply Crabfork safe defaults (numbered):

1. `crabfork security audit --fix`

Be explicit that `--fix` only tightens Crabfork defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Crabfork version/update status (read-only)

As part of the default read-only checks, run `crabfork update status`.
...
- Credential hygiene notes (location of Crabfork creds, prefer disk encryption)
...
- Crabfork security audit (re-run)
...
After Crabfork install or first hardening pass, run at least one baseline audit and version check:

- `crabfork security audit`
- `crabfork security audit --deep`
- `crabfork update status`

Ongoing monitoring is recommended. Use the Crabfork cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `crabfork cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `crabfork update status`
...
Before creating, `crabfork cron list` and match on exact `name`. If found, `crabfork cron edit <id> ...`.
If not found, `crabfork cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `crabfork update status` (preferred for source checkouts and channels)
2. `npm view crabfork version` (published npm version)

## Crabfork command accuracy

Use only supported commands and flags:

- `crabfork security audit [--deep] [--fix] [--json]`
- `crabfork status` / `crabfork status --deep`
- `crabfork health --json`
- `crabfork update status`
- `crabfork cron add|list|runs|run`

Do not invent CLI flags or imply Crabfork enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Crabfork compaction:
