...
type DriverMode = "token" | "webhook" | "crabfork";
...
function resolveStateDir(): string {
  const override = process.env.CRABFORK_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.CRABFORK_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".crabfork");
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver crabfork] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) Crabfork spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|crabfork> Driver transport mode (default: token)\n" +
    ...
    "  --crabfork-bin <path>        Crabfork CLI binary for driver=crabfork (default: crabfork)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  CRABFORK_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  CRABFORK_DISCORD_SMOKE_DRIVER\n" +
    "  CRABFORK_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  CRABFORK_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  CRABFORK_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  CRABFORK_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  CRABFORK_DISCORD_SMOKE_AGENT\n" +
    "  CRABFORK_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  CRABFORK_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  CRABFORK_DISCORD_SMOKE_POLL_MS\n" +
    "  CRABFORK_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  CRABFORK_DISCORD_SMOKE_CRABFORK_BIN"
  );
}

function parseArgs(): Args {
  const channelId = resolveArg("--channel") || process.env.CRABFORK_DISCORD_SMOKE_CHANNEL_ID || "";
  const driverModeRaw =
    resolveArg("--driver") || process.env.CRABFORK_DISCORD_SMOKE_DRIVER || "token";
  const normalizedDriverMode = driverModeRaw.trim().toLowerCase();
  const driverMode: DriverMode =
    normalizedDriverMode === "webhook"
      ? "webhook"
      : normalizedDriverMode === "crabfork"
        ? "crabfork"
        : normalizedDriverMode === "token"
          ? "token"
          : "token";
  const driverToken =
    resolveArg("--token") || process.env.CRABFORK_DISCORD_SMOKE_DRIVER_TOKEN || "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.CRABFORK_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.CRABFORK_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") ||
    process.env.CRABFORK_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent = resolveArg("--agent") || process.env.CRABFORK_DISCORD_SMOKE_AGENT || "codex";
  const mentionUserId =
    resolveArg("--mention") || process.env.CRABFORK_DISCORD_SMOKE_MENTION_USER_ID || undefined;
  const instruction =
    resolveArg("--instruction") || process.env.CRABFORK_DISCORD_SMOKE_INSTRUCTION || undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.CRABFORK_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.CRABFORK_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.CRABFORK_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const crabforkBin =
    resolveArg("--crabfork-bin") || process.env.CRABFORK_DISCORD_SMOKE_CRABFORK_BIN || "crabfork";
...
async function crabforkCliJson<T>(params: { crabforkBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.crabforkBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`crabfork ${params.args.join(" ")} returned empty stdout`);
  }
  return JSON.parse(stdout) as T;
}

async function readMessagesWithCrabfork(params: {
  crabforkBin: string;
  target: string;
  limit: number;
}): Promise<DiscordMessage[]> {
  const response = await crabforkCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    crabforkBin: params.crabforkBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "crabfork") {
    return await readMessagesWithCrabfork({
      crabforkBin: params.args.crabforkBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `crabfork-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      const sent = await crabforkCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
      ...
      if (!sentMessageId) {
        throw new Error("crabfork message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    while (Date.now() < deadline && !ackMessage) {
      try {
        const threadMessages =
          args.driverMode === "crabfork"
            ? await readMessagesWithCrabfork({
                crabforkBin: args.crabforkBin,
                target: threadId,
                limit: 50,
              })
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from Crabfork.`,
        diagnostics: {
          bindingCandidates: [
            {
