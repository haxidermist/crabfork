#!/usr/bin/env bash
# ClawDock - Docker helpers for Crabfork
# Inspired by Simon Willison's "Running Crabfork in Docker"
# https://til.simonwillison.net/llms/crabfork-docker
#
# Installation:
...
# Common paths to check for Crabfork
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/crabfork"
  "${HOME}/workspace/crabfork"
  "${HOME}/projects/crabfork"
  "${HOME}/dev/crabfork"
  "${HOME}/code/crabfork"
  "${HOME}/src/crabfork"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found Crabfork at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/crabfork"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ Crabfork not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/crabfork/crabfork.git ~/crabfork"
    echo "  cd ~/crabfork && ./scripts/docker/setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^CRABFORK_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d crabfork-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart crabfork-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f crabfork-gateway
}
...
clawdock-show-config() {
  ...
  local config_dir="${HOME}/.crabfork"
  echo -e "${_CLR_BOLD}Config directory:${_CLR_RESET} ${_CLR_CYAN}${config_dir}${_CLR_RESET}"
  echo ""

  # Show crabfork.json
  if [[ -f "${config_dir}/crabfork.json" ]]; then
    echo -e "${_CLR_BOLD}${config_dir}/crabfork.json${_CLR_RESET}"
    echo -e "${_CLR_DIM}$(cat "${config_dir}/crabfork.json")${_CLR_RESET}"
  else
    echo -e "${_CLR_YELLOW}No crabfork.json found${_CLR_RESET}"
  fi
  echo ""
...
clawdock-workspace() {
  cd ~/.crabfork/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec crabfork-gateway \
    bash -c 'echo "alias crabfork=\"./crabfork.mjs\"" > /tmp/.bashrc_crabfork && bash --rcfile /tmp/.bashrc_crabfork'
}
...
clawdock-cli() {
  _clawdock_compose run --rm crabfork-cli "$@"
}
...
clawdock-update() {
  ...
  _clawdock_compose build crabfork-gateway || { echo "❌ Build failed"; return 1; }

  echo ""
  echo "♻️  Recreating container with new image..."
  _clawdock_compose down 2>&1 | _clawdock_filter_warnings
  _clawdock_compose up -d crabfork-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-rebuild() {
  _clawdock_compose build crabfork-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "CRABFORK_GATEWAY_TOKEN=$token" crabfork-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" crabfork-gateway \
    bash -c './crabfork.mjs config set gateway.remote.token "$TOKEN" && ./crabfork.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec crabfork-gateway \
    bash -c "./crabfork.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart crabfork-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm crabfork-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec crabfork-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'crabfork config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for Crabfork${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (crabfork alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to crabfork project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.crabfork)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'crabfork channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'crabfork status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.crabfork.ai${_CLR_RESET}"
  echo ""
}
