...
export DISPLAY=:1
export HOME=/tmp/crabfork-home
export XDG_CONFIG_HOME="${HOME}/.config"
export XDG_CACHE_HOME="${HOME}/.cache"

CDP_PORT="${CRABFORK_BROWSER_CDP_PORT:-9222}"
CDP_SOURCE_RANGE="${CRABFORK_BROWSER_CDP_SOURCE_RANGE:-}"
VNC_PORT="${CRABFORK_BROWSER_VNC_PORT:-5900}"
NOVNC_PORT="${CRABFORK_BROWSER_NOVNC_PORT:-6080}"
ENABLE_NOVNC="${CRABFORK_BROWSER_ENABLE_NOVNC:-1}"
HEADLESS="${CRABFORK_BROWSER_HEADLESS:-0}"
ALLOW_NO_SANDBOX="${CRABFORK_BROWSER_NO_SANDBOX:-0}"
NOVNC_PASSWORD="${CRABFORK_BROWSER_NOVNC_PASSWORD:-}"

DISABLE_GRAPHICS_FLAGS="${CRABFORK_BROWSER_DISABLE_GRAPHICS_FLAGS:-1}"
DISABLE_EXTENSIONS="${CRABFORK_BROWSER_DISABLE_EXTENSIONS:-1}"
RENDERER_PROCESS_LIMIT="${CRABFORK_BROWSER_RENDERER_PROCESS_LIMIT:-2}"
AUTO_START_TIMEOUT_MS="${CRABFORK_BROWSER_AUTO_START_TIMEOUT_MS:-12000}"
...
if [[ -z "${CDP_SOURCE_RANGE}" ]]; then
  ...
  echo "[sandbox-browser] Set CRABFORK_BROWSER_CDP_SOURCE_RANGE to an explicit CIDR to enable CDP access." >&2
else
