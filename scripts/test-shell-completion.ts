 ...
 * `crabfork update`. Use it to verify the completion installation flow
...
\`crabfork update\`, \`crabfork doctor\`, and \`crabfork onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# Crabfork Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`crabfork completion --install\` later to enable.`));
    return;
  }
