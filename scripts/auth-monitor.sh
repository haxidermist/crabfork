...
#
# Suggested cron: */30 * * * * /home/admin/crabfork/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Crabfork notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., crabfork-alerts)
...
STATE_FILE="$HOME/.crabfork/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Crabfork if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use crabfork
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Crabfork to $NOTIFY_PHONE..."
            crabfork send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Crabfork is down. Run: ssh l36 '~/crabfork/scripts/mobile-reauth.sh'" "urgent"
    exit 1
