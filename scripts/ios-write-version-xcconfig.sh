...
- CRABFORK_IOS_VERSION = exact canonical iOS version
- CRABFORK_MARKETING_VERSION = short iOS/App Store version
- CRABFORK_BUILD_VERSION = explicit build number or local numeric fallback
EOF
}
...
while IFS='=' read -r key value; do
  case "${key}" in
    CRABFORK_IOS_VERSION)
      IOS_VERSION="${value}"
      ;;
    CRABFORK_MARKETING_VERSION)
...
CRABFORK_IOS_VERSION = ${IOS_VERSION}
CRABFORK_MARKETING_VERSION = ${MARKETING_VERSION}
CRABFORK_BUILD_VERSION = ${BUILD_NUMBER}
EOF
