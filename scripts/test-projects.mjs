...
function applyDefaultParallelVitestWorkerBudget(specs, env) {
  if (env.CRABFORK_VITEST_MAX_WORKERS || env.CRABFORK_TEST_WORKERS || isCiLikeEnv(env)) {
    return specs;
  }
  ...
  return specs.map((spec) => ({
    ...
    env: {
      ...spec.env,
      CRABFORK_VITEST_MAX_WORKERS: String(vitestMaxWorkers),
    },
  }));
}
  ...
  if (isFullSuiteRun) {
    ...
    if (concurrency > 1) {
      ...
      if (
        ...
        !process.env.CRABFORK_TEST_PROJECTS_PARALLEL &&
        !process.env.CRABFORK_VITEST_MAX_WORKERS &&
        !process.env.CRABFORK_TEST_WORKERS &&
