...
const EXPECTED_REPOSITORY_URL = "https://github.com/crabfork/crabfork";
...
const skipPackValidationEnv = "CRABFORK_NPM_RELEASE_SKIP_PACK_CHECK";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "crabfork") {
    errors.push(`package.json name must be "crabfork"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.crabfork !== "crabfork.mjs") {
    errors.push(
      `package.json bin.crabfork must be "crabfork.mjs"; found "${pkg.bin?.crabfork ?? ""}".`,
    );
  }
...
function main(): number {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`crabfork-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `crabfork-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta${skipPackValidation ? "; metadata-only" : ""}).`,
  );
  return 0;
}
