...
async function main() {
  const stateDir = process.env.CRABFORK_STATE_DIR?.trim() || path.join(os.homedir(), ".crabfork");
  const configPath =
    process.env.CRABFORK_CONFIG_PATH?.trim() || path.join(stateDir, "crabfork.json");
