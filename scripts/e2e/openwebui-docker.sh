...
IMAGE_NAME="crabfork-openwebui-e2e"
...
MODEL="${CRABFORK_OPENWEBUI_MODEL:-openai/gpt-4.1-mini}"
PROMPT_NONCE="OPENWEBUI_DOCKER_E2E_$(date +%s)_$$"
PROMPT="${CRABFORK_OPENWEBUI_PROMPT:-Reply with exactly this token and nothing else: ${PROMPT_NONCE}}"
PORT="${CRABFORK_OPENWEBUI_GATEWAY_PORT:-18789}"
WEBUI_PORT="${CRABFORK_OPENWEBUI_PORT:-8080}"
TOKEN="openwebui-e2e-$(date +%s)-$$"
ADMIN_EMAIL="${CRABFORK_OPENWEBUI_ADMIN_EMAIL:-openwebui-e2e@example.com}"
ADMIN_PASSWORD="${CRABFORK_OPENWEBUI_ADMIN_PASSWORD:-OpenWebUI-E2E-Password-$(date +%s)-$$}"
NET_NAME="crabfork-openwebui-e2e-$$"
GW_NAME="crabfork-openwebui-gateway-$$"
OW_NAME="crabfork-openwebui-$$"
...
docker run -d \
  ...
  -e "CRABFORK_OPENWEBUI_MODEL=$MODEL" \
  -e OPENAI_API_KEY \
  ...
  bash -lc '
    ...
    batch_file="$(mktemp /tmp/crabfork-openwebui-config.XXXXXX.json)"
    CRABFORK_CONFIG_BATCH_PATH="$batch_file" node - <<'"'"'NODE'"'"' "$openai_api_key"
const fs = require("node:fs");

const openaiApiKey = process.argv[2];
const batchPath = process.env.CRABFORK_CONFIG_BATCH_PATH;
const entries = [
  ...
  { path: "gateway.auth.token", value: process.env.CRABFORK_GATEWAY_TOKEN },
  { path: "gateway.http.endpoints.chatCompletions.enabled", value: true },
  { path: "agents.defaults.model.primary", value: process.env.CRABFORK_OPENWEBUI_MODEL },
];
...
docker run -d \
  ...
  --network "$NET_NAME" \
  -e ENV=prod \
  -e WEBUI_NAME="Crabfork E2E" \
  -e WEBUI_SECRET_KEY="crabfork-openwebui-e2e-secret" \
  -e OFFLINE_MODE=True \
  ...
  -e WEBUI_ADMIN_NAME="Crabfork E2E" \
  -e ENABLE_SIGNUP=False \
  -e DEFAULT_MODELS="crabfork/default" \
...
echo "Running Open WebUI -> Crabfork smoke..."
if ! docker exec \
