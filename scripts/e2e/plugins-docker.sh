...
IMAGE_NAME="crabfork-plugins-e2e"
...
RUN_LOG="$(mktemp "${TMPDIR:-/tmp}/crabfork-plugins-run.XXXXXX.log")"
if ! docker run --rm "${DOCKER_ENV_ARGS[@]}" -i "$IMAGE_NAME" bash -s >"$RUN_LOG" 2>&1 <<'EOF'
set -euo pipefail

if [ -f dist/index.mjs ]; then
  CRABFORK_ENTRY="dist/index.mjs"
elif [ -f dist/index.js ]; then
  CRABFORK_ENTRY="dist/index.js"
else
  echo "Missing dist/index.(m)js (build output):"
  ls -la dist || true
  exit 1
fi
export CRABFORK_ENTRY
...
home_dir=$(mktemp -d "/tmp/crabfork-plugins-e2e.XXXXXX")
export HOME="$home_dir"
BUNDLED_PLUGIN_ROOT_DIR="extensions"
CRABFORK_PLUGIN_HOME="$HOME/.crabfork/$BUNDLED_PLUGIN_ROOT_DIR"
...
const configPath = path.join(process.env.HOME, ".crabfork", "crabfork.json");
...
run_logged() {
  local label="$1"
  shift
  local log_file="/tmp/crabfork-plugins-e2e-${label}.log"
...
start_gateway() {
  local log_file="$1"
  : > "$log_file"
  node "$CRABFORK_ENTRY" gateway --port 18789 --bind loopback --allow-unconfigured \
    >"$log_file" 2>&1 &
  gateway_pid=$!
...
wait_for_gateway_health() {
  for _ in $(seq 1 120); do
    if node "$CRABFORK_ENTRY" gateway health \
...
run_gateway_chat_json() {
  ...
  node - <<'NODE' "$CRABFORK_ENTRY" "$session_key" "$message" "$output_file" "$timeout_ms"
...
JS
  cat > "$dir/crabfork.plugin.json" <<'JSON'
{
  "id": "placeholder",
  "configSchema": {
    "type": "object",
...
JSON
  node - <<'NODE' "$dir/crabfork.plugin.json" "$id"
const fs = require("node:fs");
...
demo_plugin_root="$CRABFORK_PLUGIN_HOME/$demo_plugin_id"
...
JS
cat > "$demo_plugin_root/crabfork.plugin.json" <<'JSON'
{
  "id": "demo-plugin",
  "configSchema": {
    "type": "object",
...
node "$CRABFORK_ENTRY" plugins list --json > /tmp/plugins.json
node "$CRABFORK_ENTRY" plugins inspect demo-plugin --json > /tmp/plugins-inspect.json
...
pack_dir="$(mktemp -d "/tmp/crabfork-plugin-pack.XXXXXX")"
mkdir -p "$pack_dir/package"
cat > "$pack_dir/package/package.json" <<'JSON'
{
  "name": "@crabfork/demo-plugin-tgz",
  "version": "0.0.1",
  "crabfork": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$pack_dir/package/crabfork.plugin.json" <<'JSON'
{
  "id": "demo-plugin-tgz",
  "configSchema": {
    "type": "object",
...
run_logged install-tgz node "$CRABFORK_ENTRY" plugins install /tmp/demo-plugin-tgz.tgz
node "$CRABFORK_ENTRY" plugins list --json > /tmp/plugins2.json
node "$CRABFORK_ENTRY" plugins inspect demo-plugin-tgz --json > /tmp/plugins2-inspect.json
...
dir_plugin="$(mktemp -d "/tmp/crabfork-plugin-dir.XXXXXX")"
cat > "$dir_plugin/package.json" <<'JSON'
{
  "name": "@crabfork/demo-plugin-dir",
  "version": "0.0.1",
  "crabfork": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$dir_plugin/crabfork.plugin.json" <<'JSON'
{
  "id": "demo-plugin-dir",
  "configSchema": {
    "type": "object",
...
run_logged install-dir node "$CRABFORK_ENTRY" plugins install "$dir_plugin"
node "$CRABFORK_ENTRY" plugins list --json > /tmp/plugins3.json
node "$CRABFORK_ENTRY" plugins inspect demo-plugin-dir --json > /tmp/plugins3-inspect.json
...
file_pack_dir="$(mktemp -d "/tmp/crabfork-plugin-filepack.XXXXXX")"
mkdir -p "$file_pack_dir/package"
cat > "$file_pack_dir/package/package.json" <<'JSON'
{
  "name": "@crabfork/demo-plugin-file",
  "version": "0.0.1",
  "crabfork": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$file_pack_dir/package/crabfork.plugin.json" <<'JSON'
{
  "id": "demo-plugin-file",
  "configSchema": {
    "type": "object",
...
run_logged install-file node "$CRABFORK_ENTRY" plugins install "file:$file_pack_dir/package"
node "$CRABFORK_ENTRY" plugins list --json > /tmp/plugins4.json
node "$CRABFORK_ENTRY" plugins inspect demo-plugin-file --json > /tmp/plugins4-inspect.json
...
bundle_root="$CRABFORK_PLUGIN_HOME/$bundle_plugin_id"
...
gateway_log="/tmp/crabfork-plugin-command-e2e.log"
start_gateway "$gateway_log"
wait_for_gateway_health

echo "Testing /plugin install with auto-restart..."
slash_install_dir="$(mktemp -d "/tmp/crabfork-plugin-slash-install.XXXXXX")"
cat > "$slash_install_dir/package.json" <<'JSON'
{
  "name": "@crabfork/slash-install-plugin",
  "version": "0.0.1",
  "crabfork": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$slash_install_dir/crabfork.plugin.json" <<'JSON'
{
  "id": "slash-install-plugin",
  "configSchema": {
    "type": "object",
...
cat > "$HOME/.claude/plugins/known_marketplaces.json" <<JSON
{
  "claude-fixtures": {
    ...
    "source": {
      "type": "github",
      "repo": "crabfork/fixture-marketplace"
    }
  }
}
JSON

node "$CRABFORK_ENTRY" plugins marketplace list claude-fixtures --json > /tmp/marketplace-list.json
...
run_logged install-marketplace-shortcut node "$CRABFORK_ENTRY" plugins install marketplace-shortcut@claude-fixtures
run_logged install-marketplace-direct node "$CRABFORK_ENTRY" plugins install marketplace-direct --marketplace claude-fixtures
node "$CRABFORK_ENTRY" plugins list --json > /tmp/plugins-marketplace.json
node "$CRABFORK_ENTRY" plugins inspect marketplace-shortcut --json > /tmp/plugins-marketplace-shortcut-inspect.json
node "$CRABFORK_ENTRY" plugins inspect marketplace-direct --json > /tmp/plugins-marketplace-direct-inspect.json
...
write_fixture_plugin \
  ...
  "Marketplace Shortcut"
run_logged update-marketplace-shortcut-dry-run node "$CRABFORK_ENTRY" plugins update marketplace-shortcut --dry-run
run_logged update-marketplace-shortcut node "$CRABFORK_ENTRY" plugins update marketplace-shortcut
node "$CRABFORK_ENTRY" plugins list --json > /tmp/plugins-marketplace-updated.json
node "$CRABFORK_ENTRY" plugins inspect marketplace-shortcut --json > /tmp/plugins-marketplace-updated-inspect.json
