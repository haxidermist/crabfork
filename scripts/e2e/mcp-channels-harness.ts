...
export async function connectMcpClient(params: {
  ...
  const tokenDir = "/tmp/crabfork-mcp-client";
  ...
  const transport = new StdioClientTransport({
    ...
    env: {
      ...process.env,
      CRABFORK_ALLOW_INSECURE_PRIVATE_WS: "1",
      CRABFORK_STATE_DIR: "/tmp/crabfork-mcp-client",
    },
    stderr: "pipe",
  });
  transport.stderr?.on("data", (chunk) => {
    process.stderr.write(`[crabfork mcp] ${String(chunk)}`);
  });
