...
IMAGE_NAME="crabfork-doctor-install-switch-e2e"
...
docker run --rm -e COREPACK_ENABLE_DOWNLOAD_PROMPT=0 "$IMAGE_NAME" bash -lc '
  ...
  cat > /tmp/crabfork-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/crabfork-bin/systemctl

  cat > /tmp/crabfork-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
  chmod +x /tmp/crabfork-bin/loginctl
  ...
  npm_log="/tmp/crabfork-doctor-switch-npm-install.log"
  if ! npm install -g --prefix /tmp/npm-prefix "/app/$pkg_tgz" >"$npm_log" 2>&1; then
    cat "$npm_log"
    exit 1
  fi

	  npm_bin="/tmp/npm-prefix/bin/crabfork"
	  npm_root="/tmp/npm-prefix/lib/node_modules/crabfork"
  ...
  run_flow() {
    ...
    local install_log="/tmp/crabfork-doctor-switch-${name}-install.log"
    local doctor_log="/tmp/crabfork-doctor-switch-${name}-doctor.log"

    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/crabfork-switch-${name}.XXXXXX")
    ...
    unit_path="$HOME/.config/systemd/user/crabfork-gateway.service"
    if [ ! -f "$unit_path" ]; then
