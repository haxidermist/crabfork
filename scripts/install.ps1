# Crabfork Installer for Windows (PowerShell)
# Usage: iwr -useb https://crabfork.ai/install.ps1 | iex
# Or: & ([scriptblock]::Create((iwr -useb https://crabfork.ai/install.ps1))) -NoOnboard

param(
    [string]$InstallMethod = "npm",
    [string]$Tag = "latest",
    [string]$GitDir = "$env:USERPROFILE\crabfork",
    [switch]$NoOnboard,
    [switch]$NoGitUpdate,
    [switch]$DryRun
)
...
function Write-Banner {
    Write-Host ""
    Write-Host "${ACCENT}  🦞 Crabfork Installer$NC" -Level info
    Write-Host "${MUTED}  All your chats, one Crabfork.$NC" -Level info
    Write-Host ""
}
...
function Install-CrabforkGit {
    param([string]$RepoDir, [switch]$Update)
    
    Write-Host "Installing Crabfork from git..." -Level info
    
    if (!(Test-Path $RepoDir)) {
        Write-Host "  Cloning repository..." -Level info
        git clone https://github.com/crabfork/crabfork.git $RepoDir 2>&1
    } elseif ($Update) {
    ...
    @"
@echo off
node "%~dp0..\crabfork\dist\entry.js" %*
"@ | Out-File -FilePath "$wrapperDir\crabfork.cmd" -Encoding ASCII -Force
...
function Main {
    ...
    if ($InstallMethod -eq "git") {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install Crabfork from git to $GitDir" -Level info
        } else {
            Install-CrabforkGit -RepoDir $GitDir -Update:(-not $NoGitUpdate)
        }
    } else {
        # npm method
        if (!(Ensure-Git)) {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install Crabfork via npm ($((Resolve-PackageInstallSpec -Target $Tag)))" -Level info
        } else {
            if (!(Install-CrabforkNpm -Target $Tag)) {
                exit 1
            }
        }
    }
    ...
    if (!$NoOnboard -and !$DryRun) {
        Write-Host ""
        Write-Host "Run 'crabfork onboard' to complete setup" -Level info
    }
    
    Write-Host ""
    Write-Host "🦞 Crabfork installed successfully!" -Level success
}
