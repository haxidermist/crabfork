...
export function readBundledChannelCatalog(
  ...
  for (const dirEntry of readdirSync(extensionsDirOverride, { withFileTypes: true })) {
    ...
    try {
      ...
      const id = parsed.crabfork?.channel?.id;
      if (typeof id !== "string" || !id.trim()) {
        continue;
      }
      const orderRaw = parsed.crabfork?.channel?.order;
      const labelRaw = parsed.crabfork?.channel?.label;
      entries.push({
        id: id.trim(),
...
function createIsolatedRootHelpRenderContext(
  ...
  const stateDir = path.join(rootDir, ".crabfork-build-root-help");
  const workspaceDir = path.join(stateDir, "workspace");
  const homeDir = path.join(stateDir, "home");
  const env: NodeJS.ProcessEnv = {
    HOME: homeDir,
    LOGNAME: process.env.LOGNAME ?? process.env.USER ?? "crabfork-build",
    USER: process.env.USER ?? process.env.LOGNAME ?? "crabfork-build",
    ...
    CRABFORK_PLUGIN_DISCOVERY_CACHE_MS: "0",
    CRABFORK_PLUGIN_MANIFEST_CACHE_MS: "0",
