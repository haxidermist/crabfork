...
# Crabfork Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://crabfork.ai/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one Crabfork."
...
GUM_VERSION="${CRABFORK_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🦞 Crabfork Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://crabfork.ai/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$CRABFORK_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.crabfork.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_crabfork_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.crabfork-* "$npm_root"/crabfork 2>/dev/null || true
}

extract_crabfork_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_crabfork_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/crabfork" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/crabfork"|"/usr/local/bin/crabfork")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/crabfork/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale crabfork symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing crabfork binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing Crabfork package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_crabfork_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "Crabfork npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*crabfork" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_crabfork_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_crabfork_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "Crabfork npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an crabfork binary already exists"
            if [[ -n "$conflict" ]]; then
...
pick_tagline() {
    ...
    if [[ -n "${CRABFORK_TAGLINE_INDEX:-}" ]]; then
        if [[ "${CRABFORK_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((CRABFORK_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${CRABFORK_NO_ONBOARD:-0}
NO_PROMPT=${CRABFORK_NO_PROMPT:-0}
DRY_RUN=${CRABFORK_DRY_RUN:-0}
INSTALL_METHOD=${CRABFORK_INSTALL_METHOD:-}
CRABFORK_VERSION=${CRABFORK_VERSION:-latest}
USE_BETA=${CRABFORK_BETA:-0}
GIT_DIR_DEFAULT="${HOME}/crabfork"
GIT_DIR=${CRABFORK_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${CRABFORK_GIT_UPDATE:-1}
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${CRABFORK_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${CRABFORK_VERBOSE:-0}"
VERIFY_INSTALL="${CRABFORK_VERIFY_INSTALL:-0}"
CRABFORK_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
Crabfork installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://crabfork.ai/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/crabfork)
...
Environment variables:
  CRABFORK_INSTALL_METHOD=git|npm
  CRABFORK_VERSION=latest|next|main|<semver>|<spec>
  CRABFORK_BETA=0|1
  CRABFORK_GIT_DIR=...
  CRABFORK_GIT_UPDATE=0|1
  CRABFORK_NO_PROMPT=1
  CRABFORK_VERIFY_INSTALL=1
  CRABFORK_DRY_RUN=1
  CRABFORK_NO_ONBOARD=1
  CRABFORK_VERBOSE=1
  CRABFORK_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
  SHARP_IGNORE_GLOBAL_LIBVIPS=0|1    Default: 1 (avoid sharp building against global libvips)

Examples:
  curl -fsSL --proto '=https' --tlsv1.2 https://crabfork.ai/install.sh | bash
  curl -fsSL --proto '=https' --tlsv1.2 https://crabfork.ai/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://crabfork.ai/install.sh | bash -s -- --no-onboard --verify
  curl -fsSL --proto '=https' --tlsv1.2 https://crabfork.ai/install.sh | bash -s -- --version main
  curl -fsSL --proto '=https' --tlsv1.2 https://crabfork.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected Crabfork checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a Crabfork source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_crabfork_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"crabfork"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://crabfork.ai/install.sh | bash"
}
...
ensure_crabfork_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/crabfork" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/crabfork" ]]; then
        ln -sf "$npm_root/crabfork/dist/entry.js" "${npm_bin}/crabfork"
        ui_info "Created crabfork bin link at ${npm_bin}/crabfork"
    fi
    return 0
}

# Check for existing Crabfork installation
check_existing_crabfork() {
    if [[ -n "$(type -P crabfork 2>/dev/null || true)" ]]; then
        ui_info "Existing Crabfork installation detected, upgrading"
        return 0
    fi
    return 1
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make crabfork show as \"command not found\" in new terminals."
...
warn_crabfork_not_found() {
    ui_warn "Installed, but crabfork is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t crabfork 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named crabfork; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_crabfork_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P crabfork 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/crabfork" ]]; then
        echo "${npm_bin}/crabfork"
        return 0
    fi
...
install_crabfork_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/crabfork/crabfork.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing Crabfork from git checkout: ${repo_dir}"
    else
        ui_info "Installing Crabfork from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning Crabfork" git clone "$repo_url" "$repo_dir"
    fi
    ...
    run_quiet_step "Building Crabfork" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/crabfork" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/crabfork"
    ui_success "Crabfork wrapper installed to \$HOME/.local/bin/crabfork"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view crabfork dist-tags.beta 2>/dev/null || true)"
...
resolve_package_install_spec() {
    ...
    local value="$2"
    if [[ "${value,,}" == "main" ]]; then
        echo "github:crabfork/crabfork#main"
        return 0
    fi
    if is_explicit_package_install_spec "$value"; then
        echo "$value"
...
install_crabfork() {
    local package_name="crabfork"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            CRABFORK_VERSION="$beta_version"
            ui_info "Beta tag detected (${beta_version})"
            package_name="crabfork"
        else
            CRABFORK_VERSION="latest"
            ui_info "No beta tag found; using latest"
        fi
    fi

    if [[ -z "${CRABFORK_VERSION}" ]]; then
        CRABFORK_VERSION="latest"
    fi

    local resolved_version=""
    if can_resolve_registry_package_version "${CRABFORK_VERSION}"; then
        resolved_version="$(npm view "${package_name}@${CRABFORK_VERSION}" version 2>/dev/null || true)"
    fi
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing Crabfork v${resolved_version}"
    else
        ui_info "Installing Crabfork (${CRABFORK_VERSION})"
    fi
    local install_spec=""
    install_spec="$(resolve_package_install_spec "${package_name}" "${CRABFORK_VERSION}")"

    if ! install_crabfork_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_crabfork_paths
        install_crabfork_npm "${install_spec}"
    fi

    if [[ "${CRABFORK_VERSION}" == "latest" && "${package_name}" == "crabfork" ]]; then
        if ! resolve_crabfork_bin &> /dev/null; then
            ui_warn "npm install crabfork@latest failed; retrying crabfork@next"
            cleanup_npm_crabfork_paths
            install_crabfork_npm "crabfork@next"
        fi
    fi

    ensure_crabfork_bin_link || true

    ui_success "Crabfork installed"
}

# Run doctor for migrations (safe, non-interactive)
run_doctor() {
    ui_info "Running doctor to migrate settings"
    local claw="${CRABFORK_BIN:-}"
    if [[ -z "$claw" ]]; then
        claw="$(resolve_crabfork_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (crabfork not on PATH yet)"
        warn_crabfork_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    local profile="${CRABFORK_PROFILE:-default}"
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.crabfork/workspace-${profile}"
    else
        echo "${HOME}/.crabfork/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local config_path="${CRABFORK_CONFIG_PATH:-$HOME/.crabfork/crabfork.json}"
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        ui_info "BOOTSTRAP.md found but no TTY; run crabfork onboard to finish setup"
        return
    fi
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but crabfork not on PATH; skipping onboarding"
        warn_crabfork_not_found
        return
    fi

    "$claw" onboard || {
        ui_error "Onboarding failed; run crabfork onboard to retry"
        return
    }
}
...
if ! declare -F extract_crabfork_semver >/dev/null 2>&1; then
...
resolve_crabfork_version() {
    ...
    local claw="${CRABFORK_BIN:-}"
    if [[ -z "$claw" ]] && command -v crabfork &> /dev/null; then
        claw="$(command -v crabfork)"
    fi
    if [[ -n "$claw" ]]; then
        raw_version_output=$("$claw" --version 2>/dev/null | head -n 1 | tr -d '\r')
        version="$(extract_crabfork_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/crabfork/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/crabfork/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
verify_installation() {
    ...
    if [[ -z "$claw" ]]; then
        ui_error "Install verify failed: crabfork not on PATH yet"
        warn_crabfork_not_found
        return 1
    fi

    run_quiet_step "Checking Crabfork version" "$claw" --version || return 1

    if is_gateway_daemon_loaded "$claw"; then
        run_quiet_step "Checking gateway service" "$claw" gateway status --deep || {
            ui_error "Install verify failed: gateway service unhealthy"
            ui_info "Run: crabfork gateway status --deep"
            return 1
        }
    else
...
main() {
    ...
    detected_checkout="$(detect_crabfork_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found Crabfork checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set CRABFORK_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if check_existing_crabfork; then
        is_upgrade=true
    fi
    ...
    ui_stage "Installing Crabfork"

    local final_git_dir=""
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        # Clean up npm global install if switching to git
        if npm list -g crabfork &>/dev/null; then
            ui_info "Removing npm global install (switching to git)"
            npm uninstall -g crabfork 2>/dev/null || true
        ...
        install_crabfork_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/crabfork" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/crabfork"
    ...
    CRABFORK_BIN="$(resolve_crabfork_bin || true)"
    ...
    installed_version=$(resolve_crabfork_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🦞 Crabfork installed successfully (${installed_version})!"
    else
        ui_celebrate "🦞 Crabfork installed successfully!"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/crabfork"
        ui_kv "Update command" "crabfork update --restart"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://crabfork.ai/install.sh | bash -s -- --install-method npm"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "Running crabfork doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                CRABFORK_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/null && doctor_ok=1
            else
                CRABFORK_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                CRABFORK_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
                ui_warn "Doctor failed; skipping plugin updates"
            fi
        else
            ui_info "No TTY; run crabfork doctor and crabfork plugins update --all manually"
        fi
    else
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ui_info "Skipping onboard (requested); run crabfork onboard later"
        else
            local config_path="${CRABFORK_CONFIG_PATH:-$HOME/.crabfork/crabfork.json}"
            if [[ -f "${config_path}" || -f "$HOME/.clawdbot/clawdbot.json" ]]; then
            ...
            if [[ -r /dev/tty && -w /dev/tty ]]; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (crabfork not on PATH yet)"
                    warn_crabfork_not_found
                    return 0
                fi
                exec </dev/tty
            ...
            ui_info "No TTY; run crabfork onboard to finish setup"
            return 0
        fi
    fi

    if command -v crabfork &> /dev/null; then
        local claw="${CRABFORK_BIN:-}"
        if [[ -z "$claw" ]]; then
            claw="$(resolve_crabfork_bin || true)"
        fi
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (crabfork daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if CRABFORK_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: crabfork daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${CRABFORK_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_verbose
    main
fi
