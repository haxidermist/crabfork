#!/usr/bin/env bash
# Rootless Crabfork in Podman: run after one-time setup.
#
# One-time setup (from repo root): ./scripts/podman/setup.sh
# Then:
#   ./scripts/run-crabfork-podman.sh launch        # Start gateway
#   ./scripts/run-crabfork-podman.sh launch setup  # Onboarding wizard
#
# Manage the running container from the host CLI:
#   crabfork --container crabfork dashboard --no-open
#   crabfork --container crabfork channels login
#
...
load_podman_env_file() {
  ...
  while IFS= read -r line <&9 || [[ -n "$line" ]]; do
    ...
    case "$key" in
      CRABFORK_GATEWAY_TOKEN|CRABFORK_PODMAN_CONTAINER|CRABFORK_PODMAN_IMAGE|CRABFORK_IMAGE|CRABFORK_PODMAN_PULL|CRABFORK_PODMAN_GATEWAY_HOST_PORT|CRABFORK_GATEWAY_PORT|CRABFORK_PODMAN_BRIDGE_HOST_PORT|CRABFORK_BRIDGE_PORT|CRABFORK_GATEWAY_BIND|CRABFORK_PODMAN_USERNS|CRABFORK_BIND_MOUNT_OPTIONS|CRABFORK_PODMAN_PUBLISH_HOST)
        ;;
      *)
        continue
        ;;
...
if [[ "$(id -u)" -eq 0 ]]; then
  fail "Run run-crabfork-podman.sh as your normal user so Podman stays rootless."
fi
...
CONFIG_DIR="${CRABFORK_CONFIG_DIR:-$EFFECTIVE_HOME/.crabfork}"
ENV_FILE="${CRABFORK_PODMAN_ENV:-$CONFIG_DIR/.env}"
...
CONFIG_DIR="${CRABFORK_CONFIG_DIR:-$EFFECTIVE_HOME/.crabfork}"
ENV_FILE="${CRABFORK_PODMAN_ENV:-$CONFIG_DIR/.env}"
WORKSPACE_DIR="${CRABFORK_WORKSPACE_DIR:-$CONFIG_DIR/workspace}"
CONTAINER_NAME="${CRABFORK_PODMAN_CONTAINER:-crabfork}"
CRABFORK_IMAGE="${CRABFORK_PODMAN_IMAGE:-${CRABFORK_IMAGE:-crabfork:local}}"
PODMAN_PULL="${CRABFORK_PODMAN_PULL:-never}"
HOST_GATEWAY_PORT="${CRABFORK_PODMAN_GATEWAY_HOST_PORT:-${CRABFORK_GATEWAY_PORT:-18789}}"
HOST_BRIDGE_PORT="${CRABFORK_PODMAN_BRIDGE_HOST_PORT:-${CRABFORK_BRIDGE_PORT:-18790}}"
PUBLISH_HOST="${CRABFORK_PODMAN_PUBLISH_HOST:-127.0.0.1}"
...
validate_single_line_value "image name" "$CRABFORK_IMAGE"
...
resolve_config_gateway_bind() {
  local config_dir="$1"
  if ! command -v crabfork >/dev/null 2>&1; then
    return 0
  fi
  CRABFORK_CONTAINER="" CRABFORK_CONFIG_DIR="$config_dir" \
    crabfork config get gateway.bind 2>/dev/null || true
}
...
# CRABFORK_GATEWAY_BIND first, then gateway.bind in local config.
CONFIG_GATEWAY_BIND="$(resolve_config_gateway_bind "$CONFIG_DIR")"
GATEWAY_BIND="${CRABFORK_GATEWAY_BIND:-${CONFIG_GATEWAY_BIND:-lan}}"
...
create_token_env_file() {
  ...
  chmod 600 "$tmp"
  printf 'CRABFORK_GATEWAY_TOKEN=%s\n' "$token" >"$tmp"
  printf '%s' "$tmp"
}
...
sync_local_control_ui_origins_via_cli() {
  ...
  if ! command -v crabfork >/dev/null 2>&1; then
    echo "Warning: crabfork not found; unable to sync gateway.controlUi.allowedOrigins in $file." >&2
    return 0
  fi
  if ! command -v python3 >/dev/null 2>&1; then
    CRABFORK_CONTAINER="" CRABFORK_CONFIG_DIR="$config_dir" \
      crabfork config set gateway.controlUi.allowedOrigins \
  ...
  allowed_json="$(
    CRABFORK_CONTAINER="" CRABFORK_CONFIG_DIR="$config_dir" \
      crabfork config get gateway.controlUi.allowedOrigins --json 2>/dev/null || true
  )"
...
PY
  ...
  CRABFORK_CONTAINER="" CRABFORK_CONFIG_DIR="$config_dir" \
    crabfork config set gateway.controlUi.allowedOrigins "$merged_json" --strict-json >/dev/null
}
...
if [[ -z "${CRABFORK_GATEWAY_TOKEN:-}" ]]; then
  export CRABFORK_GATEWAY_TOKEN="$(generate_token_hex_32)"
  mkdir -p "$(dirname "$ENV_FILE")"
  ensure_safe_existing_dir "env file directory" "$(dirname "$ENV_FILE")"
  upsert_env_var "$ENV_FILE" "CRABFORK_GATEWAY_TOKEN" "$CRABFORK_GATEWAY_TOKEN"
  echo "Generated CRABFORK_GATEWAY_TOKEN and wrote it to $ENV_FILE." >&2
fi

CONFIG_JSON="$CONFIG_DIR/crabfork.json"
...
PODMAN_USERNS="${CRABFORK_PODMAN_USERNS:-keep-id}"
USERNS_ARGS=()
RUN_USER_ARGS=()
case "$PODMAN_USERNS" in
  ...
  *)
    echo "Unsupported CRABFORK_PODMAN_USERNS=$PODMAN_USERNS (expected: keep-id, auto, host)." >&2
    exit 2
    ;;
esac
...
if [[ "$PODMAN_USERNS" == "keep-id" ]]; then
  ...
  echo "Starting container without --user (CRABFORK_PODMAN_USERNS=$PODMAN_USERNS), mounts may require ownership fixes." >&2
fi

SELINUX_MOUNT_OPTS=""
if [[ -z "${CRABFORK_BIND_MOUNT_OPTIONS:-}" ]]; then
  ...
  SELINUX_MOUNT_OPTS="${CRABFORK_BIND_MOUNT_OPTIONS#:}"
  [[ -n "$SELINUX_MOUNT_OPTS" ]] && SELINUX_MOUNT_OPTS=",$SELINUX_MOUNT_OPTS"
fi

if [[ "$RUN_SETUP" == true ]]; then
  TOKEN_ENV_FILE="$(create_token_env_file "$ENV_FILE" "$CRABFORK_GATEWAY_TOKEN")"
  podman run --pull="$PODMAN_PULL" --rm -it \
    --init \
    "${USERNS_ARGS[@]}" "${RUN_USER_ARGS[@]}" \
    -e HOME=/home/node -e TERM=xterm-256color -e BROWSER=echo \
    -e NPM_CONFIG_CACHE=/home/node/.crabfork/.npm \
    -e CRABFORK_NO_RESPAWN=1 \
    --env-file "$TOKEN_ENV_FILE" \
    -v "$CONFIG_DIR:/home/node/.crabfork:rw${SELINUX_MOUNT_OPTS}" \
    -v "$WORKSPACE_DIR:/home/node/.crabfork/workspace:rw${SELINUX_MOUNT_OPTS}" \
    "$CRABFORK_IMAGE" \
...
echo "podman exec -it $CONTAINER_NAME crabfork dashboard --no-open"
echo "podman exec -it $CONTAINER_NAME crabfork devices approve --latest  # if pairing required"
