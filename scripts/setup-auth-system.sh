#!/bin/bash
# Setup Crabfork Auth Management System
...
echo "=== Crabfork Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'crabfork-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Crabfork message: Send warning via Crabfork itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/crabfork-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/crabfork-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/crabfork-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now crabfork-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status crabfork-auth-monitor.timer"
echo ""
echo "Quick commands:"
