...
function buildCatalogEntry(packageJson) {
  ...
  const manifest = isRecord(packageJson.crabfork) ? packageJson.crabfork : null;
...
export function buildOfficialChannelCatalog(params = {}) {
  ...
  entries.sort((left, right) => {
    const leftId = trimString(left.crabfork?.channel?.id) || left.name;
    const rightId = trimString(right.crabfork?.channel?.id) || right.name;
