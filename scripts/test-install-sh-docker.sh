...
resolve_default_smoke_platform() {
  local host_os
  local host_arch
  if [[ -n "${CRABFORK_INSTALL_SMOKE_PLATFORM:-}" ]]; then
    printf "%s" "$CRABFORK_INSTALL_SMOKE_PLATFORM"
    return
  fi
...
const parsed = JSON.parse(raw);
const budgetOverride = process.env.CRABFORK_INSTALL_SMOKE_PACK_UNPACKED_BUDGET_BYTES;
const budgetBytes = budgetOverride ? Number(budgetOverride) : undefined;
if (budgetOverride && !Number.isFinite(budgetBytes)) {
  throw new Error(
    `CRABFORK_INSTALL_SMOKE_PACK_UNPACKED_BUDGET_BYTES must be numeric, got ${JSON.stringify(
      budgetOverride,
    )}`,
  );
}
...
SMOKE_IMAGE="${CRABFORK_INSTALL_SMOKE_IMAGE:-crabfork-install-smoke:local}"
NONROOT_IMAGE="${CRABFORK_INSTALL_NONROOT_IMAGE:-crabfork-install-nonroot:local}"
SMOKE_PLATFORM="$(resolve_default_smoke_platform)"
NONROOT_PLATFORM="${CRABFORK_INSTALL_NONROOT_PLATFORM:-$SMOKE_PLATFORM}"
INSTALL_URL="${CRABFORK_INSTALL_URL:-https://crabfork.bot/install.sh}"
CLI_INSTALL_URL="${CRABFORK_INSTALL_CLI_URL:-https://crabfork.bot/install-cli.sh}"
PACKAGE_NAME="${CRABFORK_INSTALL_PACKAGE:-crabfork}"
SKIP_NONROOT="${CRABFORK_INSTALL_SMOKE_SKIP_NONROOT:-0}"
SKIP_SMOKE_IMAGE_BUILD="${CRABFORK_INSTALL_SMOKE_SKIP_IMAGE_BUILD:-0}"
SKIP_NONROOT_IMAGE_BUILD="${CRABFORK_INSTALL_NONROOT_SKIP_IMAGE_BUILD:-0}"
SKIP_UPDATE="${CRABFORK_INSTALL_SMOKE_SKIP_UPDATE:-0}"
UPDATE_BASELINE_VERSION="${CRABFORK_INSTALL_SMOKE_UPDATE_BASELINE:-2026.4.10}"
UPDATE_PACKAGE_SPEC="${CRABFORK_INSTALL_SMOKE_UPDATE_PACKAGE_SPEC:-}"
UPDATE_SKIP_LOCAL_BUILD="${CRABFORK_INSTALL_SMOKE_UPDATE_SKIP_LOCAL_BUILD:-0}"
UPDATE_HOST_ALIAS="${CRABFORK_INSTALL_SMOKE_UPDATE_HOST:-host.docker.internal}"
UPDATE_PORT="${CRABFORK_INSTALL_SMOKE_UPDATE_PORT:-}"
UPDATE_EXPECT_VERSION="${CRABFORK_INSTALL_SMOKE_UPDATE_EXPECT_VERSION:-}"
LATEST_DIR="$(mktemp -d)"
...
if [[ "$SKIP_UPDATE" == "1" ]]; then
  echo "==> Skip update smoke (CRABFORK_INSTALL_SMOKE_SKIP_UPDATE=1)"
else
  prepare_update_tarball
  prepare_update_host_access
  start_update_server
  ...
  docker run --rm -t \
    ...
    -e CRABFORK_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e CRABFORK_INSTALL_METHOD=npm \
    -e CRABFORK_INSTALL_FRESH_VERSION="$UPDATE_EXPECT_VERSION" \
    -e CRABFORK_INSTALL_FRESH_TAG_URL="$FRESH_TAG_URL" \
    -e CRABFORK_INSTALL_LATEST_OUT="/out/latest" \
    -e CRABFORK_NO_ONBOARD=1 \
    -e CRABFORK_NO_PROMPT=1 \
  ...
  docker run --rm -t \
    ...
    "${UPDATE_DOCKER_HOST_ARGS[@]}" \
    -e CRABFORK_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e CRABFORK_INSTALL_SMOKE_MODE=update \
    -e CRABFORK_INSTALL_UPDATE_BASELINE="$UPDATE_BASELINE_VERSION" \
    -e CRABFORK_INSTALL_UPDATE_BASELINE_TAG_URL="$BASELINE_TAG_URL" \
    -e CRABFORK_INSTALL_UPDATE_EXPECT_VERSION="$UPDATE_EXPECT_VERSION" \
    -e CRABFORK_INSTALL_UPDATE_TAG_URL="$UPDATE_TAG_URL" \
    -e CRABFORK_NO_ONBOARD=1 \
    -e CRABFORK_NO_PROMPT=1 \
    -e DEBIAN_FRONTEND=noninteractive \
    "$SMOKE_IMAGE"
...
if [[ "$SKIP_NONROOT" == "1" ]]; then
  echo "==> Skip non-root installer smoke (CRABFORK_INSTALL_SMOKE_SKIP_NONROOT=1)"
else
  ...
  docker run --rm -t \
    ...
    -e CRABFORK_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e CRABFORK_INSTALL_METHOD=npm \
    -e CRABFORK_INSTALL_EXPECT_VERSION="$LATEST_VERSION" \
    -e CRABFORK_NO_ONBOARD=1 \
    -e CRABFORK_NO_PROMPT=1 \
    -e DEBIAN_FRONTEND=noninteractive \
    "$NONROOT_IMAGE"
...
if [[ "${CRABFORK_INSTALL_SMOKE_SKIP_CLI:-0}" == "1" ]]; then
  echo "==> Skip CLI installer smoke (CRABFORK_INSTALL_SMOKE_SKIP_CLI=1)"
  exit 0
fi
...
docker run --rm -t \
  ...
  --entrypoint /bin/bash \
  -e CRABFORK_INSTALL_URL="$INSTALL_URL" \
  -e CRABFORK_INSTALL_CLI_URL="$CLI_INSTALL_URL" \
  -e CRABFORK_NO_ONBOARD=1 \
  -e CRABFORK_NO_PROMPT=1 \
  -e DEBIAN_FRONTEND=noninteractive \
