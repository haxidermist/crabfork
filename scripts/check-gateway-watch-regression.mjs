...
const WATCH_GATEWAY_SKIP_ENV = {
  ...
  CRABFORK_SKIP_ACPX_RUNTIME: "1",
  CRABFORK_SKIP_ACPX_RUNTIME_PROBE: "1",
...
function buildTimedWatchCommand(pidFilePath, timeFilePath, isolatedHomeDir, port) {
  const shellSource = [
    'echo "$$" > "$CRABFORK_WATCH_PID_FILE"',
    'mkdir -p "$CRABFORK_HOME/.crabfork"',
    `printf '%s\n' '{"gateway":{"controlUi":{"enabled":false}}}' > "$CRABFORK_HOME/.crabfork/crabfork.json"`,
    `exec node scripts/watch-node.mjs gateway --force --allow-unconfigured --port ${String(port)} --token watch-regression-token`,
  ].join("\n");
  const env = {
    CRABFORK_WATCH_PID_FILE: pidFilePath,
    HOME: isolatedHomeDir,
    CRABFORK_HOME: isolatedHomeDir,
...
async function runTimedWatch(options, outputDir) {
  ...
  const isolatedHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), "crabfork-gateway-watch-"));
