...
const { clearPluginCommands, getPluginCommandSpecs, loadCrabforkPlugins, matchPluginCommand } =
  await import(pathToFileURL(smokeEntryPath).href);

assert.equal(typeof loadCrabforkPlugins, "function", "built loader export missing");
...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "crabfork-build-smoke-"));
...
fs.writeFileSync(
  ...
  JSON.stringify(
    {
      name: "@crabfork/build-smoke-plugin",
      type: "module",
      crabfork: {
        extensions: ["./index.js"],
      },
...
fs.writeFileSync(
  path.join(distPluginDir, "crabfork.plugin.json"),
  JSON.stringify(
    {
      id: pluginId,
      configSchema: {
...
fs.writeFileSync(
  ...
  [
    "import sdk from 'crabfork/plugin-sdk';",
...
const registry = loadCrabforkPlugins({
  ...
  env: {
    ...process.env,
    CRABFORK_BUNDLED_PLUGINS_DIR: path.join(tempRoot, "dist-runtime", "extensions"),
