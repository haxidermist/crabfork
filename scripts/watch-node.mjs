...
const logWatcher = (message, deps) => {
  deps.process.stderr?.write?.(`[crabfork] ${message}\n`);
};
...
export async function runWatchMain(params = {}) {
  ...
  childEnv.CRABFORK_WATCH_MODE = "1";
  childEnv.CRABFORK_WATCH_SESSION = watchSession;
  ...
  childEnv.CRABFORK_NO_RESPAWN = "1";
  if (deps.args.length > 0) {
    childEnv.CRABFORK_WATCH_COMMAND = deps.args.join(" ");
  }
