...
# Crabfork Auth Widget for Termux
...
SERVER="${CRABFORK_SERVER:-l36}"

# Check auth status
termux-toast "Checking Crabfork auth..."

STATUS=$(ssh "$SERVER" '$HOME/crabfork/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/crabfork/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|CRABFORK_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/crabfork/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|CRABFORK_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: Crabfork won't work until re-auth"
                ;;
        esac
        ;;
