...
- regenerates apps/ios/Crabfork.xcodeproj via xcodegen
EOF
}
...
PUSH_RELAY_BASE_URL="${CRABFORK_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid CRABFORK_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid CRABFORK_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid CRABFORK_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid CRABFORK_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
if [[ -z "${PUSH_RELAY_BASE_URL}" ]]; then
  echo "Missing CRABFORK_PUSH_RELAY_BASE_URL (or IOS_PUSH_RELAY_BASE_URL) for beta relay registration." >&2
  exit 1
fi
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(CRABFORK_URL_SLASH)$(CRABFORK_URL_SLASH)#g'
)"
...
CRABFORK_CODE_SIGN_STYLE = Automatic
CRABFORK_DEVELOPMENT_TEAM = ${TEAM_ID}
CRABFORK_IOS_SELECTED_TEAM = ${TEAM_ID}
CRABFORK_APP_BUNDLE_ID = ai.crabfork.client
CRABFORK_SHARE_BUNDLE_ID = ai.crabfork.client.share
CRABFORK_ACTIVITY_WIDGET_BUNDLE_ID = ai.crabfork.client.activitywidget
CRABFORK_WATCH_APP_BUNDLE_ID = ai.crabfork.client.watchkitapp
CRABFORK_WATCH_EXTENSION_BUNDLE_ID = ai.crabfork.client.watchkitapp.extension
CRABFORK_APP_PROFILE =
CRABFORK_SHARE_PROFILE =
CRABFORK_PUSH_TRANSPORT = relay
CRABFORK_PUSH_DISTRIBUTION = official
CRABFORK_URL_SLASH = /
CRABFORK_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
CRABFORK_PUSH_APNS_ENVIRONMENT = production
EOF
