...
DEFAULT_PROVIDER="${CRABFORK_DOCKER_CLI_BACKEND_PROVIDER:-claude-cli}"
CLI_MODEL="${CRABFORK_LIVE_CLI_BACKEND_MODEL:-}"
CLI_PROVIDER="${CLI_MODEL%%/*}"
CLI_DISABLE_MCP_CONFIG="${CRABFORK_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG:-}"
CLI_AUTH_MODE="${CRABFORK_LIVE_CLI_BACKEND_AUTH:-auto}"
...
case "$CLI_AUTH_MODE" in
  auto | api-key | subscription)
    ;;
  *)
    echo "ERROR: CRABFORK_LIVE_CLI_BACKEND_AUTH must be one of: auto, api-key, subscription." >&2
    exit 1
    ;;
esac

if [[ "$CLI_AUTH_MODE" == "subscription" && "$CLI_PROVIDER" != "claude-cli" ]]; then
  echo "ERROR: CRABFORK_LIVE_CLI_BACKEND_AUTH=subscription is only supported for claude-cli." >&2
  exit 1
fi
...
if [[ "$CLI_PROVIDER" == "claude-cli" && "$CLI_AUTH_MODE" == "subscription" ]]; then
  ...
  if [[ -z "${CRABFORK_LIVE_CLI_BACKEND_PRESERVE_ENV:-}" ]]; then
    if [[ "$CLAUDE_SUBSCRIPTION_AUTH_SOURCE" == "env-token" ]]; then
      export CRABFORK_LIVE_CLI_BACKEND_PRESERVE_ENV='["CLAUDE_CODE_OAUTH_TOKEN"]'
    else
      export CRABFORK_LIVE_CLI_BACKEND_PRESERVE_ENV="[]"
    fi
  fi
  if [[ "$CRABFORK_LIVE_CLI_BACKEND_PRESERVE_ENV" == *ANTHROPIC_API_KEY* ]]; then
    echo "ERROR: subscription auth smoke must not preserve Anthropic API-key env vars." >&2
    exit 1
  fi
  if [[ "$CLAUDE_SUBSCRIPTION_AUTH_SOURCE" == "env-token" && "$CRABFORK_LIVE_CLI_BACKEND_PRESERVE_ENV" != *CLAUDE_CODE_OAUTH_TOKEN* ]]; then
  ...
  export CRABFORK_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE="${CRABFORK_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE:-0}"
  export CRABFORK_LIVE_CLI_BACKEND_RESUME_PROBE="${CRABFORK_LIVE_CLI_BACKEND_RESUME_PROBE:-1}"
  export CRABFORK_LIVE_CLI_BACKEND_IMAGE_PROBE="${CRABFORK_LIVE_CLI_BACKEND_IMAGE_PROBE:-0}"
  export CRABFORK_LIVE_CLI_BACKEND_MCP_PROBE="${CRABFORK_LIVE_CLI_BACKEND_MCP_PROBE:-0}"
fi
...
if [[ -n "${CRABFORK_DOCKER_AUTH_DIRS:-}" ]]; then
  ...
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
  done < <(crabfork_live_collect_auth_dirs_from_csv "$CLI_PROVIDER")
  while IFS= read -r auth_file; do
    [[ -n "$auth_file" ]] || continue
    AUTH_FILES+=("$auth_file")
  done < <(crabfork_live_collect_auth_files_from_csv "$CLI_PROVIDER")
fi
AUTH_DIRS_CSV=""
if ((${#AUTH_DIRS[@]} > 0)); then
...
provider="${CRABFORK_DOCKER_CLI_BACKEND_PROVIDER:-claude-cli}"
default_command="${CRABFORK_DOCKER_CLI_BACKEND_COMMAND_DEFAULT:-}"
docker_package="${CRABFORK_DOCKER_CLI_BACKEND_NPM_PACKAGE:-}"
binary_name="${CRABFORK_DOCKER_CLI_BACKEND_BINARY_NAME:-}"
if [ -z "$binary_name" ] && [ -n "$default_command" ]; then
  binary_name="$(basename "$default_command")"
fi
if [ -z "${CRABFORK_LIVE_CLI_BACKEND_COMMAND:-}" ] && [ -n "$binary_name" ]; then
  export CRABFORK_LIVE_CLI_BACKEND_COMMAND="$HOME/.npm-global/bin/$binary_name"
fi
if [ -n "${CRABFORK_LIVE_CLI_BACKEND_COMMAND:-}" ] && [ ! -x "${CRABFORK_LIVE_CLI_BACKEND_COMMAND}" ] && [ -n "$docker_package" ]; then
  npm_config_prefix="$HOME/.npm-global" npm install -g "$docker_package"
fi
if [ "$provider" = "claude-cli" ]; then
  auth_mode="${CRABFORK_LIVE_CLI_BACKEND_AUTH:-auto}"
...
if [ -n "\${CRABFORK_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY:-}" ]; then
  export ANTHROPIC_API_KEY="\${CRABFORK_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY}"
fi
if [ -n "\${CRABFORK_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY_OLD:-}" ]; then
  export ANTHROPIC_API_KEY_OLD="\${CRABFORK_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY_OLD}"
fi
...
WRAP
  ...
  if [ -z "${CRABFORK_LIVE_CLI_BACKEND_PRESERVE_ENV:-}" ]; then
    export CRABFORK_LIVE_CLI_BACKEND_PRESERVE_ENV='["ANTHROPIC_API_KEY","ANTHROPIC_API_KEY_OLD"]'
  fi
  if [ "$auth_mode" = "subscription" ]; then
    claude --version
    direct_token="CRABFORK-CLAUDE-SUBSCRIPTION-DIRECT"
    direct_output="$(
      claude \
...
if [[ "${CRABFORK_SKIP_DOCKER_BUILD:-}" == "1" ]]; then
  echo "==> Reuse live-test image: $LIVE_IMAGE_NAME (CRABFORK_SKIP_DOCKER_BUILD=1)"
else
...
DOCKER_AUTH_ENV=(
  -e CRABFORK_LIVE_CLI_BACKEND_AUTH="$CLI_AUTH_MODE"
)
if [[ "$CLI_PROVIDER" == "claude-cli" && "$CLI_AUTH_MODE" == "subscription" ]]; then
  DOCKER_AUTH_ENV+=(
    -e CLAUDE_CODE_OAUTH_TOKEN="${CLAUDE_CODE_OAUTH_TOKEN:-}"
    -e CRABFORK_LIVE_CLI_BACKEND_PRESERVE_ENV="$CRABFORK_LIVE_CLI_BACKEND_PRESERVE_ENV"
  )
else
  DOCKER_AUTH_ENV+=(
    -e ANTHROPIC_API_KEY
    -e ANTHROPIC_API_KEY_OLD
    -e CRABFORK_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY="${ANTHROPIC_API_KEY:-}"
    -e CRABFORK_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY_OLD="${ANTHROPIC_API_KEY_OLD:-}"
    -e CRABFORK_LIVE_CLI_BACKEND_PRESERVE_ENV="${CRABFORK_LIVE_CLI_BACKEND_PRESERVE_ENV:-}"
  )
fi
...
docker run --rm -t \
  ...
  -e CRABFORK_DOCKER_CLI_BACKEND_PROVIDER="$CLI_PROVIDER" \
  -e CRABFORK_DOCKER_CLI_BACKEND_COMMAND_DEFAULT="$CLI_DEFAULT_COMMAND" \
  -e CRABFORK_DOCKER_CLI_BACKEND_NPM_PACKAGE="$CLI_DOCKER_NPM_PACKAGE" \
  -e CRABFORK_DOCKER_CLI_BACKEND_BINARY_NAME="$CLI_DOCKER_BINARY_NAME" \
  -e CRABFORK_LIVE_TEST=1 \
  -e CRABFORK_LIVE_CLI_BACKEND=1 \
  -e CRABFORK_LIVE_CLI_BACKEND_DEBUG="${CRABFORK_LIVE_CLI_BACKEND_DEBUG:-}" \
  -e CRABFORK_CLI_BACKEND_LOG_OUTPUT="${CRABFORK_CLI_BACKEND_LOG_OUTPUT:-}" \
  -e CRABFORK_LIVE_CLI_BACKEND_MODEL="$CLI_MODEL" \
  -e CRABFORK_LIVE_CLI_BACKEND_COMMAND="${CRABFORK_LIVE_CLI_BACKEND_COMMAND:-}" \
  -e CRABFORK_LIVE_CLI_BACKEND_ARGS="${CRABFORK_LIVE_CLI_BACKEND_ARGS:-}" \
  -e CRABFORK_LIVE_CLI_BACKEND_CLEAR_ENV="${CRABFORK_LIVE_CLI_BACKEND_CLEAR_ENV:-}" \
  -e CRABFORK_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG="$CLI_DISABLE_MCP_CONFIG" \
  -e CRABFORK_LIVE_CLI_BACKEND_RESUME_PROBE="${CRABFORK_LIVE_CLI_BACKEND_RESUME_PROBE:-}" \
  -e CRABFORK_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE="${CRABFORK_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE:-}" \
  -e CRABFORK_LIVE_CLI_BACKEND_IMAGE_PROBE="${CRABFORK_LIVE_CLI_BACKEND_IMAGE_PROBE:-}" \
  -e CRABFORK_LIVE_CLI_BACKEND_MCP_PROBE="${CRABFORK_LIVE_CLI_BACKEND_MCP_PROBE:-}" \
  -e CRABFORK_LIVE_CLI_BACKEND_IMAGE_ARG="${CRABFORK_LIVE_CLI_BACKEND_IMAGE_ARG:-}" \
  -e CRABFORK_LIVE_CLI_BACKEND_IMAGE_MODE="${CRABFORK_LIVE_CLI_BACKEND_IMAGE_MODE:-}" \
  -v "$ROOT_DIR":/src:ro \
