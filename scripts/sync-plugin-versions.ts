...
const CRABFORK_VERSION_RANGE_RE = /^>=\d{4}\.\d{1,2}\.\d{1,2}(?:[-.][^"\s]+)?$/u;

function syncCrabforkDependencyRange(
  deps: Record<string, string> | undefined,
  targetVersion: string,
): boolean {
  const current = deps?.crabfork;
  if (!current || current === "workspace:*" || !CRABFORK_VERSION_RANGE_RE.test(current)) {
    return false;
  }
...
function syncPluginApiVersion(pkg: PackageJson, targetVersion: string): boolean {
  const compat = pkg.crabfork?.compat;
  const current = compat?.pluginApi;
  if (!current || !CRABFORK_VERSION_RANGE_RE.test(current)) {
    return false;
  }
  const next = `>=${targetVersion}`;
  if (current === next) {
...
function syncBuildCrabforkVersion(pkg: PackageJson, targetVersion: string): boolean {
  const build = pkg.crabfork?.build;
  const current = build?.crabforkVersion;
  if (!current) {
    return false;
  }
  ...
  build.crabforkVersion = targetVersion;
  return true;
}
...
function ensureChangelogEntry(changelogPath: string, version: string): boolean {
  ...
  const entry = `## ${version}\n\n### Changes\n- Version alignment with core Crabfork release numbers.\n\n`;
...
export function syncPluginVersions(rootDir = resolve(".")) {
  ...
  for (const dir of dirs) {
    ...
    const devDependencyChanged = syncCrabforkDependencyRange(pkg.devDependencies, targetVersion);
    const peerDependencyChanged = syncCrabforkDependencyRange(pkg.peerDependencies, targetVersion);
    ...
    const buildCrabforkVersionChanged = syncBuildCrabforkVersion(pkg, targetVersion);
    const packageChanged =
      versionChanged ||
      devDependencyChanged ||
      peerDependencyChanged ||
      pluginApiChanged ||
      buildCrabforkVersionChanged;
    if (!packageChanged) {
