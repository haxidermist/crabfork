...
run_protocol_ci_mirror() {
  local targets=(
    ...
    "apps/macos/Sources/CrabforkProtocol/GatewayModels.swift"
    "apps/shared/CrabforkKit/Sources/CrabforkProtocol/GatewayModels.swift"
  )
  local before after
...
has_native_swift_changes() {
  if git rev-parse --verify --quiet origin/main >/dev/null; then
    if git diff --name-only --relative origin/main...HEAD -- apps/macos apps/ios apps/shared/CrabforkKit | rg -q .; then
      return 0
    fi
  fi

  if git rev-parse --verify --quiet HEAD^ >/dev/null; then
    git diff --name-only --relative HEAD^..HEAD -- apps/macos apps/ios apps/shared/CrabforkKit | rg -q .
    return $?
  fi

  git show --name-only --relative --pretty='' HEAD -- apps/macos apps/ios apps/shared/CrabforkKit | rg -q .
}
...
run_linux_ci_mirror() {
  ...
  log_step "CRABFORK_VITEST_MAX_WORKERS=${CRABFORK_VITEST_MAX_WORKERS:-1} NODE_OPTIONS=${NODE_OPTIONS:---max-old-space-size=6144} pnpm test"
  CRABFORK_VITEST_MAX_WORKERS="${CRABFORK_VITEST_MAX_WORKERS:-1}" \
  NODE_OPTIONS="${NODE_OPTIONS:---max-old-space-size=6144}" \
    pnpm test
}

run_macos_ci_mirror() {
  if [[ "${CRABFORK_PREPUSH_SKIP_MACOS:-0}" == "1" ]]; then
    log_step "Skipping macOS mirror because CRABFORK_PREPUSH_SKIP_MACOS=1"
    return 0
  fi
