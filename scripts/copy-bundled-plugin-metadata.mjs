...
function isManifestlessBundledRuntimeSupportPackage(params) {
  ...
  if (packageName !== `@crabfork/${params.dirName}`) {
    return false;
  }
...
export function copyBundledPluginMetadata(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const distManifestPath = path.join(distPluginDir, "crabfork.plugin.json");
    ...
    if (packageJson.crabfork && "extensions" in packageJson.crabfork) {
      packageJson.crabfork = {
        ...packageJson.crabfork,
        extensions: rewritePackageExtensions(packageJson.crabfork.extensions),
        ...(typeof packageJson.crabfork.setupEntry === "string"
          ? { setupEntry: rewritePackageEntry(packageJson.crabfork.setupEntry) }
          : {}),
      };
    }
