...
function collectPluginSourceEntries(packageJson) {
  let packageEntries = Array.isArray(packageJson?.crabfork?.extensions)
    ? packageJson.crabfork.extensions.filter(
        (entry) => typeof entry === "string" && entry.trim().length > 0,
      )
    : [];
  const setupEntry =
    typeof packageJson?.crabfork?.setupEntry === "string" &&
    packageJson.crabfork.setupEntry.trim().length > 0
      ? packageJson.crabfork.setupEntry
      : undefined;
  if (setupEntry) {
...
export function listBundledPluginPackArtifacts(params = {}) {
  ...
  for (const { id, hasManifest, hasPackageJson, sourceEntries } of entries) {
    if (hasManifest) {
      artifacts.add(bundledDistPluginFile(id, "crabfork.plugin.json"));
    }
    if (hasPackageJson) {
