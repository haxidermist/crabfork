...
# Build and bundle Crabfork into a minimal .app we can open.
# Outputs to dist/Crabfork.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Crabfork.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Crabfork"
BUNDLE_ID="${BUNDLE_ID:-ai.crabfork.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/crabfork/crabfork/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Crabfork/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :CrabforkBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :CrabforkGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Crabfork"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Crabfork"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Crabfork"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Crabfork" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Crabfork/Resources/Crabfork.icns" "$APP_ROOT/Contents/Resources/Crabfork.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Crabfork/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying CrabforkKit resources"
CRABFORKKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/CrabforkKit_CrabforkKit.bundle"
if [ -d "$CRABFORKKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/CrabforkKit_CrabforkKit.bundle"
  cp -R "$CRABFORKKIT_BUNDLE" "$APP_ROOT/Contents/Resources/CrabforkKit_CrabforkKit.bundle"
else
  echo "WARN: CrabforkKit resource bundle not found at $CRABFORKKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Crabfork"
killall -q Crabfork 2>/dev/null || true
