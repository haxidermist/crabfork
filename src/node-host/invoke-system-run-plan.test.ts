...
function withFakeRuntimeBin<T>(params: { binName: string; run: () => T }): T {
  return withFakeRuntimeBins({
    ...
    tmpPrefix: `crabfork-${params.binName}-bin-`,
    run: params.run,
  });
}
...
function withFakeRuntimeBins<T>(params: {
  ...
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), params.tmpPrefix ?? "crabfork-runtime-bins-"));
...
const unsafeRuntimeInvocationCases: UnsafeRuntimeInvocationCase[] = [
  ...
  },
  {
    ...
    binName: "node",
    tmpPrefix: "crabfork-node-import-inline-",
  ...
  },
  {
    ...
    binName: "ruby",
    tmpPrefix: "crabfork-ruby-require-",
  ...
  },
  {
    ...
    binName: "ruby",
    tmpPrefix: "crabfork-ruby-load-path-",
  ...
  },
  {
    ...
    binName: "perl",
    tmpPrefix: "crabfork-perl-module-preload-",
  ...
  },
  {
    ...
    binName: "perl",
    tmpPrefix: "crabfork-perl-load-path-",
  ...
  },
  {
    ...
    binName: "perl",
    tmpPrefix: "crabfork-perl-preload-load-path-",
  ...
  },
  {
    ...
    binName: "node",
    tmpPrefix: "crabfork-inline-shell-node-",
  ...
  },
  {
    ...
    binName: "pnpm",
    tmpPrefix: "crabfork-pnpm-dlx-unknown-flag-",
  ...
  },
  {
    ...
    binName: "pnpm",
    tmpPrefix: "crabfork-pnpm-dlx-unknown-prefix-",
  ...
  },
  {
    ...
    binName: "pnpm",
    tmpPrefix: "crabfork-pnpm-dlx-unknown-prefix-value-",
  ...
  },
  {
    ...
    binName: "pnpm",
    tmpPrefix: "crabfork-pnpm-dlx-global-double-dash-",
    command: ["pnpm", "--", "dlx", "--future-flag", "tsx", "./run.ts"],
    setup: (tmp) => {
      fs.writeFileSync(path.join(tmp, "run.ts"), 'console.log("SAFE")\n');
    },
...
describe("hardenApprovedExecutionPaths", () => {
  ...
  it.runIf(process.platform !== "win32").each(cases)("$name", (testCase) => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "crabfork-approval-hardening-"));
    const oldPath = process.env.PATH;
      ...
      withFakeRuntimeBins({
        ...
        run: () => {
          withScriptOperandPlanFixture(
            {
              tmpPrefix: "crabfork-approval-script-plan-",
              fixture: runtimeCase,
  ...
  it("detects rewritten script operands for pnpm dlx approval plans", () => {
    withFakeRuntimeBins({
      ...
      run: () => {
        withScriptOperandPlanFixture(
          {
            tmpPrefix: "crabfork-pnpm-dlx-approval-",
            fixture: {
  ...
  it("does not bind pnpm dlx shell-mode commands to a mutable file operand", () => {
    withFakeRuntimeBins({
      binNames: ["pnpm", "tsx"],
      run: () => {
        const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "crabfork-pnpm-dlx-shell-mode-"));
        try {
          fs.writeFileSync(path.join(tmp, "run.ts"), 'console.log("SAFE");\n');
          expect(
            resolveMutableFileOperandSnapshotSync({
  ...
  it("allows pnpm dlx package binaries that do not bind a mutable local file", () => {
    withFakeRuntimeBin({
      binName: "pnpm",
      run: () => {
        const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "crabfork-pnpm-dlx-package-bin-"));
        try {
    ...
    withFakeRuntimeBin({
      ...
      run: () => {
        const tmp = fs.mkdtempSync(
          path.join(os.tmpdir(), "crabfork-pnpm-dlx-package-runtime-token-"),
        );
        try {
  ...
  it("allows pnpm dlx package binaries with multi-token data-like runtime names", () => {
    withFakeRuntimeBin({
      ...
      run: () => {
        const tmp = fs.mkdtempSync(
          path.join(os.tmpdir(), "crabfork-pnpm-dlx-package-runtime-token-multi-"),
        );
        try {
  ...
  it("allows pnpm dlx package binaries with local file arguments", () => {
    withFakeRuntimeBins({
      ...
      run: () => {
        const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "crabfork-pnpm-dlx-package-file-"));
        try {
  ...
  it("allows pnpm dlx package binaries with interpreter-like data tails", () => {
    withFakeRuntimeBin({
      binName: "pnpm",
      run: () => {
        const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "crabfork-pnpm-dlx-package-data-tail-"));
        try {
          fs.writeFileSync(path.join(tmp, "run.ts"), 'console.log("SAFE");\n');
  ...
  it("treats -- as the end of pnpm dlx option parsing", () => {
    withFakeRuntimeBins({
      ...
      run: () => {
        withScriptOperandPlanFixture(
          {
            tmpPrefix: "crabfork-pnpm-dlx-double-dash-",
            fixture: {
  ...
  it("captures the real shell script operand after value-taking shell flags", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "crabfork-shell-option-value-"));
    try {
