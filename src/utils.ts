...
export function resolveConfigDir(
  ...
  const configPath = env.CRABFORK_CONFIG_PATH?.trim();
  if (configPath) {
    return path.dirname(resolveUserPath(configPath, env, homedir));
  }
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".crabfork");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.CRABFORK_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$CRABFORK_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via CRABFORK_STATE_DIR.
