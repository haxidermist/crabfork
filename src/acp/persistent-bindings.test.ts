...
type ConfiguredBinding = NonNullable<CrabforkConfig["bindings"]>[number];
...
function createCfgWithBindings(
  ...
  overrides?: Partial<CrabforkConfig>,
): CrabforkConfig {
  return {
    ...baseCfg,
    ...overrides,
...
function resolveBindingRecord(cfg: CrabforkConfig, overrides: Partial<BindingRecordInput> = {}) {
...
describe("resolveConfiguredAcpBindingRecord", () => {
  ...
  it("derives configured binding cwd from an explicit agent workspace", () => {
    const cfg = createCfgWithBindings(
      ...
      {
        agents: {
          list: [{ id: "codex", workspace: "/workspace/crabfork" }, { id: "claude" }],
        },
      },
    );
    const resolved = resolveBindingRecord(cfg);
