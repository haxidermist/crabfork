...
const createTempDir = () => tempDirs.make("crabfork-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets CRABFORK_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "crabfork",
    });

    expect(env.CRABFORK_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("crabfork");
  });

  it("overrides pre-existing CRABFORK_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      CRABFORK_SHELL: "wrong",
    });
    expect(env.CRABFORK_SHELL).toBe("acp-client");
  });
  ...
  it("preserves CRABFORK_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        CRABFORK_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["CRABFORK_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default Crabfork bridge", () => {
    ...
    expect(env.CRABFORK_API_KEY).toBe("keep-me");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.CRABFORK_SHELL).toBe("acp-client");
  });
...
describe("shouldStripProviderAuthEnvVarsForAcpServer", () => {
  it("strips provider auth env vars for the default bridge", () => {
    ...
    expect(
      shouldStripProviderAuthEnvVarsForAcpServer({
        serverCommand: "crabfork",
        serverArgs: ["acp"],
        defaultServerCommand: "crabfork",
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("CRABFORK_API_KEY")).toBe(false);
  });
});
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "crabfork", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "crabfork", "dist", "entry.js");
    const shimPath = path.join(dir, "crabfork.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\crabfork\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/crabfork-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/crabfork-acp-cwd",
    });
  ...
  it("prompts for read when rawInput path escapes cwd via traversal", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/crabfork-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);
