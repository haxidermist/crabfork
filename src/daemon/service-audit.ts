...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds CRABFORK_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `crabfork gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service CRABFORK_GATEWAY_TOKEN does not match gateway.auth.token in crabfork.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (command.environmentValueSources?.CRABFORK_GATEWAY_TOKEN === "file") {
    return undefined;
  }
  return normalizeOptionalString(command.environment?.CRABFORK_GATEWAY_TOKEN);
}
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `crabfork gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
