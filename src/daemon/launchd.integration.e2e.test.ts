...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `crabfork-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      CRABFORK_LAUNCHD_LABEL: `ai.crabfork.launchd-int-${testId}`,
      CRABFORK_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
