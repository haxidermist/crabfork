...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when CRABFORK_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".crabfork", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when CRABFORK_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", CRABFORK_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".crabfork-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers CRABFORK_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        CRABFORK_PROFILE: "rescue",
        CRABFORK_STATE_DIR: "C:\\State\\crabfork",
      },
      expected: path.join("C:\\State\\crabfork", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", CRABFORK_PROFILE: "default" },
      expected: path.join("/home/test", ".crabfork", "gateway.cmd"),
    },
  ])("$name", ({ env, expected }) => {
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "crabfork-schtasks-test-"));
    try {
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          "rem Crabfork Gateway",
          "cd /d C:\\Projects\\crabfork",
          "set NODE_ENV=production",
          "set CRABFORK_PORT=18789",
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\crabfork",
          environment: {
            NODE_ENV: "production",
            CRABFORK_PORT: "18789",
          },
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\crabfork\\dist\\index.js gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "C:\\Program Files\\nodejs\\node.exe",
            "C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\crabfork\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\Crabfork Share\\node.exe" "\\\\fileserver\\Crabfork Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\Crabfork Share\\node.exe",
            "\\\\fileserver\\Crabfork Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from CRABFORK_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ CRABFORK_STATE_DIR: path.join(tmpDir, "custom-state") }),
