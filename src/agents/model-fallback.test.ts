...
function makeFallbacksOnlyCfg(): CrabforkConfig {
  return {
    agents: {
      defaults: {
        model: {
...
function makeProviderFallbackCfg(provider: string): CrabforkConfig {
  return makeCfg({
    agents: {
      defaults: {
        model: {
...
async function withTempAuthStore<T>(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "crabfork-auth-"));
...
// https://github.com/crabfork/crabfork/issues/23440
const INSUFFICIENT_QUOTA_PAYLOAD =
  '{"type":"error","error":{"type":"insufficient_quota","message":"Your account has insufficient quota balance to run this request."}}';
// Internal Crabfork compatibility marker, not a provider API contract.
...
describe("runWithModelFallback", () => {
  it("skips auth store bootstrap when no auth profile sources exist", async () => {
    ...
    try {
      const result = await runWithModelFallback({
        ...
        provider: "openai",
        model: "gpt-4.1-mini",
        agentDir: "/tmp/crabfork-no-auth-profiles",
        run,
      });

      expect(result.result).toBe("ok");
      expect(hasSourcesSpy).toHaveBeenCalledWith("/tmp/crabfork-no-auth-profiles");
  ...
  it("sanitizes model identifiers in model_not_found warnings", async () => {
    const warnLogs = createWarnLogCapture("crabfork-model-fallback-test");
    try {
      const cfg = makeCfg();
      const run = vi
        .fn()
  ...
  describe("fallback behavior with provider cooldowns", () => {
    async function makeAuthStoreWithCooldown(
      ...
      const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "crabfork-test-"));
      const now = Date.now();
      const store: AuthProfileStore = {
        version: AUTH_STORE_VERSION,
        profiles: {
