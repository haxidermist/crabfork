...
import { createCrabforkTools } from "./crabfork-tools.js";
...
function resolveExecConfig(params: { cfg?: CrabforkConfig; agentId?: string }) {
...
export function createCrabforkCodingTools(options?: {
  agentId?: string;
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createCrabforkReadTool(freshReadTool, {
        modelContextWindowTokens: options?.modelContextWindowTokens,
        imageSanitization,
      });
