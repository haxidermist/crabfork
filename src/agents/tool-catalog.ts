...
type CoreToolDefinition = {
  ...
  includeInCrabforkGroup?: boolean;
};
...
function buildCoreToolGroupMap() {
  ...
  const crabforkTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInCrabforkGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:crabfork": crabforkTools,
