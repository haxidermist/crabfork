...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("crabfork-bundled-");
  const workspaceDir = await createTempDir("crabfork-workspace-");
...
async function resolveBundledDiffsSkillEntries(config?: CrabforkConfig) {
