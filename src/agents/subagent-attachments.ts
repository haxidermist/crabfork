...
function resolveAttachmentLimits(config: CrabforkConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".crabfork", "attachments");
  const relDir = path.posix.join(".crabfork", "attachments", attachmentId);
