...
describe("provider attribution", () => {
  it("resolves the canonical Crabfork product and runtime version", () => {
  ...
  it("returns a documented OpenRouter attribution policy", () => {
    ...
    expect(policy).toEqual({
      ...
      reviewNote: "Documented app attribution headers. Verified in Crabfork runtime wrapper.",
      product: "Crabfork",
      version: "2026.3.22",
      headers: {
  ...
  it("returns a hidden-spec OpenAI attribution policy", () => {
    expect(resolveProviderAttributionPolicy("openai", { CRABFORK_VERSION: "2026.3.22" })).toEqual({
      provider: "openai",
      enabledByDefault: true,
      ...
      headers: {
        originator: "crabfork",
        version: "2026.3.22",
        "User-Agent": "crabfork/2026.3.22",
      },
    });
    expect(resolveProviderAttributionHeaders("openai", { CRABFORK_VERSION: "2026.3.22" })).toEqual({
      originator: "crabfork",
      version: "2026.3.22",
      "User-Agent": "crabfork/2026.3.22",
    });
  ...
  it("returns a hidden-spec OpenAI Codex attribution policy", () => {
    expect(
      resolveProviderAttributionPolicy("openai-codex", { CRABFORK_VERSION: "2026.3.22" }),
    ).toEqual({
  ...
  it("lists the current attribution support matrix", () => {
    expect(
      listProviderAttributionPolicies({ CRABFORK_VERSION: "2026.3.22" }).map((policy) => [
        policy.provider,
