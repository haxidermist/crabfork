...
async function resolveAllowedTmpMediaPath(params: {
  ...
  const crabForkTmpDir = path.resolve(resolvePreferredCrabforkTmpDir());
  if (!isPathInside(crabForkTmpDir, resolved)) {
    return undefined;
  }
  await assertNoTmpAliasEscape({ filePath: resolved, tmpRoot: crabForkTmpDir });
  return resolved;
}
