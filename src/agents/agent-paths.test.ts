...
describe("resolveCrabforkAgentDir", () => {
  const withTempStateDir = async (run: (stateDir: string) => void) => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "crabfork-agent-"));
    try {
      run(stateDir);
    } finally {
      ...
      withEnv(
        {
          ...
          CRABFORK_AGENT_DIR: undefined,
          PI_CODING_AGENT_DIR: undefined,
        },
        () => {
          const resolved = resolveCrabforkAgentDir();
  ...
  it("honors CRABFORK_AGENT_DIR overrides", async () => {
    await withTempStateDir((stateDir) => {
      const override = path.join(stateDir, "agent");
      withEnv(
        {
          CRABFORK_STATE_DIR: undefined,
          CRABFORK_AGENT_DIR: override,
          PI_CODING_AGENT_DIR: undefined,
        },
        () => {
          const resolved = resolveCrabforkAgentDir();
  ...
  it("honors PI_CODING_AGENT_DIR when CRABFORK_AGENT_DIR is unset", async () => {
    await withTempStateDir((stateDir) => {
  ...
  it("prefers CRABFORK_AGENT_DIR over PI_CODING_AGENT_DIR when both are set", async () => {
    await withTempStateDir((stateDir) => {
      ...
      withEnv(
        {
          CRABFORK_STATE_DIR: undefined,
          CRABFORK_AGENT_DIR: primaryOverride,
