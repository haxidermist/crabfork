...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["CRABFORK_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "crabfork-subagent-"));
  ...
  it("uses isolated temp state when CRABFORK_STATE_DIR is unset in tests", async () => {
    delete process.env.CRABFORK_STATE_DIR;
    const registryPath = resolveSubagentRegistryPath();
    expect(registryPath).toContain(path.join(os.tmpdir(), "crabfork-test-state"));
  });
});
