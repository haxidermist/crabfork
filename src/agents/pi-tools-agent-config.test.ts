...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "crabfork-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: CrabforkConfig) {
    return createCrabforkCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<CrabforkConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<CrabforkConfig["agents"]>["list"]>[number]["tools"];
  }): CrabforkConfig {
    return {
      tools: params.tools,
      agents: {
