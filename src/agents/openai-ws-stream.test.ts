...
describe("createOpenAIWebSocketStreamFn", () => {
  ...
  it("keeps native turn metadata stable across websocket retries and increments attempt", async () => {
    ...
    expect(firstPayload.metadata?.crabfork_session_id).toBe("sess-turn-metadata-retry");
    expect(firstPayload.metadata?.crabfork_transport).toBe("websocket");
    expect(firstPayload.metadata?.crabfork_turn_id).toBeTruthy();
    expect(secondPayload.metadata?.crabfork_turn_id).toBe(firstPayload.metadata?.crabfork_turn_id);
    expect(firstPayload.metadata?.crabfork_turn_attempt).toBe("1");
    expect(secondPayload.metadata?.crabfork_turn_attempt).toBe("2");
  });
