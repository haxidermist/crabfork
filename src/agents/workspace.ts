...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.CRABFORK_PROFILE?.trim();
  if (profile && normalizeOptionalLowercaseString(profile) !== "default") {
    return path.join(home, ".crabfork", `workspace-${profile}`);
  }
  return path.join(home, ".crabfork", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".crabfork";
