...
import { planCrabforkModelsJson } from "./models-config.plan.js";
...
function resolveModelsConfigInput(config?: CrabforkConfig): {
  config: CrabforkConfig;
  sourceConfigForSecrets: CrabforkConfig;
} {
...
export async function ensureCrabforkModelsJson(
  config?: CrabforkConfig,
  agentDirOverride?: string,
): Promise<{ agentDir: string; wrote: boolean }> {
  const resolved = resolveModelsConfigInput(config);
  const cfg = resolved.config;
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : resolveCrabforkAgentDir();
