...
describe("resolveAgentConfig", () => {
  ...
  it("should return undefined when agent id does not exist", () => {
    const cfg: CrabforkConfig = {
      agents: {
        list: [{ id: "main", workspace: "~/crabfork" }],
      },
    };
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: CrabforkConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: CrabforkConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: CrabforkConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgDisable: CrabforkConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: CrabforkConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: CrabforkConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
    ...
    const cfgAgentOverrideOnly: CrabforkConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/crabfork");
  });

  it("uses CRABFORK_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "crabfork-home");
    vi.stubEnv("CRABFORK_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as CrabforkConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".crabfork", "workspace"));
  });

  it("uses CRABFORK_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "crabfork-home");
    vi.stubEnv("CRABFORK_HOME", home);
    // Clear state dir so it falls back to CRABFORK_HOME
    vi.stubEnv("CRABFORK_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as CrabforkConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".crabfork", "agents", "main", "agent"));
  });
...
describe("resolveAgentIdByWorkspacePath", () => {
  it("returns the most specific workspace match for a directory", () => {
    const workspaceRoot = `/tmp/crabfork-agent-scope-${Date.now()}-root`;
  ...
  it("returns undefined when directory has no matching workspace", () => {
    ...
    expect(
      resolveAgentIdByWorkspacePath(cfg, `/tmp/crabfork-agent-scope-${Date.now()}-unrelated`),
    ).toBeUndefined();
  });

  it("matches workspace paths through symlink aliases", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "crabfork-agent-scope-"));
