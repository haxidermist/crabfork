...
export function installModelsConfigTestHooks(opts?: {
  ...
  let previousCrabforkAgentDir: string | undefined;
  ...
  beforeEach(() => {
    ...
    previousCrabforkAgentDir = process.env.CRABFORK_AGENT_DIR;
  ...
  afterEach(() => {
    ...
    if (previousCrabforkAgentDir === undefined) {
      delete process.env.CRABFORK_AGENT_DIR;
    } else {
      process.env.CRABFORK_AGENT_DIR = previousCrabforkAgentDir;
    }
...
const COPILOT_DISCOVERY_ENV_VARS = [
  ...
  "CRABFORK_TEST_ONLY_PROVIDER_PLUGIN_IDS",
];
...
export async function withUnsetCopilotTokenEnv<T>(fn: () => Promise<T>): Promise<T> {
  return withTempEnv(COPILOT_DISCOVERY_ENV_VARS, async () => {
    ...
    process.env.CRABFORK_TEST_ONLY_PROVIDER_PLUGIN_IDS = "github-copilot";
    return fn();
  });
}
...
export function snapshotImplicitProviderEnv(env?: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  snapshot.CRABFORK_BUNDLED_PLUGINS_DIR ??=
...
export async function resolveImplicitProvidersForTest(
  ...
  if (inferredPluginIds.length > 0) {
    env.CRABFORK_TEST_ONLY_PROVIDER_PLUGIN_IDS = inferredPluginIds.join(",");
  }
...
export const CUSTOM_PROXY_MODELS_CONFIG: CrabforkConfig = {
  models: {
    providers: {
      "custom-proxy": {
