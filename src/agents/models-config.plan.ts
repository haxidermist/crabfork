...
type ModelsConfig = NonNullable<CrabforkConfig["models"]>;
...
function resolveExplicitBaseUrlProviders(
  providers: CrabforkConfig["models"] | undefined,
): ReadonlySet<string> {
  return new Set(
...
export async function planCrabforkModelsJsonWithDeps(
  params: {
    cfg: CrabforkConfig;
    sourceConfigForSecrets?: CrabforkConfig;
    agentDir: string;
    env: NodeJS.ProcessEnv;
    existingRaw: string;
    existingParsed: unknown;
...
export async function planCrabforkModelsJson(
  params: Parameters<typeof planCrabforkModelsJsonWithDeps>[0],
): Promise<ModelsJsonPlan> {
  return planCrabforkModelsJsonWithDeps(params);
}
