...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("crabfork-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("crabfork-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("crabfork-sbx-");
