...
const GOOGLE_LIVE_SINGLE_KEY = "CRABFORK_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "CRABFORK_LIVE_ANTHROPIC_KEY",
    listVar: "CRABFORK_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "CRABFORK_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `CRABFORK_LIVE_${base}_KEY`;
