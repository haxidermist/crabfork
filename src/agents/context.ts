...
function isLikelyCrabforkCliProcess(argv: string[] = process.argv): boolean {
  const entryBasename = normalizeLowercaseStringOrEmpty(path.basename(argv[1] ?? ""));
  return (
    entryBasename === "crabfork" ||
    entryBasename === "crabfork.mjs" ||
...
function shouldEagerWarmContextWindowCache(argv: string[] = process.argv): boolean {
  // Keep this gate tied to the real Crabfork CLI entrypoints.
  //
  ...
  // built plugin-sdk can call ensureCrabforkModelsJson(), which cascades into
  // plugin discovery and breaks dist/source singleton assumptions.
  if (!isLikelyCrabforkCliProcess(argv)) {
    return false;
  }
...
function primeConfiguredContextWindows(): CrabforkConfig | undefined {
...
function ensureContextWindowCacheLoaded(): Promise<void> {
  ...
  CONTEXT_WINDOW_RUNTIME_STATE.loadPromise = (async () => {
    try {
      await (await loadModelsConfigRuntime()).ensureCrabforkModelsJson(cfg);
    } catch {
