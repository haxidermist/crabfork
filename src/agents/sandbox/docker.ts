...
import { markCrabforkExecEnv } from "../../infra/crabfork-exec-env.js";
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "crabfork.sandbox=1");
  args.push("--label", `crabfork.sessionKey=${params.scopeKey}`);
  args.push("--label", `crabfork.createdAtMs=${createdAtMs}`);
  args.push("--label", `crabfork.mountFormatVersion=${SANDBOX_MOUNT_FORMAT_VERSION}`);
  if (params.configHash) {
    args.push("--label", `crabfork.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markCrabforkExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "crabfork.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`crabfork sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`crabfork sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("crabfork sandbox recreate --all");
}
