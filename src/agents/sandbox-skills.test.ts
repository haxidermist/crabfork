...
vi.mock("./sandbox/docker.js", () => ({
  ensureSandboxContainer: vi.fn(async () => "crabfork-sbx-test"),
}));
...
describe("sandbox skill mirroring", () => {
  ...
  const runContext = async (workspaceAccess: "none" | "ro") => {
    const bundledDir = await fs.mkdtemp(path.join(os.tmpdir(), "crabfork-bundled-skills-"));
    await fs.mkdir(bundledDir, { recursive: true });

    process.env.CRABFORK_BUNDLED_SKILLS_DIR = bundledDir;
