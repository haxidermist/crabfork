...
let configOverride: CrabforkConfig = {
  session: {
    mainKey: "main",
    scope: "per-sender",
  },
...
function setConfigOverride(next: CrabforkConfig): void {
  configOverride = next;
...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    // See: https://github.com/crabfork/crabfork/issues/31298
    previousFastTestEnv = process.env.CRABFORK_TEST_FAST;
  ...
  afterAll(() => {
    ...
    process.env.CRABFORK_TEST_FAST = previousFastTestEnv;
  });

  beforeEach(() => {
    // CRABFORK_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("Crabfork runtime context (internal):");
