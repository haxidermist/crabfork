    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "crabfork.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "crabfork.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "crabfork.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("crabfork-");
  const acpxRoot = await tempDirs.make("crabfork-acpx-plugin-");
  const helperRoot = await tempDirs.make("crabfork-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("crabfork-");
  const pluginRoot = await tempDirs.make("crabfork-plugin-");
  const outsideDir = await tempDirs.make("crabfork-outside-");
...
describe("resolvePluginSkillDirs", () => {
  ...
  it("resolves Claude bundle command roots through the normal plugin skill path", async () => {
    const workspaceDir = await tempDirs.make("crabfork-");
    const pluginRoot = await tempDirs.make("crabfork-claude-bundle-");
  ...
  it("resolves enabled plugin skills through legacy manifest aliases", async () => {
    const workspaceDir = await tempDirs.make("crabfork-");
    const pluginRoot = await tempDirs.make("crabfork-legacy-plugin-");
    await fs.mkdir(path.join(pluginRoot, "skills"), { recursive: true });
