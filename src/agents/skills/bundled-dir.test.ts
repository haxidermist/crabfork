...
describe("resolveBundledSkillsDir", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["CRABFORK_BUNDLED_SKILLS_DIR"]);
  });
  ...
  it("returns CRABFORK_BUNDLED_SKILLS_DIR override when set", async () => {
    const overrideDir = await fs.mkdtemp(path.join(os.tmpdir(), "crabfork-bundled-override-"));
    process.env.CRABFORK_BUNDLED_SKILLS_DIR = ` ${overrideDir} `;
  ...
  it("resolves bundled skills under a flattened dist layout", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "crabfork-bundled-"));
    await fs.writeFile(path.join(root, "package.json"), JSON.stringify({ name: "crabfork" }));
