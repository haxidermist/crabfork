...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses CRABFORK_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      CRABFORK_HOME: "/srv/crabfork-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/crabfork-home"), ".crabfork", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".crabfork", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("crabfork-workspace-");
  ...
  it("migrates legacy onboardingCompletedAt markers to setupCompletedAt", async () => {
    const tempDir = await makeTempWorkspace("crabfork-workspace-");
    await fs.mkdir(path.join(tempDir, ".crabfork"), { recursive: true });
    await fs.writeFile(
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "crabfork-workspace-hardlink-"));
    try {
