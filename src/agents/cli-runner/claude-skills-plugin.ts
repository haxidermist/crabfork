...
const CRABFORK_CLAUDE_PLUGIN_NAME = "crabfork-skills";
...
export async function prepareClaudeCliSkillsPlugin(params: {
  ...
  const tempDir = await fs.mkdtemp(
    path.join(resolvePreferredCrabforkTmpDir(), "crabfork-claude-skills-"),
  );
  const pluginDir = path.join(tempDir, CRABFORK_CLAUDE_PLUGIN_NAME);
  ...
  const manifest = {
    name: CRABFORK_CLAUDE_PLUGIN_NAME,
    version: "0.0.0",
    description: "Session-scoped Crabfork skills selected for this agent run.",
    skills: "./skills",
  };
  await fs.writeFile(
