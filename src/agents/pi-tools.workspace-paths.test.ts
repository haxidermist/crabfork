...
import "./test-helpers/fast-crabfork-tools.js";
...
describe("workspace path resolution", () => {
  it("resolves relative read/write/edit paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("crabfork-ws-", async (workspaceDir) => {
      await withTempDir("crabfork-cwd-", async (otherDir) => {
        const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(otherDir);
        try {
          const tools = createCrabforkCodingTools({ workspaceDir });
          ...
          await editTool.execute("ws-edit", {
            path: editFile,
            edits: [{ oldText: "world", newText: "crabfork" }],
          });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("crabfork-ws-", async (workspaceDir) => {
      await withTempDir("crabfork-override-", async (overrideDir) => {
        const execTool = createExecTool(workspaceDir);
  ...
  it("rejects @-prefixed absolute paths outside workspace when workspaceOnly is enabled", async () => {
    await withTempDir("crabfork-ws-", async (workspaceDir) => {
      const cfg: CrabforkConfig = { tools: { fs: { workspaceOnly: true } } };
      const tools = createCrabforkCodingTools({ workspaceDir, config: cfg });
      const { readTool } = expectReadWriteEditTools(tools);

      const outsideAbsolute = path.resolve(path.parse(workspaceDir).root, "outside-crabfork.txt");
      await expect(
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("crabfork-sandbox-", async (sandboxDir) => {
      await withTempDir("crabfork-workspace-", async (workspaceDir) => {
        ...
        const tools = createCrabforkCodingTools({ workspaceDir, sandbox });
        const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
