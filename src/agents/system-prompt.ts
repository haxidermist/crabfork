...
function buildWebchatCanvasSection(params: {
  ...
  return [
    ...
    '- You may also use an explicit hosted URL: `[embed url="/__crabfork__/canvas/documents/cv_123/index.html" title="Status" height="320" /]`.',
    '- Never use local filesystem paths or `file://...` URLs in `[embed ...]`. Hosted embeds must point at `/__crabfork__/canvas/...` URLs or use `ref="..."`.',
    ...
    "- Quote all attribute values. Prefer `ref` for hosted documents unless you already have the full `/__crabfork__/canvas/documents/<id>/index.html` URL.",
    "",
  ];
}
...
function buildMessagingSection(params: {
  ...
  return [
    ...
    "- Never use exec/curl for provider messaging; Crabfork handles all routing internally.",
...
function buildDocsSection(params: { docsPath?: string; isMinimal: boolean; readToolName: string }) {
  ...
  return [
    "## Documentation",
    `Crabfork docs: ${docsPath}`,
    "Mirror: https://docs.crabfork.ai",
    "Source: https://github.com/crabfork/crabfork",
    "Community: https://discord.com/invite/clawd",
    "Find new skills: https://clawhub.ai",
    "For Crabfork behavior, commands, config, or architecture: consult local docs first.",
    "When diagnosing issues, run `crabfork status` yourself when possible; only ask the user if you lack access (e.g., sandboxed).",
    "",
  ];
}
...
export function buildAgentSystemPrompt(params: {
  ...
  const coreToolSummaries: Record<string, string> = {
    ...
    gateway: "Restart, apply config, or run updates on the running Crabfork process",
    agents_list: acpSpawnRuntimeEnabled
      ? 'List Crabfork agent ids allowed for sessions_spawn when runtime="subagent" (not ACP harness ids)'
      : "List Crabfork agent ids allowed for sessions_spawn",
  ...
  if (promptMode === "none") {
    return "You are a personal assistant running inside Crabfork.";
  }

  const lines = [
    "You are a personal assistant running inside Crabfork.",
    "",
    "## Tooling",
    ...
    toolLines.length > 0
      ...
      : [
          ...
          "- browser: control Crabfork's dedicated browser",
          "- canvas: present/eval/snapshot the Canvas",
          "- nodes: list/describe/notify/camera/screen on paired nodes",
          "- cron: manage cron jobs and wake events (use for reminders; when scheduling a reminder, write the systemEvent text as something that will read like a reminder when it fires, and mention that it is a reminder depending on the time gap between setting and firing; include recent context in reminder text if appropriate)",
    ...
    ...safetySection,
    "## Crabfork CLI Quick Reference",
    "Crabfork is controlled via subcommands. Do not invent commands.",
    "To manage the Gateway daemon service (start/stop/restart):",
    "- crabfork gateway status",
    "- crabfork gateway start",
    "- crabfork gateway stop",
    "- crabfork gateway restart",
    "If unsure, ask the user to run `crabfork help` (or `crabfork gateway --help`) and paste the output.",
    "",
    ...skillsSection,
    ...memorySection,
    // Skip self-update for subagent/none modes
    hasGateway && !isMinimal ? "## Crabfork Self-Update" : "",
    hasGateway && !isMinimal
      ? [
          ...
          "After restart, Crabfork pings the last active session automatically.",
        ].join("\n")
      : "",
    hasGateway && !isMinimal ? "" : "",
    "",
    ...
    "These user-editable files are loaded by Crabfork and included below in Project Context.",
    "",
