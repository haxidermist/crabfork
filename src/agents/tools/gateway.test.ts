...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    crabfork: process.env.CRABFORK_GATEWAY_TOKEN,
  };
  ...
  afterAll(() => {
    if (envSnapshot.crabfork === undefined) {
      delete process.env.CRABFORK_GATEWAY_TOKEN;
    } else {
      process.env.CRABFORK_GATEWAY_TOKEN = envSnapshot.crabfork;
    }
  });
  ...
  it("uses CRABFORK_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.CRABFORK_GATEWAY_TOKEN = "local-env-token";
    configState.value = {
      gateway: {
