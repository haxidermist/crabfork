...
import { isCrabforkOwnerOnlyCoreToolName } from "./owner-only-tools.js";
...
function assertGatewayConfigMutationAllowed(params: {
  ...
  // Uses the same flag enumeration as `crabfork security audit`.
  const currentFlags = new Set(
    collectEnabledInsecureOrDangerousFlags(params.currentConfig as CrabforkConfig),
  );
  const nextFlags = collectEnabledInsecureOrDangerousFlags(nextConfig as CrabforkConfig);
...
export function createGatewayTool(opts?: {
  ...
  return {
    label: "Gateway",
    name: "gateway",
    ownerOnly: isCrabforkOwnerOnlyCoreToolName("gateway"),
    description:
