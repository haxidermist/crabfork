...
type HomeEnvSnapshot = {
  ...
  CRABFORK_STATE_DIR: string | undefined;
  CRABFORK_AGENT_DIR: string | undefined;
...
function snapshotHomeEnv(): HomeEnvSnapshot {
  return {
    ...
    CRABFORK_AGENT_DIR: process.env.CRABFORK_AGENT_DIR,
...
export function createTempHomeHarness(options: { prefix: string; beforeEachCase?: () => void }) {
  ...
  async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    await fs.mkdir(path.join(home, ".crabfork", "agents", "main", "sessions"), { recursive: true });
    const envSnapshot = snapshotHomeEnv();
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.CRABFORK_STATE_DIR = path.join(home, ".crabfork");
    process.env.CRABFORK_AGENT_DIR = path.join(home, ".crabfork", "agent");
    process.env.PI_CODING_AGENT_DIR = path.join(home, ".crabfork", "agent");
