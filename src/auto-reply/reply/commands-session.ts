...
import { scheduleGatewaySigusr1Restart, triggerCrabforkRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Crabfork in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerCrabforkRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Crabfork via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
