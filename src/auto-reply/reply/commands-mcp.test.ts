...
const workspaceHarness = createCommandWorkspaceHarness("crabfork-command-mcp-");
...
function buildCfg(): CrabforkConfig {
  return {
    commands: {
      text: true,
      mcp: true,
...
describe("handleCommands /mcp", () => {
  ...
  it("writes MCP config and shows it back", async () => {
    await withTempHome("crabfork-command-mcp-home-", async () => {
