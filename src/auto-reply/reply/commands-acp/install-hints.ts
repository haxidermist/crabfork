...
export function resolveAcpInstallCommandHint(cfg: CrabforkConfig): string {
  ...
  if (backendId === "acpx") {
    ...
    if (existsSync(workspaceLocalPath)) {
      return `crabfork plugins install ${workspaceLocalPath}`;
    }
    ...
    if (bundledInstallHint) {
      const localPath = bundledInstallHint.replace(/^crabfork plugins install /u, "");
    ...
    return "crabfork plugins install acpx";
  }
