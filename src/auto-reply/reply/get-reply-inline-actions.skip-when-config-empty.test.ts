...
const { buildStatusReplyMock, createCrabforkToolsMock, getChannelPluginMock, handleCommandsMock } =
  vi.hoisted(() => ({
    buildStatusReplyMock: vi.fn(),
    createCrabforkToolsMock: vi.fn(),
...
vi.mock("../../agents/crabfork-tools.runtime.js", () => ({
  createCrabforkTools: (...args: unknown[]) => createCrabforkToolsMock(...args),
}));
...
describe("handleInlineActions", () => {
  ...
  it("passes requesterAgentIdOverride into inline tool runtimes", async () => {
    ...
    expect(result).toEqual({ kind: "reply", reply: { text: "✅ Done." } });
    expect(createCrabforkToolsMock).toHaveBeenCalledWith(
      expect.objectContaining({
