...
export function createStatusScanSharedMocks(configPathLabel: string): StatusScanSharedMocks {
  return {
    resolveConfigPath: vi.fn(() => `/tmp/crabfork-${configPathLabel}-missing-${process.pid}.json`),
...
export function createStatusScanConfig<T extends object = CrabforkConfig>(
  overrides: T = {} as T,
): CrabforkConfig & T {
  return {
    session: {},
...
export function createStatusMemorySearchConfig(): CrabforkConfig {
