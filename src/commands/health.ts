...
const debugHealth = (...args: unknown[]) => {
  if (isTruthyEnvValue(process.env.CRABFORK_DEBUG_HEALTH)) {
...
const resolveHeartbeatSummary = (cfg: CrabforkConfig, agentId: string) =>
  resolveHeartbeatSummaryForAgent(cfg, agentId);

const resolveAgentOrder = (cfg: CrabforkConfig) => {
...
async function inspectHealthAccount(plugin: ChannelPlugin, cfg: CrabforkConfig, accountId: string) {
  return (
...
export async function healthCommand(
  opts: { json?: boolean; timeoutMs?: number; verbose?: boolean; config?: CrabforkConfig },
  runtime: RuntimeEnv,
) {
  ...
  if (opts.json) {
    ...
    const debugEnabled = isTruthyEnvValue(process.env.CRABFORK_DEBUG_HEALTH);
    const rich = isRich();
    if (opts.verbose) {
...
async function readBestEffortHealthConfig(): Promise<CrabforkConfig> {
