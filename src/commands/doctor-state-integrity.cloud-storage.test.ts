...
describe("detectMacCloudSyncedStateDir", () => {
  ...
  it("detects state dir under Library/CloudStorage", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "Dropbox", "Crabfork", ".crabfork");
  ...
  it("detects cloud-synced target when state dir resolves via symlink", () => {
    const symlinkPath = "/tmp/crabfork-state";
  ...
  it("ignores cloud-synced symlink prefix when resolved target is local", () => {
    ...
    const resolvedLocalPath = path.join(home, ".crabfork");
  ...
  it("anchors cloud detection to OS homedir when CRABFORK_HOME is overridden", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "iCloud Drive", ".crabfork");
    const originalCrabforkHome = process.env.CRABFORK_HOME;
    process.env.CRABFORK_HOME = "/tmp/crabfork-home-override";
    ...
    try {
      ...
      if (originalCrabforkHome === undefined) {
