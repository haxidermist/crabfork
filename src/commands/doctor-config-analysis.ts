...
import { CrabforkSchema } from "../config/zod-schema.js";
...
export function stripUnknownConfigKeys(config: CrabforkConfig): {
  config: CrabforkConfig;
  removed: string[];
} {
  const parsed = CrabforkSchema.safeParse(config);
  if (parsed.success) {
...
export function noteOpencodeProviderOverrides(cfg: CrabforkConfig): void {
