...
const SEARCH_TOOL = "https://docs.crabfork.ai/mcp.SearchCrabfork";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.crabfork.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('crabfork docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.crabfork.ai/");
      runtime.log(`Search: ${formatCliCommand('crabfork docs "your query"')}`);
    }
    return;
  }
