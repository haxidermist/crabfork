...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under CRABFORK_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("crabfork-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "crabfork-oauth-sync-"));
    ...
    process.env.CRABFORK_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "crabfork-oauth-nosync-"));
    process.env.CRABFORK_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside CRABFORK_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "crabfork-oauth-external-"));
    process.env.CRABFORK_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* CRABFORK_STATE_DIR
...
describe("upsertApiKeyProfile", () => {
  ...
  it("writes to CRABFORK_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("crabfork-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
