...
function noteFlowRecoveryHints() {
  ...
  note(
    [
      ...
      `Inspect: ${formatCliCommand("crabfork tasks flow show <flow-id>")}`,
      `Cancel: ${formatCliCommand("crabfork tasks flow cancel <flow-id>")}`,
    ]
...
export function noteWorkspaceStatus(cfg: CrabforkConfig) {
