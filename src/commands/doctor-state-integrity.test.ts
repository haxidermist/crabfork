...
type EnvSnapshot = {
  HOME?: string;
  CRABFORK_HOME?: string;
  CRABFORK_STATE_DIR?: string;
  CRABFORK_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    CRABFORK_HOME: process.env.CRABFORK_HOME,
    CRABFORK_STATE_DIR: process.env.CRABFORK_STATE_DIR,
    CRABFORK_OAUTH_DIR: process.env.CRABFORK_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: CrabforkConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
function createAgentDir(agentId: string, includeNestedAgentDir = true) {
  const stateDir = process.env.CRABFORK_STATE_DIR;
  if (!stateDir) {
    throw new Error("CRABFORK_STATE_DIR is not set");
  }
...
async function runStateIntegrity(cfg: CrabforkConfig) {
...
async function runStateIntegrityText(cfg: CrabforkConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "crabfork-doctor-state-integrity-"));
    ...
    fs.mkdirSync(process.env.CRABFORK_STATE_DIR, { recursive: true, mode: 0o700 });
    noteMock.mockClear();
  });
  ...
  it("prompts for oauth dir when CRABFORK_OAUTH_DIR is explicitly configured", async () => {
    process.env.CRABFORK_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: CrabforkConfig = {};
    const confirmRuntimeRepair = await runStateIntegrity(cfg);
    expect(confirmRuntimeRepair).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
  ...
  it("does not warn when a case-mismatched dir resolves to the configured agent path", async () => {
    ...
    const resolvedResearchAgentDir = realpathNative(
      path.join(process.env.CRABFORK_STATE_DIR ?? "", "agents", "Research", "agent"),
    );
    const realpathSpy = vi
      .spyOn(fs.realpathSync, "native")
      .mockImplementation((target, options) => {
  ...
  it("prints crabfork-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: CrabforkConfig = {};
    ...
    expect(text).toMatch(/crabfork sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/crabfork sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /crabfork sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
