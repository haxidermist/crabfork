...
function usesClaudeCliModelSelection(cfg: CrabforkConfig): boolean {
...
function hasClaudeCliConfigSignals(cfg: CrabforkConfig): boolean {
...
function resolveClaudeCliCommand(cfg: CrabforkConfig): string {
...
function formatWorkspaceHealthLine(workspaceDir: string, health: ClaudeCliDirHealth): string {
  ...
  if (health === "missing") {
    return `- Workspace: ${display} (missing; Crabfork will create it on first run).`;
  }
  ...
  if (credential) {
    ...
    fixHints.push(
      `- Fix: run ${formatCliCommand("claude auth login")}, then ${formatCliCommand(
        "crabfork models auth login --provider anthropic --method cli --set-default",
      )}.`,
    );
  }

  if (!storedProfile) {
    lines.push(`- Crabfork auth profile: missing (${CLAUDE_CLI_PROFILE_ID}) in ${authStorePath}.`);
    fixHints.push(
    ...
    lines.push(
      `- Crabfork auth profile: ${CLAUDE_CLI_PROFILE_ID} is wired to provider "${storedProfile.provider}" instead of "${CLAUDE_CLI_PROVIDER}".`,
    );
    fixHints.push(
    ...
    lines.push(
      `- Crabfork auth profile: ${CLAUDE_CLI_PROFILE_ID} (provider ${CLAUDE_CLI_PROVIDER}).`,
    );
  }
