...
export function hasExplicitMemorySearchConfig(cfg: CrabforkConfig, agentId: string): boolean {
  if (
    cfg.agents?.defaults &&
...
export function resolveMemoryPluginStatus(cfg: CrabforkConfig): MemoryPluginStatus {
...
export async function resolveSharedMemoryStatusSnapshot(params: {
  ...
  resolveMemoryConfig: (cfg: CrabforkConfig, agentId: string) => { store: { path: string } } | null;
