...
export function normalizeLegacyTalkConfig(cfg: CrabforkConfig, changes: string[]): CrabforkConfig {
  const rawTalk = cfg.talk;
  if (!isRecord(rawTalk)) {
    return cfg;
  }

  const normalizedTalk = normalizeTalkSection(rawTalk as CrabforkConfig["talk"]) ?? {};
