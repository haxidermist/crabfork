...
const ensureCrabforkModelsJson = vi.fn().mockResolvedValue(undefined);
const resolveCrabforkAgentDir = vi.fn().mockReturnValue("/tmp/crabfork-agent");
...
vi.mock("./models/list.runtime.js", () => {
  ...
  return {
    ...
    ensureCrabforkModelsJson,
    resolveCrabforkAgentDir,
...
beforeEach(() => {
  ...
  ensureCrabforkModelsJson.mockClear();
...
describe("models list/status", () => {
  ...
  it("loadModelRegistry does not persist models.json as a side effect", async () => {
    ...
    expect(ensureCrabforkModelsJson).not.toHaveBeenCalled();
  });
  ...
  it("modelsListCommand persists using the source snapshot config when provided", async () => {
    ...
    expect(ensureCrabforkModelsJson).toHaveBeenCalled();
    expect(ensureCrabforkModelsJson.mock.calls[0]?.[0]).toEqual(sourceConfig);
  });
