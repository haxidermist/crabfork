...
import type { CrabforkConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: CrabforkConfig): string[] {
...
function collectExecPolicyConflictWarnings(cfg: CrabforkConfig): string[] {
  const warnings: string[] = [];
  const approvals = loadExecApprovals();
  const defaultRequestedSecuritySource = "Crabfork default (full)";
  const defaultRequestedAskSource = "Crabfork default (off)";
  ...
  const maybeWarn = (params: {
    ...
    warnings.push(
      [
        ...
        `  Inspect with: ${formatCliCommand("crabfork approvals get --gateway")}`,
      ].join("\n"),
    );
  };
...
function collectDurableExecApprovalWarnings(cfg: CrabforkConfig): string[] {
  void cfg;
  return [];
}

export async function noteSecurityWarnings(cfg: CrabforkConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("crabfork security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.crabfork/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("crabfork approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.crabfork.ai/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("crabfork configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("crabfork config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("crabfork doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("crabfork config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('crabfork config set session.dmScope "per-channel-peer"') +
