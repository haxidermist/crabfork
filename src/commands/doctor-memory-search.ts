...
function buildMemoryRecallIssueNote(audit: ShortTermAuditSummary): string | null {
  ...
  const guidance = hasFixableIssue
    ? `Fix: ${formatCliCommand("crabfork doctor --fix")} or ${formatCliCommand("crabfork memory status --fix")}`
    : `Verify: ${formatCliCommand("crabfork memory status --deep")}`;
  return [
...
export async function noteMemoryRecallHealth(cfg: CrabforkConfig): Promise<void> {
  try {
 ...
 * Runs as part of `crabfork doctor` — config-only checks where possible;
    ...
    if (!qmdCheck.available) {
      note(
        [
          ...
          `- Set an explicit binary path: ${formatCliCommand("crabfork config set memory.qmd.command /absolute/path/to/qmd")}`,
          `- Or switch back to builtin memory: ${formatCliCommand("crabfork config set memory.backend builtin")}`,
          "",
          `Verify: ${formatCliCommand("crabfork memory status --deep")}`,
        ]
          .filter(Boolean)
  ...
  if (resolved.provider !== "auto") {
      ...
      note(
        [
          ...
          suggestedRemoteProvider
            ? `- Switch to a remote provider: ${formatCliCommand(`crabfork config set agents.defaults.memorySearch.provider ${suggestedRemoteProvider}`)}`
    ...
    note(
      [
        ...
        `- Configure credentials: ${formatCliCommand("crabfork configure --section model")}`,
        `- To disable: ${formatCliCommand("crabfork config set agents.defaults.memorySearch.enabled false")}`,
        "",
        `Verify: ${formatCliCommand("crabfork memory status --deep")}`,
      ].join("\n"),
      "Memory search",
