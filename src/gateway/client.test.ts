...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("crabfork doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  it("allows ws:// to private addresses only with CRABFORK_ALLOW_INSECURE_PRIVATE_WS=1", () => {
  ...
  it("allows ws:// hostnames with CRABFORK_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.CRABFORK_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://crabfork-gateway.ai:18789",
      onConnectError,
    });
