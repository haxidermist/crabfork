...
function resolveMemoryToolDisableReasons(cfg: CrabforkConfig): string[] {
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-crabfork-message-channel") ?? "",
  );
  const accountId = normalizeOptionalString(getHeader(req, "x-crabfork-account-id"));
  const agentTo = normalizeOptionalString(getHeader(req, "x-crabfork-message-to"));
  const agentThreadId = normalizeOptionalString(getHeader(req, "x-crabfork-thread-id"));
