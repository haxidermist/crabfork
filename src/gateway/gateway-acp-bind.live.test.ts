...
const ACP_BIND_LIVE = isTruthyEnvValue(process.env.CRABFORK_LIVE_ACP_BIND);
  ...
  it(
    ...
    async () => {
      const previous = {
        ...
        port: process.env.CRABFORK_GATEWAY_PORT,
      ...
      const liveAgent = normalizeAcpAgent(process.env.CRABFORK_LIVE_ACP_BIND_AGENT);
      const agentCommandOverride =
        process.env.CRABFORK_LIVE_ACP_BIND_AGENT_COMMAND?.trim() || undefined;
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "crabfork-live-acp-bind-"));
      const tempStateDir = path.join(tempRoot, "state");
      const tempConfigPath = path.join(tempRoot, "crabfork.json");
      ...
      try {
        ...
        if (previous.port === undefined) {
          ...
          process.env.CRABFORK_GATEWAY_PORT = previous.port;
        }
