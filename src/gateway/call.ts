...
function loadGatewayConfig(): CrabforkConfig {
  const loadConfigFn =
...
function resolveGatewayPortValue(config?: CrabforkConfig, env?: NodeJS.ProcessEnv): number {
...
function resolveGatewayCallContext(opts: CallGatewayBaseOptions): ResolvedGatewayCallContext {
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : trimToUndefined(process.env.CRABFORK_GATEWAY_URL);
  ...
  const config = opts.config ?? (canSkipConfigLoad ? ({} as CrabforkConfig) : loadGatewayConfig());
