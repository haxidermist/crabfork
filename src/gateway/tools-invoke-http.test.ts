...
let lastCreateCrabforkToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/crabfork-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateCrabforkToolsContext?.agentTo,
          agentThreadId: lastCreateCrabforkToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createCrabforkTools: (ctx: Record<string, unknown>) => {
      lastCreateCrabforkToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateCrabforkToolsContext = undefined;
...
const gatewayAuthHeaders = () => ({ "x-crabfork-scopes": "operator.write" });
const gatewayAdminHeaders = () => ({ "x-crabfork-scopes": "operator.admin" });
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateCrabforkToolsContext?.allowMediaInvokeCommands).toBe(true);
    expect(lastCreateCrabforkToolsContext?.disablePluginTools).toBe(true);
  ...
  it("opts direct gateway tool invocation into gateway subagent binding", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateCrabforkToolsContext?.allowGatewaySubagentBinding).toBe(true);
  });
  ...
  it("keeps plugin tools enabled for non-core tool invokes", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateCrabforkToolsContext?.disablePluginTools).toBe(false);
  });
  ...
  it("threads senderIsOwner into tool creation before owner-only filtering", async () => {
    ...
    expect(lastCreateCrabforkToolsContext?.senderIsOwner).toBe(false);
    ...
    expect(lastCreateCrabforkToolsContext?.senderIsOwner).toBe(true);
  });
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-crabfork-message-to": "channel:24514",
        "x-crabfork-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
  ...
  it("treats shared-secret bearer auth as owner on /tools/invoke", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        authorization: "Bearer secret",
        "x-crabfork-scopes": "operator.approvals",
      },
      tool: "owner_only_test",
      sessionKey: "main",
    });
