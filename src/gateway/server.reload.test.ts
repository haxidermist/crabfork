...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.CRABFORK_SKIP_CHANNELS;
    prevSkipGmail = process.env.CRABFORK_SKIP_GMAIL_WATCHER;
    prevSkipProviders = process.env.CRABFORK_SKIP_PROVIDERS;
    prevOpenAiApiKey = process.env.OPENAI_API_KEY;
    process.env.CRABFORK_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.CRABFORK_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.CRABFORK_SKIP_GMAIL_WATCHER;
    } else {
      process.env.CRABFORK_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
    if (prevSkipProviders === undefined) {
      delete process.env.CRABFORK_SKIP_PROVIDERS;
    } else {
      process.env.CRABFORK_SKIP_PROVIDERS = prevSkipProviders;
    }
  ...
  async function writeConfigFile(config: unknown) {
    ...
    if (!configPath) {
      throw new Error("CRABFORK_CONFIG_PATH is not set");
    }
...
`,
    ...
    const previousGatewayTokenEnv = process.env.CRABFORK_GATEWAY_TOKEN;
    ...
    try {
      ...
      if (previousGatewayTokenEnv === undefined) {
        ...
        process.env.CRABFORK_GATEWAY_TOKEN = previousGatewayTokenEnv;
      }
