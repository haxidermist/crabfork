...
export function registerDefaultAuthTokenSuite(): void {
  describe("default auth (token)", () => {
    ...
    const testsWithoutDefaultServer = new Set([
      ...
      "prefers CRABFORK_HANDSHAKE_TIMEOUT_MS and falls back on empty string",
    ]);
    ...
    test("closes silent handshakes after timeout", async () => {
      vi.useRealTimers();
      const prevHandshakeTimeout = process.env.CRABFORK_TEST_HANDSHAKE_TIMEOUT_MS;
      process.env.CRABFORK_TEST_HANDSHAKE_TIMEOUT_MS = "20";
      try {
        ...
        if (prevHandshakeTimeout === undefined) {
          delete process.env.CRABFORK_TEST_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.CRABFORK_TEST_HANDSHAKE_TIMEOUT_MS = prevHandshakeTimeout;
        }
      }
    });

    test("prefers CRABFORK_HANDSHAKE_TIMEOUT_MS and falls back on empty string", () => {
      const prevHandshakeTimeout = process.env.CRABFORK_HANDSHAKE_TIMEOUT_MS;
      const prevTestHandshakeTimeout = process.env.CRABFORK_TEST_HANDSHAKE_TIMEOUT_MS;
      process.env.CRABFORK_HANDSHAKE_TIMEOUT_MS = "75";
      process.env.CRABFORK_TEST_HANDSHAKE_TIMEOUT_MS = "20";
      try {
        expect(getPreauthHandshakeTimeoutMsFromEnv()).toBe(75);
        process.env.CRABFORK_HANDSHAKE_TIMEOUT_MS = "";
        expect(getPreauthHandshakeTimeoutMsFromEnv()).toBe(20);
      } finally {
        if (prevHandshakeTimeout === undefined) {
          delete process.env.CRABFORK_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.CRABFORK_HANDSHAKE_TIMEOUT_MS = prevHandshakeTimeout;
        }
        if (prevTestHandshakeTimeout === undefined) {
          delete process.env.CRABFORK_TEST_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.CRABFORK_TEST_HANDSHAKE_TIMEOUT_MS = prevTestHandshakeTimeout;
        }
      }
    });
      ...
      for (const testCase of [
        {
          env: {
            CRABFORK_VERSION: " ",
            CRABFORK_SERVICE_VERSION: "2.4.6-service",
    ...
    test("does not grant admin when scopes are omitted", async () => {
      ...
      const { identity, device } = await createSignedDevice({
        ...
        identityPath: path.join(os.tmpdir(), `crabfork-test-device-${randomUUID()}.json`),
        nonce,
      });
