...
describe("gateway auth", () => {
  ...
  it("resolves token/password from CRABFORK gateway env vars", () => {
    expect(
      resolveGatewayAuth({
        authConfig: {},
        env: {
          CRABFORK_GATEWAY_TOKEN: "env-token",
          CRABFORK_GATEWAY_PASSWORD: "env-password",
  ...
  it("accepts password mode when env provides CRABFORK_GATEWAY_PASSWORD", () => {
...
describe("trusted-proxy auth", () => {
  ...
  it.each([
    ...
    },
    {
      ...
      env: {
        CRABFORK_GATEWAY_TOKEN: "shared-secret",
      } as NodeJS.ProcessEnv,
    },
