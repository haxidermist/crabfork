...
const { createCrabforkTools } = await import("../agents/crabfork-tools.js");
...
type SessionSendTool = ReturnType<typeof createCrabforkTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createCrabforkTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["CRABFORK_GATEWAY_PORT", "CRABFORK_GATEWAY_TOKEN"]);
  ...
  process.env.CRABFORK_GATEWAY_PORT = String(gatewayPort);
  process.env.CRABFORK_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("CRABFORK_CONFIG_PATH missing in gateway test environment");
      }
      ...
      const tool = createCrabforkTools({
        config: {
          tools: {
            sessions: {
              visibility: "all",
