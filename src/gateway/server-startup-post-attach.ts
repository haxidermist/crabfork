...
import type { loadCrabforkPlugins } from "../plugins/loader.js";
...
async function prewarmConfiguredPrimaryModel(params: {
  ...
  try {
    await ensureCrabforkModelsJson(params.cfg, agentDir);
...
export async function startGatewaySidecars(params: {
  cfg: CrabforkConfig;
  pluginRegistry: ReturnType<typeof loadCrabforkPlugins>;
  ...
  if (!skipChannels) {
    ...
    params.logChannels.info(
      "skipping channel start (CRABFORK_SKIP_CHANNELS=1 or CRABFORK_SKIP_PROVIDERS=1)",
    );
  }
...
export async function startGatewayPostAttachRuntime(
  params: {
    ...
    gatewayPluginConfigAtStart: CrabforkConfig;
    pluginRegistry: ReturnType<typeof loadCrabforkPlugins>;
    defaultWorkspaceDir: string;
    deps: CliDeps;
    startChannels: () => Promise<void>;
