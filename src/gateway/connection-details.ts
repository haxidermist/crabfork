...
type GatewayConnectionDetailResolvers = {
  ...
  resolveGatewayPort?: (cfg?: CrabforkConfig, env?: NodeJS.ProcessEnv) => number;
};
...
export function buildGatewayConnectionDetailsWithResolvers(
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : normalizeOptionalString(process.env.CRABFORK_GATEWAY_URL);
  ...
  const allowPrivateWs = process.env.CRABFORK_ALLOW_INSECURE_PRIVATE_WS === "1";
  if (!isSecureWebSocketUrl(url, { allowPrivateWs })) {
    throw new Error(
      [
        ...
        allowPrivateWs
          ? undefined
          : "Break-glass (trusted private networks only): set CRABFORK_ALLOW_INSECURE_PRIVATE_WS=1",
        "Doctor: crabfork doctor --fix",
