...
import { attachCrabforkTranscriptMeta, readSessionMessages } from "./session-utils.js";
...
export type SessionHistoryMessage = Record<string, unknown> & {
  __crabfork?: SessionHistoryTranscriptMeta;
};
...
export function resolveMessageSeq(message: SessionHistoryMessage | undefined): number | undefined {
  const seq = message?.__crabfork?.seq;
...
export class SessionHistorySseState {
  ...
  appendInlineMessage(update: {
    ...
    const nextMessage = attachCrabforkTranscriptMeta(update.message, {
