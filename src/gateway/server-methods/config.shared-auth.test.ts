...
vi.mock("../../config/config.js", async () => {
  ...
  return {
    ...actual,
    createConfigIO: () => ({ configPath: "/tmp/crabfork.json" }),
...
beforeEach(() => {
  validateConfigObjectWithPluginsMock.mockImplementation((config: CrabforkConfig) => ({
    ok: true,
    config,
  }));
...
describe("config shared auth disconnects", () => {
  it("does not disconnect shared-auth clients for config.set auth writes without restart", async () => {
    const prevConfig: CrabforkConfig = {
      gateway: {
        auth: {
          mode: "token",
          token: "old-token",
