...
type GatewayInstallAuthMode = NonNullable<NonNullable<CrabforkConfig["gateway"]>["auth"]>["mode"];
...
function hasConfiguredGatewayPasswordForInstall(cfg: CrabforkConfig): boolean {
...
function hasDurableGatewayPasswordEnvForInstall(
  ...
  return Boolean(
    normalizeOptionalString(durableServiceEnv.CRABFORK_GATEWAY_PASSWORD) ||
