...
function cfg(input: Partial<CrabforkConfig>): CrabforkConfig {
  return input as CrabforkConfig;
}
...
const DEFAULT_ENV = {
  CRABFORK_GATEWAY_TOKEN: "env-token",
  CRABFORK_GATEWAY_PASSWORD: "env-password", // pragma: allowlist secret
...
describe("resolveGatewayConnectionAuth", () => {
  ...
  it("resolves local SecretRef token when CRABFORK env is absent", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first token SecretRef even when CRABFORK env token exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first password SecretRef even when CRABFORK env password exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
