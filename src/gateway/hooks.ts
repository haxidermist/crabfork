...
export function resolveHooksConfig(cfg: CrabforkConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: CrabforkConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken = normalizeOptionalString(req.headers["x-crabfork-token"]) ?? "";
  if (headerToken) {
    return headerToken;
  }
  return undefined;
...
export function resolveHookIdempotencyKey(params: {
  ...
  return (
    ...
    resolveOptionalHookIdempotencyKey(params.headers?.["x-crabfork-idempotency-key"]) ||
