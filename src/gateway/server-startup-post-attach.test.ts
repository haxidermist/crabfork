...
vi.mock("../config/paths.js", async () => {
  ...
  return {
    ...actual,
    STATE_DIR: "/tmp/crabfork-state",
    resolveConfigPath: vi.fn(() => "/tmp/crabfork-state/crabfork.json"),
    resolveGatewayPort: vi.fn(() => 18789),
    resolveStateDir: vi.fn(() => "/tmp/crabfork-state"),
  };
});
...
function createPostAttachParams(overrides: Partial<PostAttachParams> = {}): PostAttachParams {
  return {
    ...
    defaultWorkspaceDir: "/tmp/crabfork-workspace",
    deps: {} as never,
