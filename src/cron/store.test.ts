...
const { makeStorePath } = createCronStoreHarness({ prefix: "crabfork-cron-store-" });
...
describe("resolveCronStorePath", () => {
  ...
  it("uses CRABFORK_HOME for tilde expansion", () => {
    vi.stubEnv("CRABFORK_HOME", "/srv/crabfork-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/crabfork-home", "cron", "jobs.json"));
  });
});
