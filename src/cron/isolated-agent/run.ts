...
function hasConfiguredAuthProfiles(cfg: CrabforkConfig): boolean {
  return (
...
async function prepareCronRunContext(params: {
  ...
  const cfgWithAgentDefaults: CrabforkConfig = {
    ...input.cfg,
...
export async function runCronIsolatedAgentTurn(params: {
  ...
  const isFastTestEnv = process.env.CRABFORK_TEST_FAST === "1";
