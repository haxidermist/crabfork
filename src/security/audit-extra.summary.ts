...
function summarizeGroupPolicy(cfg: CrabforkConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function hasWebSearchKey(cfg: CrabforkConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: CrabforkConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: CrabforkConfig): boolean {
...
function isBrowserEnabled(cfg: CrabforkConfig): boolean {
  return cfg.browser?.enabled !== false;
}

export function collectAttackSurfaceSummaryFindings(cfg: CrabforkConfig): SecurityAuditFinding[] {
