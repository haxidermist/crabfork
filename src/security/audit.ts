...
import type { ConfigFileSnapshot, CrabforkConfig } from "../config/config.js";
  ...
  if (stateDirPerms.ok) {
    ...
    if (stateDirPerms.worldWritable) {
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; other users can write into your Crabfork state.`,
      ...
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; group users can write into your Crabfork state.`,
        remediation: formatPermissionRemediation({
          targetPath: params.stateDir,
          perms: stateDirPerms,
          isDir: true,
...
export function collectGatewayConfigFindings(
  ...
  const envTokenConfigured = hasNonEmptyString(env.CRABFORK_GATEWAY_TOKEN);
  const envPasswordConfigured = hasNonEmptyString(env.CRABFORK_GATEWAY_PASSWORD);
...
export function collectLoggingFindings(cfg: CrabforkConfig): SecurityAuditFinding[] {
...
export function collectElevatedFindings(cfg: CrabforkConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExecRuntimeFindings(cfg: CrabforkConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectOpenExecSurfacePaths(cfg: CrabforkConfig): string[] {
