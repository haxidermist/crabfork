...
export function collectDeepProbeFindings(params: {
  ...
  if (params.deep?.gateway?.attempted && !params.deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("crabfork status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("crabfork security audit --deep")}".`,
    });
  }
  if (params.authWarning) {
    findings.push({
      ...
      remediation: `Set CRABFORK_GATEWAY_TOKEN/CRABFORK_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("crabfork security audit --deep")}".`,
    });
  }
  return findings;
}
