...
const ENV_CATALOG_PATHS = ["CRABFORK_PLUGIN_CATALOG_PATHS", "CRABFORK_MPM_CATALOG_PATHS"];
...
function parseExternalCatalogChannelEntries(raw: unknown): ExternalCatalogChannelEntry[] {
  ...
  for (const entry of list) {
    if (!isRecord(entry) || !isRecord(entry.crabfork) || !isRecord(entry.crabfork.channel)) {
      continue;
    }
...
export function shouldSkipPreferredPluginAutoEnable(params: {
  ...
  env: NodeJS.ProcessEnv;
  registry: PluginManifestRegistry;
  isPluginDenied: (config: CrabforkConfig, pluginId: string) => boolean;
  isPluginExplicitlyDisabled: (config: CrabforkConfig, pluginId: string) => boolean;
