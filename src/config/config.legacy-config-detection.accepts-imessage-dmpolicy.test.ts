...
function expectCrabforkSchemaInvalidPreservesField(params: {
  config: unknown;
  ...
  const res = CrabforkSchema.safeParse(params.config);
  expect(res.success).toBe(false);
  if (!res.success) {
...
describe("legacy config detection", () => {
  ...
  it("rejects removed legacy provider sections", () => {
    expectCrabforkSchemaInvalidPreservesField({
  ...
  it("preserves claude-cli auth profile mode during validation", () => {
    ...
    const res = CrabforkSchema.safeParse(config);
    expect(res.success).toBe(true);
    if (res.success) {
