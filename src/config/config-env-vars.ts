...
function collectConfigEnvVarsByTarget(cfg?: CrabforkConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: CrabforkConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: CrabforkConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** @deprecated Use `collectConfigRuntimeEnvVars` or `collectConfigServiceEnvVars`. */
export function collectConfigEnvVars(cfg?: CrabforkConfig): Record<string, string> {
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.CRABFORK_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
