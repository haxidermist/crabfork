...
function collectModelRefs(cfg: CrabforkConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: CrabforkConfig, providerId: string): boolean {
...
function hasPluginOwnedWebSearchConfig(cfg: CrabforkConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
  return isRecord(pluginConfig) && isRecord(pluginConfig.webSearch);
}

function hasPluginOwnedWebFetchConfig(cfg: CrabforkConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
...
function hasPluginOwnedToolConfig(cfg: CrabforkConfig, plugin: PluginManifestRecord): boolean {
...
function collectCandidateChannelIds(cfg: CrabforkConfig, env: NodeJS.ProcessEnv): string[] {
  return listPotentialConfiguredChannelIds(cfg, env).map(
    (channelId) => normalizeChatChannelId(channelId) ?? channelId,
  );
}

function hasConfiguredWebSearchPluginEntry(cfg: CrabforkConfig): boolean {
...
function hasConfiguredWebFetchPluginEntry(cfg: CrabforkConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    !!entries &&
    typeof entries === "object" &&
...
function hasConfiguredPluginConfigEntry(cfg: CrabforkConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    !!entries &&
    typeof entries === "object" &&
...
function hasBrowserToolReference(cfg: CrabforkConfig): boolean {
...
function hasSetupAutoEnableRelevantConfig(cfg: CrabforkConfig): boolean {
  const entries = cfg.plugins?.entries;
...
function hasPluginEntries(cfg: CrabforkConfig): boolean {
  const entries = cfg.plugins?.entries;
  return !!entries && typeof entries === "object" && Object.keys(entries).length > 0;
}

function configMayNeedPluginManifestRegistry(cfg: CrabforkConfig): boolean {
...
function isPluginExplicitlyDisabled(cfg: CrabforkConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: CrabforkConfig, pluginId: string): boolean {
  const deny = cfg.plugins?.deny;
  return Array.isArray(deny) && deny.includes(pluginId);
}

function isBuiltInChannelAlreadyEnabled(cfg: CrabforkConfig, channelId: string): boolean {
  const channels = cfg.channels as Record<string, unknown> | undefined;
...
function registerPluginEntry(cfg: CrabforkConfig, pluginId: string): CrabforkConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
