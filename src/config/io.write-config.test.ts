...
describe("config io write", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "crabfork-config-io-" });
  const silentLogger = {
    warn: () => {},
    error: () => {},
  };
  ...
  it("keeps writes inside an CRABFORK_STATE_DIR override even when the real home config exists", async () => {
    await withSuiteHome(async (home) => {
      const liveConfigPath = path.join(home, ".crabfork", "crabfork.json");
      ...
      expect(io.configPath).toBe(path.join(overrideDir, "crabfork.json"));
      ...
      const overridePersisted = JSON.parse(
        await fs.readFile(path.join(overrideDir, "crabfork.json"), "utf-8"),
      ) as {
      ...
      plugins: [
        {
          ...
          skills: [],
          hooks: [],
          rootDir: "/tmp/crabfork-test-required-plugin",
          source: "/tmp/crabfork-test-required-plugin/index.ts",
          manifestPath: "/tmp/crabfork-test-required-plugin/crabfork.plugin.json",
          configSchema: {
            type: "object",
            properties: {
              token: { type: "string" },
