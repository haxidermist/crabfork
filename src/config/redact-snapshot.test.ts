...
import type { ConfigFileSnapshot } from "./types.crabfork.js";
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/crabfork/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/crabfork/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/crabfork/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/crabfork/nickserv-password.txt");
