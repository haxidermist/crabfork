...
function logConfigDocBaselineDebug(message: string): void {
  if (process.env.CRABFORK_CONFIG_DOC_BASELINE_DEBUG === "1") {
    console.error(`[config-doc-baseline] ${message}`);
  }
}

function resolveRepoRoot(): string {
  const fromPackage = resolveCrabforkPackageRootSync({
...
async function loadBundledConfigSchemaResponse(): Promise<ConfigSchemaResponse> {
  ...
  const env = {
    ...process.env,
    HOME: os.tmpdir(),
    CRABFORK_STATE_DIR: path.join(os.tmpdir(), "crabfork-config-doc-baseline-state"),
    CRABFORK_BUNDLED_PLUGINS_DIR: path.join(repoRoot, "extensions"),
  };
