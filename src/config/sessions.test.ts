...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "crabfork-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ CRABFORK_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-crabfork",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-crabfork#general");
  });
  ...
  it("derives session transcripts dir from CRABFORK_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { CRABFORK_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when CRABFORK_STATE_DIR differs from stored paths", () => {
