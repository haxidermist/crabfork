...
// Keep integration tests deterministic: never read a real crabfork.json.
...
const suiteRootTracker = createSuiteTempRootTracker({ prefix: "crabfork-pruning-integ-" });
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeEach(async () => {
    ...
    savedCacheTtl = process.env.CRABFORK_SESSION_CACHE_TTL_MS;
    process.env.CRABFORK_SESSION_CACHE_TTL_MS = "0";
  ...
  afterEach(() => {
    ...
    if (savedCacheTtl === undefined) {
      delete process.env.CRABFORK_SESSION_CACHE_TTL_MS;
    } else {
      process.env.CRABFORK_SESSION_CACHE_TTL_MS = savedCacheTtl;
    }
  });
