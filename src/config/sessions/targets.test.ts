...
function createCustomRootCfg(customRoot: string, defaultAgentId = "ops"): CrabforkConfig {
  return {
    session: {
...
const discoveryResolvers = [
  {
    label: "async",
    resolve: async (cfg: CrabforkConfig, env: NodeJS.ProcessEnv) =>
...
describe("resolveAllAgentSessionStoreTargets", () => {
  ...
  it("respects the caller env when resolving configured and discovered store roots", async () => {
    await withTempHome(async (home) => {
      ...
      const env = {
        ...process.env,
        CRABFORK_STATE_DIR: envStateDir,
      };
      const cfg: CrabforkConfig = {};
