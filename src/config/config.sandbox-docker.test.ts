...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("crabfork-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "crabfork-sandbox-browser-global" },
      agentBrowser: { network: "crabfork-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("crabfork-sandbox-browser-agent");
  });
