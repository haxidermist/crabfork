...
describe("image-generation runtime", () => {
  ...
  it("lists runtime image-generation providers through the provider registry", () => {
    ...
    expect(listRuntimeImageGenerationProviders({ config: {} as CrabforkConfig })).toEqual(
      providers,
    );
    expect(mocks.listImageGenerationProviders).toHaveBeenCalledWith({} as CrabforkConfig);
  });
  ...
  it("builds a generic config hint without hardcoded provider ids", async () => {
    ...
    const promise = generateImage({ cfg: {} as CrabforkConfig, prompt: "draw a cat" });
