...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        CRABFORK_BASE_ENV: "base",
        CRABFORK_TO_REMOVE: undefined,
      },
      env: {
        CRABFORK_TEST_ENV: "ok",
      },
    });

    expect(resolved.CRABFORK_BASE_ENV).toBe("base");
    expect(resolved.CRABFORK_TEST_ENV).toBe("ok");
    expect(resolved.CRABFORK_TO_REMOVE).toBeUndefined();
    expect(resolved.CRABFORK_CLI).toBe(CRABFORK_CLI_ENV_VALUE);
  });
