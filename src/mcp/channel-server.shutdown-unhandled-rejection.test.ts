...
vi.mock("./channel-bridge.js", () => ({
  CrabforkChannelBridge: class MockCrabforkChannelBridge {
...
describe("serveCrabforkChannelMcp shutdown", () => {
  ...
  it("does not leak unhandled rejections when shutdown close fails", async () => {
    ...
    const { serveCrabforkChannelMcp } = await import("./channel-server.js");

    const servePromise = serveCrabforkChannelMcp({ verbose: false });
