...
import { z } from "zod";
import { createCrabforkChannelMcpServer, CrabforkChannelBridge } from "./channel-server.js";
...
async function connectMcpWithoutGateway(params?: { claudeChannelMode?: "auto" | "on" | "off" }) {
  const serverHarness = await createCrabforkChannelMcpServer({
...
describe("crabfork channel mcp server", () => {
  describe("gateway-backed flows", () => {
    ...
    test("sendMessage normalizes route metadata for gateway send", async () => {
      const bridge = new CrabforkChannelBridge({} as never, {
        claudeChannelMode: "off",
        verbose: false,
      });
