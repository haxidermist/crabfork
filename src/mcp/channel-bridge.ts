...
export class CrabforkChannelBridge {
  ...
  async start(): Promise<void> {
    ...
    this.gateway = new GatewayClient({
      ...
      clientDisplayName: "Crabfork MCP",
  ...
  async handleClaudePermissionRequest(params: {
    ...
    if (this.verbose) {
      process.stderr.write(`crabfork mcp: pending Claude permission ${params.requestId}\n`);
    }
  }
    ...
    try {
      ...
      if (this.verbose && !this.closed) {
        process.stderr.write(
          `crabfork mcp: notification ${notification.method} failed: ${String(error)}\n`,
        );
      }
    }
  }
