...
describe("canvas host state dir defaults", () => {
  ...
  it("uses CRABFORK_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("crabfork-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("Crabfork Canvas");
      } finally {
        await handler.close();
      }
    });
