...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.crabforkCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.crabforkCanvasA2UIAction &&
        typeof window.crabforkCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.crabforkSendUserAction === "function";
  ...
  window.addEventListener("crabfork:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android Crabfork node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.crabforkSendUserAction === "function"
        ? window.crabforkSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.CRABFORK_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.CRABFORK_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>Crabfork Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
