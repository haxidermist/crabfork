...
const mocks = vi.hoisted(() => {
  ...
  let configState: CrabforkConfig = {
    tools: {
      exec: {
        host: "auto",
        security: "allowlist",
  ...
  return {
    ...
    defaultRuntime,
    runtimeErrors,
    mutateConfigFile: vi.fn(async ({ mutate }: { mutate: (draft: CrabforkConfig) => void }) => {
      ...
      return {
        ...
        previousHash: "hash-1",
        snapshot: { path: "/tmp/crabfork.json" },
        nextConfig: draft,
        result: undefined,
      };
    }),
    replaceConfigFile: vi.fn(
      async ({ nextConfig }: { nextConfig: CrabforkConfig; baseHash?: string }) => {
    ...
    readConfigFileSnapshot: vi.fn<
      () => Promise<{ path: string; hash: string; config: CrabforkConfig }>
    >(async () => ({
      path: "/tmp/crabfork.json",
      hash: "config-hash-1",
      config: configState,
...
describe("exec-policy CLI", () => {
  ...
  beforeEach(() => {
    ...
    mocks.mutateConfigFile.mockImplementation(
      async ({ mutate }: { mutate: (draft: CrabforkConfig) => void }) => {
  ...
  it("sanitizes terminal control content before rendering the text table", async () => {
    ...
    expect(output).toContain("/tmp/crabfork.json");
    ...
    expect(output).not.toContain("/tmp/crabfork.json\nforged");
