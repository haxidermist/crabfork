...
describe("tryRouteCli", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    originalDisableRouteFirst = process.env.CRABFORK_DISABLE_ROUTE_FIRST;
    originalHideBanner = process.env.CRABFORK_HIDE_BANNER;
    delete process.env.CRABFORK_DISABLE_ROUTE_FIRST;
  ...
  afterEach(() => {
    ...
    if (originalDisableRouteFirst === undefined) {
      ...
      process.env.CRABFORK_DISABLE_ROUTE_FIRST = originalDisableRouteFirst;
    }
  ...
  it("skips config guard for routed status --json commands", async () => {
    await expect(tryRouteCli(["node", "crabfork", "status", "--json"])).resolves.toBe(true);
  ...
  it("does not pass suppressDoctorStdout for routed non-json commands", async () => {
    await expect(tryRouteCli(["node", "crabfork", "status"])).resolves.toBe(true);
  ...
  it("routes logs to stderr during plugin loading in --json mode and restores after", async () => {
    ...
    await tryRouteCli(["node", "crabfork", "agents", "--json"]);
  ...
  it("does not route logs to stderr during plugin loading without --json", async () => {
    ...
    await tryRouteCli(["node", "crabfork", "agents"]);
  ...
  it("routes status when root options precede the command", async () => {
    await expect(tryRouteCli(["node", "crabfork", "--log-level", "debug", "status"])).resolves.toBe(
      true,
    );
  ...
  it("respects CRABFORK_HIDE_BANNER for routed commands", async () => {
