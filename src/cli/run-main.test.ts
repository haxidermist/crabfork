  ...
  plugins: [
    {
      ...
      source: "bundled",
      manifestPath: "/tmp/memory-wiki/crabfork.plugin.json",
...
describe("shouldEnsureCliPath", () => {
  it("skips path bootstrap for help/version invocations", () => {
    expect(shouldEnsureCliPath(["node", "crabfork", "--help"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "crabfork", "-V"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "crabfork", "-v"])).toBe(false);
  });

  it("skips path bootstrap for read-only fast paths", () => {
    expect(shouldEnsureCliPath(["node", "crabfork", "status"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "crabfork", "--log-level", "debug", "status"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "crabfork", "sessions", "--json"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "crabfork", "config", "get", "update"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "crabfork", "models", "status", "--json"])).toBe(false);
  });

  it("keeps path bootstrap for mutating or unknown commands", () => {
    expect(shouldEnsureCliPath(["node", "crabfork", "message", "send"])).toBe(true);
    expect(shouldEnsureCliPath(["node", "crabfork", "voicecall", "status"])).toBe(true);
    expect(shouldEnsureCliPath(["node", "crabfork", "acp", "-v"])).toBe(true);
  });
});

describe("shouldUseRootHelpFastPath", () => {
  it("uses the fast path for root help only", () => {
    expect(shouldUseRootHelpFastPath(["node", "crabfork", "--help"])).toBe(true);
    expect(shouldUseRootHelpFastPath(["node", "crabfork", "--profile", "work", "-h"])).toBe(true);
    expect(shouldUseRootHelpFastPath(["node", "crabfork", "status", "--help"])).toBe(false);
    expect(shouldUseRootHelpFastPath(["node", "crabfork", "--help", "status"])).toBe(false);
  });
});
...
describe("resolveMissingPluginCommandMessage", () => {
  ...
  it("explains that dreaming is a runtime slash command, not a CLI command", () => {
    ...
    expect(message).toContain("crabfork memory");
  });
