...
describe("proxy cli runtime", () => {
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(path.join(os.tmpdir(), "crabfork-proxy-cli-runtime-"));
    ...
    process.env.CRABFORK_DEBUG_PROXY_CERT_DIR = path.join(tempDir, "certs");
    delete process.env.CRABFORK_DEBUG_PROXY_ENABLED;
    delete process.env.CRABFORK_DEBUG_PROXY_SESSION_ID;
