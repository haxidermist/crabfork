...
const serveCrabforkChannelMcp = mocks.serveCrabforkChannelMcp;
...
vi.mock("../mcp/channel-server.js", () => ({
  serveCrabforkChannelMcp: mocks.serveCrabforkChannelMcp,
}));

const tempDirs: string[] = [];

async function createWorkspace(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "crabfork-cli-mcp-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("mcp cli", () => {
  ...
  it("sets and shows a configured MCP server", async () => {
    await withTempHome("crabfork-cli-mcp-home-", async () => {
  ...
  it("starts the channel bridge with parsed serve options", async () => {
    await withTempHome("crabfork-cli-mcp-home-", async () => {
      ...
      expect(serveCrabforkChannelMcp).toHaveBeenCalledWith({
