...
export function resolveMissingPluginCommandMessage(
  ...
  if (parentPluginId) {
    ...
    if (config?.plugins?.entries?.[parentPluginId]?.enabled === false) {
      return (
        `The \`crabfork ${normalizedPluginId}\` command is unavailable because ` +
    ...
    if (commandAlias.kind === "runtime-slash") {
      const cliHint = commandAlias.cliCommand
        ? `Use \`crabfork ${commandAlias.cliCommand}\` for related CLI operations, or `
        : "Use ";
      return (
...
export async function runCli(argv: string[] = process.argv) {
  ...
  const containerTargetName =
    parsedContainer.container ?? normalizeOptionalString(process.env.CRABFORK_CONTAINER) ?? null;
  ...
  try {
    ...
    process.on("uncaughtException", (error) => {
      console.error("[crabfork] Uncaught exception:", formatUncaughtError(error));
