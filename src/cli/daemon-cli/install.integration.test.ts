...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("crabfork-daemon-install-int-");
    configPath = path.join(tempHome, "crabfork.json");
    process.env.HOME = tempHome;
    process.env.CRABFORK_STATE_DIR = tempHome;
  ...
  it("auto-mints token when no source exists without embedding it into service env", async () => {
    ...
    expect(installEnv?.CRABFORK_GATEWAY_TOKEN).toBeUndefined();
  });
});
