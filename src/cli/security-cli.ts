...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["crabfork security audit", "Run a local security audit."],
          ["crabfork security audit --deep", "Include best-effort live Gateway probe checks."],
          ["crabfork security audit --deep --token <token>", "Use explicit token for deep probe."],
          [
            "crabfork security audit --deep --password <password>",
            "Use explicit password for deep probe.",
          ],
          ["crabfork security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["crabfork security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.crabfork.ai/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("Crabfork security audit"));
      lines.push(muted(`Summary: ${formatSummary(report.summary)}`));
      lines.push(muted(`Run deeper: ${formatCliCommand("crabfork security audit --deep")}`));
      for (const diagnostic of secretDiagnostics) {
        lines.push(muted(`[secrets] ${diagnostic}`));
      }

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("crabfork security audit --fix")}`));
        if (!fixResult) {
