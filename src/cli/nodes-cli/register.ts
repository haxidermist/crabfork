    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["crabfork nodes status", "List known nodes with live status."],
          ["crabfork nodes pairing pending", "Show pending node pairing requests."],
          [
            'crabfork nodes invoke --node <id> --command system.which --params \'{"name":"uname"}\'',
            "Invoke a node command directly.",
          ],
          ["crabfork nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.crabfork.ai/cli/nodes")}\n`,
    );
