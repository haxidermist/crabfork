    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "crabfork node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["crabfork node status", "Check node host service status."],
          ["crabfork node install", "Install the node host service."],
          ["crabfork node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.crabfork.ai/cli/node")}\n`,
    );
