...
describe("resolveGatewayInstallEntrypointCandidates", () => {
  it("prefers index.js before legacy entry.js", () => {
    expect(resolveGatewayInstallEntrypointCandidates("/tmp/crabfork-root")).toEqual([
      path.join("/tmp/crabfork-root", "dist", "index.js"),
      path.join("/tmp/crabfork-root", "dist", "index.mjs"),
      path.join("/tmp/crabfork-root", "dist", "entry.js"),
      path.join("/tmp/crabfork-root", "dist", "entry.mjs"),
    ]);
  });
});
