...
export function registerUpdateCli(program: Command) {
  ...
  const update = program
    .command("update")
    .description("Update Crabfork and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["crabfork update", "Update a source checkout (git)"],
        ["crabfork update --channel beta", "Switch to beta channel (git + npm)"],
        ["crabfork update --channel dev", "Switch to dev channel (git + npm)"],
        ["crabfork update --tag beta", "One-off update to a dist-tag or version"],
        ["crabfork update --tag main", "One-off package install from GitHub main"],
        ["crabfork update --dry-run", "Preview actions without changing anything"],
        ["crabfork update --no-restart", "Update without restarting the service"],
        ["crabfork update --json", "Output result as JSON"],
        ["crabfork update --yes", "Non-interactive (accept downgrade prompts)"],
        ["crabfork update wizard", "Interactive update wizard"],
        ["crabfork --update", "Shorthand for crabfork update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run crabfork update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.crabfork.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["crabfork update status", "Show channel + version status."],
          ["crabfork update status --json", "JSON output."],
          ["crabfork update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.crabfork.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
