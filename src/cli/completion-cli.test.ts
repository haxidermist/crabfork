...
describe("completion-cli", () => {
  it("generates zsh functions for nested subcommands", () => {
    ...
    expect(script).toContain("_crabfork_gateway()");
    expect(script).toContain("(status) _crabfork_gateway_status ;;");
    expect(script).toContain("(restart) _crabfork_gateway_restart ;;");
  ...
  it("defers zsh registration until compinit is available", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "crabfork-zsh-completion-"));
    try {
      const scriptPath = path.join(tempDir, "crabfork.zsh");
      ...
      const result = spawnSync(
        ...
        [
          ...
          `
            ...
            [[ -z "\${_comps[crabfork]-}" ]] || exit 10
            [[ "\${precmd_functions[(r)_crabfork_register_completion]}" = "_crabfork_register_completion" ]] || exit 11
            autoload -Uz compinit
            compinit -C
            _crabfork_register_completion
            [[ -z "\${precmd_functions[(r)_crabfork_register_completion]}" ]] || exit 12
            [[ "\${_comps[crabfork]-}" = "_crabfork_root_completion" ]]
          `,
        ],
        {
          encoding: "utf8",
  ...
  it("generates PowerShell command paths without the executable prefix", () => {
    ...
    expect(script).not.toContain("if ($commandPath -eq 'crabfork gateway') {");
  ...
  it("generates fish completions for root and nested command contexts", () => {
    ...
    expect(script).toContain(
      'complete -c crabfork -n "__fish_use_subcommand" -a "gateway" -d \'Gateway commands\'',
    );
    expect(script).toContain(
      'complete -c crabfork -n "__fish_seen_subcommand_from gateway" -a "status" -d \'Show gateway status\'',
    );
    expect(script).toContain(
      "complete -c crabfork -n \"__fish_seen_subcommand_from gateway\" -l force -d 'Force the action'",
    );
  });
});
