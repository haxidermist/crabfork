      ...
      .addHelpText(
        ...
        () =>
          `\n${theme.heading("Examples:")}\n${formatHelpExamples([
            ["crabfork gateway run", "Run the gateway in the foreground."],
            ["crabfork gateway status", "Show service status and probe reachability."],
            ["crabfork gateway discover", "Find local and wide-area gateway beacons."],
            ["crabfork gateway call health", "Call a gateway RPC method directly."],
          ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/gateway", "docs.crabfork.ai/cli/gateway")}\n`,
      ),
  );
