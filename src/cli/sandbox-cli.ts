...
const SANDBOX_EXAMPLES = {
  main: [
    ["crabfork sandbox list", "List all sandbox containers."],
    ["crabfork sandbox list --browser", "List only browser containers."],
    ["crabfork sandbox recreate --all", "Recreate all containers."],
    ["crabfork sandbox recreate --session main", "Recreate a specific session."],
    ["crabfork sandbox recreate --agent mybot", "Recreate agent containers."],
    ["crabfork sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["crabfork sandbox list", "List all sandbox containers."],
    ["crabfork sandbox list --browser", "List only browser containers."],
  ...
  recreate: [
    ["crabfork sandbox recreate --all", "Recreate all containers."],
    ["crabfork sandbox recreate --session main", "Recreate a specific session."],
    ["crabfork sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["crabfork sandbox recreate --browser --all", "Recreate only browser containers."],
    ["crabfork sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["crabfork sandbox explain", "Show effective sandbox config."],
    ["crabfork sandbox explain --session agent:main:main", "Explain a specific session."],
    ["crabfork sandbox explain --agent work", "Explain an agent sandbox."],
    ["crabfork sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.crabfork.ai/cli/sandbox")}\n`,
    )
    .action(() => {
