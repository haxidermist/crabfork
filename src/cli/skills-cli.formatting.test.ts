...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "crabfork-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "crabfork-bundled-skills-test-"));
    process.env.CRABFORK_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
