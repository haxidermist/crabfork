...
describe("completion-cli write-state", () => {
  ...
  it("keeps completion cache generation alive when a subcli fails to register", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "crabfork-completion-state-"));
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "crabfork-completion-home-"));
    ...
    expect(await fs.readdir(cacheDir)).toEqual(
      expect.arrayContaining(["crabfork.bash", "crabfork.fish", "crabfork.ps1", "crabfork.zsh"]),
    );
