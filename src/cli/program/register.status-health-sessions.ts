...
export function registerStatusHealthSessionsCommands(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["crabfork status", "Show channel health + session summary."],
          ["crabfork status --all", "Full diagnosis (read-only)."],
          ["crabfork status --json", "Machine-readable output."],
          ["crabfork status --usage", "Show model provider usage/quota snapshots."],
          [
            "crabfork status --deep",
            "Run channel probes (WA + Telegram + Discord + Slack + Signal).",
          ],
          ["crabfork status --deep --timeout 5000", "Tighten probe timeout."],
        ])}`,
    )
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/status", "docs.crabfork.ai/cli/status")}\n`,
    )
    .action(async (opts) => {
  ...
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/health", "docs.crabfork.ai/cli/health")}\n`,
    )
    .action(async (opts) => {
      await runWithVerboseAndTimeout(opts, async ({ verbose, timeoutMs }) => {
        await healthCommand(
  ...
  const sessionsCmd = program
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["crabfork sessions", "List all sessions."],
          ["crabfork sessions --agent work", "List sessions for one agent."],
          ["crabfork sessions --all-agents", "Aggregate sessions across agents."],
          ["crabfork sessions --active 120", "Only last 2 hours."],
          ["crabfork sessions --json", "Machine-readable output."],
          ["crabfork sessions --store ./tmp/sessions.json", "Use a specific session store."],
        ])}\n\n${theme.muted(
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sessions", "docs.crabfork.ai/cli/sessions")}\n`,
    )
    .action(async (opts) => {
  ...
  sessionsCmd
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["crabfork sessions cleanup --dry-run", "Preview stale/cap cleanup."],
          [
            "crabfork sessions cleanup --dry-run --fix-missing",
            "Also preview pruning entries with missing transcript files.",
          ],
          ["crabfork sessions cleanup --enforce", "Apply maintenance now."],
          ["crabfork sessions cleanup --agent work --dry-run", "Preview one agent store."],
          ["crabfork sessions cleanup --all-agents --dry-run", "Preview all agent stores."],
          [
            "crabfork sessions cleanup --enforce --store ./tmp/sessions.json",
