...
${formatHelpExamples([
  ['crabfork agent --to +15555550123 --message "status update"', "Start a new session."],
  ['crabfork agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'crabfork agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'crabfork agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['crabfork agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'crabfork agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.crabfork.ai/cli/agent")}`,
    )
    .action(async (opts) => {
      const verboseLevel =
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.crabfork.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['crabfork agents set-identity --agent main --name "Crabfork" --emoji "🦞"', "Set name + emoji."],
  ["crabfork agents set-identity --agent main --avatar avatars/crabfork.png", "Set avatar path."],
  [
    "crabfork agents set-identity --workspace ~/.crabfork/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "crabfork agents set-identity --identity-file ~/.crabfork/workspace/IDENTITY.md --agent main",
