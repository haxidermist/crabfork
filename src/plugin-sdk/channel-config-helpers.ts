...
function formatPairingApproveHint(channelId: string): string {
  const listCmd = formatCliCommand(`crabfork pairing list ${channelId}`);
  const approveCmd = formatCliCommand(`crabfork pairing approve ${channelId} <code>`);
...
type ChannelConfigAccessorParams<Config extends CrabforkConfig = CrabforkConfig> = {
  cfg: Config;
  accountId?: string | null;
};
...
type MultiAccountChannelConfigAdapterParams<
  ...
  Config extends CrabforkConfig = CrabforkConfig,
> = {
  sectionKey: string;
...
export function adaptScopedAccountAccessor<Result, Config extends CrabforkConfig = CrabforkConfig>(
...
> {
  const base = {
    resolveAllowFrom({ cfg, accountId }: { cfg: CrabforkConfig; accountId?: string | null }) {
...
function setTopLevelChannelEnabledInConfigSection<Config extends CrabforkConfig>(params: {
  cfg: Config;
  sectionKey: string;
  enabled: boolean;
}): Config {
...
function removeTopLevelChannelConfigSection<Config extends CrabforkConfig>(params: {
  cfg: Config;
  sectionKey: string;
}): Config {
...
function clearTopLevelChannelConfigFields<Config extends CrabforkConfig>(params: {
  cfg: Config;
  sectionKey: string;
  clearBaseFields: string[];
}): Config {
