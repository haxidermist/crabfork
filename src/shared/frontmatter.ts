...
export function resolveCrabforkManifestBlock(params: {
  frontmatter: Record<string, unknown>;
  key?: string;
...
export type CrabforkManifestRequires = {
  bins: string[];
  anyBins: string[];
  env: string[];
  config: string[];
...
export function resolveCrabforkManifestRequires(
  metadataObj: Record<string, unknown>,
): CrabforkManifestRequires | undefined {
  const requiresRaw =
...
export function resolveCrabforkManifestInstall<T>(
  metadataObj: Record<string, unknown>,
...
export function resolveCrabforkManifestOs(metadataObj: Record<string, unknown>): string[] {
  return normalizeStringList(metadataObj.os);
}

export type ParsedCrabforkManifestInstallBase = {
...
export function parseCrabforkManifestInstallBase(
  input: unknown,
  allowedKinds: readonly string[],
): ParsedCrabforkManifestInstallBase | undefined {
  ...
  const spec: ParsedCrabforkManifestInstallBase = {
    raw,
    kind,
  };
...
export function applyCrabforkManifestInstallCommonFields<
  T extends { id?: string; label?: string; bins?: string[] },
>(spec: T, parsed: Pick<ParsedCrabforkManifestInstallBase, "id" | "label" | "bins">): T {
  if (parsed.id) {
    spec.id = parsed.id;
  }
  if (parsed.label) {
