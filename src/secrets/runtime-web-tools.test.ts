...
function setConfiguredFetchProviderKey(configTarget: CrabforkConfig, value: unknown): void {
  const plugins = ensureRecord(configTarget as Record<string, unknown>, "plugins");
  const entries = ensureRecord(plugins, "entries");
...
async function runRuntimeWebTools(params: { config: CrabforkConfig; env?: NodeJS.ProcessEnv }) {
...
function readProviderKey(config: CrabforkConfig, provider: ProviderUnderTest): unknown {
