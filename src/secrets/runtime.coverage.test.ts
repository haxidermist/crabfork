...
const DEBUG_COVERAGE_BATCHES = process.env.CRABFORK_DEBUG_RUNTIME_COVERAGE === "1";
const COVERAGE_LOADABLE_PLUGIN_ORIGINS =
  buildCoverageLoadablePluginOrigins(COVERAGE_REGISTRY_ENTRIES);
const PLUGIN_OWNED_CRABFORK_COVERAGE_EXCLUSIONS = new Set([
...
function applyConfigForCrabforkTarget(
...
describe("secrets runtime target coverage", () => {
  ...
  it("handles every crabfork.json registry target when configured as active", async () => {
    const entries = COVERAGE_REGISTRY_ENTRIES.filter(
      (entry) =>
        entry.configFile === "crabfork.json" &&
        !PLUGIN_OWNED_CRABFORK_COVERAGE_EXCLUSIONS.has(entry.id),
    );
    for (const batch of buildCoverageBatches(entries)) {
      logCoverageBatch("crabfork.json", batch);
      const config = {} as CrabforkConfig;
      const env: Record<string, string> = {};
      for (const [index, entry] of batch.entries()) {
        const envId = `CRABFORK_SECRET_TARGET_${entry.id}`;
        ...
        applyConfigForCrabforkTarget(config, entry, envId, wildcardToken);
      }
  ...
  it("handles every auth-profiles registry target", async () => {
    ...
    for (const batch of buildCoverageBatches(entries)) {
      ...
      for (const [index, entry] of batch.entries()) {
        const envId = `CRABFORK_AUTH_SECRET_TARGET_${entry.id}`;
