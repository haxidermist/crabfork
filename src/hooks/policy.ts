import type { CrabforkConfig, HookConfig } from "../config/config.js";
...
const HOOK_SOURCE_POLICIES: Record<HookSource, HookSourcePolicy> = {
  "crabfork-bundled": {
    ...
    canOverride: ["crabfork-bundled"],
    canBeOverriddenBy: ["crabfork-managed", "crabfork-plugin"],
  },
  "crabfork-plugin": {
    precedence: 20,
    trustedLocalCode: true,
    defaultEnableMode: "default-on",
    canOverride: ["crabfork-bundled", "crabfork-plugin"],
    canBeOverriddenBy: ["crabfork-managed"],
  },
  "crabfork-managed": {
    precedence: 30,
    trustedLocalCode: true,
    defaultEnableMode: "default-on",
    canOverride: ["crabfork-bundled", "crabfork-managed", "crabfork-plugin"],
    canBeOverriddenBy: ["crabfork-managed"],
  },
  "crabfork-workspace": {
    precedence: 40,
    ...
    canOverride: ["crabfork-workspace"],
    canBeOverriddenBy: ["crabfork-workspace"],
  },
};
...
export function resolveHookEnableState(params: {
  ...
  if (entry.hook.source === "crabfork-plugin") {
    return { enabled: true };
  }
