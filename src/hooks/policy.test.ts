...
describe("hook policy", () => {
  describe("resolveHookEnableState", () => {
    it("keeps workspace hooks disabled by default", () => {
      const entry = makeHookEntry("workspace-hook", "crabfork-workspace");
    ...
    it("keeps plugin hooks enabled without local hook toggles", () => {
      const entry = makeHookEntry("plugin-hook", "crabfork-plugin");
  ...
  describe("resolveHookEntries", () => {
    it("lets managed hooks override bundled and plugin hooks", () => {
      const bundled = makeHookEntry("shared", "crabfork-bundled");
      const plugin = makeHookEntry("shared", "crabfork-plugin");
      const managed = makeHookEntry("shared", "crabfork-managed");

      const resolved = resolveHookEntries([bundled, plugin, managed]);
      expect(resolved).toHaveLength(1);
      expect(resolved[0]?.hook.source).toBe("crabfork-managed");
    });

    it("prevents workspace hooks from overriding non-workspace hooks", () => {
      const managed = makeHookEntry("shared", "crabfork-managed");
      const workspace = makeHookEntry("shared", "crabfork-workspace");
    ...
    it("keeps later workspace entries for the same source/name", () => {
      const first = makeHookEntry("shared", "crabfork-workspace");
      const second = makeHookEntry("shared", "crabfork-workspace");
      second.hook.handlerPath = "/tmp/crabfork-workspace/shared/handler-2.js";
