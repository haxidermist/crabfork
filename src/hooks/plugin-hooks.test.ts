...
describe("bundle plugin hooks", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "crabfork-plugin-hooks-"));
  });
  ...
  beforeEach(async () => {
    ...
    previousBundledHooksDir = process.env.CRABFORK_BUNDLED_HOOKS_DIR;
    process.env.CRABFORK_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
  ...
  afterEach(() => {
    ...
    if (previousBundledHooksDir === undefined) {
      ...
      process.env.CRABFORK_BUNDLED_HOOKS_DIR = previousBundledHooksDir;
    }
  });
  ...
  async function writeBundleHookFixture(): Promise<string> {
    const bundleRoot = path.join(workspaceDir, ".crabfork", "extensions", "sample-bundle");
  ...
  function createConfig(enabled: boolean): CrabforkConfig {
    return {
      hooks: {
        internal: {
          enabled: true,
  ...
  it("exposes enabled bundle hook dirs as plugin-managed hook entries", async () => {
    ...
    expect(entries[0]?.hook.source).toBe("crabfork-plugin");
  ...
  it("does not treat Claude hooks.json bundles as Crabfork hook packs", async () => {
    const bundleRoot = path.join(workspaceDir, ".crabfork", "extensions", "claude-bundle");
