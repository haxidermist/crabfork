...
This directory contains hooks that ship with Crabfork. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.crabfork/workspace`)
...
homepage: https://docs.crabfork.ai/automation/hooks#my-hook
metadata:
...
- **Managed hooks**: `~/.crabfork/hooks/` (shared across workspaces)
...
Hooks can be configured in `~/.crabfork/crabfork.json`:
...
2. Restart gateway: `pkill -9 -f 'crabfork.*gateway' && pnpm crabfork gateway`
3. Enable the hook: `crabfork hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.crabfork.ai/automation/hooks
