...
homepage: https://docs.crabfork.ai/automation/hooks#command-logger
metadata:
  {
    "crabfork":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.crabfork/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.crabfork/logs/commands.log ~/.crabfork/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/crabfork`:
   ```
   /home/username/.crabfork/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
