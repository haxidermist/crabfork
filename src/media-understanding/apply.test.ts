...
const TEMP_MEDIA_PREFIX = "crabfork-media-";
...
function createGroqAudioConfig(): CrabforkConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): CrabforkConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): CrabforkConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: CrabforkConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  beforeAll(async () => {
    ...
    const baseDir = resolvePreferredCrabforkTmpDir();
