...
const PRIMARY_PACKAGE_NAME = "crabfork";
const ALL_PACKAGE_NAMES = [PRIMARY_PACKAGE_NAME] as const;
const GLOBAL_RENAME_PREFIX = ".";
export const CRABFORK_MAIN_PACKAGE_SPEC = "github:crabfork/crabfork#main";
...
async function resolvePortableGitPathPrepend(
  ...
  const portableGitRoot = path.join(localAppData, "Crabfork", "deps", "portable-git");
  const candidates = [
...
export function resolveGlobalInstallSpec(params: {
  ...
  const override =
    params.env?.CRABFORK_UPDATE_PACKAGE_SPEC?.trim() ||
    process.env.CRABFORK_UPDATE_PACKAGE_SPEC?.trim();
  if (override) {
    return override;
  }
  const target = normalizePackageTarget(params.tag);
  if (isMainPackageTarget(target)) {
    return CRABFORK_MAIN_PACKAGE_SPEC;
  }
