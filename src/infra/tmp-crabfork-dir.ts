...
export const POSIX_CRABFORK_TMP_DIR = "/tmp/crabfork";
const TMP_DIR_ACCESS_MODE = fs.constants.W_OK | fs.constants.X_OK;

type ResolvePreferredCrabforkTmpDirOptions = {
...
export function resolvePreferredCrabforkTmpDir(
  options: ResolvePreferredCrabforkTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "crabfork" : `crabfork-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[crabfork] tightened permissions on temp dir: ${candidatePath}`);
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback Crabfork temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback Crabfork temp dir: ${fallbackPath}`);
    }
  ...
  const existingPreferredState = resolveDirState(POSIX_CRABFORK_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_CRABFORK_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_CRABFORK_TMP_DIR)) {
      return POSIX_CRABFORK_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_CRABFORK_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_CRABFORK_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_CRABFORK_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_CRABFORK_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_CRABFORK_TMP_DIR;
