...
vi.mock("./crabfork-root.js", () => ({
  resolveCrabforkPackageRoot: vi.fn(async () => null),
  resolveCrabforkPackageRootSync: vi.fn(() => null),
}));
...
let crabforkRoot: typeof import("./crabfork-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    crabforkRoot = await import("./crabfork-root.js");
  });
  ...
  it("resolves dist control-ui index path for symlinked argv1 via realpath", async () => {
    const pkgRoot = abs("fixtures/bun-global/crabfork");
    const wrapperArgv1 = abs("fixtures/bin/crabfork");
  ...
  it("uses resolveCrabforkPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/crabfork");
    (
      crabforkRoot.resolveCrabforkPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/crabfork"))).resolves.toBe(
      path.join(pkgRoot, "dist", "control-ui", "index.html"),
    );
  });
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "crabfork" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "crabfork.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-crabfork");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/crabfork-bundle");
    (
      crabforkRoot.resolveCrabforkPackageRootSync as unknown as ReturnType<typeof vi.fn>
  ...
  it("prefers packaged app Control UI assets in Contents/Resources", () => {
    const execPath = abs("fixtures/Crabfork.app/Contents/MacOS/Crabfork");
    const bundledUiDir = abs("fixtures/Crabfork.app/Contents/Resources/control-ui");
  ...
  it("detects package-proven control-ui roots", () => {
    const pkgRoot = abs("fixtures/crabfork-package-root");
    const uiDir = path.join(pkgRoot, "dist", "control-ui");
    setDir(uiDir);
    setFile(path.join(uiDir, "index.html"), "<html></html>\n");
    (
