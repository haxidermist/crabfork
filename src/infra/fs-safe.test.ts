...
async function withOutsideHardlinkAlias(params: {
  ...
  const outside = await tempDirs.make("crabfork-fs-safe-outside-");
...
async function setupSymlinkWriteRaceFixture(options?: { seedInsideTarget?: boolean }): Promise<{
  ...
  const root = await tempDirs.make("crabfork-fs-safe-root-");
...
describe("fs-safe", () => {
  it("reads a local file safely", async () => {
    const dir = await tempDirs.make("crabfork-fs-safe-");
  ...
  it("copies a file within root safely", async () => {
    const root = await tempDirs.make("crabfork-fs-safe-root-");
    const sourceDir = await tempDirs.make("crabfork-fs-safe-source-");
  ...
  it("writes a file within root from another local source path safely", async () => {
    const root = await tempDirs.make("crabfork-fs-safe-root-");
    const outside = await tempDirs.make("crabfork-fs-safe-src-");
...
describe("tilde expansion in file tools", () => {
  it("keeps tilde expansion behavior aligned", async () => {
    ...
    const originalCrabforkHome = process.env.CRABFORK_HOME;
    const fakeHome = path.resolve(path.sep, "tmp", "fake-home-test");
    process.env.HOME = fakeHome;
    process.env.CRABFORK_HOME = fakeHome;
    try {
      const result = expandHomePrefix("~/file.txt");
      expect(path.normalize(result)).toBe(path.join(fakeHome, "file.txt"));
    } finally {
      process.env.HOME = originalHome;
      process.env.CRABFORK_HOME = originalCrabforkHome;
    }

    const root = await tempDirs.make("crabfork-tilde-test-");
    process.env.HOME = root;
    process.env.CRABFORK_HOME = root;
    try {
    ...
    const outsideRoot = await tempDirs.make("crabfork-tilde-outside-");
    await expect(
      openFileWithinRoot({
        rootDir: outsideRoot,
        relativePath: "~/escape.txt",
