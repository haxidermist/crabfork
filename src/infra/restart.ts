...
export function triggerCrabforkRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.CRABFORK_SYSTEMD_UNIT,
      process.env.CRABFORK_PROFILE,
    );
  ...
  const label =
    process.env.CRABFORK_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.CRABFORK_PROFILE);
