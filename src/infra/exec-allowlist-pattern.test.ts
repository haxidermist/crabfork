...
describe("matchesExecAllowlistPattern", () => {
  ...
  it("expands home-prefix patterns", () => {
    const prevCrabforkHome = process.env.CRABFORK_HOME;
    const prevHome = process.env.HOME;
    process.env.CRABFORK_HOME = "/srv/crabfork-home";
    process.env.HOME = "/home/other";
    const crabForkHome = path.join(path.resolve("/srv/crabfork-home"), "bin", "tool");
    const fallbackHome = path.join(path.resolve("/home/other"), "bin", "tool");
    try {
      expect(matchesExecAllowlistPattern("~/bin/tool", crabForkHome)).toBe(true);
      expect(matchesExecAllowlistPattern("~/bin/tool", fallbackHome)).toBe(false);
    } finally {
      if (prevCrabforkHome === undefined) {
        delete process.env.CRABFORK_HOME;
      } else {
        process.env.CRABFORK_HOME = prevCrabforkHome;
      }
