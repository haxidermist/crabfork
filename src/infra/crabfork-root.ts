...
import { crabForkRootFs, crabForkRootFsSync } from "./crabfork-root.fs.runtime.js";

const CORE_PACKAGE_NAMES = new Set(["crabfork"]);
...
async function readPackageName(dir: string): Promise<string | null> {
  try {
    return parsePackageName(await crabForkRootFs.readFile(path.join(dir, "package.json"), "utf-8"));
  } catch {
    return null;
  }
}
...
function readPackageNameSync(dir: string): string | null {
  try {
    return parsePackageName(
      crabForkRootFsSync.readFileSync(path.join(dir, "package.json"), "utf-8"),
    );
  } catch {
    return null;
  }
...
function candidateDirsFromArgv1(argv1: string): string[] {
  ...
  try {
    const resolved = crabForkRootFsSync.realpathSync(normalized);
...
export async function resolveCrabforkPackageRoot(opts: {
  cwd?: string;
  argv1?: string;
  moduleUrl?: string;
}): Promise<string | null> {
...
export function resolveCrabforkPackageRootSync(opts: {
  cwd?: string;
  argv1?: string;
  moduleUrl?: string;
}): string | null {
