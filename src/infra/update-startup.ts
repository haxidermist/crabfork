...
import { resolveCrabforkPackageRoot } from "./crabfork-root.js";
...
function resolveAutoUpdatePolicy(cfg: CrabforkConfig): AutoUpdatePolicy {
...
function resolveCheckIntervalMs(cfg: CrabforkConfig): number {
...
export async function runGatewayUpdateCheck(params: {
  ...
  const root = await resolveCrabforkPackageRoot({
  ...
  if (cmp != null && cmp < 0) {
    ...
    if (shouldRunUpdateHints && shouldNotify) {
      params.log.info(
        `update available (${tag}): v${resolved.version} (current v${VERSION}). Run: ${formatCliCommand("crabfork update")}`,
      );
