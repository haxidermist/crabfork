...
function candidateBinDirs(opts: EnsureCrabforkPathOpts): { prepend: string[]; append: string[] } {
  ...
  // subprocesses keep using the same Node/Bun the current Crabfork process is on.
  try {
  ...
  // Bundled macOS app: `crabfork` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "crabfork");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.CRABFORK_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "crabfork"))) {
  ...
  // This includes Brew/Homebrew dirs, which are useful for finding `crabfork`
...
/**
 * Best-effort PATH bootstrap so skills that require the `crabfork` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureCrabforkCliOnPath(opts: EnsureCrabforkPathOpts = {}) {
  if (isTruthyEnvValue(process.env.CRABFORK_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.CRABFORK_PATH_BOOTSTRAPPED = "1";
