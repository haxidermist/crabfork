...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.CRABFORK_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*crabfork-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "crabfork";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "crabfork");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "crabfork-gw");
  ...
  records.push(`_crabfork-gw._tcp IN PTR ${instanceLabel}._crabfork-gw._tcp`);
  records.push(`${instanceLabel}._crabfork-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._crabfork-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; crabfork-content-hash: ${contentHash}\n${contentBody}`;
}
