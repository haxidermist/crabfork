...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.CRABFORK_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "Crabfork";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(Crabfork\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  try {
    ...
    const hostnameRaw = process.env.CRABFORK_MDNS_HOSTNAME?.trim() || "crabfork";
    const hostname =
      hostnameRaw
        .replace(/\.local$/i, "")
        .split(".")[0]
