...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: CrabforkConfig) => void;
};
...
function hasExplicitHeartbeatAgents(cfg: CrabforkConfig) {
...
function resolveHeartbeatAgents(cfg: CrabforkConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: CrabforkConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: CrabforkConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: CrabforkConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
