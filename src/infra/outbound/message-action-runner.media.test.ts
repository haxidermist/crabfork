...
describe("runMessageAction media behavior", () => {
  ...
  describe("sandboxed media validation", () => {
    ...
    it("allows media paths under preferred Crabfork tmp root", async () => {
      ...
      try {
        ...
        const hostTmpOutsideCrabfork = path.join(os.tmpdir(), "outside-crabfork", "test-media.png");
        await expect(
          runMessageAction({
            cfg: slackConfig,
            action: "send",
