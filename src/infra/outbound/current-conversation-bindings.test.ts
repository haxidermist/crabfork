...
describe("generic current-conversation bindings", () => {
  ...
  beforeEach(async () => {
    previousStateDir = process.env.CRABFORK_STATE_DIR;
    testStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "crabfork-current-bindings-"));
    process.env.CRABFORK_STATE_DIR = testStateDir;
