...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: CrabforkConfig | null = null;
  ...
  get(key: string, cfg: CrabforkConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: CrabforkConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: CrabforkConfig): void {
