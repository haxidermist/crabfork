...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__crabfork_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "crabfork-root");
...
function setPackageRoot(root: string, name = "crabfork") {
  setFile(path.join(root, "package.json"), JSON.stringify({ name }));
}

function expectResolvedPackageRoot(
  syncResolver: typeof import("./crabfork-root.js").resolveCrabforkPackageRootSync,
  asyncResolver: typeof import("./crabfork-root.js").resolveCrabforkPackageRoot,
  opts: Parameters<typeof import("./crabfork-root.js").resolveCrabforkPackageRootSync>[0],
...
vi.mock("./crabfork-root.fs.runtime.js", () => ({
  crabForkRootFsSync: mockFsModule(),
  crabForkRootFs: mockFsPromisesModule(),
}));

describe("resolveCrabforkPackageRoot", () => {
  let resolveCrabforkPackageRoot: typeof import("./crabfork-root.js").resolveCrabforkPackageRoot;
  let resolveCrabforkPackageRootSync: typeof import("./crabfork-root.js").resolveCrabforkPackageRootSync;
  ...
  beforeEach(async () => {
    vi.resetModules();
    ({ resolveCrabforkPackageRoot, resolveCrabforkPackageRootSync } =
      await import("./crabfork-root.js"));
  });
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const argv1 = path.join(project, "node_modules", ".bin", "crabfork");
        const pkgRoot = path.join(project, "node_modules", "crabfork");
    ...
    },
    {
      ...
      setup: () => {
        ...
        const bin = path.join(project, "bin", "crabfork");
        const realPkg = path.join(project, "real-pkg");
        state.realpaths.set(abs(bin), abs(path.join(realPkg, "crabfork.mjs")));
    ...
    {
      name: "falls through from a non-crabfork moduleUrl candidate to cwd",
      setup: () => {
        const wrongPkgRoot = fx("moduleurl-fallthrough", "wrong");
        const cwdPkgRoot = fx("moduleurl-fallthrough", "cwd");
        setPackageRoot(wrongPkgRoot, "not-crabfork");
    ...
    {
      name: "returns null for non-crabfork package roots",
      setup: () => {
        const pkgRoot = fx("not-crabfork");
        setPackageRoot(pkgRoot, "not-crabfork");
    ...
    },
    {
      ...
      setup: () => {
        ...
        state.realpaths.set(
          abs(argv1),
          abs(path.join(project, "versions", "current", "crabfork.mjs")),
        );
        const pkgRoot = path.join(project, "node_modules", "crabfork");
        setPackageRoot(pkgRoot);
        return { opts: { argv1 }, expected: pkgRoot };
    ...
    await expectResolvedPackageRoot(
      resolveCrabforkPackageRootSync,
      resolveCrabforkPackageRoot,
      opts,
      expected,
    );
  });
