...
const suiteRootTracker = createSuiteTempRootTracker({ prefix: "crabfork-provider-auth-suite-" });
...
describe("resolveProviderAuths key normalization", () => {
  ...
  async function withSuiteHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    const stateDir = path.join(base, ".crabfork");
  ...
  function agentDirForHome(home: string): string {
    return path.join(home, ".crabfork", "agents", "main", "agent");
  }
  ...
  function buildSuiteEnv(
    ...
    const suiteEnv: NodeJS.ProcessEnv = {
      ...
      HOME: home,
      USERPROFILE: home,
      CRABFORK_STATE_DIR: path.join(home, ".crabfork"),
      ...env,
    };
  ...
  async function writeConfig(home: string, config: Record<string, unknown>) {
    ...
    await fs.writeFile(
      path.join(stateDir, "crabfork.json"),
