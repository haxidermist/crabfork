...
export function getTestTailscaleBinaryOverride(
  ...
  const forcedBinary = env.CRABFORK_TEST_TAILSCALE_BINARY?.trim();
  if (!forcedBinary) {
    return null;
  }
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for Crabfork. You can keep running the web gateway without it: \`${formatCliCommand("crabfork gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
