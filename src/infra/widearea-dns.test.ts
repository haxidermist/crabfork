...
const baseZoneOpts: WideAreaGatewayZoneOpts = {
  domain: "crabfork.internal.",
  gatewayPort: 18789,
  displayName: "Mac Studio (Crabfork)",
...
describe("wide-area DNS discovery domain helpers", () => {
  it.each([
    { value: "crabfork.internal", expected: "crabfork.internal." },
    { value: "crabfork.internal.", expected: "crabfork.internal." },
    { value: "  crabfork.internal  ", expected: "crabfork.internal." },
    { value: "", expected: null },
    { value: "   ", expected: null },
  ...
  it.each([
    {
      ...
      params: {
        env: { CRABFORK_WIDE_AREA_DOMAIN: "env.internal" } as NodeJS.ProcessEnv,
    ...
    },
    {
      ...
      params: {
        env: { CRABFORK_WIDE_AREA_DOMAIN: "   " } as NodeJS.ProcessEnv,
        configDomain: " ",
      },
      expected: null,
    },
  ...
  it("builds the default zone path from the normalized domain", () => {
    expect(getWideAreaZonePath("crabfork.internal.")).toBe(
      path.join(utils.CONFIG_DIR, "dns", "crabfork.internal.db"),
    );
  });
});
...
describe("wide-area DNS-SD zone rendering", () => {
  it("renders a zone with gateway PTR/SRV/TXT records", () => {
    const txt = renderZoneText({
      ...
      sshPort: 22,
      cliPath: "/opt/homebrew/bin/crabfork",
    });

    expectZoneRecords(txt, [
      `$ORIGIN crabfork.internal.`,
      `studio-london IN A 100.123.224.76`,
      `studio-london IN AAAA fd7a:115c:a1e0::8801:e04c`,
      `_crabfork-gw._tcp IN PTR studio-london._crabfork-gw._tcp`,
      `studio-london._crabfork-gw._tcp IN SRV 0 0 18789 studio-london`,
      `displayName=Mac Studio (Crabfork)`,
      `gatewayPort=18789`,
      `sshPort=22`,
      `cliPath=/opt/homebrew/bin/crabfork`,
  ...
  it.each([
    ...
    },
    {
      ...
      records: [
        `$ORIGIN crabfork.internal.`,
        `studio-london IN A 100.123.224.76`,
        `studio-london._crabfork-gw._tcp IN TXT`,
        `displayName=Mac Studio (Crabfork)`,
        `gatewayTls=1`,
...
describe("wide-area DNS zone writes", () => {
  ...
  it("skips rewriting unchanged content", async () => {
    ...
    expect(result).toEqual({
      zonePath: getWideAreaZonePath("crabfork.internal."),
      changed: false,
    });
  ...
  it("increments same-day serials when content changes", async () => {
    ...
    expect(writeSpy).toHaveBeenCalledWith(
      getWideAreaZonePath("crabfork.internal."),
