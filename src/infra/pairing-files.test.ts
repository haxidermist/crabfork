...
describe("pairing file helpers", () => {
  it("resolves pairing file paths from explicit base dirs", () => {
    expect(resolvePairingPaths("/tmp/crabfork-state", "devices")).toEqual({
      dir: path.join("/tmp/crabfork-state", "devices"),
      pendingPath: path.join("/tmp/crabfork-state", "devices", "pending.json"),
      pairedPath: path.join("/tmp/crabfork-state", "devices", "paired.json"),
    });
  });
