...
describe("resolveEffectiveHomeDir", () => {
  it.each([
    {
      name: "prefers CRABFORK_HOME over HOME and USERPROFILE",
      env: {
...
describe("resolveRequiredHomeDir", () => {
  it.each([
    ...
    {
      name: "returns a fully resolved path for CRABFORK_HOME",
      env: { CRABFORK_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      homedir: () => "/fallback",
      expected: path.resolve("/custom/home"),
    },
    {
      name: "returns cwd when CRABFORK_HOME is tilde-only and no fallback home exists",
      env: { CRABFORK_HOME: "~" } as NodeJS.ProcessEnv,
      homedir: () => {
        throw new Error("no home");
      },
      expected: process.cwd(),
...
describe("resolveOsHomeDir", () => {
  it("ignores CRABFORK_HOME and uses HOME", () => {
    expect(
      resolveOsHomeDir(
        {
          CRABFORK_HOME: "/srv/crabfork-home",
  ...
  it.each([
    {
      ...
      input: "~/x",
      opts: {
        env: { CRABFORK_HOME: "/srv/crabfork-home" } as NodeJS.ProcessEnv,
      },
      expected: `${path.resolve("/srv/crabfork-home")}/x`,
    },
    {
      name: "expands exact ~ using explicit home",
      input: "~",
      opts: { home: " /srv/crabfork-home " },
      expected: "/srv/crabfork-home",
    },
    {
...
describe("resolveHomeRelativePath", () => {
  it.each([
    ...
    },
    {
      ...
      expected: path.resolve("/srv/crabfork-home/docs"),
    },
    {
...
describe("resolveOsHomeRelativePath", () => {
  it("expands tilde paths using the OS home instead of CRABFORK_HOME", () => {
    expect(
