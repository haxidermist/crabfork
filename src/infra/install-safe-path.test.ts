...
describe("unscopedPackageName", () => {
  it.each([
    { value: "@crabfork/matrix", expected: "matrix" },
...
describe("packageNameMatchesId", () => {
  it.each([
    { packageName: "@crabfork/matrix", id: "matrix", expected: true },
    { packageName: "@crabfork/matrix", id: "@crabfork/matrix", expected: true },
    { packageName: "@crabfork/matrix", id: "signal", expected: false },
    { packageName: " ", id: "matrix", expected: false },
    { packageName: "@crabfork/matrix", id: " ", expected: false },
...
describe("resolveSafeInstallDir", () => {
  it("resolves install dirs under the base directory", () => {
    expect(
      ...
      ok: true,
      path: path.join("/tmp/plugins", "@crabfork__matrix"),
    });
  });
...
describe("assertCanonicalPathWithinBase", () => {
  it("accepts in-base directories", async () => {
    await withTempDir({ prefix: "crabfork-install-safe-" }, async (baseDir) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "crabfork-install-safe-" }, async (baseDir) => {
        await withTempDir({ prefix: "crabfork-install-safe-outside-" }, async (outsideDir) => {
  ...
  it.runIf(process.platform !== "win32")("rejects symlinked base directories", async () => {
    await withTempDir({ prefix: "crabfork-install-safe-" }, async (parentDir) => {
