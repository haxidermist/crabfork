...
export type SearchProvider = NonNullable<
  NonNullable<NonNullable<NonNullable<CrabforkConfig["tools"]>["web"]>["search"]>["provider"]
>;
type SearchConfig = NonNullable<NonNullable<NonNullable<CrabforkConfig["tools"]>["web"]>["search"]>;
...
function rawKeyValue(config: CrabforkConfig, provider: SearchProvider): unknown {
...
export function hasExistingKey(config: CrabforkConfig, provider: SearchProvider): boolean {
  return hasConfiguredSecretInput(rawKeyValue(config, provider));
}

function buildSearchEnvRef(config: CrabforkConfig, provider: SearchProvider): SecretRef {
  const entry =
...
function preserveDisabledState(original: CrabforkConfig, result: CrabforkConfig): CrabforkConfig {
  ...
  return {
    ...next,
    plugins: nextPlugins as CrabforkConfig["plugins"],
  };
}
...
export async function runSearchSetupFlow(
  ...
  const choice = await prompter.select({
    ...
    options: [
      ...options,
      {
        ...
        label: "Skip for now",
        hint: "Configure later with crabfork configure --section web",
      },
    ],
  ...
  if (!needsCredential) {
    await prompter.note(
      [
        ...
        "Crabfork will enable the plugin and use it as your web_search provider.",
        `Docs: ${entry.docsUrl ?? "https://docs.crabfork.ai/tools/web"}`,
      ].join("\n"),
      "Web search",
    );
    return await finalizeSearchProviderSetup({
  ...
  if (useSecretRefMode) {
    ...
    await prompter.note(
      [
        "Secret references enabled — Crabfork will store a reference instead of the API key.",
