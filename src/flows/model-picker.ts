...
export type PromptDefaultModelResult = { model?: string; config?: CrabforkConfig };
...
function resolveConfiguredModelRaw(cfg: CrabforkConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: CrabforkConfig): string[] {
...
export function applyModelAllowlist(cfg: CrabforkConfig, models: string[]): CrabforkConfig {
