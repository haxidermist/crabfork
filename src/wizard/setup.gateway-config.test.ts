...
describe("configureGatewayForSetup", () => {
  ...
  it("prefers CRABFORK_GATEWAY_TOKEN during quickstart token setup", async () => {
    const prevToken = process.env.CRABFORK_GATEWAY_TOKEN;
    process.env.CRABFORK_GATEWAY_TOKEN = "token-from-env";
    mocks.randomToken.mockReturnValue("generated-token");
  ...
  it("honors secretInputMode=ref for gateway password prompts", async () => {
    ...
    process.env.CRABFORK_GATEWAY_PASSWORD = "gateway-secret"; // pragma: allowlist secret
    try {
      const prompter = createPrompter({
        selectQueue: ["loopback", "password", "off", "env"],
        textQueue: ["18789", "CRABFORK_GATEWAY_PASSWORD"],
      });
      const runtime = createRuntime();
