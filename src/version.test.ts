...
describe("version resolution", () => {
  it("resolves package version from nested dist/plugin-sdk module URL", async () => {
    await withTempDir({ prefix: "crabfork-version-" }, async (root) => {
      await writeJsonFixture(root, "package.json", { name: "crabfork", version: "1.2.3" });
  ...
  it("ignores unrelated nearby package.json files", async () => {
    await withTempDir({ prefix: "crabfork-version-" }, async (root) => {
      await writeJsonFixture(root, "package.json", { name: "crabfork", version: "2.3.4" });
  ...
  it("ignores non-crabfork package and blank build-info versions", async () => {
    await withTempDir({ prefix: "crabfork-version-" }, async (root) => {
  ...
  it("prefers CRABFORK_VERSION over service and package versions", () => {
    expect(
      resolveRuntimeServiceVersion({
        CRABFORK_VERSION: "9.9.9",
        CRABFORK_SERVICE_VERSION: "2.2.2",
        npm_package_version: "1.1.1",
      }),
    ).toBe("9.9.9");
  });

  it("prefers runtime VERSION over stale CRABFORK_VERSION for compatibility checks", () => {
    const previous = process.env.CRABFORK_VERSION;
    const previousService = process.env.CRABFORK_SERVICE_VERSION;
    const previousPackage = process.env.npm_package_version;
    try {
      process.env.CRABFORK_VERSION = "2026.3.25";
      process.env.CRABFORK_SERVICE_VERSION = "2026.3.25-service";
      process.env.npm_package_version = "2026.3.25-package";
      expect(resolveCompatibilityHostVersion()).toBe(VERSION);
    } finally {
      process.env.CRABFORK_VERSION = previous;
      process.env.CRABFORK_SERVICE_VERSION = previousService;
  ...
  it("keeps explicit env-object overrides for compatibility checks in tests", () => {
    expect(
      resolveCompatibilityHostVersion({
        CRABFORK_VERSION: "2026.3.99",
        CRABFORK_SERVICE_VERSION: "2026.3.98",
  ...
  it("prefers explicit compatibility host overrides over runtime and stale env versions", () => {
    expect(
      resolveCompatibilityHostVersion({
        CRABFORK_COMPATIBILITY_HOST_VERSION: "2026.4.8",
        CRABFORK_VERSION: "2026.3.99",
        CRABFORK_SERVICE_VERSION: "2026.3.98",
        npm_package_version: "2026.3.97",
      }),
  ...
  it("prefers runtime VERSION over service/package markers and ignores blank env values", () => {
    expect(
      resolveRuntimeServiceVersion({
        CRABFORK_VERSION: "   ",
        CRABFORK_SERVICE_VERSION: "  2.0.0  ",
