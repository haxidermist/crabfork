...
describe("video-generation runtime", () => {
  ...
  it("lists runtime video-generation providers through the provider registry", () => {
    ...
    expect(listRuntimeVideoGenerationProviders({ config: {} as CrabforkConfig })).toEqual(
      providers,
    );
    expect(mocks.listVideoGenerationProviders).toHaveBeenCalledWith({} as CrabforkConfig);
  });
  ...
  it("builds a generic config hint without hardcoded provider ids", async () => {
    ...
    const promise = generateVideo({ cfg: {} as CrabforkConfig, prompt: "animate a cat" });
