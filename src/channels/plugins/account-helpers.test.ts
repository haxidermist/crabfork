...
function cfg(accounts?: Record<string, unknown> | null, defaultAccount?: string): CrabforkConfig {
  if (accounts === null) {
    return {
      channels: {
  ...
  if (accounts === undefined && !defaultAccount) {
    return {} as unknown as CrabforkConfig;
  }
  return {
    channels: {
      testchannel: {
...
function expectResolvedAccountIdsCase(params: {
  resolve: (cfg: CrabforkConfig) => string[];
  input: CrabforkConfig;
  expected: string[];
}) {
  expect(params.resolve(params.input)).toEqual(params.expected);
}

function expectResolvedDefaultAccountCase(input: CrabforkConfig, expected: string) {
