...
describe("resolveInitialTuiAgentId", () => {
  const cfg: CrabforkConfig = {
    agents: {
      list: [
        { id: "main", workspace: "/tmp/crabfork" },
        { id: "ops", workspace: "/tmp/crabfork/projects/ops" },
      ],
    },
  };
  ...
  it("infers agent from cwd when session is not agent-prefixed", () => {
    expect(
      resolveInitialTuiAgentId({
        ...
        initialSessionInput: "",
        cwd: "/tmp/crabfork/projects/ops/src",
      }),
    ).toBe("ops");
  });
...
describe("resolveGatewayDisconnectState", () => {
  it("returns pairing recovery guidance when disconnect reason requires pairing", () => {
    ...
    expect(state.activityStatus).toBe("pairing required: run crabfork devices list");
    expect(state.pairingHint).toContain("crabfork devices list");
  });
