...
export function maybeWarnAboutDebugProxyCoverage(
  ...
  warn(
    `[crabfork proxy] debug proxy coverage: ${summary.captured}/${summary.total} captured, ${summary.proxyOnly} proxy-only, ${summary.uncovered} uncovered.`,
  );
  warn(
    `[crabfork proxy] remaining gaps: ${partial.map((entry) => entry.id).join(", ")}. Run \`crabfork proxy coverage\` for details.`,
  );
}
