...
import { clearPluginLoaderCache, loadCrabforkPlugins } from "./loader.js";
import { clearPluginManifestRegistryCache } from "./manifest-registry.js";
import { resetPluginRuntimeStateForTest } from "./runtime.js";

export type TempPlugin = { dir: string; file: string; id: string };
export type PluginLoadConfig = NonNullable<Parameters<typeof loadCrabforkPlugins>[0]>["config"];
export type PluginRegistry = ReturnType<typeof loadCrabforkPlugins>;
...
const fixtureRoot = mkdtempSafe(path.join(os.tmpdir(), "crabfork-plugin-"));
let tempDirIndex = 0;
const prevBundledDir = process.env.CRABFORK_BUNDLED_PLUGINS_DIR;
...
export function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "crabfork.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
export function loadBundleFixture(params: {
  ...
  const bundleRoot = path.join(workspaceDir, ".crabfork", "extensions", params.pluginId);
  params.build(bundleRoot);
  return withEnv({ CRABFORK_STATE_DIR: stateDir, ...params.env }, () =>
    loadCrabforkPlugins({
      workspaceDir,
...
export function resetPluginLoaderTestStateForTest() {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.CRABFORK_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
}
