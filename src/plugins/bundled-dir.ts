...
const DISABLED_BUNDLED_PLUGINS_DIR = path.join(os.tmpdir(), "crabfork-empty-bundled-plugins");

function bundledPluginsDisabled(env: NodeJS.ProcessEnv): boolean {
  const raw = normalizeOptionalLowercaseString(env.CRABFORK_DISABLE_BUNDLED_PLUGINS);
  ...
  try {
    return fs.readdirSync(pluginsDir, { withFileTypes: true }).some((entry) => {
      ...
      return (
        ...
        fs.existsSync(path.join(pluginDir, "crabfork.plugin.json"))
      );
    });
  } catch {
    return false;
...
export function resolveBundledPluginsDir(env: NodeJS.ProcessEnv = process.env): string | undefined {
  ...
  const override = env.CRABFORK_BUNDLED_PLUGINS_DIR?.trim();
  if (override) {
    ...
    try {
      const argvPackageRoot = resolveCrabforkPackageRootSync({ argv1: process.argv[1] });
  ...
  try {
    const packageRoots = [
      resolveCrabforkPackageRootSync({ argv1: process.argv[1] }),
