...
function expectLoaderCall(overrides: Record<string, unknown>) {
  expect(loadCrabforkPluginsMock).toHaveBeenCalledWith(expect.objectContaining(overrides));
}
  ...
  it.each([
    {
      ...
      expectedLoaderCall: {
        env: { CRABFORK_HOME: "/srv/crabfork-home" },
      },
    },
    {
