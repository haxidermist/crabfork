...
import type { CrabforkPluginService, CrabforkPluginServiceContext } from "./types.js";
...
function createRegistry(services: CrabforkPluginService[]) {
...
function expectServiceContext(
  ctx: CrabforkPluginServiceContext,
...
function expectServiceLogger(ctx: CrabforkPluginServiceContext) {
...
function expectServiceContexts(
  contexts: CrabforkPluginServiceContext[],
  config: Parameters<typeof startPluginServices>[0]["config"],
) {
...
async function startTrackingServices(params: {
  services: CrabforkPluginService[];
  config?: Parameters<typeof startPluginServices>[0]["config"];
  workspaceDir?: string;
}) {
...
describe("startPluginServices", () => {
  ...
  it("starts services and stops them in reverse order", async () => {
    const starts: string[] = [];
    const stops: string[] = [];
    const contexts: CrabforkPluginServiceContext[] = [];
