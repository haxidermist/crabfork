...
import type {
  CrabforkPluginHookOptions,
  CrabforkPluginToolContext,
  CrabforkPluginToolFactory,
  CrabforkPluginToolOptions,
} from "./tool-types.js";
 ...
 */
export type CrabforkPluginConfigSchema = {
...
export type ProviderAuthResult = {
   ...
   */
  configPatch?: Partial<CrabforkConfig>;
  defaultModel?: string;
  notes?: string[];
};
...
export type ProviderAuthMethod = {
   ...
   * key + OAuth, or region-specific login flows). Crabfork uses this to expose
 ...
 *
 * Runs after Crabfork resolves an explicit/discovered/dynamic model and before
 ...
 *
 * The helper methods cover the common Crabfork auth resolution paths:
 *
 ...
 *
 * Called when OAuth refresh fails and Crabfork wants a provider-specific repair
...
/**
 * Provider-owned extra-param normalization before Crabfork builds its generic
 ...
 * `buildReplayPolicy`, `normalizeToolSchemas`, and `wrapStreamFn`. Crabfork no
...
/**
 * Provider-owned stream wrapper hook after Crabfork applies its generic
 ...
 *
 * Return `true` or `false` to override Crabfork's built-in provider cache TTL
 ...
 *
 * Runs only after Crabfork exhausts normal env/profile/config auth resolution
 ...
 * hint suffix that Crabfork should append to the generic `Unknown model`
 * error.
 */
 ...
 *
 * Runs after Crabfork loads the discovered model catalog and merges configured
...
export type ProviderOAuthProfileIdRepair = {
   ...
   *
   * When omitted, Crabfork falls back to `<provider>:default`.
   */
...
export type ProviderPlugin = {
   ...
   *
   * Crabfork calls this only from async model resolution paths. After it
   ...
   *
   * Runs after Crabfork performs generic transcript cleanup. Use this for
   ...
   *
   * Use this for transport-family schema cleanup before Crabfork registers
  ...
  /**
   * Provider-owned stream wrapper applied after generic Crabfork wrappers.
   *
   ...
   *
   * Called after Crabfork resolves the raw configured credential but before the
   ...
   * hint than Crabfork's generic auth-store guidance.
   */
   ...
   * Crabfork's generic `Unknown model` error after catalog/runtime lookup
   * fails.
   */
   ...
   * `errorMessage` when Crabfork should surface a provider-specific hint for
   ...
   * merging. Crabfork deduplicates by `provider/id`, so plugins only need to
   ...
   * without replacing the full Crabfork-owned system prompt.
   */
   ...
   * rewrites after Crabfork has assembled the complete prompt. Return
   ...
   * `output` applies to assistant text deltas/final text before Crabfork handles
   ...
   *
   * Crabfork uses this when a stored auth profile is already valid and needs to
  ...
  /**
   * Legacy auth-profile ids that should be retired by `crabfork doctor`.
   *
  ...
  /**
   * Legacy OAuth profile-id migrations that `crabfork doctor` should offer.
   *
   ...
   *
   * Crabfork calls this before falling back to the shared `pi-ai` OAuth
   ...
   * path. Return nothing to keep Crabfork's generic error text.
   */
  buildAuthDoctorHint?: (
   ...
   * Crabfork falls back to that stored profile.
   */
...
export type CrabforkPluginGatewayMethod = {
  method: string;
...
export type PluginCommandContext = {
  ...
  sessionId?: string;
  /** Transcript file for the active Crabfork session when available. */
  sessionFile?: string;
  /** Raw command arguments after the command name */
  args?: string;
  /** The full normalized command body */
  commandBody: string;
  /** Current Crabfork configuration */
  config: CrabforkConfig;
 ...
 */
export type CrabforkPluginCommandDefinition = {
...
export type CrabforkPluginHttpRouteAuth = "gateway" | "plugin";
export type CrabforkPluginHttpRouteMatch = "exact" | "prefix";
export type CrabforkPluginGatewayRuntimeScopeSurface = "write-default" | "trusted-operator";

export type CrabforkPluginHttpRouteHandler = (
  req: IncomingMessage,
  res: ServerResponse,
) => Promise<boolean | void> | boolean | void;

export type CrabforkPluginHttpRouteParams = {
  path: string;
...
export type CrabforkPluginCliContext = {
  program: Command;
  config: CrabforkConfig;
  workspaceDir?: string;
  logger: PluginLogger;
...
export type CrabforkPluginCliRegistrar = (ctx: CrabforkPluginCliContext) => void | Promise<void>;

/**
 * Top-level CLI metadata for plugin-owned commands.
 *
 * Descriptors are the parse-time contract for lazy plugin CLI registration.
 * If you want Crabfork to keep a plugin command lazy-loaded while still
 ...
 */
export type CrabforkPluginCliCommandDescriptor = {
  name: string;
  description: string;
  hasSubcommands: boolean;
};

export type CrabforkPluginReloadRegistration = {
  restartPrefixes?: string[];
  hotPrefixes?: string[];
  noopPrefixes?: string[];
};

export type CrabforkPluginNodeHostCommand = {
  command: string;
  cap?: string;
  handle: (paramsJSON?: string | null) => Promise<string>;
};

export type CrabforkPluginSecurityAuditContext = {
  config: CrabforkConfig;
...
export type CrabforkPluginSecurityAuditCollector = (
  ctx: CrabforkPluginSecurityAuditContext,
) => SecurityAuditFinding[] | Promise<SecurityAuditFinding[]>;

/** Context passed to long-lived plugin services. */
export type CrabforkPluginServiceContext = {
  config: CrabforkConfig;
  workspaceDir?: string;
  stateDir: string;
  logger: PluginLogger;
...
export type CrabforkPluginService = {
  id: string;
  start: (ctx: CrabforkPluginServiceContext) => void | Promise<void>;
  stop?: (ctx: CrabforkPluginServiceContext) => void | Promise<void>;
};

export type CrabforkPluginChannelRegistration = {
  plugin: ChannelPlugin;
};

/** Module-level plugin definition loaded from a native plugin entry file. */
export type CrabforkPluginDefinition = {
  id?: string;
  name?: string;
  description?: string;
  version?: string;
  ...
  reload?: CrabforkPluginReloadRegistration;
  nodeHostCommands?: CrabforkPluginNodeHostCommand[];
  securityAuditCollectors?: CrabforkPluginSecurityAuditCollector[];
  register?: (api: CrabforkPluginApi) => void | Promise<void>;
  activate?: (api: CrabforkPluginApi) => void | Promise<void>;
};

export type CrabforkPluginModule =
  | CrabforkPluginDefinition
  | ((api: CrabforkPluginApi) => void | Promise<void>);

export type PluginRegistrationMode = "full" | "setup-only" | "setup-runtime" | "cli-metadata";

export type PluginConfigMigration = (config: CrabforkConfig) =>
  | {
      config: CrabforkConfig;
      changes: string[];
    }
...
export type CrabforkPluginApi = {
  ...
  logger: PluginLogger;
  registerTool: (
    tool: AnyAgentTool | CrabforkPluginToolFactory,
    opts?: CrabforkPluginToolOptions,
  ) => void;
  registerHook: (
    events: string | string[],
    handler: InternalHookHandler,
    opts?: CrabforkPluginHookOptions,
  ) => void;
  registerHttpRoute: (params: CrabforkPluginHttpRouteParams) => void;
  /** Register a native messaging channel plugin (channel capability). */
  registerChannel: (registration: CrabforkPluginChannelRegistration | ChannelPlugin) => void;
  /**
  ...
  registerCli: (
    registrar: CrabforkPluginCliRegistrar,
    opts?: {
       ...
       *
       * When descriptors cover every top-level command root, Crabfork can keep
  ...
  registerReload: (registration: CrabforkPluginReloadRegistration) => void;
  registerNodeHostCommand: (command: CrabforkPluginNodeHostCommand) => void;
  registerSecurityAuditCollector: (collector: CrabforkPluginSecurityAuditCollector) => void;
  registerService: (service: CrabforkPluginService) => void;
   ...
   */
  registerCommand: (command: CrabforkPluginCommandDefinition) => void;
