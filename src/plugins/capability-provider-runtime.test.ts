...
const mocks = vi.hoisted(() => ({
  ...
  withBundledPluginAllowlistCompat: vi.fn(
    ({ config, pluginIds }: { config?: CrabforkConfig; pluginIds: string[] }) =>
      ({
        ...config,
        plugins: {
          ...config?.plugins,
...
function expectBundledCompatLoadPath(params: {
  cfg: CrabforkConfig;
  allowlistCompat: CrabforkConfig;
  enablementCompat: {
    plugins: {
      allow?: string[];
...
function createCompatChainConfig() {
  const cfg = { plugins: { allow: ["custom-plugin"] } } as CrabforkConfig;
...
describe("resolvePluginCapabilityProviders", () => {
  ...
  it("keeps active capability providers even when cfg is passed", () => {
    ...
    const providers = resolvePluginCapabilityProviders({
      key: "speechProviders",
      cfg: { messages: { tts: { provider: "edge" } } } as CrabforkConfig,
    });
