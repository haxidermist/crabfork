...
const PLUGIN_SOURCE_ROOTS = {
  stock: path.resolve(path.sep, "opt", "homebrew", "lib", "node_modules", "crabfork", "extensions"),
  global: path.resolve(path.sep, "Users", "x", ".crabfork", "extensions"),
  workspace: path.resolve(path.sep, "Users", "x", "ws", ".crabfork", "extensions"),
};
...
describe("formatPluginSourceForTable", () => {
  ...
  it("resolves source roots from an explicit env override", () => {
    const homeDir = path.resolve(path.sep, "tmp", "crabfork-home");
    expectResolvedSourceRoots({
      ...
      expected: {
        ...
        workspace: path.join(homeDir, "ws", ".crabfork", "extensions"),
      },
    });
  });
});
