...
function collectConfiguredPluginEntryIds(config: CrabforkConfig): string[] {
...
function resolveRegister(mod: CrabforkPluginModule): {
...
export function resolvePluginSetupRegistry(params?: {
  ...
  for (const record of manifestRegistry.plugins) {
    ...
    let mod: CrabforkPluginModule;
    try {
      mod = getJiti(setupSource)(setupSource) as CrabforkPluginModule;
    } catch {
      continue;
    }

    const resolved = resolveRegister((mod as { default?: CrabforkPluginModule }).default ?? mod);
