...
const mocks = vi.hoisted(() => ({
  loadCrabforkPlugins: vi.fn<typeof import("../loader.js").loadCrabforkPlugins>(),
...
vi.mock("../loader.js", () => ({
  loadCrabforkPlugins: (...args: Parameters<typeof mocks.loadCrabforkPlugins>) =>
    mocks.loadCrabforkPlugins(...args),
}));
...
describe("ensurePluginRegistryLoaded", () => {
  ...
  it("does not cache scoped loads by explicit plugin ids", () => {
    ...
    expect(mocks.loadCrabforkPlugins).toHaveBeenCalledTimes(2);
    expect(mocks.loadCrabforkPlugins).toHaveBeenNthCalledWith(
      1,
  ...
  it("does not forward empty channel scopes for broad channel loads", () => {
    ...
    expect(
      (mocks.loadCrabforkPlugins.mock.calls[0]?.[0] as { onlyPluginIds?: string[] }).onlyPluginIds,
    ).toBeUndefined();
  });
});
