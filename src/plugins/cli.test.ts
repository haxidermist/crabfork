...
const mocks = vi.hoisted(() => ({
  ...
  loadCrabforkPluginCliRegistry: vi.fn(),
  loadCrabforkPlugins: vi.fn(),
...
vi.mock("./loader.js", () => ({
  loadCrabforkPluginCliRegistry: (...args: unknown[]) =>
    mocks.loadCrabforkPluginCliRegistry(...args),
  loadCrabforkPlugins: (...args: unknown[]) => mocks.loadCrabforkPlugins(...args),
}));
...
function expectAutoEnabledCliLoad(params: {
  ...
  autoEnabledConfig: CrabforkConfig;
  ...
  expect(mocks.loadCrabforkPlugins).toHaveBeenCalledWith(
...
describe("registerPluginCliCommands", () => {
  ...
  beforeEach(() => {
    ...
    mocks.loadCrabforkPluginCliRegistry.mockReset();
    mocks.loadCrabforkPluginCliRegistry.mockResolvedValue(createCliRegistry());
    mocks.loadCrabforkPlugins.mockReset();
    mocks.loadCrabforkPlugins.mockReturnValue({
      ...createCliRegistry(),
      diagnostics: [],
    });
    ...
    mocks.loadConfig.mockReturnValue({} as CrabforkConfig);
  ...
  it("skips plugin CLI registrars when commands already exist", async () => {
    ...
    await registerPluginCliCommands(program, {} as CrabforkConfig);
  ...
  it("forwards an explicit env to plugin loading", async () => {
    ...
    await registerPluginCliCommands(createProgram(), {} as CrabforkConfig, env);
  ...
  it("loads root-help descriptors through the dedicated non-activating CLI collector", async () => {
    ...
    mocks.loadCrabforkPluginCliRegistry.mockResolvedValue({
      cliRegistrars: [
        {
          pluginId: "matrix",
          register: vi.fn(),
    ...
    expect(mocks.loadCrabforkPluginCliRegistry).toHaveBeenCalledWith(
      expect.objectContaining({
        config: autoEnabledConfig,
  ...
  it("keeps runtime CLI command registration on the full plugin loader for legacy channel plugins", async () => {
    ...
    expect(mocks.loadCrabforkPluginCliRegistry).not.toHaveBeenCalled();
  });
  ...
  it("falls back to awaited CLI metadata collection when runtime loading ignored async registration", async () => {
    ...
    await registerPluginCliCommands(program, {} as CrabforkConfig, undefined, undefined, {
      mode: "lazy",
    });

    expect(mocks.loadCrabforkPluginCliRegistry).toHaveBeenCalledTimes(1);
  ...
  it("falls back to eager registration when descriptors do not cover every command root", async () => {
    ...
    await registerPluginCliCommands(createProgram(), {} as CrabforkConfig, undefined, undefined, {
      mode: "lazy",
    });
  ...
  it("loads validated plugin CLI config when the snapshot is valid", async () => {
    const loadedConfig = { plugins: { enabled: true } } as CrabforkConfig;
    mocks.readConfigFileSnapshot.mockResolvedValueOnce({
      valid: true,
      config: loadedConfig,
    });
  ...
  it("skips plugin CLI registration from validated config when the snapshot is invalid", async () => {
    ...
    expect(mocks.loadCrabforkPlugins).not.toHaveBeenCalled();
  });
});
