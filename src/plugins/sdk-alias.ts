...
function hasTrustedCrabforkRootIndicator(params: {
  packageRoot: string;
  ...
  const hasCrabforkBin =
    (typeof params.packageJson.bin === "string" &&
      normalizeLowercaseStringOrEmpty(params.packageJson.bin).includes("crabfork")) ||
    (typeof params.packageJson.bin === "object" &&
      params.packageJson.bin !== null &&
      typeof params.packageJson.bin.crabfork === "string");
  const hasCrabforkEntrypoint = fs.existsSync(path.join(params.packageRoot, "crabfork.mjs"));
  return hasCliEntryExport || hasCrabforkBin || hasCrabforkEntrypoint;
}
...
function readPluginSdkSubpathsFromPackageRoot(packageRoot: string): string[] | null {
  ...
  if (!hasTrustedCrabforkRootIndicator({ packageRoot, packageJson: pkg })) {
    return null;
  }
...
function resolveTrustedCrabforkRootFromArgvHint(params: {
  argv1?: string;
  cwd: string;
}): string | null {
  if (!params.argv1) {
  ...
  const packageRoot = resolveCrabforkPackageRootSync({
    cwd: params.cwd,
    argv1: params.argv1,
  });
  if (!packageRoot) {
  ...
  return hasTrustedCrabforkRootIndicator({ packageRoot, packageJson }) ? packageRoot : null;
}
...
function resolveLoaderPluginSdkPackageRoot(
  ...
  const cwd = params.cwd ?? path.dirname(params.modulePath);
  const fromCwd = resolveCrabforkPackageRootSync({ cwd });
  const fromExplicitHints =
    resolveTrustedCrabforkRootFromArgvHint({ cwd, argv1: params.argv1 }) ??
    (params.moduleUrl
...
const PLUGIN_SDK_PACKAGE_NAMES = ["crabfork/plugin-sdk", "@crabfork/plugin-sdk"] as const;
...
export function resolvePluginSdkScopedAliasMap(
  ...
  for (const subpath of listPluginSdkExportedSubpaths({
    ...
    for (const kind of orderedKinds) {
      ...
      if (Object.prototype.hasOwnProperty.call(aliasMap, `crabfork/plugin-sdk/${subpath}`)) {
        break;
      }
    }
  }
...
export function buildPluginLoaderAliasMap(
  ...
  return {
    ...(extensionApiAlias
      ? { "crabfork/extension-api": normalizeJitiAliasTargetPath(extensionApiAlias) }
      : {}),
    ...(pluginSdkAlias
      ? Object.fromEntries(
...
export function isBundledPluginExtensionPath(params: {
  ...
  const roots = [
    ...
    path.join(params.crabForkPackageRoot, "extensions"),
    path.join(params.crabForkPackageRoot, "dist", "extensions"),
    path.join(params.crabForkPackageRoot, "dist-runtime", "extensions"),
