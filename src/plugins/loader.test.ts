...
function writeBundledPlugin(params: {
  ...
  process.env.CRABFORK_BUNDLED_PLUGINS_DIR = bundledDir;
...
function writeWorkspacePlugin(params: {
  ...
  const workspacePluginDir = path.join(workspaceDir, ".crabfork", "extensions", params.id);
...
function withStateDir<T>(run: (stateDir: string) => T) {
  ...
  return withEnv({ CRABFORK_STATE_DIR: stateDir }, () => run(stateDir));
}
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  if (!options && cachedBundledMemoryDir) {
    process.env.CRABFORK_BUNDLED_PLUGINS_DIR = cachedBundledMemoryDir;
      ...
      JSON.stringify(
        {
          ...
          crabfork: { extensions: [`./${pluginFilename}`] },
        },
        null,
        2,
      ),
...
function setupBundledTelegramPlugin() {
  ...
  process.env.CRABFORK_BUNDLED_PLUGINS_DIR = cachedBundledTelegramDir;
}

function expectTelegramLoaded(registry: ReturnType<typeof loadCrabforkPlugins>) {
...
function loadRegistryFromSinglePlugin(params: {
  ...
  options?: Omit<Parameters<typeof loadCrabforkPlugins>[0], "cache" | "workspaceDir" | "config">;
}) {
...
function loadRegistryFromAllowedPlugins(
  plugins: TempPlugin[],
  options?: Omit<Parameters<typeof loadCrabforkPlugins>[0], "cache" | "config">,
) {
  return loadCrabforkPlugins({
    cache: false,
    ...options,
...
function resolveLoadedPluginSource(
  registry: ReturnType<typeof loadCrabforkPlugins>,
  pluginId: string,
) {
...
function expectCachePartitionByPluginSource(params: {
  pluginId: string;
  loadFirst: () => ReturnType<typeof loadCrabforkPlugins>;
  loadSecond: () => ReturnType<typeof loadCrabforkPlugins>;
...
function expectCacheMissThenHit(params: {
  loadFirst: () => ReturnType<typeof loadCrabforkPlugins>;
  loadVariant: () => ReturnType<typeof loadCrabforkPlugins>;
}) {
  const first = params.loadFirst();
...
function createEnvResolvedPluginFixture(pluginId: string) {
  useNoBundledPlugins();
  const crabforkHome = makeTempDir();
  const ignoredHome = makeTempDir();
  const stateDir = makeTempDir();
  const pluginDir = path.join(crabforkHome, "plugins", pluginId);
  mkdirSafe(pluginDir);
  const plugin = writePlugin({
    id: pluginId,
    dir: pluginDir,
  ...
  const env = {
    ...process.env,
    CRABFORK_HOME: crabforkHome,
    HOME: ignoredHome,
    CRABFORK_STATE_DIR: stateDir,
    CRABFORK_BUNDLED_PLUGINS_DIR: "/nonexistent/bundled/plugins",
  };
  return { plugin, env };
}
...
function expectEscapingEntryRejected(params: {
  ...
  const registry = loadCrabforkPlugins({
    cache: false,
    config: {
      plugins: {
  ...
  it.each([
    {
      ...
      assert: (registry: ReturnType<typeof loadCrabforkPlugins>) => {
  ...
  it.each([
    {
      ...
      run: () => {
        process.env.CRABFORK_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
        const plugin = writePlugin({
...
module.exports = { id: "manifest-only-plugin", register() { throw new Error("manifest-only snapshot should not register"); } };`,
  ...
  register() {},
        ...
        fs.writeFileSync(
          path.join(memoryPlugin.dir, "crabfork.plugin.json"),
          JSON.stringify(
            {
              id: "memory-demo",
              kind: "memory",
    ...
    },
    {
      ...
      run: () => {
        useNoBundledPlugins();
        const importMarker = "__crabfork_loader_import_throw_marker";
...
module.exports = { id: "throws-after-import", register() {} };`,
    ...
    },
    {
      ...
      run: () => {
        useNoBundledPlugins();
        const marker = "__crabfork_loader_reentry_error";
        const reenterFnMarker = "__crabfork_loader_reentry_fn";
        Reflect.deleteProperty(globalThis, marker);
        Reflect.set(
          globalThis,
          reenterFnMarker,
          (options: Parameters<typeof loadCrabforkPlugins>[0]) => loadCrabforkPlugins(options),
        );
        const pluginDir = makeTempDir();
        ...
        } satisfies Parameters<typeof loadCrabforkPlugins>[0];
        writePlugin({
  ...
  register() {
    try {
      ...
      throw error;
        ...
        const registry = loadCrabforkPlugins(nestedOptions);
    ...
    },
    {
      ...
      run: () => {
        useNoBundledPlugins();
        const marker = "__crabfork_runtime_registry_reentry_marker";
        const resolverMarker = "__crabfork_runtime_registry_reentry_fn";
        Reflect.deleteProperty(globalThis, marker);
        Reflect.set(
          globalThis,
          resolverMarker,
  ...
  register() {
    ...
    },
    {
      ...
      run: () => {
        ...
        const full = loadCrabforkPlugins(options);
        const scoped = loadCrabforkPlugins({
          ...options,
          onlyPluginIds: ["allowed-cache-scope"],
        });
        const scopedAgain = loadCrabforkPlugins({
          ...options,
          onlyPluginIds: ["allowed-cache-scope"],
        });
  ...
  it("only publishes plugin commands to the global registry during activating loads", async () => {
    ...
    const active = loadCrabforkPlugins({
      cache: false,
      workspaceDir: plugin.dir,
      config: {
        plugins: {
  ...
  it("replaces prior plugin hook registrations on activating reloads", async () => {
    ...
    loadCrabforkPlugins(loadOptions);
    loadCrabforkPlugins(loadOptions);
  ...
  it("restores cached memory capability public artifacts on cache hits", async () => {
    ...
    const first = loadCrabforkPlugins(options);
    ...
    const second = loadCrabforkPlugins(options);
  ...
  it("throws when activate:false is used without cache:false", () => {
    expect(() => loadCrabforkPlugins({ activate: false })).toThrow(
      "activate:false requires cache:false",
    );
    expect(() => loadCrabforkPlugins({ activate: false, cache: true })).toThrow(
      "activate:false requires cache:false",
    );
  });
        ...
        return {
          ...
          loadFirst: () =>
            loadCrabforkPlugins({
              ...
              env: {
                ...process.env,
                CRABFORK_BUNDLED_PLUGINS_DIR: bundledA,
              },
            }),
          loadSecond: () =>
            loadCrabforkPlugins({
              ...
              env: {
                ...process.env,
                CRABFORK_BUNDLED_PLUGINS_DIR: bundledB,
              },
            }),
        };
      },
          ...
          loadFirst: () =>
            loadCrabforkPlugins({
              ...
              env: {
                ...
                CRABFORK_STATE_DIR: stateDir,
                CRABFORK_BUNDLED_PLUGINS_DIR: bundledDir,
              },
            }),
          loadSecond: () =>
            loadCrabforkPlugins({
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const ignoredHome = makeTempDir();
        const stateDir = makeTempDir();
        const pluginDir = path.join(crabforkHome, "plugins", "tracked-install-cache");
        mkdirSafe(pluginDir);
        const plugin = writePlugin({
    ...
    },
    {
      ...
      setup: () => {
        ...
        return {
          loadFirst: () => loadCrabforkPlugins(options),
          loadVariant: () =>
            loadCrabforkPlugins({
              ...options,
  ...
  it("normalizes bundled plugin env overrides against the provided env", () => {
    ...
    const registry = loadCrabforkPlugins({
      env: {
        ...
        HOME: homeDir,
        CRABFORK_HOME: undefined,
        CRABFORK_BUNDLED_PLUGINS_DIR: override,
      },
      config: {
        plugins: {
          allow: ["tilde-bundled"],
  ...
  it("prefers CRABFORK_HOME over HOME for env-expanded load paths", () => {
    const ignoredHome = makeTempDir();
    const crabforkHome = makeTempDir();
    const stateDir = makeTempDir();
    const bundledDir = makeTempDir();
    const plugin = writePlugin({
      id: "crabfork-home-demo",
      dir: path.join(crabforkHome, "plugins", "crabfork-home-demo"),
      filename: "index.cjs",
      body: `module.exports = { id: "crabfork-home-demo", register() {} };`,
    });
    ...
    const registry = loadCrabforkPlugins({
      ...
      config: {
        plugins: {
          allow: ["crabfork-home-demo"],
          entries: {
            "crabfork-home-demo": { enabled: true },
          },
          load: {
            paths: ["~/plugins/crabfork-home-demo"],
          },
        },
      },
    });
    ...
    expect(
      fs.realpathSync(
        registry.plugins.find((entry) => entry.id === "crabfork-home-demo")?.source ?? "",
      ),
    ).toBe(fs.realpathSync(plugin.file));
  });
  ...
  api.registerHook("gateway:startup", () => {}, { name: "shared-hook" });
        ...
        selectCount: (registry: ReturnType<typeof loadCrabforkPlugins>) =>
  ...
  api.registerContextEngine("shared-context-engine-loader-test", () => ({}));
        ...
        assertPrimaryOwner: (registry: ReturnType<typeof loadCrabforkPlugins>) => {
          expect(
  ...
  it("loads bundled channel entries through nested default export wrappers", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          name: "@crabfork/nested-default-channel",
          crabfork: {
            extensions: ["./index.cjs"],
          },
        },
  ...
  it("does not treat manifest channel ids as scoped plugin id matches", () => {
    ...
    fs.writeFileSync(
      path.join(unrelated.dir, "crabfork.plugin.json"),
      JSON.stringify(
        {
          id: "unrelated-plugin",
          configSchema: EMPTY_PLUGIN_SCHEMA,
...
module.exports = {
  ...
  register(api) {
    ...
    fs.writeFileSync(
      path.join(plugin.dir, "crabfork.plugin.json"),
      JSON.stringify(
        {
          id: "lazy-channel-plugin",
          configSchema: EMPTY_PLUGIN_SCHEMA,
    ...
    const broadSetupRegistry = loadCrabforkPlugins({
      cache: false,
      config,
    ...
    const scopedSetupRegistry = loadCrabforkPlugins({
      cache: false,
      config,
      includeSetupOnlyChannelPlugins: true,
  ...
  it.each([
    {
      ...
      fixture: {
        ...
        packageName: "@crabfork/setup-entry-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@crabfork/setup-runtime-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@crabfork/setup-runtime-bundled-contract-test",
        fullBlurb: "full entry should not run while unconfigured",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@crabfork/setup-runtime-bundled-contract-secrets-test",
        fullBlurb: "full entry should not run while unconfigured",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@crabfork/setup-runtime-not-preferred-test",
  ...
  it("isolates loadSetupPlugin errors as per-plugin diagnostics instead of crashing registry load", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          name: "@crabfork/setup-entry-throws-test",
          crabfork: {
            extensions: ["./index.cjs"],
            setupEntry: "./setup-entry.cjs",
          },
  ...
  it("keeps healthy sibling channel plugins loadable when a setup entry throws", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          name: "@crabfork/setup-entry-throws-sibling-test",
          crabfork: {
            extensions: ["./index.cjs"],
            setupEntry: "./setup-entry.cjs",
          },
    ...
    fs.writeFileSync(
      path.join(brokenDir, "crabfork.plugin.json"),
      JSON.stringify(
        {
    ...
    const scenarios = [
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          fs.writeFileSync(
            path.join(memoryADir, "crabfork.plugin.json"),
            JSON.stringify(
              {
                id: "memory-a",
                kind: "memory",
          ...
          fs.writeFileSync(
            path.join(memoryBDir, "crabfork.plugin.json"),
            JSON.stringify(
              {
                id: "memory-b",
                kind: "memory",
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          fs.writeFileSync(
            path.join(memoryCoreDir, "crabfork.plugin.json"),
            JSON.stringify(
          ...
          fs.writeFileSync(
            path.join(memoryLanceDir, "crabfork.plugin.json"),
            JSON.stringify(
  ...
  it("evaluates load-path provenance warnings", () => {
    ...
    const scenarios = [
      ...
      },
      {
        ...
        loadRegistry: () => {
          const stateDir = makeTempDir();
          return withEnv({ CRABFORK_STATE_DIR: stateDir }, () => {
            const globalDir = path.join(stateDir, "extensions", "rogue");
            mkdirSafe(globalDir);
            writePlugin({
              id: "rogue",
  ...
  if (!Object.getOwnPropertyDescriptor(runtime, "channel")) {
    ...
    const registry = withEnv({ CRABFORK_STATE_DIR: stateDir }, () =>
      loadRegistryFromSinglePlugin({
        plugin,
        pluginConfig: {
  ...
  it("supports legacy plugins importing monolithic plugin-sdk root", async () => {
    ...
    const plugin = writePlugin({
      ...
      body: `module.exports = {
  id: "legacy-root-import",
  configSchema: (require("crabfork/plugin-sdk").emptyPluginConfigSchema)(),
        register() {},
      };`,
    });

    const registry = withEnv({ CRABFORK_BUNDLED_PLUGINS_DIR: "/nonexistent/bundled/plugins" }, () =>
      loadCrabforkPlugins({
        cache: false,
        workspaceDir: plugin.dir,
        config: {
  ...
  it("supports legacy plugins subscribing to diagnostic events from the root sdk", async () => {
    useNoBundledPlugins();
    const seenKey = "__crabforkLegacyRootDiagnosticSeen";
  ...
  configSchema: (require("crabfork/plugin-sdk").emptyPluginConfigSchema)(),
  register() {
    const { onDiagnosticEvent } = require("crabfork/plugin-sdk");
  ...
  it("suppresses trust warning logs for non-activating snapshot loads", () => {
    useNoBundledPlugins();
    const stateDir = makeTempDir();
    withEnv({ CRABFORK_STATE_DIR: stateDir }, () => {
      const globalDir = path.join(stateDir, "extensions", "rogue");
      mkdirSafe(globalDir);
      writePlugin({
        id: "rogue",
