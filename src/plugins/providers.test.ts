...
type LoadCrabforkPlugins = typeof import("./loader.js").loadCrabforkPlugins;
...
const loadCrabforkPluginsMock = vi.fn<LoadCrabforkPlugins>();
...
function createManifestProviderPlugin(params: {
  ...
  return {
    ...
    manifestPath: `/tmp/${params.id}/crabfork.plugin.json`,
  };
}
...
function getLastSetupLoadedPluginConfig() {
  const call = loadCrabforkPluginsMock.mock.calls.at(-1)?.[0];
  expect(call).toBeDefined();
...
function createAutoEnabledProviderConfig() {
  const rawConfig: CrabforkConfig = {
    plugins: {},
  };
  const autoEnabledConfig: CrabforkConfig = {
    ...rawConfig,
    plugins: {
      entries: {
        google: { enabled: true },
...
describe("resolvePluginProviders", () => {
  beforeAll(async () => {
    ...
    vi.doMock("./loader.js", () => ({
      loadCrabforkPlugins: (...args: Parameters<LoadCrabforkPlugins>) =>
        loadCrabforkPluginsMock(...args),
