...
const APPROVALS_PATH = "~/.crabfork/plugin-binding-approvals.json";
...
const LEGACY_CODEX_PLUGIN_SESSION_PREFIXES = [
  "crabfork-app-server:thread:",
  "crabfork-codex-app-server:thread:",
] as const;
...
const PLUGIN_BINDING_PENDING_REQUESTS_KEY = Symbol.for("crabfork.pluginBindingPendingRequests");
...
const pluginBindingGlobalStateKey = Symbol.for("crabfork.plugins.binding.global-state");
...
export function buildPluginBindingUnavailableText(binding: PluginConversationBinding): string {
  return `The bound plugin ${resolvePluginBindingDisplayName(binding)} is not currently loaded. Routing this message to Crabfork instead.${buildDetachHintSuffix(binding.detachHint)}`;
}
