...
type PluginConfig = NonNullable<CrabforkConfig["plugins"]>;
...
function createPluginConfig(params: {
  ...
  loadPaths?: string[];
  channels?: CrabforkConfig["channels"];
}): CrabforkConfig {
...
function createSinglePluginWithEmptySlotsConfig(): CrabforkConfig {
...
function createSingleNpmInstallConfig(installPath: string): CrabforkConfig {
  return createPluginConfig({
    entries: createSinglePluginEntries(),
    installs: {
...
describe("resolveUninstallDirectoryTarget", () => {
  ...
  it("falls back to default path when configured installPath is untrusted", () => {
    const extensionsDir = path.join(os.tmpdir(), "crabfork-uninstall-safe");
    const target = resolveUninstallDirectoryTarget({
      ...
      installRecord: {
        ...
        installPath: "/tmp/not-crabfork-plugin-install/my-plugin",
      },
      extensionsDir,
    });
