...
import type { CrabforkPluginDefinition, CrabforkPluginModule, PluginLogger } from "./types.js";
...
function shouldProfilePluginLoader(): boolean {
  return process.env.CRABFORK_PLUGIN_LOAD_PROFILE === "1";
}
...
export function resolveRuntimePluginRegistry(
  ...
  // plugin registration is still in flight. Let direct loadCrabforkPlugins(...)
  // callers surface the hard error instead.
  if (isPluginRegistryLoadInFlight(options)) {
    return undefined;
  }
  return loadCrabforkPlugins(options);
}
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: CrabforkPluginDefinition;
  register?: CrabforkPluginDefinition["register"];
} {
  const resolved = unwrapDefaultModuleExport(moduleExport);
  if (typeof resolved === "function") {
    return {
      register: resolved as CrabforkPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as CrabforkPluginDefinition;
...
function recordPluginError(params: {
  ...
  const errorText =
    process.env.CRABFORK_PLUGIN_LOADER_DEBUG_STACKS === "1" &&
...
function resolveCandidateDuplicateRank(params: {
  candidate: ReturnType<typeof discoverCrabforkPlugins>["candidates"][number];
...
function compareDuplicateCandidateOrder(params: {
  left: ReturnType<typeof discoverCrabforkPlugins>["candidates"][number];
  right: ReturnType<typeof discoverCrabforkPlugins>["candidates"][number];
  manifestByRoot: Map<string, ReturnType<typeof loadPluginManifestRegistry>["plugins"][number]>;
  provenance: PluginProvenanceIndex;
  env: NodeJS.ProcessEnv;
}): number {
...
export function loadCrabforkPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  if (options.activate === false && options.cache !== false) {
    throw new Error(
      "loadCrabforkPlugins: activate:false requires cache:false to prevent command registry divergence",
    );
  }
  const {
    env,
  ...
  try {
    ...
    for (const candidate of orderedCandidates) {
      ...
      if (record.format === "bundle") {
        ...
        for (const capability of unsupportedCapabilities) {
          registry.diagnostics.push({
            ...
            pluginId: record.id,
            source: record.source,
            message: `bundle capability detected but not wired into Crabfork yet: ${capability}`,
          });
        }
        if (
          enableState.enabled &&
      ...
      let mod: CrabforkPluginModule | null = null;
      try {
        ...
        mod = profilePluginLoaderSync({
          ...
          pluginId: record.id,
          source: safeSource,
          run: () => getJiti(safeSource)(safeImportSource) as CrabforkPluginModule,
        });
      } catch (err) {
        recordPluginError({
          logger,
      ...
      if (failedPlugins.length > 0) {
        logger.warn(
          `[plugins] ${failedPlugins.length} plugin(s) failed to initialize (${formatPluginFailureSummary(
            failedPlugins,
          )}). Run 'crabfork plugins list' for details.`,
        );
      }
    }
...
export async function loadCrabforkPluginCliRegistry(
  options: PluginLoadOptions = {},
