...
let originalCrabforkLogLevel: string | undefined;
...
beforeEach(() => {
  originalTestFileLog = process.env.CRABFORK_TEST_FILE_LOG;
  originalCrabforkLogLevel = process.env.CRABFORK_LOG_LEVEL;
  delete process.env.CRABFORK_TEST_FILE_LOG;
  delete process.env.CRABFORK_LOG_LEVEL;
...
afterEach(() => {
  if (originalTestFileLog === undefined) {
    ...
    process.env.CRABFORK_TEST_FILE_LOG = originalTestFileLog;
  }
  if (originalCrabforkLogLevel === undefined) {
    delete process.env.CRABFORK_LOG_LEVEL;
  } else {
    process.env.CRABFORK_LOG_LEVEL = originalCrabforkLogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();
...
describe("getResolvedLoggerSettings", () => {
  ...
  it("reads logging config when test file logging is explicitly enabled", () => {
    process.env.CRABFORK_TEST_FILE_LOG = "1";
    const settings = logging.getResolvedLoggerSettings();
