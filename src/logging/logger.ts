...
function resolveDefaultLogDir(): string {
  return canUseNodeFs() ? resolvePreferredCrabforkTmpDir() : POSIX_CRABFORK_TMP_DIR;
}

function resolveDefaultLogFile(defaultLogDir: string): string {
  return canUseNodeFs()
    ? path.join(defaultLogDir, "crabfork.log")
    : `${POSIX_CRABFORK_TMP_DIR}/crabfork.log`;
}
...
function resolveSettings(): ResolvedSettings {
  ...
  let cfg: CrabforkConfig["logging"] | undefined =
  ...
  const defaultLevel =
    process.env.VITEST === "true" && process.env.CRABFORK_TEST_FILE_LOG !== "1" ? "silent" : "info";
      ...
      if (nextBytes > settings.maxFileBytes) {
        if (!warnedAboutSizeCap) {
          ...
          process.stderr.write(
            `[crabfork] log file size cap reached; suppressing writes file=${settings.file} maxFileBytes=${settings.maxFileBytes}\n`,
          );
        }
        return;
      }
