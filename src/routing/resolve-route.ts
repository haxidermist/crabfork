...
function listAgents(cfg: CrabforkConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: CrabforkConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<CrabforkConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: CrabforkConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: CrabforkConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: CrabforkConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<CrabforkConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  CrabforkConfig,
  {
    bindingsRef: CrabforkConfig["bindings"];
    agentsRef: CrabforkConfig["agents"];
    sessionRef: CrabforkConfig["session"];
...
function resolveRouteCacheForConfig(cfg: CrabforkConfig): Map<string, ResolvedAgentRoute> {
