...
```bash
export CRABFORK_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.crabfork/crabfork.json' | tr -d '\n'
)"

pnpm test:parallels:macos \
  --discord-token-env CRABFORK_PARALLELS_DISCORD_TOKEN \
...
- Use the `crabfork` wrapper for guest `message send/read`; `node crabfork.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/crabfork-parallels-smoke.*`
...
- host inbound nonce appears in `crabfork message read` output
