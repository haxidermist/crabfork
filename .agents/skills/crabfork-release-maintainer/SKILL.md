---
name: crabfork-release-maintainer
description: Maintainer workflow for Crabfork releases, prereleases, changelog release notes, and publish validation. Use when Codex needs to prepare or verify stable or beta release steps, align version naming, assemble release notes, check release auth requirements, or validate publish-time commands and artifacts.
---

# Crabfork Release Maintainer
...
- Core `crabfork` publish is manual `workflow_dispatch`; creating or pushing a tag does not publish by itself.
...
- Version locations include:
  ...
  - `apps/macos/Sources/Crabfork/Resources/Info.plist`
...
- The production Sparkle feed lives at `https://raw.githubusercontent.com/crabfork/crabfork/main/appcast.xml`, and the canonical published file is `appcast.xml` on `main` in the `crabfork` repo.
...
- When cutting a mac release with a beta GitHub prerelease:
  ...
  - create a prerelease titled `crabfork YYYY.M.D-beta.N`
...
```bash
  CRABFORK_INSTALL_SMOKE_SKIP_NONROOT=1 pnpm test:install:smoke
```

After npm publish, run:

```bash
node --import tsx scripts/crabfork-npm-postpublish-verify.ts <published-version>
```
...
- Use `pnpm test:live:media video` for bounded video-provider smoke when video
  ...
  each provider operation with `CRABFORK_LIVE_VIDEO_GENERATION_TIMEOUT_MS`
  (`180000` by default).
...
- Set `CRABFORK_LIVE_VIDEO_GENERATION_FULL_MODES=1` only when intentionally
  validating the slower image-to-video and video-to-video transform lanes.

## Check all relevant release builds

- Always validate the Crabfork npm release path before creating the tag.
...
- Include mac release readiness in preflight by running the public validation
  workflow in `crabfork/crabfork` and the real mac preflight in
  `crabfork/releases-private` for every release.
...
- Crabfork publish uses GitHub trusted publishing.
- Stable npm promotion from `beta` to `latest` is an explicit mode on
  `.github/workflows/crabfork-npm-release.yml`, but it still needs a valid
...
- `.github/workflows/macos-release.yml` in `crabfork/crabfork` is now a
  public validation-only handoff. It validates the tag/release state and points
  operators to the private repo. It still rebuilds the JS outputs needed for
  release validation, but it does not sign, notarize, or publish macOS
  artifacts.
- `crabfork/releases-private/.github/workflows/crabfork-macos-validate.yml`
...
- Real mac preflight and real mac publish both use
  `crabfork/releases-private/.github/workflows/crabfork-macos-publish.yml`.
...
- The `npm-release` environment must be approved by `@crabfork/crabfork-release-managers` before publish continues.
- Mac publish uses
  `crabfork/releases-private/.github/workflows/crabfork-macos-publish.yml` for
...
- Real private mac publish uploads the packaged `.zip`, `.dmg`, and
  `.dSYM.zip` assets to the existing GitHub release in `crabfork/crabfork`
  automatically when `CRABFORK_PUBLIC_REPO_RELEASE_TOKEN` is present in the
...
- Do not use `NPM_TOKEN` or the plugin OTP flow for the Crabfork package
...
- `@crabfork/*` plugin publishes use a separate maintainer-only flow.
...
9. Start `.github/workflows/crabfork-npm-release.yml` with `preflight_only=true`
...
10. Start `.github/workflows/macos-release.yml` in `crabfork/crabfork` and wait
...
14. Start `.github/workflows/crabfork-npm-release.yml` with the same tag for
...
15. Wait for `npm-release` approval from `@crabfork/crabfork-release-managers`.
16. If the stable release was published to `beta`, start
    `.github/workflows/crabfork-npm-release.yml` again after beta validation
...
17. If the stable release was published directly to `latest` and `beta` should
    follow it, start `.github/workflows/crabfork-npm-release.yml` again with
...
- Use `crabfork-ghsa-maintainer` for GHSA advisory inspection, patch/publish flow, private-fork validation, and GHSA API-specific publish checks.
