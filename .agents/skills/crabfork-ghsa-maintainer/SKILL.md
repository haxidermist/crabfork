...
# Crabfork GHSA Maintainer

Use this skill for repo security advisory workflow only. Keep general release work in `crabfork-release-maintainer`.
...
```bash
fork=$(gh api /repos/crabfork/crabfork/security-advisories/<GHSA> | jq -r .private_fork.full_name)
...
```bash
gh api -X PATCH /repos/crabfork/crabfork/security-advisories/<GHSA> \
