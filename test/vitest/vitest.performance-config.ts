...
export function loadVitestExperimentalConfig(
  ...
  if (!windowsEnv && !isDisabled(env.CRABFORK_VITEST_FS_MODULE_CACHE)) {
    experimental.fsModuleCache = true;
  }
  if (windowsEnv && isEnabled(env.CRABFORK_VITEST_FS_MODULE_CACHE)) {
    experimental.fsModuleCache = true;
  }
  if (experimental.fsModuleCache && env.CRABFORK_VITEST_FS_MODULE_CACHE_PATH?.trim()) {
    experimental.fsModuleCachePath = env.CRABFORK_VITEST_FS_MODULE_CACHE_PATH.trim();
  }
  if (isEnabled(env.CRABFORK_VITEST_IMPORT_DURATIONS)) {
    experimental.importDurations = { print: true };
  }
  if (isEnabled(env.CRABFORK_VITEST_PRINT_IMPORT_BREAKDOWN)) {
