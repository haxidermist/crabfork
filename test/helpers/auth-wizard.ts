...
export function requireCrabforkAgentDir(): string {
  const agentDir = process.env.CRABFORK_AGENT_DIR;
  if (!agentDir) {
    throw new Error("CRABFORK_AGENT_DIR not set");
  }
  return agentDir;
}
