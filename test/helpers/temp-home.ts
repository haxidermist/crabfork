...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked CRABFORK_HOME.
  delete process.env.CRABFORK_HOME;
  process.env.CRABFORK_STATE_DIR = path.join(base, ".crabfork");
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "crabfork-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".crabfork", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
