  ...
  it("keeps `update status --json` stdout parseable even with legacy doctor preflight inputs", async () => {
    await withTempHome(
      async (tempHome) => {
        ...
        delete env.CRABFORK_HOME;
        delete env.CRABFORK_STATE_DIR;
        delete env.CRABFORK_CONFIG_PATH;
        delete env.VITEST;

        const entry = path.resolve(process.cwd(), "crabfork.mjs");
