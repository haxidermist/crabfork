...
function createTempHome(): string {
  return makeTempDir(tempDirs, "crabfork-test-env-real-home-");
}
...
describe("installTestEnv", () => {
  it("keeps live tests on a temp HOME while copying config and auth state", () => {
    ...
    writeFile(
      path.join(realHome, "custom-crabfork.json5"),
      `{
        // Preserve provider config, strip host-bound paths.
        agents: {
          defaults: {
            workspace: "/Users/peter/Projects",
            agentDir: "/Users/peter/.crabfork/agents/main/agent",
          },
          list: [
            {
              id: "dev",
              workspace: "/Users/peter/dev-workspace",
              agentDir: "/Users/peter/.crabfork/agents/dev/agent",
            },
          ],
        },
        models: {
    ...
    writeFile(path.join(realHome, ".crabfork", "credentials", "token.txt"), "secret\n");
    writeFile(
      path.join(realHome, ".crabfork", "external-plugins", "glueclaw", "crabfork.plugin.json"),
      '{"id":"glueclaw"}\n',
    );
    writeFile(
      path.join(realHome, ".crabfork", "agents", "main", "agent", "auth-profiles.json"),
    ...
    process.env.CRABFORK_LIVE_TEST_QUIET = "1";
    process.env.CRABFORK_CONFIG_PATH = "~/custom-crabfork.json5";
    process.env.CRABFORK_TEST_HOME = priorIsolatedHome;
    process.env.CRABFORK_STATE_DIR = path.join(priorIsolatedHome, ".crabfork");
    ...
    expect(process.env.CRABFORK_TEST_HOME).toBe(testEnv.tempHome);
    expect(process.env.TEST_PROFILE_ONLY).toBe("from-profile");

    const copiedConfigPath = path.join(testEnv.tempHome, ".crabfork", "crabfork.json");
    ...
    expect(
      fs.existsSync(path.join(testEnv.tempHome, ".crabfork", "credentials", "token.txt")),
    ).toBe(true);
    expect(
      fs.existsSync(
        path.join(
    ...
    expect(
      fs.existsSync(
        path.join(testEnv.tempHome, ".crabfork", "agents", "main", "agent", "auth-profiles.json"),
      ),
    ).toBe(true);
  ...
  it("allows explicit live runs against the real HOME", () => {
    ...
    process.env.USERPROFILE = realHome;
    process.env.CRABFORK_LIVE_TEST = "1";
    process.env.CRABFORK_LIVE_USE_REAL_HOME = "1";
    process.env.CRABFORK_LIVE_TEST_QUIET = "1";
  ...
  it("does not load ~/.profile for normal isolated test runs", () => {
    ...
    delete process.env.CRABFORK_LIVE_TEST;
    delete process.env.CRABFORK_LIVE_GATEWAY;
    delete process.env.CRABFORK_LIVE_USE_REAL_HOME;
    delete process.env.CRABFORK_LIVE_TEST_QUIET;
