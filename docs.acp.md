...
This document describes how the Crabfork ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`crabfork acp` exposes an ACP agent over stdio and forwards prompts to a running
Crabfork Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
runtime. It is designed to route IDE prompts into an existing Crabfork Gateway
...
- ACP client spawns `crabfork acp` and speaks ACP messages over stdio.
...
`crabfork acp` resolves the Gateway URL and auth from CLI flags or config:
