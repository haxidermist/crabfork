...
```bash
crabfork nodes describe --node <idOrNameOrIp>
crabfork nodes canvas snapshot --node <idOrNameOrIp>
crabfork logs --follow
```
...
crabfork nodes status
crabfork approvals get --node <idOrNameOrIp>
crabfork approvals allowlist add --node <idOrNameOrIp> "/usr/bin/uname"
```
...
Node pairing is an identity/trust gate, not a per-command approval surface. For `system.run`, the per-node policy lives in that node's exec approvals file (`crabfork approvals get --node ...`), not in the gateway pairing record.
