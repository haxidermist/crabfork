---
summary: "How Crabfork builds prompt context and reports token usage + costs"
read_when:
...
Crabfork tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

Crabfork assembles its own system prompt on every run. It includes:
...
For images, Crabfork downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from Crabfork session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `crabfork status --usage` and `crabfork channels list` show
...
available; otherwise Crabfork falls back to matching OAuth/API-key credentials
...
`cacheWrite`. If pricing is missing, Crabfork shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. Crabfork can
...
Anthropic's 1M context window is currently beta-gated. Crabfork can inject the
...
Crabfork skips the `context-1m-*` beta header because Anthropic currently
