...
This document explains how Crabfork manages sessions end-to-end:
...
Crabfork is designed around a single **Gateway process** that owns session state.
...
Crabfork persists sessions in two layers:
...
- Transcripts: `~/.crabfork/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

Crabfork resolves these via `src/config/sessions.ts`.
...
In `mode: "warn"`, Crabfork reports potential evictions but does not mutate the store/files.

Run maintenance on demand:

```bash
crabfork sessions cleanup --dry-run
crabfork sessions cleanup --enforce
```
...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.crabfork/cron/runs/<jobId>.jsonl` files (defaults: `2_000_000` bytes and `2000` lines).
...
Crabfork intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
When Crabfork splits a long transcript into compaction chunks, it keeps
assistant tool calls paired with their matching `toolResult` entries.

- If the token-share split lands between a tool call and its result, Crabfork
...
- If a trailing tool-result block would otherwise push the chunk over target,
  Crabfork preserves that pending tool block and keeps the unsummarized tail
  intact.
...
These are Pi runtime semantics (Crabfork consumes the events, but Pi decides when to compact).
...
Crabfork also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, Crabfork bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, Crabfork leaves it alone.
...
- If the provider fails or returns an empty result, Crabfork falls back to built-in LLM summarization automatically.
...
- `crabfork status` (CLI)
- `crabfork sessions` / `sessions --json`
...
Crabfork supports “silent” turns for background tasks where the user should not see intermediate output.
...
- Crabfork strips/suppresses this in the delivery layer.
...
As of `2026.1.10`, Crabfork also suppresses **draft/typing streaming** when a
...
Crabfork uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but Crabfork’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `crabfork status`.
