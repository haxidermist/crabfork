...
Crabfork integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Crabfork owns lifecycle when `channels.signal.autoStart=true`.
...
- Crabfork spawns `imsg rpc` as a child process (legacy iMessage integration).
