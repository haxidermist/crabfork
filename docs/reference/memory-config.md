...
This page lists every configuration knob for Crabfork memory search. For
...
`crabfork.json` unless noted otherwise.
...
When `provider` is not set, Crabfork selects the first available:
...
If Crabfork runs on EC2 with a Bedrock-enabled instance role, just set the
provider and model:
...
When sqlite-vec is unavailable, Crabfork falls back to in-process cosine
...
| Key                   | Type     | Default                               | Description                                 |
| --------------------- | -------- | ------------------------------------- | ------------------------------------------- |
| `store.path`          | `string` | `~/.crabfork/memory/{agentId}.sqlite` | Index location (supports `{agentId}` token) |
...
Crabfork prefers the current QMD collection and MCP query shapes, but keeps
older QMD releases working by falling back to legacy `--mask` collection flags
and older MCP tool names when needed.

QMD model overrides stay on the QMD side, not Crabfork config. If you need to
