...
Crabfork features that can generate provider usage or paid API calls.
...
- Gemini CLI note: when the CLI returns JSON output, Crabfork reads usage from
...
Anthropic still does not expose a per-message dollar estimate that Crabfork can
show in `/usage full`.

**CLI usage windows (provider quotas)**

- `crabfork status --usage` and `crabfork channels list` show provider **usage windows**
...
- MiniMax note: its raw `usage_percent` / `usagePercent` fields mean remaining
  quota, so Crabfork inverts them before display. Count-based fields still win
  when present. If the provider returns `model_remains`, Crabfork prefers the
...
- Usage auth for those quota windows comes from provider-specific hooks when
  available; otherwise Crabfork falls back to matching OAuth/API-key
...
Crabfork can pick up credentials from:
...
Crabfork's local UI, such as **OpenAI Codex**, **Alibaba Cloud Model Studio
Coding Plan**, **MiniMax Coding Plan**, **Z.AI / GLM Coding Plan**, and
Anthropic's Crabfork Claude-login path with **Extra Usage** enabled.
...
- `crabfork status --usage`
- `crabfork models status --json`
...
`crabfork models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
