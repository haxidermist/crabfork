...
CLI onboarding is the **recommended** way to set up Crabfork on macOS,
...
`crabfork dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`crabfork configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.crabfork/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, onboarding asks you to run `crabfork doctor` first.
</Note>
...
Use `crabfork agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.crabfork/workspace-<agentId>`.
...
- CLI command reference: [`crabfork onboard`](/cli/onboard)
