---
summary: "Get Crabfork installed and run your first chat in minutes."
read_when:
...
Install Crabfork, run onboarding, and chat with your AI assistant — all in
...
```bash
mkdir -p "$HOME/.crabfork/control-ui-custom"
...
```json
{
  "gateway": {
    "controlUi": {
      "enabled": true,
      "root": "$HOME/.crabfork/control-ui-custom"
    }
  }
}
```
...
<Accordion title="Advanced: environment variables">
  If you run Crabfork as a service account or want custom paths:

- `CRABFORK_HOME` — home directory for internal path resolution
- `CRABFORK_STATE_DIR` — override the state directory
- `CRABFORK_CONFIG_PATH` — override the config file path
