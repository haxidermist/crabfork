---
summary: "Scripted onboarding and agent setup for the Crabfork CLI"
read_when:
...
Use `--non-interactive` to automate `crabfork onboard`.
...
Anthropic setup-token remains available as a supported onboarding token path, but Crabfork now prefers Claude CLI reuse when available.
