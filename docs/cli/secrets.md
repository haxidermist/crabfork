---
summary: "CLI reference for `crabfork secrets` (reload, audit, configure, apply)"
read_when:
...
Use `crabfork secrets` to manage SecretRefs and keep the active runtime snapshot healthy.
...
```bash
crabfork secrets reload
crabfork secrets reload --json
crabfork secrets reload --url ws://127.0.0.1:18789 --token <token>
```
...
Scan Crabfork state for:

- plaintext secret storage
- unresolved refs
- precedence drift (`auth-profiles.json` credentials shadowing `crabfork.json` refs)
...
```bash
crabfork secrets audit
crabfork secrets audit --check
crabfork secrets audit --json
crabfork secrets audit --allow-exec
```
...
```bash
crabfork secrets configure
crabfork secrets configure --plan-out /tmp/crabfork-secrets-plan.json
crabfork secrets configure --apply --yes
crabfork secrets configure --providers-only
crabfork secrets configure --skip-provider-setup
crabfork secrets configure --agent ops
crabfork secrets configure --json
```
...
- `configure` targets secret-bearing fields in `crabfork.json` plus `auth-profiles.json` for the selected agent scope.
...
- On Windows, if ACL verification is unavailable for a provider path, Crabfork fails closed. For trusted paths only, set `allowInsecurePath: true` on that provider to bypass path security checks.
...
crabfork secrets apply --from /tmp/crabfork-secrets-plan.json --json
```
...
- `crabfork.json` (SecretRef targets + provider upserts/deletes)
- `auth-profiles.json` (provider-target scrubbing)
- legacy `auth.json` residues
- `~/.crabfork/.env` known secret keys whose values were migrated
