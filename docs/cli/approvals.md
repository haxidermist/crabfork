---
summary: "CLI reference for `crabfork approvals` and `crabfork exec-policy`"
read_when:
...
## `crabfork exec-policy`

`crabfork exec-policy` is the local convenience command for keeping the requested
...
- synchronize local `tools.exec.*` and local `~/.crabfork/exec-approvals.json`

Examples:

```bash
crabfork exec-policy show
crabfork exec-policy show --json

crabfork exec-policy preset yolo
crabfork exec-policy preset cautious --json

crabfork exec-policy set --host gateway --security full --ask off --ask-fallback full
```
...
- `crabfork exec-policy show` marks `host=node` scopes as node-managed at runtime instead of deriving an effective policy from the local approvals file

If you need to edit remote host approvals directly, keep using `crabfork approvals set --gateway`
or `crabfork approvals set --node <id|name|ip>`.

## Common commands

```bash
crabfork approvals get
crabfork approvals get --node <id|name|ip>
crabfork approvals get --gateway
```

`crabfork approvals get` now shows the effective exec policy for local, gateway, and node targets:
...
```bash
crabfork approvals set --file ./exec-approvals.json
crabfork approvals set --stdin <<'EOF'
{ version: 1, defaults: { security: "full", ask: "off" } }
EOF
crabfork approvals set --node <id|name|ip> --file ./exec-approvals.json
crabfork approvals set --gateway --file ./exec-approvals.json
```
...
This changes the **host approvals file** only. To keep the requested Crabfork policy aligned, also set:
...
```bash
crabfork approvals allowlist add "~/Projects/**/bin/rg"
crabfork approvals allowlist add --agent main --node <id|name|ip> "/usr/bin/uptime"
crabfork approvals allowlist add --agent "*" "/usr/bin/uname"

crabfork approvals allowlist remove "~/Projects/**/bin/rg"
```
...
- `--node` uses the same resolver as `crabfork nodes` (id, name, ip, or id prefix).
- `--agent` defaults to `"*"`, which applies to all agents.
- The node host must advertise `system.execApprovals.get/set` (macOS app or headless node host).
- Approvals files are stored per host at `~/.crabfork/exec-approvals.json`.
