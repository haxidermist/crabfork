---
summary: "CLI reference for `crabfork update` (safe-ish source update + gateway auto-restart)"
read_when:
...
Safely update Crabfork and switch between stable/beta/dev channels.
...
- `--tag <dist-tag|version|spec>`: override the package target for this update only. For package installs, `main` maps to `github:crabfork/crabfork#main`.
...
crabfork update status --timeout 10
```
...
When you switch channels explicitly (`--channel ...`), Crabfork also keeps the
install method aligned:

- `dev` → ensures a git checkout (default: `~/crabfork`, override with `CRABFORK_GIT_DIR`),
...
8. Runs `crabfork doctor` as the final “safe update” check.
...
`crabfork --update` rewrites to `crabfork update` (useful for shells and launcher scripts).

## See also

- `crabfork doctor` (offers to run update first on git checkouts)
