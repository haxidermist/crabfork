---
summary: "CLI reference for `crabfork dns` (wide-area discovery helpers)"
read_when:
  - You want wide-area discovery (DNS-SD) via Tailscale + CoreDNS
  - You’re setting up split DNS for a custom discovery domain (example: crabfork.internal)
title: "dns"
---
...
```bash
crabfork dns setup
crabfork dns setup --domain crabfork.internal
...
- `--domain <domain>`: wide-area discovery domain (for example `crabfork.internal`)
...
- zone file path
- current tailnet IPs
- recommended `crabfork.json` discovery config
...
- If `--domain` is omitted, Crabfork uses `discovery.wideArea.domain` from config.
