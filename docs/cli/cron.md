---
summary: "CLI reference for `crabfork cron` (schedule and run background jobs)"
read_when:
...
Tip: run `crabfork cron --help` for the full command surface.
...
Note: `crabfork cron run` now returns as soon as the manual run is queued for execution. Successful responses include `{ ok: true, enqueued: true, runId }`; use `crabfork cron runs --id <job-id>` to follow the eventual outcome.

Note: `crabfork cron run <job-id>` force-runs by default. Use `--due` to keep the
...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.crabfork/cron/runs/<jobId>.jsonl`.

Upgrade note: if you have older cron jobs from before the current delivery/store format, run
`crabfork doctor --fix`. Doctor now normalizes legacy cron fields (`jobId`, `schedule.cron`,
...
```bash
crabfork cron edit <job-id> --announce --channel telegram --to "123456789"
```

Disable delivery for an isolated job:

```bash
crabfork cron edit <job-id> --no-deliver
```

Enable lightweight bootstrap context for an isolated job:

```bash
crabfork cron edit <job-id> --light-context
```

Announce to a specific channel:

```bash
crabfork cron edit <job-id> --announce --channel slack --to "channel:C1234567890"
```
...
```bash
crabfork cron run <job-id>
crabfork cron run <job-id> --due
crabfork cron runs --id <job-id> --limit 50
```

Agent/session retargeting:

```bash
crabfork cron edit <job-id> --agent ops
crabfork cron edit <job-id> --clear-agent
crabfork cron edit <job-id> --session current
crabfork cron edit <job-id> --session "session:daily-brief"
```
...
```bash
crabfork cron edit <job-id> --announce --channel slack --to "channel:C1234567890"
crabfork cron edit <job-id> --best-effort-deliver
crabfork cron edit <job-id> --no-best-effort-deliver
crabfork cron edit <job-id> --no-deliver
```
