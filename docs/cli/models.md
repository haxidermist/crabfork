---
summary: "CLI reference for `crabfork models` (status/list/set/scan, aliases, fallbacks, auth)"
read_when:
...
crabfork models set <model-or-alias>
crabfork models scan
```

`crabfork models status` shows the resolved default/fallbacks plus an auth overview.
...
when available; otherwise Crabfork falls back to matching OAuth/API-key
...
the command uses `CRABFORK_AGENT_DIR`/`PI_CODING_AGENT_DIR` if set, otherwise the
...
- If you omit the provider, Crabfork resolves the input as an alias first, then
...
- `--probe-concurrency <n>`
- `--probe-max-tokens <n>`
- `--agent <id>` (configured agent id; overrides `CRABFORK_AGENT_DIR`/`PI_CODING_AGENT_DIR`)
...
- `no_model`: provider auth exists, but Crabfork could not resolve a probeable
...
```bash
crabfork models auth add
crabfork models auth login --provider <id>
crabfork models auth setup-token --provider <id>
crabfork models auth paste-token
```
...
`crabfork plugins list` to see which providers are installed.

Examples:

```bash
crabfork models auth login --provider openai-codex --set-default
```
