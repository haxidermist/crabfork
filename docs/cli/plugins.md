---
summary: "CLI reference for `crabfork plugins` (list, install, marketplace, uninstall, enable/disable, doctor)"
read_when:
...
crabfork plugins list --enabled
crabfork plugins list --verbose
crabfork plugins list --json
crabfork plugins install <path-or-spec>
crabfork plugins inspect <id>
crabfork plugins inspect <id> --json
crabfork plugins inspect --all
crabfork plugins info <id>
crabfork plugins enable <id>
crabfork plugins disable <id>
crabfork plugins uninstall <id>
crabfork plugins doctor
crabfork plugins update <id>
crabfork plugins update --all
crabfork plugins marketplace list <marketplace>
crabfork plugins marketplace list <marketplace> --json
```

Bundled plugins ship with Crabfork. Some are enabled by default (for example
bundled model providers, bundled speech providers, and the bundled browser
plugin); others require `plugins enable`.

Native Crabfork plugins must ship `crabfork.plugin.json` with an inline JSON
Schema (`configSchema`, even if empty). Compatible bundles use their own bundle
manifests instead.

`plugins list` shows `Format: crabfork` or `Format: bundle`. Verbose list/info
...
```bash
crabfork plugins install <package>                      # ClawHub first, then npm
crabfork plugins install clawhub:<package>              # ClawHub only
crabfork plugins install <package> --force              # overwrite existing install
crabfork plugins install <package> --pin                # pin version
crabfork plugins install <package> --dangerously-force-unsafe-install
crabfork plugins install <path>                         # local path
crabfork plugins install <plugin>@<marketplace>         # marketplace
crabfork plugins install <plugin> --marketplace <name>  # marketplace (explicit)
...
run `crabfork doctor --fix` first. The only documented exception is a narrow
...
override, while `crabfork skills install` remains a separate ClawHub skill
download/install flow.

`plugins install` is also the install surface for hook packs that expose
`crabfork.hooks` in `package.json`. Use `crabfork hooks` for filtered hook
...
those to a prerelease, Crabfork stops and asks you to opt in explicitly with a
prerelease tag such as `@beta`/`@rc` or an exact prerelease version such as
`@1.2.3-beta.4`.

If a bare install spec matches a bundled plugin id (for example `diffs`), Crabfork
...
```bash
crabfork plugins install clawhub:crabfork-codex-app-server
crabfork plugins install clawhub:crabfork-codex-app-server@1.2.3
```

Crabfork now also prefers ClawHub for bare npm-safe plugin specs. It only falls
...
Crabfork downloads the package archive from ClawHub, checks the advertised
...
```bash
crabfork plugins marketplace list <marketplace-name>
crabfork plugins install <plugin-name>@<marketplace-name>
```

Use `--marketplace` when you want to pass the marketplace source explicitly:

```bash
crabfork plugins install <plugin-name> --marketplace <marketplace-name>
crabfork plugins install <plugin-name> --marketplace <owner/repo>
crabfork plugins install <plugin-name> --marketplace https://github.com/<owner>/<repo>
crabfork plugins install <plugin-name> --marketplace ./my-marketplace
```
...
inside the cloned marketplace repo. Crabfork accepts relative path sources from
that repo and rejects HTTP(S), absolute-path, git, GitHub, and other non-path
plugin sources from remote manifests.

For local paths and archives, Crabfork auto-detects:

- native Crabfork plugins (`crabfork.plugin.json`)
...
```bash
crabfork plugins install -l ./my-plugin
```
...
```bash
crabfork plugins uninstall <id>
crabfork plugins uninstall <id> --dry-run
...
```bash
crabfork plugins update <id-or-npm-spec>
crabfork plugins update --all
crabfork plugins update <id-or-npm-spec> --dry-run
crabfork plugins update @crabfork/voice-call@beta
crabfork plugins update crabfork-codex-app-server --dangerously-force-unsafe-install
```

Updates apply to tracked installs in `plugins.installs` and tracked hook-pack
installs in `hooks.internal.installs`.

When you pass a plugin id, Crabfork reuses the recorded install spec for that
...
or exact version. Crabfork resolves that package name back to the tracked plugin
record, updates that installed plugin, and records the new npm spec for future
id-based updates.

When a stored integrity hash exists and the fetched artifact hash changes,
Crabfork prints a warning and asks for confirmation before proceeding. Use
