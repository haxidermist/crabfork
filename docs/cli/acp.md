...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an Crabfork Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`crabfork acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
runtime. It focuses on session routing, prompt delivery, and basic streaming
updates.

If you want an external MCP client to talk directly to Crabfork channel
conversations instead of hosting an ACP harness session, use
[`crabfork mcp serve`](/cli/mcp) instead.
...
- Crabfork acts as an ACP server
- an IDE or ACP client connects to Crabfork
- Crabfork forwards that work into a Gateway session

This is different from [ACP Agents](/tools/acp-agents), where Crabfork runs an
external harness such as Codex or Claude Code through `acpx`.

Quick rule:

- editor/client wants to talk ACP to Crabfork: use `crabfork acp`
- Crabfork should launch Codex/Claude/Gemini as an ACP harness: use `/acp spawn` and [ACP Agents](/tools/acp-agents)
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the Crabfork gateway or agent instead.                                                                                                                                     |
...
# Remote Gateway
crabfork acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
crabfork acp --url wss://gateway-host:18789 --token-file ~/.crabfork/gateway.token

# Attach to an existing session key
crabfork acp --session agent:main:main

# Attach by label (must already exist)
crabfork acp --session-label "support inbox"

# Reset the session key before the first prompt
crabfork acp --session agent:main:main --reset-session
```
...
crabfork acp client --server-args --url wss://gateway-host:18789 --token-file ~/.crabfork/gateway.token

# Override the server command (default: crabfork)
crabfork acp client --server "node" --server-args crabfork.mjs acp --url ws://127.0.0.1:19001
```
...
- This ACP bridge policy is separate from ACPX harness permissions. If you run Crabfork through the `acpx` backend, `plugins.entries.acpx.config.permissionMode=approve-all` is the break-glass “yolo” switch for that harness session.

## How to use this

Use ACP when an IDE (or other client) speaks Agent Client Protocol and you want
it to drive an Crabfork Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `crabfork acp` over stdio.

Example config (persisted):

```bash
crabfork config set gateway.remote.url wss://gateway-host:18789
crabfork config set gateway.remote.token <token>
```
...
```bash
crabfork acp --session agent:main:main
crabfork acp --session agent:design:main
crabfork acp --session agent:qa:bug-123
```
...
If you want ACPX-backed sessions to see Crabfork plugin tools, enable the
...
Crabfork bot over ACP, use `acpx` with its built-in `crabfork` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx crabfork` at `crabfork acp`.
3. Target the Crabfork session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default Crabfork ACP session
acpx crabfork exec "Summarize the active Crabfork session state."

# Persistent named session for follow-up turns
acpx crabfork sessions ensure --name codex-bridge
acpx crabfork -s codex-bridge --cwd /path/to/repo \
  "Ask my Crabfork work agent for recent context relevant to this repo."
```

If you want `acpx crabfork` to target a specific Gateway and session key every
time, override the `crabfork` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "crabfork": {
      "command": "env CRABFORK_HIDE_BANNER=1 CRABFORK_SUPPRESS_NOTES=1 crabfork acp --url ws://127.0.0.1:18789 --token-file ~/.crabfork/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local Crabfork checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env CRABFORK_HIDE_BANNER=1 CRABFORK_SUPPRESS_NOTES=1 node crabfork.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an Crabfork agent without scraping a terminal.
...
In Zed, open the Agent panel and select “Crabfork ACP” to start a thread.
...
- Prefer `--token-file`/`--password-file` or environment variables (`CRABFORK_GATEWAY_TOKEN`, `CRABFORK_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`CRABFORK_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
...
- ACP runtime backend child processes receive `CRABFORK_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `crabfork acp client` sets `CRABFORK_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `crabfork`).
