---
summary: "CLI reference for `crabfork memory` (status/index/search/promote/promote-explain/rem-harness)"
read_when:
...
```bash
crabfork memory status
crabfork memory status --deep
crabfork memory status --fix
crabfork memory index --force
crabfork memory search "meeting notes"
crabfork memory search --query "deployment" --max-results 20
crabfork memory promote --limit 10 --min-score 0.75
crabfork memory promote --apply
crabfork memory promote --json --min-recall-count 0 --min-unique-queries 0
crabfork memory promote-explain "router vlan"
crabfork memory promote-explain "router vlan" --json
crabfork memory rem-harness
crabfork memory rem-harness --json
crabfork memory status --json
crabfork memory status --deep --index
crabfork memory status --deep --index --verbose
crabfork memory status --agent main
crabfork memory index --agent main --verbose
```
...
```bash
crabfork memory promote [--apply] [--limit <n>] [--include-promoted]
```
...
- When dreaming is enabled, `memory-core` auto-manages one cron job that runs a full sweep (`light -> REM -> deep`) in the background (no manual `crabfork cron add` required).
- `--agent <id>`: scope to a single agent (default: the default agent).
...
```bash
crabfork memory promote-explain <selector> [--agent <id>] [--include-promoted] [--json]
```
...
```bash
crabfork memory rem-harness [--agent <id>] [--include-promoted] [--json]
```
