---
summary: "CLI reference for `crabfork onboard` (interactive onboarding)"
read_when:
...
```bash
crabfork onboard
crabfork onboard --flow quickstart
crabfork onboard --flow manual
crabfork onboard --mode remote --remote-url wss://gateway-host:18789
```

For plaintext private-network `ws://` targets (trusted networks only), set
`CRABFORK_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
- `--install-daemon` starts the managed gateway install path first. Without it, you must already have a local gateway running, for example `crabfork gateway run`.
...
- Fastest first chat: `crabfork dashboard` (Control UI, no channel setup).
