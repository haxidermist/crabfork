---
summary: "CLI reference for `crabfork qr` (generate mobile pairing QR + setup code)"
read_when:
...
```bash
crabfork qr
crabfork qr --setup-code-only
crabfork qr --json
crabfork qr --remote
crabfork qr --url wss://gateway.example/ws
```
...
- With `--remote`, Crabfork requires either `gateway.remote.url` or
