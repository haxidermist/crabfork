---
summary: "CLI reference for `crabfork message` (send + channel actions)"
read_when:
...
```
crabfork message <subcommand> [flags]
```
...
- On cache miss, Crabfork will attempt a live directory lookup when the provider supports it.
...
- `crabfork message` resolves supported channel SecretRefs before running the selected action.
- Resolution is scoped to the active action target when possible:
  ...
  - when `--account` is omitted, Crabfork does not force a `default` account SecretRef scope
...
```
crabfork message send --channel discord \
...
```bash
crabfork message send --channel googlechat --target spaces/AAA... \
  --message "Choose:" \
...
```
crabfork message poll --channel discord \
  --target channel:123 \
  --poll-question "Snack?" \
...
```
crabfork message poll --channel telegram \
  --target @mychat \
  --poll-question "Lunch?" \
  --poll-option Pizza --poll-option Sushi \
...
```
crabfork message poll --channel msteams \
...
```
crabfork message react --channel slack \
...
```
crabfork message react --channel signal \
...
```
crabfork message send --channel telegram --target @mychat --message "Choose:" \
...
```bash
crabfork message send --channel telegram --target @mychat \
