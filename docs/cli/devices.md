---
summary: "CLI reference for `crabfork devices` (device pairing + token rotation/revocation)"
read_when:
...
```
crabfork devices remove <deviceId>
crabfork devices remove <deviceId> --json
```

### `crabfork devices clear --yes [--pending]`

Clear paired devices in bulk.

```
crabfork devices clear --yes
crabfork devices clear --yes --pending
crabfork devices clear --yes --pending --json
```

### `crabfork devices approve [requestId] [--latest]`

Approve a pending device pairing request by exact `requestId`. If `requestId`
is omitted or `--latest` is passed, Crabfork only prints the selected pending
...
key), Crabfork supersedes the previous pending entry and issues a new
`requestId`. Run `crabfork devices list` right before approval to use the
current ID.

```
crabfork devices approve
...
### `crabfork devices rotate --device <id> --role <role> [--scope <scope...>]`
...
```
crabfork devices rotate --device <deviceId> --role operator --scope operator.read --scope operator.write
```

Returns the new token payload as JSON.

### `crabfork devices revoke --device <id> --role <role>`
...
```
crabfork devices revoke --device <deviceId> --role node
```
...
```bash
crabfork devices rotate --device <deviceId> --role operator
```
