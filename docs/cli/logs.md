---
summary: "CLI reference for `crabfork logs` (tail gateway logs via RPC)"
read_when:
...
`crabfork logs` also accepts the standard Gateway client flags:
...
crabfork logs --follow --interval 2000
crabfork logs --limit 500 --max-bytes 500000
crabfork logs --json
crabfork logs --plain
crabfork logs --no-color
crabfork logs --limit 500
crabfork logs --local-time
crabfork logs --follow --local-time
crabfork logs --url ws://127.0.0.1:18789 --token "$CRABFORK_GATEWAY_TOKEN"
```

## Notes

- Use `--local-time` to render timestamps in your local timezone.
- If the local loopback Gateway asks for pairing, `crabfork logs` falls back to the configured local log file automatically. Explicit `--url` targets do not use this fallback.
