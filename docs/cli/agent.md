---
summary: "CLI reference for `crabfork agent` (send one agent turn via the Gateway)"
read_when:
...
```bash
crabfork agent --to +15555550123 --message "status update" --deliver
...
crabfork agent --agent ops --message "Run locally" --local
```
...
- Marker writes are source-authoritative: Crabfork persists markers from the active source config snapshot, not from resolved runtime secret values.
