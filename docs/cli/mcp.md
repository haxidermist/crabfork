---
summary: "Expose Crabfork channel conversations over MCP and manage saved MCP server definitions"
read_when:
  - Connecting Codex, Claude Code, or another MCP client to Crabfork-backed channels
  - Running `crabfork mcp serve`
  - Managing Crabfork-saved MCP server definitions
title: "mcp"
---

# mcp

`crabfork mcp` has two jobs:

- run Crabfork as an MCP server with `crabfork mcp serve`
- manage Crabfork-owned outbound MCP server definitions with `list`, `show`,
  `set`, and `unset`

In other words:

- `serve` is Crabfork acting as an MCP server
- `list` / `show` / `set` / `unset` is Crabfork acting as an MCP client-side
  registry for other MCP servers its runtimes may consume later

Use [`crabfork acp`](/cli/acp) when Crabfork should host a coding harness
session itself and route that runtime through ACP.

## Crabfork as an MCP server

This is the `crabfork mcp serve` path.

## When to use `serve`

Use `crabfork mcp serve` when:

- Codex, Claude Code, or another MCP client should talk directly to
  Crabfork-backed channel conversations
- you already have a local or remote Crabfork Gateway with routed sessions
- you want one MCP server that works across Crabfork's channel backends instead
  of running separate per-channel bridges

Use [`crabfork acp`](/cli/acp) instead when Crabfork should host the coding
runtime itself and keep the agent session inside Crabfork.

## How it works

`crabfork mcp serve` starts a stdio MCP server. The MCP client owns that
process. While the client keeps the stdio session open, the bridge connects to a
local or remote Crabfork Gateway over WebSocket and exposes routed channel
conversations over MCP.

Lifecycle:

1. the MCP client spawns `crabfork mcp serve`
...
conversations. A conversation appears when Crabfork already has session state
...
# Remote Gateway
crabfork mcp serve --url wss://gateway-host:18789 --token-file ~/.crabfork/gateway.token

# Remote Gateway with password auth
crabfork mcp serve --url wss://gateway-host:18789 --password-file ~/.crabfork/gateway.password

# Enable verbose bridge logs
crabfork mcp serve --verbose

# Disable Claude-specific push notifications
crabfork mcp serve --claude-channel-mode off
```
...
Crabfork equivalent of a Claude Code channel adapter: standard MCP tools remain
...
- sender allowlists, pairing, and channel-level trust still belong to the
  underlying Crabfork channel configuration
...
Crabfork ships a deterministic Docker smoke for this bridge:
...
- starts a second container that spawns `crabfork mcp serve`
...
## Crabfork as an MCP client registry

This is the `crabfork mcp list`, `show`, `set`, and `unset` path.

These commands do not expose Crabfork over MCP. They manage Crabfork-owned MCP
server definitions under `mcp.servers` in Crabfork config.

Those saved definitions are for runtimes that Crabfork launches or configures
later, such as embedded Pi and other runtime adapters. Crabfork stores the
definitions centrally so those runtimes do not need to keep their own duplicate
MCP server lists.

Important behavior:

- these commands only read or write Crabfork config
...
Crabfork also stores a lightweight MCP server registry in config for surfaces
that want Crabfork-managed MCP definitions.

Commands:

- `crabfork mcp list`
- `crabfork mcp show [name]`
- `crabfork mcp set <name> <json>`
- `crabfork mcp unset <name>`
...
```bash
crabfork mcp list
crabfork mcp show context7 --json
crabfork mcp set context7 '{"command":"uvx","args":["context7-mcp"]}'
crabfork mcp set docs '{"url":"https://mcp.example.com"}'
crabfork mcp unset context7
```
...
| --------------------- | -------------------------------------------------------------------------------------- |
| `url`                 | HTTP or HTTPS URL of the remote server (required)                                      |
| `transport`           | Set to `"streamable-http"` to select this transport; when omitted, Crabfork uses `sse` |
| `headers`             | Optional key-value map of HTTP headers (for example auth tokens)                       |
| `connectionTimeoutMs` | Per-server connection timeout in ms (optional)                                         |
