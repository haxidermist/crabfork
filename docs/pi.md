...
summary: "Architecture of Crabfork's embedded Pi agent integration and session lifecycle"
read_when:
  - Understanding Pi SDK integration design in Crabfork
  - Modifying agent session lifecycle, tooling, or provider wiring for Pi
---

# Pi Integration Architecture

This document describes how Crabfork integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

Crabfork uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, Crabfork directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
| `pi-tui`          | Terminal UI components (used in Crabfork's local TUI mode)                                             |
...
├── pi-tools.ts                    # createCrabforkCodingTools()
...
├── crabfork-tools.ts              # Crabfork-specific tools
...
Image injection is prompt-local: Crabfork loads image refs from the current prompt and
...
2. **Custom Replacements**: Crabfork replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **Crabfork Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
Crabfork wraps this with `guardSessionManager()` for tool result safety.
...
Crabfork maintains an auth profile store with multiple API keys per provider:
...
Crabfork loads custom pi extensions for specialized behavior:
...
Crabfork also has a local TUI mode that uses pi-tui components directly:
...
| Aspect          | Pi CLI                  | Crabfork Embedded                                                                              |
| --------------- | ----------------------- | ---------------------------------------------------------------------------------------------- |
| Invocation      | `pi` command / RPC      | SDK via `createAgentSession()`                                                                 |
| Tools           | Default coding tools    | Custom Crabfork tool suite                                                                     |
| System prompt   | AGENTS.md + prompts     | Dynamic per-channel/context                                                                    |
| Session storage | `~/.pi/agent/sessions/` | `~/.crabfork/agents/<agentId>/sessions/` (or `$CRABFORK_STATE_DIR/agents/<agentId>/sessions/`) |
...
- `src/agents/pi-embedded-runner-extraparams.live.test.ts` (enable `CRABFORK_LIVE_TEST=1`)
