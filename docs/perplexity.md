...
Crabfork supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, Crabfork also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `crabfork configure --section web`. It stores the key in
`~/.crabfork/crabfork.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.crabfork/.env` (or your service environment). See [Env vars](/help/faq#env-vars-and-env-loading).
