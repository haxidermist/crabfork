---
summary: "Run Crabfork Gateway 24/7 on an Azure Linux VM with durable state"
read_when:
  - You want Crabfork running 24/7 on Azure with Network Security Group hardening
  - You want a production-grade, always-on Crabfork Gateway on your own Azure Linux VM
  - You want secure administration with Azure Bastion SSH
title: "Azure"
---

# Crabfork on Azure Linux VM

This guide sets up an Azure Linux VM with the Azure CLI, applies Network Security Group (NSG) hardening, configures Azure Bastion for SSH access, and installs Crabfork.
...
- Install Crabfork with the installer script
- Verify the Gateway
...
<Steps>
  ...
  <Step title="Set deployment variables">
    ...
    RG="rg-crabfork"
    LOCATION="westus2"
    VNET_NAME="vnet-crabfork"
    VNET_PREFIX="10.40.0.0/16"
    VM_SUBNET_NAME="snet-crabfork-vm"
    VM_SUBNET_PREFIX="10.40.2.0/24"
    BASTION_SUBNET_PREFIX="10.40.1.0/26"
    NSG_NAME="nsg-crabfork-vm"
    VM_NAME="vm-crabfork"
    ADMIN_USERNAME="crabfork"
    BASTION_NAME="bas-crabfork"
    BASTION_PIP_NAME="pip-crabfork-bastion"
    ```
...
<Steps>
  ...
  <Step title="Install Crabfork (in the VM shell)">
    ```bash
    curl -fsSL https://crabfork.ai/install.sh -o /tmp/install.sh
    bash /tmp/install.sh
    rm -f /tmp/install.sh
    ```

    The installer installs Node LTS and dependencies if not already present, installs Crabfork, and launches the onboarding wizard. See [Install](/install) for details.
  ...
  <Step title="Verify the Gateway">
    ...
    Most enterprise Azure teams already have GitHub Copilot licenses. If that is your case, we recommend choosing the GitHub Copilot provider in the Crabfork onboarding wizard. See [GitHub Copilot provider](/providers/github-copilot).
...
- **Deallocate the VM** when not in use (stops compute billing; disk charges remain). The Crabfork Gateway will not be reachable while the VM is deallocated — restart it when you need it live again:
...
- For more details on Crabfork Azure deployment with the GitHub Copilot model provider: [Crabfork on Azure with GitHub Copilot](https://github.com/johnsonshi/crabfork-azure-github-copilot)
