---
summary: "Host Crabfork on Oracle Cloud's Always Free ARM tier"
read_when:
  - Setting up Crabfork on Oracle Cloud
  - Looking for free VPS hosting for Crabfork
...
Run a persistent Crabfork Gateway on Oracle Cloud's **Always Free** ARM tier (up to 4 OCPU, 24 GB RAM, 200 GB storage) at no cost.
...
<Steps>
  ...
  <Step title="Install Tailscale">
    ...
    From now on, connect via Tailscale: `ssh ubuntu@crabfork`.
...
**Tailscale will not connect** -- Run `sudo tailscale up --ssh --hostname=crabfork --reset` to re-authenticate.

**Gateway will not start** -- Run `crabfork doctor --non-interactive` and check logs with `journalctl --user -u crabfork-gateway.service -n 50`.
...
- [Updating](/install/updating) -- keep Crabfork up to date
