---
summary: "Install Crabfork — installer script, npm/pnpm/bun, from source, Docker, and more"
read_when:
...
The fastest way to install. It detects your OS, installs Node if needed, installs Crabfork, and launches onboarding.
...
<Tabs>
  <Tab title="macOS / Linux / WSL2">
    ```bash
    curl -fsSL https://crabfork.ai/install.sh | bash -s -- --no-onboard
    ```
  </Tab>
  <Tab title="Windows (PowerShell)">
    ```powershell
...
Use this when you want Crabfork and Node kept under a local prefix such as
`~/.crabfork`, without depending on a system-wide Node install:

```bash
curl -fsSL https://crabfork.ai/install-cli.sh | bash
```
...
```bash
SHARP_IGNORE_GLOBAL_LIBVIPS=1 npm install -g crabfork@latest
```
...
Or skip the link and use `pnpm crabfork ...` from inside the repo. See [Setup](/start/setup) for full development workflows.

### Install from GitHub main

```bash
npm install -g github:crabfork/crabfork#main
```
...
```bash
crabfork --version      # confirm the CLI is available
crabfork doctor         # check for config issues
crabfork gateway status # verify the Gateway is running
```

If you want managed startup after install:

- macOS: LaunchAgent via `crabfork onboard --install-daemon` or `crabfork gateway install`
- Linux/WSL2: systemd user service via the same commands
- Native Windows: Scheduled Task first, with a per-user Startup-folder login item fallback if task creation is denied

## Hosting and deployment

Deploy Crabfork on a cloud server or VPS:
...
<CardGroup cols={3}>
  ...
  <Card title="Uninstall" href="/install/uninstall" icon="trash-2">
    Remove Crabfork completely.
  </Card>
</CardGroup>

## Troubleshooting: `crabfork` not found

If the install succeeded but `crabfork` is not found in your terminal:
