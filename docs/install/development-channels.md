...
Crabfork ships three update channels:
...
```bash
crabfork update --channel stable
...
- **`dev`**: ensures a git checkout (default `~/crabfork`, override with
  `CRABFORK_GIT_DIR`), switches to `main`, rebases on upstream, builds, and
...
crabfork update --tag 2026.4.1-beta.1
...
crabfork update --tag crabfork@2026.4.1-beta.1
```

Notes:

- `--tag` applies to **package (npm) installs only**. Git installs ignore it.
- The tag is not persisted. Your next `crabfork update` uses your configured
  channel as usual.
- Downgrade protection: if the target version is older than your current version,
  Crabfork prompts for confirmation (skip with `--yes`).
...
Preview what `crabfork update` would do without making changes:

```bash
crabfork update --dry-run
crabfork update --channel beta --dry-run
crabfork update --tag 2026.4.1-beta.1 --dry-run
crabfork update --dry-run --json
```
...
When you switch channels with `crabfork update`, Crabfork also syncs plugin
sources:
