---
summary: "ClawDock shell helpers for Docker-based Crabfork installs"
read_when:
  - You run Crabfork with Docker often and want shorter day-to-day commands
...
ClawDock is a small shell-helper layer for Docker-based Crabfork installs.
...
| `clawdock-cli <command>`  | Run Crabfork CLI commands in Docker           |
...
- `~/.crabfork/.env` for env-backed provider keys and bot tokens
- `~/.crabfork/agents/<agentId>/agent/auth-profiles.json` for stored provider OAuth/API-key auth
- `~/.crabfork/crabfork.json` for behavior config

Use `clawdock-show-config` when you want to inspect the `.env` files and `crabfork.json` quickly. It redacts `.env` values in its printed output.
