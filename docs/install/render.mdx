---
summary: "Deploy Crabfork on Render with Infrastructure-as-Code"
read_when:
  - Deploying Crabfork to Render
  - You want a declarative cloud deploy with Render Blueprints
title: "Render"
---

Deploy Crabfork on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/crabfork/crabfork)
...
repository configures everything needed to run Crabfork:
...
services:
  - type: web
    ...
    envVars:
      - key: CRABFORK_GATEWAY_PORT
        value: "8080"
      - key: CRABFORK_STATE_DIR
        value: /data/.crabfork
      - key: CRABFORK_WORKSPACE_DIR
        value: /data/workspace
      - key: CRABFORK_GATEWAY_TOKEN
...
your fork's `render.yaml` (but note: no persistent disk means Crabfork state
...
`CRABFORK_GATEWAY_TOKEN` (find it in **Dashboard → your service →
...
If you use the original Crabfork repository, Render will not auto-deploy your Crabfork. To update it, run a manual Blueprint sync from the dashboard.
...
For Crabfork, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This creates a portable backup archive with Crabfork state plus any configured
...
- Missing `CRABFORK_GATEWAY_TOKEN` — verify it is set in **Dashboard → Environment**
- Port mismatch — ensure `CRABFORK_GATEWAY_PORT=8080` is set so the gateway binds to the port Render expects
...
regularly export a full backup via `crabfork backup create` in the Render shell.
