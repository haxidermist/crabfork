---
summary: "Deploy Crabfork Gateway to a Kubernetes cluster with Kustomize"
read_when:
  - You want to run Crabfork on a Kubernetes cluster
  - You want to test Crabfork in a Kubernetes environment
title: "Kubernetes"
---

# Crabfork on Kubernetes

A minimal starting point for running Crabfork on Kubernetes — not a production-ready deployment. It covers the core resources and is meant to be adapted to your environment.

## Why not Helm?

Crabfork is a single container with some config files. The interesting customization is in agent content (markdown files, skills, config overrides), not infrastructure templating. Kustomize handles overlays without the overhead of a Helm chart. If your deployment grows more complex, a Helm chart can be layered on top of these manifests.
...
kubectl port-forward svc/crabfork 18789:18789 -n crabfork
...
```bash
kubectl get secret crabfork-secrets -n crabfork -o jsonpath='{.data.CRABFORK_GATEWAY_TOKEN}' | base64 -d
```
...
```
Namespace: crabfork (configurable via CRABFORK_NAMESPACE)
├── Deployment/crabfork        # Single pod, init container + gateway
├── Service/crabfork           # ClusterIP on port 18789
├── PersistentVolumeClaim      # 10Gi for agent state and config
├── ConfigMap/crabfork-config  # crabfork.json + AGENTS.md
└── Secret/crabfork-secrets    # Gateway token + API keys
```
...
Edit `crabfork.json` in `scripts/k8s/manifests/configmap.yaml`. See [Gateway configuration](/gateway/configuration) for the full reference.
...
```bash
kubectl patch secret crabfork-secrets -n crabfork \
  -p '{"stringData":{"<PROVIDER>_API_KEY":"..."}}'
kubectl rollout restart deployment/crabfork -n crabfork
```

### Custom namespace

```bash
CRABFORK_NAMESPACE=my-namespace ./scripts/k8s/deploy.sh
```
...
```yaml
image: ghcr.io/crabfork/crabfork:latest # or pin to a specific version from https://github.com/crabfork/crabfork/releases
```
...
└── manifests/
    ...
    ├── configmap.yaml          # crabfork.json + AGENTS.md
