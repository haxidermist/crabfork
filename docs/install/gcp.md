---
summary: "Run Crabfork Gateway 24/7 on a GCP Compute Engine VM (Docker) with durable state"
read_when:
  - You want Crabfork running 24/7 on GCP
  - You want a production-grade, always-on Gateway on your own VM
  - You want full control over persistence, binaries, and restart behavior
title: "GCP"
---

# Crabfork on GCP Compute Engine (Docker, Production VPS Guide)

## Goal

Run a persistent Crabfork Gateway on a GCP Compute Engine VM using Docker, with durable state, baked-in binaries, and safe restart behavior.

If you want "Crabfork 24/7 for ~$5-12/mo", this is a reliable setup on Google Cloud.
...
- Start the Crabfork Gateway in Docker
- Persist `~/.crabfork` + `~/.crabfork/workspace` on the host (survives restarts/rebuilds)
- Access the Control UI from your laptop via an SSH tunnel

That mounted `~/.crabfork` state includes `crabfork.json`, per-agent
...
<Steps>
  ...
  <Step title="Create a GCP project">
    **CLI:**
    ...
    ```bash
    gcloud projects create my-crabfork-project --name="Crabfork Gateway"
    gcloud config set project my-crabfork-project
    ```
  ...
  <Step title="Create the VM">
    ...
    **CLI:**
    ...
    ```bash
    gcloud compute instances create crabfork-gateway \
      --zone=us-central1-a \
  ...
  <Step title="SSH into the VM">
    **CLI:**
    ...
    ```bash
    gcloud compute ssh crabfork-gateway --zone=us-central1-a
    ```
  ...
  <Step title="Clone the Crabfork repository">
    ```bash
  ...
  <Step title="Configure environment variables">
    ...
    ```bash
    CRABFORK_IMAGE=crabfork:latest
    CRABFORK_GATEWAY_TOKEN=change-me-now
    CRABFORK_GATEWAY_BIND=lan
    CRABFORK_GATEWAY_PORT=18789

    CRABFORK_CONFIG_DIR=/home/$USER/.crabfork
    CRABFORK_WORKSPACE_DIR=/home/$USER/.crabfork/workspace

    GOG_KEYRING_PASSWORD=change-me-now
    XDG_CONFIG_HOME=/home/node/.crabfork
    ```
    ...
    **Do not commit this file.**
    ...
    This `.env` file is for container/runtime env such as `CRABFORK_GATEWAY_TOKEN`.
    ...
    services:
      crabfork-gateway:
        ...
        environment:
          ...
          - NODE_ENV=production
          - TERM=xterm-256color
          - CRABFORK_GATEWAY_BIND=${CRABFORK_GATEWAY_BIND}
          - CRABFORK_GATEWAY_PORT=${CRABFORK_GATEWAY_PORT}
          - CRABFORK_GATEWAY_TOKEN=${CRABFORK_GATEWAY_TOKEN}
        ...
        ports:
          ...
          - "127.0.0.1:${CRABFORK_GATEWAY_PORT}:18789"
        command:
          [
            "node",
            "dist/index.js",
  ...
  <Step title="GCP-specific launch notes">
    ...
    When binding to LAN (`CRABFORK_GATEWAY_BIND=lan`), configure a trusted browser origin before continuing:

    ```bash
    docker compose run --rm crabfork-cli config set gateway.controlUi.allowedOrigins '["http://127.0.0.1:18789"]' --strict-json
    ```
  ...
  <Step title="Access from your laptop">
    ...
    ```bash
    gcloud compute ssh crabfork-gateway --zone=us-central1-a -- -L 18789:127.0.0.1:18789
    ```
...
```bash
# Stop the VM first
gcloud compute instances stop crabfork-gateway --zone=us-central1-a

# Change machine type
gcloud compute instances set-machine-type crabfork-gateway \
  --zone=us-central1-a \
  --machine-type=e2-small

# Start the VM
gcloud compute instances start crabfork-gateway --zone=us-central1-a
```
...
1. Create a service account:
   ...
   ```bash
   gcloud iam service-accounts create crabfork-deploy \
     --display-name="Crabfork Deployment"
   ```

2. Grant Compute Instance Admin role (or narrower custom role):

   ```bash
   gcloud projects add-iam-policy-binding my-crabfork-project \
     --member="serviceAccount:crabfork-deploy@my-crabfork-project.iam.gserviceaccount.com" \
