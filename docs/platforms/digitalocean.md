---
summary: "Crabfork on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up Crabfork on DigitalOcean
  - Looking for cheap VPS hosting for Crabfork
title: "DigitalOcean (Platform)"
---

# Crabfork on DigitalOcean

## Goal

Run a persistent Crabfork Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
```bash
crabfork config set gateway.bind tailnet
crabfork gateway restart
```
...
```bash
crabfork channels login whatsapp
# Scan QR code
```
...
- `~/.crabfork/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl --user -u crabfork-gateway.service --no-pager -n 50
```
