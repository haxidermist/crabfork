---
summary: "Crabfork on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Crabfork on a Raspberry Pi
  - Running Crabfork on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi (Platform)"
---

# Crabfork on Raspberry Pi

## Goal

Run a persistent, always-on Crabfork Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
# View logs
journalctl --user -u crabfork-gateway.service -f
```

## 9) Access the Crabfork Dashboard
...
```bash
ssh user@gateway-host 'crabfork dashboard --no-open'
```
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/crabfork-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF' # pragma: allowlist secret
export NODE_COMPILE_CACHE=/var/tmp/crabfork-compile-cache
mkdir -p /var/tmp/crabfork-compile-cache
export CRABFORK_NO_RESPAWN=1
EOF
source ~/.bashrc
```
...
- `CRABFORK_NO_RESPAWN=1` avoids extra startup cost from CLI self-respawn.
- First run warms the cache; later runs benefit most.

### systemd startup tuning (optional)

If this Pi is mostly running Crabfork, add a service drop-in to reduce restart
jitter and keep startup env stable:

```bash
systemctl --user edit crabfork-gateway.service
```

```ini
[Service]
Environment=CRABFORK_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/crabfork-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```
...
If possible, keep Crabfork state/cache on SSD-backed storage to avoid SD-card
...
Most Crabfork features work on ARM64, but some external binaries may need ARM builds:
...
systemctl --user is-enabled crabfork-gateway.service

# Enable if not
systemctl --user enable crabfork-gateway.service
...
```bash
# Check logs
journalctl --user -u crabfork-gateway.service --no-pager -n 100

# Common fix: rebuild
cd ~/crabfork  # if using hackable install
npm run build
systemctl --user restart crabfork-gateway.service
```
