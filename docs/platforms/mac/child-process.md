...
service via the external `crabfork` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.crabfork.gateway`
  (or `ai.crabfork.<profile>` when using `--profile`/`CRABFORK_PROFILE`; legacy `com.crabfork.*` is supported).
