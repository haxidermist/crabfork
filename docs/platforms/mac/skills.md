...
The macOS app surfaces Crabfork skills via the gateway; it does not parse skills locally.

## Data source

- `skills.status` (gateway) returns all skills plus eligibility and missing requirements
  (including allowlist blocks for bundled skills).
- Requirements are derived from `metadata.crabfork.requires` in each `SKILL.md`.

## Install actions

- `metadata.crabfork.install` defines install options (brew/node/go/uv).
...
- The app stores keys in `~/.crabfork/crabfork.json` under `skills.entries.<skillKey>`.
