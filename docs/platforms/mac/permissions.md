...
- Same path: run the app from a fixed location (for Crabfork, `dist/Crabfork.app`).
...
```bash
sudo tccutil reset Accessibility ai.crabfork.mac
sudo tccutil reset ScreenCapture ai.crabfork.mac
...
Workaround: move files into the Crabfork workspace (`~/.crabfork/workspace`) if you want to avoid per-folder grants.
