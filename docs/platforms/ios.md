...
- Network path:
  ...
  - Tailnet via unicast DNS-SD (example domain: `crabfork.internal.`), **or**
...
Run `crabfork devices list` again before approval.

4. Verify connection:

```bash
crabfork nodes status
crabfork gateway call node.list --params "{}"
```
...
- `CRABFORK_APNS_RELAY_BASE_URL` still works as a temporary env override for the gateway.
...
- Only genuine Crabfork iOS builds distributed through Apple can use the hosted relay.
...
- To allow hosted relay usage only for official/TestFlight Crabfork builds.
...
```bash
export CRABFORK_APNS_TEAM_ID="TEAMID"
export CRABFORK_APNS_KEY_ID="KEYID"
export CRABFORK_APNS_PRIVATE_KEY_P8="$(cat /path/to/AuthKey_KEYID.p8)"
```
...
```bash
mkdir -p ~/.crabfork/credentials/apns
chmod 700 ~/.crabfork/credentials/apns
mv /path/to/AuthKey_KEYID.p8 ~/.crabfork/credentials/apns/AuthKey_KEYID.p8
chmod 600 ~/.crabfork/credentials/apns/AuthKey_KEYID.p8
export CRABFORK_APNS_PRIVATE_KEY_PATH="$HOME/.crabfork/credentials/apns/AuthKey_KEYID.p8"
```
...
The iOS app browses `_crabfork-gw._tcp` on `local.` and, when configured, the same
...
`crabfork.internal.`) and Tailscale split DNS.
...
```bash
crabfork nodes invoke --node "iOS Node" --command canvas.navigate --params '{"url":"http://<gateway-host>:18789/__crabfork__/canvas/"}'
```

Notes:

- The Gateway canvas host serves `/__crabfork__/canvas/` and `/__crabfork__/a2ui/`.
...
```bash
crabfork nodes invoke --node "iOS Node" --command canvas.eval --params '{"javaScript":"(() => { const {ctx} = window.__crabfork; ctx.clearRect(0,0,innerWidth,innerHeight); ctx.lineWidth=6; ctx.strokeStyle=\"#ff2d55\"; ctx.beginPath(); ctx.moveTo(40,40); ctx.lineTo(innerWidth-40, innerHeight-40); ctx.stroke(); return \"ok\"; })()"}'
```

```bash
crabfork nodes invoke --node "iOS Node" --command canvas.snapshot --params '{"maxWidth":900,"format":"jpeg"}'
```
...
- Pairing prompt never appears: run `crabfork devices list` and approve manually.
