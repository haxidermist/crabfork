...
**Import:** `crabfork/plugin-sdk/plugin-entry`
...
| `kind`         | `string`                                                         | No       | —                   |
| `configSchema` | `CrabforkPluginConfigSchema \| () => CrabforkPluginConfigSchema` | No       | Empty object schema |
| `register`     | `(api: CrabforkPluginApi) => void`                               | Yes      | —                   |

- `id` must match your `crabfork.plugin.json` manifest.
- `kind` is for exclusive slots: `"memory"` or `"context-engine"`.
- `configSchema` can be a function for lazy evaluation.
- Crabfork resolves and memoizes that schema on first access, so expensive schema
  builders only run once.

## `defineChannelPluginEntry`

**Import:** `crabfork/plugin-sdk/channel-core`
...
| `registerCliMetadata` | `(api: CrabforkPluginApi) => void`                               | No       | —                   |
| `registerFull`        | `(api: CrabforkPluginApi) => void`                               | No       | —                   |
...
- Like `definePluginEntry`, `configSchema` can be a lazy factory and Crabfork
...
Crabfork loads this instead of the full entry when a channel is disabled,
...
- `crabfork/plugin-sdk/setup-runtime` for runtime-safe setup helpers such as
  import-safe setup patch adapters, lookup-note output,
  `promptResolvedAllowFrom`, `splitSetupEntries`, and delegated setup proxies
- `crabfork/plugin-sdk/channel-setup` for optional-install setup surfaces
- `crabfork/plugin-sdk/setup-tools` for setup/install CLI/archive/docs helpers
...
- use `descriptors` when the registrar owns one or more root commands and you
  want Crabfork to lazy-load the real CLI module on first invocation
...
Crabfork classifies loaded plugins by their registration behavior:
...
Use `crabfork plugins inspect <id>` to see a plugin's shape.
