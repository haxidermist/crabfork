...
read_when:
  - You see the CRABFORK_PLUGIN_SDK_COMPAT_DEPRECATED warning
  - You see the CRABFORK_EXTENSION_API_DEPRECATED warning
  - You are updating a plugin to the modern plugin architecture
  - You maintain an external Crabfork plugin
---

# Plugin SDK Migration

Crabfork has moved from a broad backwards-compatibility layer to a modern plugin
...
- **`crabfork/plugin-sdk/compat`** — a single import that re-exported dozens of
  helpers. It was introduced to keep older hook-based plugins working while the
  new plugin architecture was being built.
- **`crabfork/extension-api`** — a bridge that gave plugins direct access to
...
The modern plugin SDK fixes this: each import path (`crabfork/plugin-sdk/\<subpath\>`)
is a small, self-contained module with a clear purpose and documented contract.

Legacy provider convenience seams for bundled channels are also gone. Imports
such as `crabfork/plugin-sdk/slack`, `crabfork/plugin-sdk/discord`,
`crabfork/plugin-sdk/signal`, `crabfork/plugin-sdk/whatsapp`,
channel-branded helper seams, and
`crabfork/plugin-sdk/telegram-core` were private mono-repo shortcuts, not
...
<Steps>
  <Step title="Migrate approval-native handlers to capability facts">
    ...
    - Register channel-owned runtime objects such as clients, tokens, or Bolt
      apps through `crabfork/plugin-sdk/channel-runtime-context`
  ...
  <Step title="Audit Windows wrapper fallback behavior">
    If your plugin uses `crabfork/plugin-sdk/windows-spawn`, unresolved Windows
  ...
  <Step title="Find deprecated imports">
    ...
    grep -r "crabfork/extension-api" my-plugin/
    ```
  ...
  <Step title="Replace with focused imports">
    ...
    } from "crabfork/plugin-sdk/compat";
    ...
    import { runEmbeddedPiAgent } from "crabfork/extension-api";
...
<Accordion title="Common import path table">
  ...
  | `plugin-sdk/config-schema` | Root config schema export | `CrabforkSchema` |
...
```bash
CRABFORK_SUPPRESS_PLUGIN_SDK_COMPAT_WARNING=1 crabfork gateway run
CRABFORK_SUPPRESS_EXTENSION_API_WARNING=1 crabfork gateway run
```
