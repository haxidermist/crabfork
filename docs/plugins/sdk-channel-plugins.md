...
summary: "Step-by-step guide to building a messaging channel plugin for Crabfork"
read_when:
  - You are building a new messaging channel plugin
  - You want to connect Crabfork to a messaging platform
  - You need to understand the ChannelPlugin adapter surface
---

# Building Channel Plugins

This guide walks through building a channel plugin that connects Crabfork to a
...
<Info>
  If you have not built any Crabfork plugin before, read
...
Channel plugins do not need their own send/edit/react tools. Crabfork keeps one
...
- If a channel can infer stable owner-like DM identities from existing config, use `createResolvedApproverActionAuthAdapter` from `crabfork/plugin-sdk/approval-runtime` to restrict same-chat `/approve` without adding approval-specific core logic.
- If a channel needs native approval delivery, keep channel code focused on target normalization plus transport/presentation facts. Use `createChannelExecApprovalProfile`, `createChannelNativeOriginTargetResolver`, `createChannelApproverDmTargetResolver`, and `createApproverRestrictedNativeApprovalCapability` from `crabfork/plugin-sdk/approval-runtime`. Put the channel-specific facts behind `approvalCapability.nativeRuntime`, ideally via `createChannelApprovalNativeRuntimeAdapter(...)` or `createLazyChannelApprovalNativeRuntimeAdapter(...)`, so core can assemble the handler and own request filtering, routing, dedupe, expiry, gateway subscription, and routed-elsewhere notices. `nativeRuntime` is split into a few smaller seams:
...
- `crabfork/plugin-sdk/approval-auth-runtime`
- `crabfork/plugin-sdk/approval-client-runtime`
- `crabfork/plugin-sdk/approval-delivery-runtime`
...
- `crabfork/plugin-sdk/approval-native-runtime`
- `crabfork/plugin-sdk/approval-reply-runtime`
- `crabfork/plugin-sdk/channel-runtime-context`

Likewise, prefer `crabfork/plugin-sdk/setup-runtime`,
`crabfork/plugin-sdk/setup-adapter-runtime`,
`crabfork/plugin-sdk/reply-runtime`,
`crabfork/plugin-sdk/reply-dispatch-runtime`,
`crabfork/plugin-sdk/reply-reference`, and
`crabfork/plugin-sdk/reply-chunking` when you do not need the broader umbrella
surface.

For setup specifically:

- `crabfork/plugin-sdk/setup-runtime` covers the runtime-safe setup helpers:
...
- `crabfork/plugin-sdk/setup-adapter-runtime` is the narrow env-aware adapter
  seam for `createEnvPatchedAccountSetupAdapter`
- `crabfork/plugin-sdk/channel-setup` covers the optional-install setup
...
- use the broader `crabfork/plugin-sdk/setup` seam only when you also need the
...
- `crabfork/plugin-sdk/account-core`,
  `crabfork/plugin-sdk/account-id`,
  `crabfork/plugin-sdk/account-resolution`, and
  `crabfork/plugin-sdk/account-helpers` for multi-account config and
  default-account fallback
- `crabfork/plugin-sdk/inbound-envelope` and
  `crabfork/plugin-sdk/inbound-reply-dispatch` for inbound route/envelope and
  record-and-dispatch wiring
- `crabfork/plugin-sdk/messaging-targets` for target parsing/matching
- `crabfork/plugin-sdk/outbound-media` and
  `crabfork/plugin-sdk/outbound-runtime` for media loading plus outbound
  identity/send delegates
- `crabfork/plugin-sdk/thread-bindings-runtime` for thread-binding lifecycle
  and adapter registration
- `crabfork/plugin-sdk/agent-media-payload` only when a legacy agent/media
  payload field layout is still required
- `crabfork/plugin-sdk/telegram-command-config` for Telegram custom-command
...
Use `crabfork/plugin-sdk/channel-inbound` for the shared layer.
...
`crabfork/plugin-sdk/channel-inbound` as compatibility exports only. New code
...
<Steps>
  ...
  <Step title="Package and manifest">
    ...
    see [Plugin Setup and Config](/plugins/sdk-setup#crabfork-channel):

    <CodeGroup>
    ```json package.json
    {
      "name": "@myorg/crabfork-acme-chat",
      "version": "1.0.0",
      "type": "module",
      "crabfork": {
        "extensions": ["./index.ts"],
        ...
        "channel": {
          "id": "acme-chat",
          "label": "Acme Chat",
          "blurb": "Connect Crabfork to Acme Chat."
        }
      }
    }
    ```
  ...
  <Step title="Build the channel plugin object">
    ...
    import type { CrabforkConfig } from "crabfork/plugin-sdk/channel-core";
  ...
  <Step title="Wire the entry point">
    ...
    ```typescript index.ts
    import { defineChannelPluginEntry } from "crabfork/plugin-sdk/channel-core";
    ...
    Put channel-owned CLI descriptors in `registerCliMetadata(...)` so Crabfork
  ...
  <Step title="Add a setup entry">
    ...
    Crabfork loads this instead of the full entry when the channel is disabled
  ...
  <Step title="Handle inbound messages">
    ...
    Crabfork. The typical pattern is a webhook that verifies the request and
    ...
    registerFull(api) {
      api.registerHttpRoute({
        ...
        handler: async (req, res) => {
          ...
          // Your inbound handler dispatches the message to Crabfork.
...
├── package.json              # crabfork.channel metadata
├── crabfork.plugin.json      # Manifest with config schema
