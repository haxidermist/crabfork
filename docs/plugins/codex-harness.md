---
title: "Codex Harness"
summary: "Run Crabfork embedded agent turns through the bundled Codex app-server harness"
read_when:
...
The bundled `codex` plugin lets Crabfork run embedded agent turns through the
...
Crabfork still owns chat channels, session files, model selection, tools,
...
force `embeddedHarness.runtime: "codex"` or `CRABFORK_AGENT_RUNTIME=codex`.
...
Crabfork has separate routes for OpenAI and Codex-shaped access:

| Model ref              | Runtime path                                 | Use when                                                                |
| ---------------------- | -------------------------------------------- | ----------------------------------------------------------------------- |
| `openai/gpt-5.4`       | OpenAI provider through Crabfork/PI plumbing | You want direct OpenAI Platform API access with `OPENAI_API_KEY`.       |
...
- Crabfork with the bundled `codex` plugin available.
...
Crabfork on the protocol surface it has been tested against.
...
With fallback disabled, Crabfork fails early if the Codex plugin is disabled,
...
Crabfork session and the Codex harness creates or resumes its sidecar app-server
thread as needed. `/reset` clears the Crabfork session binding for that thread.
...
- `CRABFORK_CODEX_APP_SERVER_BIN`
- `CRABFORK_CODEX_APP_SERVER_ARGS`
- `CRABFORK_CODEX_APP_SERVER_APPROVAL_POLICY`
- `CRABFORK_CODEX_APP_SERVER_SANDBOX`
- `CRABFORK_CODEX_APP_SERVER_GUARDIAN=1`
...
Model switching stays Crabfork-controlled. When an Crabfork session is attached
...
generic and works on any channel that supports Crabfork text commands.
...
- `/codex resume <thread-id>` attaches the current Crabfork session to an existing Codex thread.
...
normal turns. On the next message, Crabfork resumes that Codex thread, passes the
currently selected Crabfork `codex/*` model into app-server, and keeps extended
history enabled.
...
Crabfork still builds the tool list and receives dynamic tool results from the
harness. Text, images, video, music, TTS, approvals, and messaging-tool output
continue through the normal Crabfork delivery path.

When the selected model uses the Codex harness, native thread compaction is
delegated to Codex app-server. Crabfork keeps a transcript mirror for channel
...
**Crabfork falls back to PI:** set `embeddedHarness.fallback: "none"` or
`CRABFORK_AGENT_HARNESS_FALLBACK=none` while testing.
