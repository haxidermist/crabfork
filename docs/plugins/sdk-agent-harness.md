...
An **agent harness** is the low level executor for one prepared Crabfork agent
...
runtime and the normal Crabfork provider transport is the wrong abstraction.
...
- a model runtime that needs its own resume id in addition to the Crabfork
  session transcript
...
Before a harness is selected, Crabfork has already resolved:

- provider and model
- runtime auth state
- thinking level and context budget
- the Crabfork transcript/session file
...
**Import:** `crabfork/plugin-sdk/agent-harness`

```typescript
import type { AgentHarness } from "crabfork/plugin-sdk/agent-harness";
...
Crabfork chooses a harness after provider/model resolution:

1. `CRABFORK_AGENT_RUNTIME=<id>` forces a registered harness with that id.
2. `CRABFORK_AGENT_RUNTIME=pi` forces the built-in PI harness.
3. `CRABFORK_AGENT_RUNTIME=auto` asks registered harnesses if they support the
   resolved provider/model.
4. If no registered harness matches, Crabfork uses PI unless PI fallback is
   disabled.

Forced plugin harness failures surface as run failures. In `auto` mode,
Crabfork may fall back to PI when the selected plugin harness fails before a
turn has produced side effects. Set `CRABFORK_AGENT_HARNESS_FALLBACK=none` or
...
Crabfork. The harness then claims that provider in `supports(...)`.
...
- app-server request: Crabfork sends the bare model id to Codex and lets the
  harness talk to the native app-server protocol

The Codex plugin is additive. Plain `openai/gpt-*` refs remain OpenAI provider
refs and continue to use the normal Crabfork provider path. Select `codex/gpt-*`
...
Crabfork requires Codex app-server `0.118.0` or newer. The Codex plugin checks
the app-server initialize handshake and blocks older or unversioned servers so
Crabfork only runs against the protocol surface it has been tested with.

### Native Codex harness mode

The bundled `codex` harness is the native Codex mode for embedded Crabfork
agent turns. Enable the bundled `codex` plugin first, and include `codex` in
`plugins.allow` if your config uses a restrictive allowlist. It is different
from `openai-codex/*`:

- `openai-codex/*` uses ChatGPT/Codex OAuth through the normal Crabfork provider
  path.
- `codex/*` uses the bundled Codex provider and routes the turn through Codex
  app-server.

When this mode runs, Codex owns the native thread id, resume behavior,
compaction, and app-server execution. Crabfork still owns the chat channel,
...
By default, Crabfork runs embedded agents with `agents.defaults.embeddedHarness`
...
plugin harness fails before producing output, Crabfork falls back to PI.
...
an explicit `runtime: "pi"` or `CRABFORK_AGENT_RUNTIME=pi`.
...
want Crabfork to silently fall back to PI, keep `runtime: "auto"` and disable
the fallback:
...
`CRABFORK_AGENT_RUNTIME` still overrides the configured runtime. Use
`CRABFORK_AGENT_HARNESS_FALLBACK=none` to disable PI fallback from the
environment.

```bash
CRABFORK_AGENT_RUNTIME=codex \
CRABFORK_AGENT_HARNESS_FALLBACK=none \
...
Keep that binding explicitly associated with the Crabfork session, and keep
mirroring user-visible assistant/tool output into the Crabfork transcript.

The Crabfork transcript remains the compatibility layer for:
...
If your harness stores a sidecar binding, implement `reset(...)` so Crabfork can
clear it when the owning Crabfork session is reset.

## Tool and media results

Core constructs the Crabfork tool list and passes it into the prepared attempt.
