...
read_when:
  - You want to place an outbound voice call from Crabfork
...
Voice calls for Crabfork via a plugin. Supports outbound notifications and
...
- Use `crabfork voicecall ...` or the `voice_call` tool
...
- Runtime fallback still accepts those old voice-call keys for now, but the rewrite path is `crabfork doctor --fix` and the compat shim is temporary.
...
Legacy keys are still auto-migrated by `crabfork doctor --fix`:
...
crabfork voicecall start --to "+15555550123"   # alias for call
...
crabfork voicecall latency                     # summarize turn latency from logs
