...
`runEmbeddedAgent(...)` is the neutral helper for starting a normal Crabfork
...
Bind a Task Flow runtime to an existing Crabfork session key or trusted tool
...
trusted Crabfork session key from your own binding layer. Do not bind from raw
user input.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: { query: "Crabfork plugin SDK", count: 5 },
});
```
