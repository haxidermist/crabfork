...
read_when:
  ...
  - You are defining plugin config schemas or package.json crabfork metadata
---

# Plugin Setup and Config

Reference for plugin packaging (`package.json` metadata), manifests
(`crabfork.plugin.json`), setup entries, and config schemas.
...
Your `package.json` needs an `crabfork` field that tells the plugin system what
...
```json
{
  "name": "@myorg/crabfork-my-channel",
  "version": "1.0.0",
  "type": "module",
  "crabfork": {
    "extensions": ["./index.ts"],
...
```json crabfork-clawhub-package.json
{
...
`crabfork.channel` is cheap package metadata for channel discovery and setup
...
`crabfork.install` is package metadata, not manifest metadata.
...
| `minHostVersion`             | `string`             | Minimum supported Crabfork version in the form `>=x.y.z`.                        |
...
still fails closed and tells the operator to run `crabfork doctor --fix`.
...
When enabled, Crabfork loads only `setupEntry` during the pre-listen startup
...
Every native plugin must ship an `crabfork.plugin.json` in the package root.
Crabfork uses this to validate config without executing plugin code.

```json
{
  "id": "my-plugin",
  "name": "My Plugin",
  "description": "Adds My Plugin capabilities to Crabfork",
  "configSchema": {
    "type": "object",
...
Crabfork loads when it only needs setup surfaces (onboarding, config repair,
...
**When Crabfork uses `setupEntry` instead of the full entry:**
...
Use `buildChannelConfigSchema` from `crabfork/plugin-sdk/core` to convert a
Zod schema into the `ChannelConfigSchema` wrapper that Crabfork validates:

```typescript
import { z } from "zod";
import { buildChannelConfigSchema } from "crabfork/plugin-sdk/core";
...
Channel plugins can provide interactive setup wizards for `crabfork onboard`.
The wizard is a `ChannelSetupWizard` object on the `ChannelPlugin`:

```typescript
import type { ChannelSetupWizard } from "crabfork/plugin-sdk/channel-setup";
...
helpers from `crabfork/plugin-sdk/setup`: `createPromptParsedAllowFromForAccount(...)`,
...
`crabfork/plugin-sdk/setup` instead of hand-rolling the same `status` object in
each plugin.

For optional setup surfaces that should only appear in certain contexts, use
`createOptionalChannelSetupSurface` from `crabfork/plugin-sdk/channel-setup`:

```typescript
import { createOptionalChannelSetupSurface } from "crabfork/plugin-sdk/channel-setup";

const setupSurface = createOptionalChannelSetupSurface({
  channel: "my-channel",
  label: "My Channel",
  npmSpec: "@myorg/crabfork-my-channel",
  docsPath: "/channels/my-channel",
});
...
```bash
crabfork plugins install @myorg/crabfork-my-plugin
```

Crabfork tries ClawHub first and falls back to npm automatically. You can also
force ClawHub explicitly:

```bash
crabfork plugins install clawhub:@myorg/crabfork-my-plugin   # ClawHub only
```
...
<Info>
  For npm-sourced installs, `crabfork plugins install` runs
