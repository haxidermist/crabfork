...
read_when:
  - Building or debugging native Crabfork plugins
...
This page covers the internal architecture of the Crabfork plugin system.

## Public capability model

Capabilities are the public **native plugin** model inside Crabfork. Every
native Crabfork plugin registers against one or more capability types:
...
Crabfork classifies every loaded plugin into a shape based on its actual
...
Use `crabfork plugins inspect <id>` to see a plugin's shape and capability
...
When you run `crabfork doctor` or `crabfork plugins inspect <id>`, you may see
one of these labels:
...
signals also appear in `crabfork status --all` and `crabfork plugins doctor`.

## Architecture overview

Crabfork's plugin system has four layers:

1. **Manifest + discovery**
   Crabfork finds candidate plugins from configured paths, workspace roots,
   global extension roots, and bundled extensions. Discovery reads native
   `crabfork.plugin.json` manifests plus supported bundle manifests first.
2. **Enablement + validation**
   Core decides whether a discovered plugin is enabled, disabled, blocked, or
   selected for an exclusive slot such as memory.
3. **Runtime loading**
   Native Crabfork plugins are loaded in-process via jiti and register
...
4. **Surface consumption**
   The rest of Crabfork reads the registry to expose tools, channels, provider
...
That keeps plugin-owned CLI code inside the plugin while still letting Crabfork
...
That split lets Crabfork validate config, explain missing/disabled plugins, and
...
normal chat actions. Crabfork keeps one shared `message` tool in core, and
...
Crabfork treats a native plugin as the ownership boundary for a **company** or a
**feature**, not as a grab bag of unrelated integrations.

That means:

- a company plugin should usually own all of that company's Crabfork-facing
  surfaces
...
So if Crabfork adds a new domain such as video, the first question is not
...
A company plugin should feel cohesive from the outside. If Crabfork has shared
...
```ts
import type { CrabforkPluginDefinition } from "crabfork/plugin-sdk/plugin-entry";
import {
  describeImageWithModel,
  transcribeOpenAiCompatibleAudio,
} from "crabfork/plugin-sdk/media-understanding";

const plugin: CrabforkPluginDefinition = {
  id: "exampleai",
  name: "ExampleAI",
  register(api) {
...
Crabfork already treats image/audio/video understanding as one shared
...
`CrabforkPluginApi`. That contract defines the supported registration points and
...
2. **contract tests**
   ...
   Crabfork can assert ownership explicitly. Today this is used for model
   providers, speech providers, web search providers, and bundled registration
   ownership.

The practical effect is that Crabfork knows, up front, which plugin owns which
...
- ad hoc runtime objects that are not part of `CrabforkPluginApi` or
  `api.runtime`
...
Native Crabfork plugins run **in-process** with the Gateway. They are not
...
- a malicious native plugin is equivalent to arbitrary code execution inside
  the Crabfork process

Compatible bundles are safer by default because Crabfork currently treats them
...
name: `@crabfork/<id>` by default, or an approved typed suffix such as
...
Crabfork exports capabilities, not implementation convenience.
...
At startup, Crabfork does roughly this:
...
The manifest is the control-plane source of truth. Crabfork uses it to:
...
Crabfork keeps short in-process caches for:
...
- Set `CRABFORK_DISABLE_PLUGIN_DISCOVERY_CACHE=1` or
  `CRABFORK_DISABLE_PLUGIN_MANIFEST_CACHE=1` to disable these caches.
- Tune cache windows with `CRABFORK_PLUGIN_DISCOVERY_CACHE_MS` and
  `CRABFORK_PLUGIN_MANIFEST_CACHE_MS`.
...
Crabfork still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, Crabfork calls hooks in this rough order.
...
| --  | _(built-in model lookup)_         | Crabfork tries the normal registry/catalog path first                                                          | _(not a plugin hook)_                                                                                                                       |
...
that still runs on Crabfork's normal inference loop.
...
- OpenRouter uses `catalog` plus `resolveDynamicModel` and
  ...
  model ids before Crabfork's static catalog updates; it also uses
...
- The preferred ownership model is company-oriented: one vendor plugin can own
  text, speech, image, and future media providers as Crabfork adds those
...
- Crabfork only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "Crabfork plugin runtime helpers",
    count: 5,
  },
});
```
...
- `auth: "gateway"` routes run inside a Gateway request runtime scope, but that scope is intentionally conservative:
  - shared-secret bearer auth (`gateway.auth.mode = "token"` / `"password"`) keeps plugin-route runtime scopes pinned to `operator.write`, even if the caller sends `x-crabfork-scopes`
  - trusted identity-bearing HTTP modes (for example `trusted-proxy` or `gateway.auth.mode = "none"` on a private ingress) honor `x-crabfork-scopes` only when the header is explicitly present
  - if `x-crabfork-scopes` is absent on those identity-bearing plugin-route requests, runtime scope falls back to `operator.write`
- Practical rule: do not assume a gateway-auth plugin route is an implicit admin surface. If your route needs admin-only behavior, require an identity-bearing auth mode and document the explicit `x-crabfork-scopes` header contract.

## Plugin SDK import paths

Use SDK subpaths instead of the monolithic `crabfork/plugin-sdk` import when
authoring plugins:

- `crabfork/plugin-sdk/plugin-entry` for plugin registration primitives.
- `crabfork/plugin-sdk/core` for the generic shared plugin-facing contract.
- `crabfork/plugin-sdk/config-schema` for the root `crabfork.json` Zod schema
  export (`CrabforkSchema`).
- Stable channel primitives such as `crabfork/plugin-sdk/channel-setup`,
  `crabfork/plugin-sdk/setup-runtime`,
  `crabfork/plugin-sdk/setup-adapter-runtime`,
  `crabfork/plugin-sdk/setup-tools`,
  `crabfork/plugin-sdk/channel-pairing`,
  `crabfork/plugin-sdk/channel-contract`,
  `crabfork/plugin-sdk/channel-feedback`,
  `crabfork/plugin-sdk/channel-inbound`,
  `crabfork/plugin-sdk/channel-lifecycle`,
  `crabfork/plugin-sdk/channel-reply-pipeline`,
  `crabfork/plugin-sdk/command-auth`,
  `crabfork/plugin-sdk/secret-input`, and
  `crabfork/plugin-sdk/webhook-ingress` for shared setup/auth/reply/webhook
  ...
  `CONFIG_DIR`).
- Domain subpaths such as `crabfork/plugin-sdk/channel-config-helpers`,
  `crabfork/plugin-sdk/allow-from`,
  `crabfork/plugin-sdk/channel-config-schema`,
  `crabfork/plugin-sdk/telegram-command-config`,
  `crabfork/plugin-sdk/channel-policy`,
  `crabfork/plugin-sdk/approval-gateway-runtime`,
  `crabfork/plugin-sdk/approval-handler-adapter-runtime`,
  `crabfork/plugin-sdk/approval-handler-runtime`,
  `crabfork/plugin-sdk/approval-runtime`,
  `crabfork/plugin-sdk/config-runtime`,
  `crabfork/plugin-sdk/infra-runtime`,
  `crabfork/plugin-sdk/agent-runtime`,
  `crabfork/plugin-sdk/lazy-runtime`,
  `crabfork/plugin-sdk/reply-history`,
  `crabfork/plugin-sdk/routing`,
  `crabfork/plugin-sdk/status-helpers`,
  `crabfork/plugin-sdk/text-runtime`,
  `crabfork/plugin-sdk/runtime-store`, and
  `crabfork/plugin-sdk/directory-runtime` for shared runtime/config helpers.
...
- `crabfork/plugin-sdk/channel-runtime` is deprecated and remains only as a
...
- Bundled extension internals remain private. External plugins should use only
  `crabfork/plugin-sdk/*` subpaths. Crabfork core/test code may use the repo
...
- Facade-loaded public entry points prefer the active runtime config snapshot
  ...
  Crabfork is not yet serving a runtime snapshot.
...
- Avoid the root `crabfork/plugin-sdk` barrel for new code.
- Prefer the narrow stable primitives first. The newer setup/pairing/reply/
  ...
  Target parsing/matching belongs on `crabfork/plugin-sdk/channel-targets`.
  Message action gates and reaction message-id helpers belong on
  `crabfork/plugin-sdk/channel-actions`.
- Bundled extension-specific helper barrels are not stable by default. If a
  ...
  `crabfork/plugin-sdk/<extension>`.
- New shared helper seams should be generic, not channel-branded. Shared target
  parsing belongs on `crabfork/plugin-sdk/channel-targets`; channel-specific
...
`crabfork/plugin-sdk/directory-runtime`.
...
`catalog.run(...)` returns the same shape Crabfork writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to Crabfork's
...
- if both `catalog` and `discovery` are registered, Crabfork uses `catalog`
...
- Read-only command paths such as `crabfork status`, `crabfork status --all`,
  `crabfork channels status`, `crabfork channels resolve`, and doctor/config
...
A plugin directory may include a `package.json` with `crabfork.extensions`:
...
Security guardrail: every `crabfork.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `crabfork plugins install` installs plugin dependencies with
...
Optional: `crabfork.setupEntry` can point at a lightweight setup-only module.
When Crabfork needs setup surfaces for a disabled channel plugin, or
...
Optional: `crabfork.startup.deferConfiguredChannelFullLoadUntilAfterListen`
...
this flag. Keep the plugin on the default behavior and let Crabfork load the
...
Channel plugins can advertise setup/discovery metadata via `crabfork.channel` and
install hints via `crabfork.install`. This keeps the core catalog data-free.

Example:

```json
{
  "name": "@crabfork/nextcloud-talk",
  "crabfork": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
    ...
    "install": {
      "npmSpec": "@crabfork/nextcloud-talk",
...
Useful `crabfork.channel` fields beyond the minimal example:
...
Crabfork can also merge **external channel catalogs** (for example, an MPM
registry export). Drop a JSON file at one of:

- `~/.crabfork/mpm/plugins.json`
- `~/.crabfork/mpm/catalog.json`
- `~/.crabfork/plugins/catalog.json`

Or point `CRABFORK_PLUGIN_CATALOG_PATHS` (or `CRABFORK_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "crabfork": { "channel": {...}, "install": {...} } } ] }`. The parser also accepts `"packages"` or `"plugins"` as legacy aliases for the `"entries"` key.
...
```ts
import { buildMemorySystemPromptAddition } from "crabfork/plugin-sdk/core";
...
2. add typed plugin registration/runtime surfaces
   Extend `CrabforkPluginApi` and/or `api.runtime` with the smallest useful
...
This is how Crabfork stays opinionated without becoming hardcoded to one
