...
read_when:
  ...
  - You want a reference for all registration methods on CrabforkPluginApi
...
prefer `crabfork/plugin-sdk/channel-core`; keep `crabfork/plugin-sdk/core` for
...
`crabfork/plugin-sdk/slack`, `crabfork/plugin-sdk/discord`,
`crabfork/plugin-sdk/signal`, `crabfork/plugin-sdk/whatsapp`, or
...
| `plugin-sdk/config-schema`  | `CrabforkSchema`                                                                                                                       |
...
<AccordionGroup>
  <Accordion title="Channel subpaths">
    ...
    | `plugin-sdk/config-schema` | Root `crabfork.json` Zod schema export (`CrabforkSchema`) |
  ...
  <Accordion title="Runtime and storage subpaths">
    ...
    | `plugin-sdk/agent-harness` | Experimental trusted-plugin surface for low-level agent harnesses: harness types, active-run steer/abort helpers, Crabfork tool bridge helpers, and attempt result utilities |
...
The `register(api)` callback receives an `CrabforkPluginApi` object with these
methods:
...
- User config still wins. Crabfork merges `agents.defaults.cliBackends.<id>` over the
...
- `registerMemoryCapability` may also expose `publicArtifacts.listArtifacts(...)`
  ...
  `crabfork/plugin-sdk/memory-host-core` instead of reaching into a specific
...
| `api.config`             | `CrabforkConfig`          | Current config snapshot (active in-memory runtime snapshot when available)                  |
...
<Warning>
  Never import your own plugin through `crabfork/plugin-sdk/<your-plugin>`
...
active runtime config snapshot when Crabfork is already running. If no runtime
...
- `@crabfork/openrouter-provider`: `api.ts` exports the provider builder plus
  onboarding/config helpers

<Warning>
  Extension production code should also avoid `crabfork/plugin-sdk/<other-plugin>`
  imports. If a helper is truly shared, promote it to a neutral SDK subpath
  such as `crabfork/plugin-sdk/speech`, `.../provider-model-shared`, or another
