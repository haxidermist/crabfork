...
summary: "Step-by-step guide to building a model provider plugin for Crabfork"
read_when:
  - You are building a new model provider plugin
  - You want to add an OpenAI-compatible proxy or custom LLM to Crabfork
...
(LLM) to Crabfork. By the end you will have a provider with a model catalog,
...
<Tip>
  Provider plugins add models to Crabfork's normal inference loop. If the model
...
<Steps>
  ...
  <Step title="Package and manifest">
    ...
    The manifest declares `providerAuthEnvVars` so Crabfork can detect
    ...
    is optional and lets Crabfork auto-load your provider plugin from shorthand
    model ids like `acme-large` before runtime hooks exist. If you publish the
    provider on ClawHub, those `crabfork.compat` and `crabfork.build` fields
  ...
  <Step title="Register the provider">
    ...
    import { createProviderApiKeyAuthMethod } from "crabfork/plugin-sdk/provider-auth";
    ...
    `crabfork onboard --acme-ai-api-key <key>` and select
    `acme-ai/acme-large` as their model.

    If the upstream provider uses different control tokens than Crabfork, add a
    ...
    Crabfork parses its own control markers or channel delivery.
    ...
    `crabfork/plugin-sdk/provider-onboard`. The narrowest helpers are
    ...
    `crabfork/plugin-sdk/provider-catalog-shared` instead of hardcoding
  ...
  <Step title="Add runtime hooks (as needed)">
    ...
    ```typescript
    import { buildProviderReplayFamilyHooks } from "crabfork/plugin-sdk/provider-model-shared";
    import { buildProviderStreamFamilyHooks } from "crabfork/plugin-sdk/provider-stream";
    import { buildProviderToolCompatFamilyHooks } from "crabfork/plugin-sdk/provider-tools";
    ...
    `crabfork/plugin-sdk/provider-model-shared` also exports the replay-family
    ...
    `crabfork/plugin-sdk/provider-stream` exposes both the family builder and
    ...
    example: `@crabfork/anthropic-provider` exports
    ...
    `crabfork/plugin-sdk/provider-tools` currently exposes one shared
    ...
    - `@crabfork/openai-provider`: `api.ts` exports provider builders,
      default-model helpers, and realtime provider builders
    - `@crabfork/openrouter-provider`: `api.ts` exports the provider builder
    ...
    <Accordion title="All available provider hooks">
      Crabfork calls hooks in this order. Most providers only use 2-3:
  ...
  <Step title="Add extra capabilities (optional)">
    ...
    Crabfork classifies this as a **hybrid-capability** plugin. This is the
...
├── package.json              # crabfork.providers metadata
├── crabfork.plugin.json      # Manifest with provider auth metadata
