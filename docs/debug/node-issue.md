...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Crabfork
...
Running Crabfork via Node with `tsx` fails at startup with:

```
[crabfork] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `crabfork status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch crabfork.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node crabfork.mjs status` works on Node 25.
