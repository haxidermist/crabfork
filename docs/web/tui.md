...
```bash
crabfork tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `crabfork tui --deliver`
...
- Local shell commands receive `CRABFORK_SHELL=tui-local` in their environment.
...
- If your terminal uses a light background and auto-detection is wrong, set `CRABFORK_THEME=light` before launching `crabfork tui`.
- To force the original dark palette instead, set `CRABFORK_THEME=dark`.
...
- Check the Gateway logs: `crabfork logs --follow`.
- Confirm the agent can run: `crabfork status` and `crabfork models status`.
- If you expect messages in a chat channel, enable delivery (`/deliver on` or `--deliver`).

## Connection troubleshooting

- `disconnected`: ensure the Gateway is running and your `--url/--token/--password` are correct.
- No agents in picker: check `crabfork agents list` and your routing config.
