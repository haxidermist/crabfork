...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/crabfork`)
...
If the page fails to load, start the Gateway first: `crabfork gateway`.
...
created. Re-run `crabfork devices list` before approval.

Once approved, the device is remembered and won't require re-approval unless
you revoke it with `crabfork devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.crabfork/crabfork.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop crabfork`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Crabfork
...
```bash
crabfork gateway --bind tailnet --token "$(openssl rand -hex 32)"
```
...
Crabfork **blocks** Control UI connections without device identity.
...
```bash
CRABFORK_CONTROL_UI_BASE_PATH=/crabfork/ pnpm ui:build
```
