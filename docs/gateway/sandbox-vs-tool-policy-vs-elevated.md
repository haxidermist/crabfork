...
Crabfork has three related (but different) controls:
...
Use the inspector to see what Crabfork is _actually_ doing:
...
- Crabfork validates bind sources twice: first on the normalized source path, then again after resolving through the deepest existing ancestor. Symlink-parent escapes do not bypass blocked-path or allowed-root checks.
...
- `group:crabfork`: all built-in Crabfork tools (excludes provider plugins)
