...
Crabfork can run **local AI CLIs** as a **text-only fallback** when API providers are down,
rate-limited, or temporarily misbehaving. This is intentionally conservative:

- **Crabfork tools are not injected directly**, but backends with `bundleMcp: true`
...
```bash
crabfork agent --message "hi" --model codex-cli/gpt-5.4
```
...
gateway host, Crabfork now auto-loads the owning bundled plugin when your config
...
- If the primary provider fails (auth, rate limits, timeouts), Crabfork will
...
2. **Builds a system prompt** using the same Crabfork prompt + workspace context.
...
told us Crabfork-style Claude CLI usage is allowed again, so Crabfork treats
`claude -p` usage as sanctioned for this integration unless Anthropic publishes
a new policy.
</Note>

The bundled OpenAI `codex-cli` backend passes Crabfork's system prompt through
Codex's `model_instructions_file` config override (`-c
model_instructions_file="..."`). Codex does not expose a Claude-style
`--append-system-prompt` flag, so Crabfork writes the assembled prompt to a
temporary file for each fresh Codex CLI session.

The bundled Anthropic `claude-cli` backend receives the Crabfork skills snapshot
two ways: the compact Crabfork skills catalog in the appended system prompt, and
...
resolver sees the same filtered set that Crabfork would otherwise advertise in
the prompt. Skill env/API key overrides are still applied by Crabfork to the
...
- Crabfork drops stored CLI session reuse when the backend auth state changes, including relogin, token rotation, or a changed auth profile credential.
...
Crabfork will write base64 images to temp files. If `imageArg` is set, those
paths are passed as CLI args. If `imageArg` is missing, Crabfork appends the
...
- For Gemini CLI JSON output, Crabfork reads reply text from `response` and
...
- `stats.cached` is normalized into Crabfork `cacheRead`.
- If `stats.input` is missing, Crabfork derives input tokens from
...
rewrites streamed assistant deltas and parsed final text before Crabfork handles
...
CLI backends do **not** receive Crabfork tool calls directly, but a backend can
...
When bundle MCP is enabled, Crabfork:

- spawns a loopback HTTP MCP server that exposes gateway tools to the CLI process
- authenticates the bridge with a per-session token (`CRABFORK_MCP_TOKEN`)
...
If no MCP servers are enabled, Crabfork still injects a strict config when a
backend opts into bundle MCP so background runs stay isolated.

## Limitations

- **No direct Crabfork tool calls.** Crabfork does not inject tool calls into
...
- **Codex CLI sessions** resume via text output (no JSONL), which is less
  structured than the initial `--json` run. Crabfork sessions still work
  normally.
