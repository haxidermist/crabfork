---
summary: "How Crabfork sandboxing works: modes, scopes, workspace access, and images"
title: Sandboxing
...
Crabfork can run **tools inside sandbox backends** to reduce blast radius.
...
- Optional sandboxed browser (`agents.defaults.sandbox.browser`).
  ...
  - By default, sandbox browser containers use a dedicated Docker network (`crabfork-sandbox-browser`) instead of the global `bridge` network.
  ...
  - noVNC observer access is password-protected by default; Crabfork emits a short-lived token URL that serves a local bootstrap page and opens noVNC with password in URL fragment (not query/header logs).
...
If you deploy the Crabfork Gateway itself as a Docker container, it orchestrates sibling sandbox containers using the host's Docker socket (DooD). This introduces a specific path mapping constraint:

- **Config Requires Host Paths**: The `crabfork.json` `workspace` configuration MUST contain the **Host's absolute path** (e.g. `/home/user/.crabfork/workspaces`), not the internal Gateway container path. When Crabfork asks the Docker daemon to spawn a sandbox, the daemon evaluates paths relative to the Host OS namespace, not the Gateway namespace.
- **FS Bridge Parity (Identical Volume Map)**: The Crabfork Gateway native process also writes heartbeat and bridge files to the `workspace` directory. Because the Gateway evaluates the exact same string (the host path) from within its own containerized environment, the Gateway deployment MUST include an identical volume map linking the host namespace natively (`-v /home/user/.crabfork:/home/user/.crabfork`).

If you map paths internally without absolute host parity, Crabfork natively throws an `EACCES` permission error attempting to write its heartbeat inside the container environment because the fully qualified path string doesn't exist natively.

### SSH backend

Use `backend: "ssh"` when you want Crabfork to sandbox `exec`, file tools, and media reads on
...
- Crabfork creates a per-scope remote root under `sandbox.ssh.workspaceRoot`.
- On first use after create or recreate, Crabfork seeds that remote workspace from the local workspace once.
...
- Crabfork does not sync remote changes back to the local workspace automatically.

Authentication material:

- `identityFile`, `certificateFile`, `knownHostsFile`: use existing local files and pass them through OpenSSH config.
- `identityData`, `certificateData`, `knownHostsData`: use inline strings or SecretRefs. Crabfork resolves them through the normal secrets runtime snapshot, writes them to temp files with `0600`, and deletes them when the SSH session ends.
...
- Host-local edits made outside Crabfork after the seed step are not visible remotely until you recreate the sandbox.
- `crabfork sandbox recreate` deletes the per-scope remote root and seeds again from local on next use.
- Browser sandboxing is not supported on the SSH backend.
- `sandbox.docker.*` settings do not apply to the SSH backend.

### OpenShell backend

Use `backend: "openshell"` when you want Crabfork to sandbox tools in an
...
- `mirror` (default): local workspace stays canonical. Crabfork syncs local files into OpenShell before exec and syncs the remote workspace back after exec.
- `remote`: OpenShell workspace is canonical after the sandbox is created. Crabfork seeds the remote workspace once from the local workspace, then file tools and exec run directly against the remote sandbox without syncing changes back.

Remote transport details:

- Crabfork asks OpenShell for sandbox-specific SSH config via `openshell sandbox ssh-config <name>`.
...
- Before `exec`, Crabfork syncs the local workspace into the OpenShell sandbox.
- After `exec`, Crabfork syncs the remote workspace back to the local workspace.
- File tools still operate through the sandbox bridge, but the local workspace remains the source of truth between turns.

Use this when:

- you edit files locally outside Crabfork and want those changes to show up in the sandbox automatically
...
- When the sandbox is first created, Crabfork seeds the remote workspace from the local workspace once.
- After that, `exec`, `read`, `write`, `edit`, and `apply_patch` operate directly against the remote OpenShell workspace.
- Crabfork does **not** sync remote changes back into the local workspace after exec.
...
- If you edit files on the host outside Crabfork after the seed step, the remote sandbox will **not** see those changes automatically.
...
- `crabfork sandbox list` shows OpenShell runtimes as well as Docker runtimes
- `crabfork sandbox recreate` deletes the current runtime and lets Crabfork recreate it on next use
...
- `"none"` (default): tools see a sandbox workspace under `~/.crabfork/sandboxes`.
...
Crabfork mirrors eligible skills into the sandbox workspace (`.../skills`) so
...
- Crabfork blocks dangerous bind sources (for example: `docker.sock`, `/etc`, `/proc`, `/sys`, `/dev`, and parent mounts that would expose them).
- Crabfork also blocks common home-directory credential roots such as `~/.aws`, `~/.cargo`, `~/.config`, `~/.docker`, `~/.gnupg`, `~/.netrc`, `~/.npm`, and `~/.ssh`.
- Bind validation is not just string matching. Crabfork normalizes the source path, then resolves it again through the deepest existing ancestor before re-checking blocked paths and allowed roots.
...
Default Docker image: `crabfork-sandbox:bookworm-slim`
...
`crabfork-sandbox-common:bookworm-slim`.
...
- The three graphics hardening flags (`--disable-3d-apis`,
  ...
  when containers lack GPU support. Set `CRABFORK_BROWSER_DISABLE_GRAPHICS_FLAGS=0`
...
- `--disable-extensions` is enabled by default and can be disabled with
  `CRABFORK_BROWSER_DISABLE_EXTENSIONS=0` for extension-reliant flows.
- `--renderer-process-limit=2` is controlled by
  `CRABFORK_BROWSER_RENDERER_PROCESS_LIMIT=<N>`, where `0` keeps Chromium's default.
...
Set `CRABFORK_SANDBOX=1` (or `true`/`yes`/`on`) to enable that path. You can
override socket location with `CRABFORK_DOCKER_SOCKET`. Full setup and env
...
- Use `crabfork sandbox explain` to inspect effective sandbox mode, tool policy, and fix-it config keys.
