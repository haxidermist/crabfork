...
read_when:
  - Running Crabfork behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of Crabfork
...
- You run Crabfork behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. Crabfork checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. Crabfork extracts the user identity from the configured header
...
- For same-host loopback proxy setups, use token/password auth instead, or route through a non-loopback trusted proxy address that Crabfork can verify.
...
- Crabfork can stay on loopback HTTP behind the proxy.
...
If Crabfork itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://crabfork.example.com
    to: http://crabfork-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
crabfork.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy crabfork:18789 {
...
location / {
    ...
    proxy_pass http://crabfork:18789;
...
Crabfork rejects ambiguous configurations where both a `gateway.auth.token` (or `CRABFORK_GATEWAY_TOKEN`) and `trusted-proxy` mode are active at the same time. Mixed token configs can cause loopback requests to silently authenticate on the wrong auth path.
...
optionally declare operator scopes with `x-crabfork-scopes`.

Examples:

- `x-crabfork-scopes: operator.read`
- `x-crabfork-scopes: operator.read,operator.write`
- `x-crabfork-scopes: operator.admin,operator.write`

Behavior:

- When the header is present, Crabfork honors the declared scope set.
...
- Gateway-auth **plugin HTTP routes** are narrower by default: when `x-crabfork-scopes` is absent, their runtime scope falls back to `operator.write`.
...
- Send `x-crabfork-scopes` explicitly when you want a trusted-proxy request to
...
`crabfork security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
Crabfork rejected a loopback-source trusted-proxy request.
...
3. Update Crabfork config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `crabfork security audit` and review findings
