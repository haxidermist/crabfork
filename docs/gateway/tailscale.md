...
Crabfork can auto-configure Tailscale **Serve** (tailnet) or **Funnel** (public) for the
...
- `funnel`: Public HTTPS via `tailscale funnel`. Crabfork requires a shared password.
...
- `token` (default when `CRABFORK_GATEWAY_TOKEN` is set)
- `password` (shared secret via `CRABFORK_GATEWAY_PASSWORD` or config)
...
(`tailscale-user-login`) without supplying a token/password. Crabfork verifies
the identity by resolving the `x-forwarded-for` address via the local Tailscale
daemon (`tailscale whois`) and matching it to the header before accepting it.
Crabfork only treats a request as Serve when it arrives from loopback with
...
Prefer `CRABFORK_GATEWAY_PASSWORD` over committing a password to disk.

## CLI examples

```bash
crabfork gateway --tailscale serve
crabfork gateway --tailscale funnel --auth password
```
...
- Set `gateway.tailscale.resetOnExit` if you want Crabfork to undo `tailscale serve`
