---
title: OpenShell
summary: "Use OpenShell as a managed sandbox backend for Crabfork agents"
read_when:
...
OpenShell is a managed sandbox backend for Crabfork. Instead of running Docker
containers locally, Crabfork delegates sandbox lifecycle to the `openshell` CLI,
...
- Crabfork Gateway running on the host
...
2. Restart the Gateway. On the next agent turn, Crabfork creates an OpenShell
...
- You edit files locally outside Crabfork and want those changes visible in the
...
- When the sandbox is first created, Crabfork seeds the remote workspace from
...
- Crabfork does **not** sync remote changes back into the local workspace.
...
Important: if you edit files on the host outside Crabfork after the initial seed,
the remote sandbox does **not** see those changes. Use
`crabfork sandbox recreate` to re-seed.
...
| `from`                    | `string`                 | `"crabfork"`  | Sandbox source for first-time create                  |
...
1. Crabfork calls `openshell sandbox create` (with `--from`, `--gateway`,
   `--policy`, `--providers`, `--gpu` flags as configured).
2. Crabfork calls `openshell sandbox ssh-config <name>` to get SSH connection
...
- [Sandbox CLI](/cli/sandbox) -- `crabfork sandbox` commands
