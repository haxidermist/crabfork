...
read_when:
  - Setting up Crabfork for the first time
...
Crabfork reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.crabfork/crabfork.json`.

If the file is missing, Crabfork uses safe defaults. Common reasons to add a config:
...
<Tip>
**New to configuration?** Start with `crabfork onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    crabfork onboard       # full onboarding flow
    crabfork configure     # config wizard
    ```
  </Tab>
  ...
  <Tab title="Direct edit">
    Edit `~/.crabfork/crabfork.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<Warning>
Crabfork only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

Schema tooling notes:

- `crabfork config schema` prints the same JSON Schema family used by Control UI
  and config validation.
- Treat that schema output as the canonical machine-readable contract for
  `crabfork.json`; this overview and the configuration reference summarize it.
...
- Only diagnostic commands work (`crabfork doctor`, `crabfork logs`, `crabfork health`, `crabfork status`)
- Run `crabfork doctor` to see exact issues
- Run `crabfork doctor --fix` (or `--yes`) to apply repairs
...
<AccordionGroup>
  ...
  <Accordion title="Enable relay-backed push for official iOS builds">
    Relay-backed push is configured in `crabfork.json`.
    ...
    ```bash
    crabfork config set gateway.push.apns.relay.baseUrl https://relay.example.com
    ```
    ...
    - `CRABFORK_APNS_RELAY_BASE_URL` and `CRABFORK_APNS_RELAY_TIMEOUT_MS` still work as temporary env overrides.
    - `CRABFORK_APNS_RELAY_ALLOW_HTTP=true` remains a loopback-only development escape hatch; do not persist HTTP relay URLs in config.
  ...
  <Accordion title="Set up webhooks (hooks)">
    ...
    - Hook auth is header-only (`Authorization: Bearer ...` or `x-crabfork-token`); query-string tokens are rejected.
...
The Gateway watches `~/.crabfork/crabfork.json` and applies changes automatically — no manual restart needed for most settings.
...
<AccordionGroup>
  <Accordion title="config.apply (full replace)">
    ...
    <Warning>
    `config.apply` replaces the **entire config**. Use `config.patch` for partial updates, or `crabfork config set` for single keys.
    </Warning>
    ...
    ```bash
    crabfork gateway call config.get --params '{}'  # capture payload.hash
    crabfork gateway call config.apply --params '{
      "raw": "{ agents: { defaults: { workspace: \"~/.crabfork/workspace\" } } }",
      "baseHash": "<hash>",
  ...
  <Accordion title="config.patch (partial update)">
    ...
    ```bash
    crabfork gateway call config.patch --params '{
...
Crabfork reads env vars from the parent process plus:

- `.env` from the current working directory (if present)
- `~/.crabfork/.env` (global fallback)
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, Crabfork runs your login shell and imports only the missing keys:
...
Env var equivalent: `CRABFORK_LOAD_SHELL_ENV=1`
</Accordion>
...
```json5
{
  gateway: { auth: { token: "${CRABFORK_GATEWAY_TOKEN}" } },
