...
crabfork nodes rename --node <id|name|ip> --name "Living Room iPad"
```
...
Pairing state is stored under the Gateway state directory (default `~/.crabfork`):

- `~/.crabfork/nodes/paired.json`
- `~/.crabfork/nodes/pending.json`

If you override `CRABFORK_STATE_DIR`, the `nodes/` folder moves with it.
