...
Crabfork’s Gateway can serve an OpenResponses-compatible `POST /v1/responses` endpoint.
...
`crabfork agent`), so routing/permissions/config match your Gateway.
...
- for shared-secret auth modes (`token` and `password`), ignore narrower bearer-declared `x-crabfork-scopes` values and restore the normal full operator defaults
- for trusted identity-bearing HTTP modes (for example trusted proxy auth or `gateway.auth.mode="none"`), honor `x-crabfork-scopes` when present and otherwise fall back to the normal operator default scope set
- select agents with `model: "crabfork"`, `model: "crabfork/default"`, `model: "crabfork/<agentId>"`, or `x-crabfork-agent-id`
- use `x-crabfork-model` when you want to override the selected agent's backend model
- use `x-crabfork-session-key` for explicit session routing
- use `x-crabfork-message-channel` when you want a non-default synthetic ingress channel context
...
- `previous_response_id`: Crabfork reuses the earlier response session when the request stays within the same agent/user/requested-session scope.
...
Crabfork normalizes common OpenAI-style aliases before those counters reach
