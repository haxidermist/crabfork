...
- `/usage cost` in chats: local cost summary aggregated from Crabfork session logs.
- CLI: `crabfork status --usage` prints a full per-provider breakdown.
- CLI: `crabfork channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
...
- **MiniMax**: API key or MiniMax OAuth auth profile. Crabfork treats
  ...
  quota, so Crabfork inverts them before display; count-based fields win when
  present.
  ...
  - If the coding-plan endpoint returns `model_remains`, Crabfork prefers the
...
can supply plugin-specific usage auth logic; otherwise Crabfork falls back to
