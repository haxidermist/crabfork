...
Crabfork selects models in this order:
...
- `agents.defaults.models` is the allowlist/catalog of models Crabfork can use (plus aliases).
...
- If you omit the provider, Crabfork resolves the input in this order:
  1. alias match
  ...
  3. deprecated fallback to the configured default provider
     If that provider no longer exposes the configured default model, Crabfork
...
crabfork models set <provider/model>
crabfork models set-image <provider/model>

crabfork models aliases list
crabfork models aliases add <alias> <provider/model>
crabfork models aliases remove <alias>

crabfork models fallbacks list
crabfork models fallbacks add <provider/model>
crabfork models fallbacks remove <provider/model>
crabfork models fallbacks clear

crabfork models image-fallbacks list
crabfork models image-fallbacks add <provider/model>
crabfork models image-fallbacks remove <provider/model>
crabfork models image-fallbacks clear
```

`crabfork models` (no subcommand) is a shortcut for `models status`.
...
`crabfork models scan` inspects OpenRouter’s **free model catalog** and can
...
agent directory (default `~/.crabfork/agents/<agentId>/agent/models.json`). This file
...
Marker persistence is source-authoritative: Crabfork writes markers from the active source config snapshot (pre-resolution), not from resolved runtime secret values.
This applies whenever Crabfork regenerates `models.json`, including command-driven paths like `crabfork agent`.
