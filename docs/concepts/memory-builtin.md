...
Crabfork indexes `MEMORY.md` and `memory/*.md` into chunks (~400 tokens with
80-token overlap) and stores them in a per-agent SQLite database.

- **Index location:** `~/.crabfork/memory/<agentId>.sqlite`
...
- **Reindex on demand:** `crabfork memory index --force`
...
**Memory search disabled?** Check `crabfork memory status`. If no provider is
detected, set one explicitly or add an API key.

**Stale results?** Run `crabfork memory index --force` to rebuild. The watcher
may miss changes in rare edge cases.

**sqlite-vec not loading?** Crabfork falls back to in-process cosine similarity
