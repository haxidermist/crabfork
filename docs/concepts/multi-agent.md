...
- **Session store** (chat history + routing state) under `~/.crabfork/agents/<agentId>/sessions`.
...
`~/.crabfork/skills`, then filtered by the effective agent skill allowlist when
...
- State dir: `~/.crabfork` (or `CRABFORK_STATE_DIR`)
- Workspace: `~/.crabfork/workspace` (or `~/.crabfork/workspace-<agentId>`)
- Agent dir: `~/.crabfork/agents/<agentId>/agent` (or `agents.list[].agentDir`)
- Sessions: `~/.crabfork/agents/<agentId>/sessions`

### Single-agent mode (default)

If you do nothing, Crabfork runs a single agent:

- `agentId` defaults to **`main`**.
- Sessions are keyed as `agent:main:<mainKey>`.
- Workspace defaults to `~/.crabfork/workspace` (or `~/.crabfork/workspace-<profile>` when `CRABFORK_PROFILE` is set).
- State defaults to `~/.crabfork/agents/main/agent`.
...
```bash
crabfork agents add coding
crabfork agents add social
```

Each agent gets its own workspace with `SOUL.md`, `AGENTS.md`, and optional `USER.md`, plus a dedicated `agentDir` and session store under `~/.crabfork/agents/<agentId>`.
...
```bash
crabfork channels login --channel whatsapp --account work
```
...
```json5
{
  agents: {
    list: [
      { id: "alex", workspace: "~/.crabfork/workspace-alex" },
      { id: "mia", workspace: "~/.crabfork/workspace-mia" },
    ],
  },
  bindings: [
    {
...
- If you later add the same binding for the same agent with an explicit account id, Crabfork upgrades the existing channel-only binding to account-scoped instead of duplicating it.
...
`channels.<channel>.defaultAccount` (optional). When unset, Crabfork falls back
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.crabfork/workspace-main" },
      { id: "coding", workspace: "~/.crabfork/workspace-coding" },
    ],
  },
  bindings: [
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.crabfork/workspace-main" },
      { id: "alerts", workspace: "~/.crabfork/workspace-alerts" },
    ],
  },
  bindings: [
...
```bash
crabfork channels login --channel whatsapp --account personal
crabfork channels login --channel whatsapp --account biz
```

`~/.crabfork/crabfork.json` (JSON5):
...
```js
{
    ...
    list: [
      {
        ...
        default: true,
        name: "Home",
        workspace: "~/.crabfork/workspace-home",
        agentDir: "~/.crabfork/agents/home/agent",
      },
      {
        id: "work",
        name: "Work",
        workspace: "~/.crabfork/workspace-work",
        agentDir: "~/.crabfork/agents/work/agent",
      },
    ],
  },
  ...
  channels: {
    whatsapp: {
      accounts: {
        personal: {
          // Optional override. Default: ~/.crabfork/credentials/whatsapp/personal
          // authDir: "~/.crabfork/credentials/whatsapp/personal",
        },
        biz: {
          // Optional override. Default: ~/.crabfork/credentials/whatsapp/biz
    ...
    list: [
      {
        id: "chat",
        name: "Everyday",
        workspace: "~/.crabfork/workspace-chat",
        model: "anthropic/claude-sonnet-4-6",
      },
      {
        id: "opus",
        name: "Deep Work",
        workspace: "~/.crabfork/workspace-opus",
