...
The private QA stack is meant to exercise Crabfork in a more realistic,
...
```bash
pnpm crabfork qa docker-build-image
pnpm qa:lab:build
pnpm qa:lab:up:fast
pnpm qa:lab:watch
```
...
disposable server. It requires `CRABFORK_QA_TELEGRAM_GROUP_ID`,
`CRABFORK_QA_TELEGRAM_DRIVER_BOT_TOKEN`, and
`CRABFORK_QA_TELEGRAM_SUT_BOT_TOKEN`, plus two distinct bots in the same
...
```bash
pnpm crabfork qa suite --runner multipass --scenario channel-chat-baseline
```

This boots a fresh Multipass guest, installs dependencies, builds Crabfork
