...
summary: "How Crabfork remembers things across sessions"
read_when:
...
Crabfork remembers things by writing **plain Markdown files** in your agent's
...
These files live in the agent workspace (default `~/.crabfork/workspace`).
...
<Info>
Crabfork auto-detects your embedding provider from available API keys. If you
...
Before [compaction](/concepts/compaction) summarizes your conversation, Crabfork
...
```bash
crabfork memory rem-backfill --rollback
crabfork memory rem-backfill --rollback-short-term
```

## CLI

```bash
crabfork memory status          # Check index status and provider
crabfork memory search "query"  # Search from the command line
crabfork memory index --force   # Rebuild the index
```
