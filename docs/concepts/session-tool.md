...
Crabfork gives agents tools to work across sessions, inspect status, and
...
After the target responds, Crabfork can run a **reply-back loop** where the
...
`subagents` is the control-plane helper for already spawned Crabfork
...
the completion origin only identifies a channel Crabfork can still reuse the
