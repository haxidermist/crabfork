---
summary: "How Crabfork manages conversation sessions"
read_when:
...
Crabfork organizes conversations into **sessions**. Each message is routed to a
...
Verify your setup with `crabfork security audit`.
...
Crabfork automatically bounds session storage over time. By default, it runs
...
Preview with `crabfork sessions cleanup --dry-run`.

## Inspecting sessions

- `crabfork status` -- session store path and recent activity.
- `crabfork sessions --json` -- all sessions (filter with `--active <minutes>`).
