...
read_when:
  - You want to understand how Crabfork assembles model context
...
A **context engine** controls how Crabfork builds model context for each run.
It decides which messages to include, how to summarize older history, and how
to manage context across subagent boundaries.

Crabfork ships with a built-in `legacy` engine. Plugins can register
...
cat ~/.crabfork/crabfork.json | jq '.plugins.slots.contextEngine'
```

### Installing a context engine plugin

Context engine plugins are installed like any other Crabfork plugin. Install
...
crabfork plugins install -l ./my-context-engine
```
...
Every time Crabfork runs a model prompt, the context engine participates at
...
Crabfork currently calls one subagent lifecycle hook:
...
The `assemble` method can return a `systemPromptAddition` string. Crabfork
...
The built-in `legacy` engine preserves Crabfork's original behavior:
...
- `estimatedTokens` (required, `number`) — the engine's estimate of total
  tokens in the assembled context. Crabfork uses this for compaction threshold
...
- `true` — the engine owns compaction behavior. Crabfork disables Pi's built-in
...
`ownsCompaction: false` does **not** mean Crabfork automatically falls back to
...
- **Delegating mode** — set `ownsCompaction: false` and have `compact()` call
  `delegateCompactionToRuntime(...)` from `crabfork/plugin-sdk/core` to use
  Crabfork's built-in compaction behavior.
...
Crabfork resolves when it needs a context engine.

## Relationship to compaction and memory

- **Compaction** is one responsibility of the context engine. The legacy engine
  delegates to Crabfork's built-in summarization. Plugin engines can implement
...
- Use `crabfork doctor` to verify your engine is loading correctly.
...
- Engine errors are logged and surfaced in diagnostics. If a plugin engine
  fails to register or the selected engine id cannot be resolved, Crabfork
  does not fall back automatically; runs fail until you fix the plugin or
  switch `plugins.slots.contextEngine` back to `"legacy"`.
- For development, use `crabfork plugins install -l ./my-engine` to link a
