---
summary: "What the Crabfork system prompt contains and how it is assembled"
read_when:
...
Crabfork builds a custom system prompt for every agent run. The prompt is **Crabfork-owned** and does not use the pi-coding-agent default prompt.

The prompt is assembled by Crabfork and injected into each agent run.

Provider plugins can contribute cache-aware prompt guidance without replacing
the full Crabfork-owned prompt. The provider runtime can:
...
- **Crabfork Self-Update**: how to inspect config safely with
...
- **Documentation**: local path to Crabfork docs (repo or npm package) and when to read them.
...
Crabfork can render smaller system prompts for sub-agents. The runtime sets a
`promptMode` for each run (not a user-facing config):

- `full` (default): includes all sections above.
- `minimal`: used for sub-agents; omits **Skills**, **Memory Recall**, **Crabfork
...
occurs, Crabfork can inject a warning block in Project Context; control this with
...
When eligible skills exist, Crabfork injects a compact **available skills list**
...
local Crabfork docs directory (either `docs/` in the repo workspace or the bundled npm
...
for Crabfork behavior, commands, configuration, or architecture, and to run
`crabfork status` itself when possible (asking the user only when it lacks access).
