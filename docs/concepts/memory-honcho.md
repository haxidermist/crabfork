...
[Honcho](https://honcho.dev) adds AI-native memory to Crabfork. It persists
...
```bash
crabfork plugins install @honcho-ai/crabfork-honcho
crabfork honcho setup
...
Settings live under `plugins.entries["crabfork-honcho"].config`:
    ...
    entries: {
      "crabfork-honcho": {
        config: {
          ...
          workspaceId: "crabfork", // memory isolation
...
`IDENTITY.md`, `memory/`, `canvas/`), `crabfork honcho setup` detects and
...
```bash
crabfork honcho setup                        # Configure API key and migrate files
crabfork honcho status                       # Check connection status
crabfork honcho ask <question>               # Query Honcho about the user
crabfork honcho search <query> [-k N] [-d D] # Semantic search over memory
```

## Further reading

- [Plugin source code](https://github.com/plastic-labs/crabfork-honcho)
- [Honcho documentation](https://docs.honcho.dev)
- [Honcho Crabfork integration guide](https://docs.honcho.dev/v3/guides/integrations/crabfork)
- [Memory](/concepts/memory) -- Crabfork memory overview
