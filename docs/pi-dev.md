...
This guide summarizes a sane workflow for working on the pi integration in Crabfork.
...
```bash
CRABFORK_LIVE_TEST=1 pnpm test src/agents/pi-embedded-runner-extraparams.live.test.ts
```
...
- Trigger the agent directly:
  - `pnpm crabfork agent --message "Hello" --thinking low`
...
State lives under the Crabfork state directory. Default is `~/.crabfork`. If `CRABFORK_STATE_DIR` is set, use that directory instead.

To reset everything:

- `crabfork.json` for config
