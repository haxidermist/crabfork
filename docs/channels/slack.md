...
```json
{
  "display_information": {
    "name": "Crabfork",
    "description": "Slack connector for Crabfork"
  },
  "features": {
    "bot_user": {
      "display_name": "Crabfork",
      "always_online": true
    },
    "app_home": {
    ...
    "slash_commands": [
      {
        "command": "/crabfork",
        "description": "Send a message to Crabfork",
        "should_escape": false
      }
    ]
  },
...
<Tabs>
  <Tab title="DM policy">
    ...
    Pairing in DMs uses `crabfork pairing approve slack <code>`.
...
- If streaming fails mid-reply, Crabfork falls back to normal delivery for remaining payloads.
...
`typingReaction` adds a temporary reaction to the inbound Slack message while Crabfork is processing a reply, then removes it when the run finishes. This is most useful outside of thread replies, which use a default "is typing..." status indicator.
...
- The interactive callback values are Crabfork-generated opaque tokens, not raw agent-authored values.
- If generated interactive blocks would exceed Slack Block Kit limits, Crabfork falls back to the original text reply instead of sending an invalid blocks payload.
...
```
  ...
  <Accordion title="Socket mode not connecting">
    ...
    If `crabfork channels status --probe --json` shows `botTokenStatus` or
