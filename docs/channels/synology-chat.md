---
summary: "Synology Chat webhook setup and Crabfork config"
read_when:
  - Setting up Synology Chat with Crabfork
...
Synology Chat ships as a bundled plugin in current Crabfork releases, so normal
...
```bash
crabfork plugins install ./path/to/local/synology-chat-plugin
```
...
1. Ensure the Synology Chat plugin is available.
   ...
   - `crabfork onboard` now shows Synology Chat in the same channel setup list as `crabfork channels add`.
   - Non-interactive setup: `crabfork channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
2. In Synology Chat integrations:
   - Create an incoming webhook and copy its URL.
   - Create an outgoing webhook with your secret token.
3. Point the outgoing webhook URL to your Crabfork gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Finish setup in Crabfork.
   - Guided: `crabfork onboard`
   - Direct: `crabfork channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
5. Restart gateway and send a DM to the Synology Chat bot.

Webhook auth details:

- Crabfork accepts the outgoing webhook token from `body.token`, then
  `?token=...`, then headers.
...
- Pairing approvals work with:
  - `crabfork pairing list synology-chat`
  - `crabfork pairing approve synology-chat <CODE>`
...
```bash
crabfork message send --channel synology-chat --target 123456 --text "Hello from Crabfork"
crabfork message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
```
...
Give each enabled account a distinct `webhookPath`. Crabfork now rejects duplicate exact paths
...
- `Invalid token`:
  ...
  - a reverse proxy stripped the token header before the request reached Crabfork
- `Rate limit exceeded`:
