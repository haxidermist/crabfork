...
Nextcloud Talk ships as a bundled plugin in current Crabfork releases, so
...
```bash
crabfork plugins install @crabfork/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
crabfork plugins install ./path/to/local/nextcloud-talk-plugin
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Crabfork" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `crabfork pairing list nextcloud-talk`
  - `crabfork pairing approve nextcloud-talk <CODE>`
