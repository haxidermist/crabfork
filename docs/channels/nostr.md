...
read_when:
  - You want Crabfork to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Crabfork to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Bundled plugin

Current Crabfork releases ship Nostr as a bundled plugin, so normal packaged
builds do not need a separate install.

### Older/custom installs

- Onboarding (`crabfork onboard`) and `crabfork channels add` still surface
  Nostr from the shared channel catalog.
- If your build excludes bundled Nostr, install it manually.

```bash
crabfork plugins install @crabfork/nostr
```

Use a local checkout (dev workflows):

```bash
crabfork plugins install --link <path-to-local-nostr-plugin>
```
...
crabfork channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY" --relay-urls "wss://relay.damus.io,wss://relay.primal.net"
```
...
```json5
{
  channels: {
    nostr: {
      ...
      profile: {
        name: "crabfork",
        displayName: "Crabfork",
        about: "Personal assistant DM bot",
        picture: "https://example.com/avatar.png",
        banner: "https://example.com/banner.png",
        website: "https://example.com",
        nip05: "crabfork@example.com",
        lud16: "crabfork@example.com",
      },
    },
  },
}
