...
read_when:
  - Setting up Zalo Personal for Crabfork
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside Crabfork.

> **Warning:** This is an unofficial integration and may result in account suspension/ban. Use at your own risk.

## Bundled plugin

Zalo Personal ships as a bundled plugin in current Crabfork releases, so normal
packaged builds do not need a separate install.

If you are on an older build or a custom install that excludes Zalo Personal,
install it manually:

- Install via CLI: `crabfork plugins install @crabfork/zalouser`
- Or from a source checkout: `crabfork plugins install ./path/to/local/zalouser-plugin`
...
- `crabfork pairing list zalouser`
- `crabfork pairing approve zalouser <code>`
...
- On startup, Crabfork resolves group/user names in allowlists to IDs and logs the mapping.
...
- When a group message is skipped because mention is required, Crabfork stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in Crabfork state. Example:
...
- Crabfork sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, Crabfork sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `crabfork channels logout --channel zalouser && crabfork channels login --channel zalouser`
...
- The channel now runs fully in Crabfork without external CLI binaries.
