---
summary: "Mattermost bot setup and Crabfork config"
read_when:
...
Mattermost ships as a bundled plugin in current Crabfork releases, so normal
...
```bash
crabfork plugins install @crabfork/mattermost
```

Local checkout (when running from a git repo):

```bash
crabfork plugins install ./path/to/local/mattermost-plugin
```
...
4. Configure Crabfork and start the gateway.
...
Native slash commands are opt-in. When enabled, Crabfork registers `oc_*` slash commands via
...
- If `callbackUrl` is omitted, Crabfork derives one from gateway host/port + `callbackPath`.
...
- Command callbacks are validated with the per-command tokens returned by
  Mattermost when Crabfork registers `oc_*` commands.
...
- Reachability requirement: the callback endpoint must be reachable from the Mattermost server.
  - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as Crabfork.
  - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to Crabfork.
  - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from Crabfork, not `404`.
...
- Approve via:
  - `crabfork pairing list mattermost`
  - `crabfork pairing approve mattermost <CODE>`
...
Use these target formats with `crabfork message send` or cron/webhooks:
...
Crabfork resolves them **user-first**:

- If the ID exists as a user (`GET /api/v4/users/<id>` succeeds), Crabfork sends a **DM** by resolving the direct channel via `/api/v4/channels/direct`.
...
When Crabfork sends to a Mattermost DM target and needs to resolve the direct channel first, it
...
- If `interactions.callbackBaseUrl` is omitted, Crabfork derives the callback URL from
...
- Reachability rule: the button callback URL must be reachable from the Mattermost server.
  `localhost` only works when Mattermost and Crabfork run on the same host/network namespace.
...
1. Derive the secret from the bot token:
   `HMAC-SHA256(key="crabfork-mattermost-interactions", data=botToken)`
...
secret = hmac.new(
    b"crabfork-mattermost-interactions",
...
`crabfork message send` and cron/webhook deliveries.
...
- Native slash commands return `Unauthorized: invalid command token.`: Crabfork
...
- If `callbackUrl` is omitted and logs warn that the callback resolved to
  ...
  Mattermost runs on the same host/network namespace as Crabfork. Set an
