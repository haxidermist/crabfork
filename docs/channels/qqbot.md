...
read_when:
  - You want to connect Crabfork to QQ
...
QQ Bot connects to Crabfork via the official QQ Bot API (WebSocket gateway). The
...
Current Crabfork releases bundle QQ Bot, so normal packaged builds do not need
a separate `crabfork plugins install` step.
...
```bash
crabfork channels add --channel qqbot --token "AppID:AppSecret"
```
...
- `crabfork channels add --channel qqbot --token-file ...` provides the
...
Run multiple QQ bots under a single Crabfork instance:
...
```bash
crabfork channels add --channel qqbot --account bot2 --token "222222222:secret-of-bot-2"
```
