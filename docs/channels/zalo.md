...
Zalo ships as a bundled plugin in current Crabfork releases, so normal packaged
builds do not need a separate install.

If you are on an older build or a custom install that excludes Zalo, install it
manually:

- Install via CLI: `crabfork plugins install @crabfork/zalo`
- Or from a source checkout: `crabfork plugins install ./path/to/local/zalo-plugin`
...
This page reflects current Crabfork behavior for **Zalo Bot Creator / Marketplace bots**.
...
- Approve via:
  - `crabfork pairing list zalo`
  - `crabfork pairing approve zalo <CODE>`
...
This table summarizes current **Zalo Bot Creator / Marketplace bot** behavior in Crabfork.
...
- Example: `crabfork message send --channel zalo --target 123456789 --message "hi"`.

## Troubleshooting

**Bot doesn't respond:**

- Check that the token is valid: `crabfork channels status --probe`
- Verify the sender is approved (pairing or allowFrom)
- Check gateway logs: `crabfork logs --follow`
