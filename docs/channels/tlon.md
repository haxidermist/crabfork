...
Tlon is a decentralized messenger built on Urbit. Crabfork connects to your Urbit ship and can
...
Tlon ships as a bundled plugin in current Crabfork releases, so normal packaged
...
```bash
crabfork plugins install @crabfork/tlon
```

Local checkout (when running from a git repo):

```bash
crabfork plugins install ./path/to/local/tlon-plugin
```
...
By default, Crabfork blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `crabfork message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Crabfork replies in-thread.
