...
read_when:
  ...
  - Reviewing Crabfork security posture
title: "Pairing"
---

# Pairing

“Pairing” is Crabfork’s explicit **owner approval** step.
...
Supported channels: `bluebubbles`, `discord`, `feishu`, `googlechat`, `imessage`, `irc`, `line`, `matrix`, `mattermost`, `msteams`, `nextcloud-talk`, `nostr`, `crabfork-weixin`, `signal`, `slack`, `synology-chat`, `telegram`, `twitch`, `whatsapp`, `zalo`, `zalouser`.

### Where the state lives

Stored under `~/.crabfork/credentials/`:
...
3. On your phone, open the Crabfork iOS app → Settings → Gateway.
...
- The legacy `node.pair.*` API (CLI: `crabfork nodes pending|approve|reject|rename`) is a
