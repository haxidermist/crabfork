...
read_when:
  - Setting up Twitch chat integration for Crabfork
title: "Twitch"
---

# Twitch

Twitch chat support via IRC connection. Crabfork connects as a Twitch user (bot account) to receive and send messages in channels.

## Bundled plugin

Twitch ships as a bundled plugin in current Crabfork releases, so normal
...
```bash
crabfork plugins install @crabfork/twitch
```

Local checkout (when running from a git repo):

```bash
crabfork plugins install ./path/to/local/twitch-plugin
```
...
5. Configure the token:
   - Env: `CRABFORK_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "crabfork", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use CRABFORK_TWITCH_ACCESS_TOKEN env var)
...
```bash
CRABFORK_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
