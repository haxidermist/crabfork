...
read_when:
  - You want to connect Crabfork to LINE
...
LINE connects to Crabfork via the LINE Messaging API. The plugin runs as a webhook
...
LINE ships as a bundled plugin in current Crabfork releases, so normal
...
```bash
crabfork plugins install @crabfork/line
```

Local checkout (when running from a git repo):

```bash
crabfork plugins install ./path/to/local/line-plugin
```
...
- LINE signature verification is body-dependent (HMAC over the raw body), so Crabfork applies strict pre-auth body limits and timeout before verification.
- Crabfork processes webhook events from the verified raw request bytes. Upstream middleware-transformed `req.body` values are ignored for signature-integrity safety.
...
```bash
crabfork pairing list line
crabfork pairing approve line <CODE>
```
