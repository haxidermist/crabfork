...
Crabfork also includes `x_search` for X (formerly Twitter) posts and
...
<Steps>
  ...
  <Step title="Use it">
    ...
    ```javascript
    await web_search({ query: "Crabfork plugin SDK" });
    ```
...
Codex-capable models can optionally use the provider-native Responses `web_search` tool instead of Crabfork's managed `web_search` function.
...
If native Codex search is enabled but the current model is not Codex-capable, Crabfork keeps the normal managed `web_search` behavior.
...
If no `provider` is set, Crabfork checks providers in this order and uses the
...
<Note>
  ...
  Crabfork resolves only the selected provider key -- non-selected SecretRefs
  stay inactive.
</Note>
...
- or omit that field and let Crabfork auto-detect the first ready web-fetch
  provider from available credentials
- today the bundled web-fetch provider is Firecrawl, configured under
  `plugins.entries.firecrawl.config.webFetch.*`

When you choose **Kimi** during `crabfork onboard` or
`crabfork configure --section web`, Crabfork can also ask for:
...
Legacy `tools.web.x_search.*` config is auto-migrated by `crabfork doctor --fix`.
When you choose Grok during `crabfork onboard` or `crabfork configure --section web`,
Crabfork can also offer optional `x_search` setup with the same key.
This is a separate follow-up step inside the Grok path, not a separate top-level
web-search provider choice. If you pick another provider, Crabfork does not
...
<Tabs>
  <Tab title="Config file">
    Run `crabfork configure --section web` or set the key directly:
  ...
  <Tab title="Environment variable">
    ...
    For a gateway install, put it in `~/.crabfork/.env`.
...
optional structured filters. Crabfork only enables the built-in xAI `x_search`
