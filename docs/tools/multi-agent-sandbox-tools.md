...
- **Debugging blocked tools**: see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `crabfork sandbox explain`.
...
Legacy `agent.*` configs are migrated by `crabfork doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=crabfork-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${CRABFORK_STATE_DIR:-$HOME/.crabfork}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
