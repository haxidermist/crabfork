...
<Steps>
  <Step title="Create the skill directory">
    ...
    ```bash
    mkdir -p ~/.crabfork/workspace/skills/hello-world
    ```
  ...
  <Step title="Load the skill">
    Start a new session so Crabfork picks up the skill:
  ...
  <Step title="Test it">
    ...
    ```bash
    crabfork agent --message "give me a greeting"
    ```
...
| `metadata.crabfork.os`              | No       | OS filter (`["darwin"]`, `["linux"]`, etc.) |
| `metadata.crabfork.requires.bins`   | No       | Required binaries on PATH                   |
| `metadata.crabfork.requires.config` | No       | Required config keys                        |
...
- **Test locally** — use `crabfork agent --message "..."` to test before sharing
...
| `~/.crabfork/skills/`           | Medium     | Shared (all agents)   |
| Bundled (shipped with Crabfork) | Low        | Global                |
