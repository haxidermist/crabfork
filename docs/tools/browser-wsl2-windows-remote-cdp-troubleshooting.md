...
read_when:
  - Running Crabfork Gateway in WSL2 while Chrome lives on Windows
...
- Crabfork Gateway runs inside WSL2
- Chrome runs on Windows
- browser control must cross the WSL2/Windows boundary

It also covers the layered failure pattern from [issue #39369](https://github.com/crabfork/crabfork/issues/39369): several independent problems can show up at once, which makes the wrong layer look broken first.
...
- Crabfork and Chrome are on the same machine
...
- Crabfork points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, Crabfork is not the problem yet.
...
Fix that before touching Crabfork config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point Crabfork at the address that is reachable from WSL2:
...
- use HTTP(S) when you want Crabfork to discover `/json/version`
- use WS(S) only when the browser provider gives you a direct DevTools socket URL
- test the same URL with `curl` before expecting Crabfork to succeed
...
```bash
crabfork browser open https://example.com --browser-profile remote
crabfork browser tabs --browser-profile remote
```

Good result:

- the tab opens in Windows Chrome
- `crabfork browser tabs` returns the target
...
- stale viewport / dark-mode / locale / offline overrides after a remote session
  - run `crabfork browser stop --browser-profile remote`
...
3. Crabfork config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug Crabfork config or Control UI auth
