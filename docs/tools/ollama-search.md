...
- an Ollama host that is reachable from Crabfork
- `ollama signin`
...
If no explicit Ollama base URL is set, Crabfork uses `http://127.0.0.1:11434`.

If your Ollama host expects bearer auth, Crabfork reuses
...
- If the Ollama host is auth-protected, Crabfork reuses the normal Ollama
  provider API key when present.
- Crabfork warns during setup if Ollama is unreachable or not signed in, but
