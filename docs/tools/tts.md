...
Crabfork can convert outbound replies into audio using ElevenLabs, Microsoft, MiniMax, or OpenAI.
It works anywhere Crabfork can send audio.
...
When `messages.tts.provider` is unset, Crabfork picks the first configured
speech provider in registry auto-select order.

## Config

TTS config lives under `messages.tts` in `crabfork.json`.
...
- If `provider` is **unset**, Crabfork uses the first configured speech provider in registry auto-select order.
...
`~/.crabfork/settings/tts.json`, override with `CRABFORK_TTS_PREFS` or
...
- **Microsoft**: uses `microsoft.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Microsoft output format fails, Crabfork retries with MP3.

OpenAI/ElevenLabs output formats are fixed per channel (see above).

## Auto-TTS behavior

When enabled, Crabfork:
...
Discord note: `/tts` is a built-in Discord command, so Crabfork registers
