...
`ask: "always"` in `~/.crabfork/exec-approvals.json` keeps prompting even if
session or config defaults request `ask: "on-miss"`.
Use `crabfork approvals get`, `crabfork approvals get --gateway`, or
`crabfork approvals get --node <id|name|ip>` to inspect the requested policy,
host policy sources, and the effective result.
For the local machine, `crabfork exec-policy show` exposes the same merged view and
`crabfork exec-policy set|preset` can synchronize the local requested policy with the
local host approvals file in one step. When a local scope requests `host=node`,
`crabfork exec-policy show` reports that scope as node-managed at runtime instead of
...
- **gateway host** → `crabfork` process on the gateway machine
...
- For shell scripts and direct interpreter/runtime file invocations, Crabfork also tries to bind
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.crabfork/exec-approvals.sock",
...
- requested exec policy in Crabfork config (`tools.exec.*`)
- host-local approvals policy in `~/.crabfork/exec-approvals.json`
...
- In YOLO mode, Crabfork does not add a separate heuristic command-obfuscation approval gate on top of the configured host exec policy.
...
```bash
crabfork config set tools.exec.host gateway
crabfork config set tools.exec.security full
crabfork config set tools.exec.ask off
...
```bash
crabfork approvals set --stdin <<'EOF'
{
  version: 1,
  defaults: {
    security: "full",
...
- `crabfork exec-policy` does not synchronize node approvals
- `crabfork exec-policy set --host node` is rejected
- node exec approvals are fetched from the node at runtime, so node-targeted updates must use `crabfork approvals --node ...`
...
When `tools.exec.strictInlineEval=true`, Crabfork treats inline code-eval forms as approval-only even if the interpreter binary itself is allowlisted.
...
- allowlist entries live in host-local `~/.crabfork/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `crabfork approvals allowlist ...`).
- `crabfork security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `crabfork doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
If you explicitly opt `jq` into `safeBins`, Crabfork still rejects the `env` builtin in safe-bin
...
`~/.crabfork/exec-approvals.json` directly.

CLI: `crabfork approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
...
After an approved async exec finishes, Crabfork sends a followup `agent` turn to the same session.
...
topics, Crabfork preserves the topic for the approval prompt and the post-approval follow-up.
...
When an async exec approval is denied, Crabfork prevents the agent from reusing
