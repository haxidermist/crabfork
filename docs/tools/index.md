---
summary: "Crabfork tools and plugins overview: what the agent can do and how to extend it"
read_when:
  - You want to understand what tools Crabfork provides
...
Crabfork has three layers that work together:

<Steps>
  <Step title="Tools are what the agent calls">
    A tool is a typed function the agent can invoke (e.g. `exec`, `browser`,
    `web_search`, `message`). Crabfork ships a set of **built-in tools** and
  ...
  <Step title="Plugins package everything together">
    ...
    Crabfork), others are **external** (published on npm by the community).
...
These tools ship with Crabfork and are available without installing any plugins:
...
| `group:crabfork`   | All built-in Crabfork tools (excludes plugin tools)                                                       |
