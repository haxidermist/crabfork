...
- `commands.config` (default `false`) enables `/config` (reads/writes `crabfork.json`).
- `commands.mcp` (default `false`) enables `/mcp` (reads/writes Crabfork-managed MCP config under `mcp.servers`).
...
- `/config show|get|set|unset` reads or writes `crabfork.json`. Owner-only. Requires `commands.config: true`.
- `/mcp show|get|set|unset` reads or writes Crabfork-managed MCP server config under `mcp.servers`. Owner-only. Requires `commands.mcp: true`.
...
- `/restart` restarts Crabfork when enabled. Default: enabled; set `commands.restart: false` to disable it.
...
- For full provider usage breakdown, use `crabfork status --usage`.
- `/allowlist add|remove` requires `commands.config=true` and honors channel `configWrites`.
- In multi-account channels, config-targeted `/allowlist --account <id>` and `/config set channels.<provider>.accounts.<id>...` also honor the target account's `configWrites`.
- `/usage` controls the per-response usage footer; `/usage cost` prints a local cost summary from Crabfork session logs.
- `/restart` is enabled by default; set `commands.restart: false` to disable it.
- `/plugins install <spec>` accepts the same plugin specs as `crabfork plugins install`: local path/archive, npm package, or `clawhub:<pkg>`.
...
- If a run is already active, Crabfork marks a live switch as pending and only restarts into the new model at a clean retry point.
...
- **Provider usage/quota** (example: “Claude 80% left”) shows up in `/status` for the current model provider when usage tracking is enabled. Crabfork normalizes provider windows to `% left`; for MiniMax, remaining-only percent fields are inverted before display, and `model_remains` responses prefer the chat-model entry plus a model-tagged plan label.
...
```
/debug show
/debug set messages.responsePrefix="[crabfork]"
...
- Overrides apply immediately to new config reads, but do **not** write to `crabfork.json`.
...
`/config` writes to your on-disk config (`crabfork.json`). Owner-only. Disabled by default; enable with `commands.config: true`.
...
/config set messages.responsePrefix="[crabfork]"
...
`/mcp` writes Crabfork-managed MCP server definitions under `mcp.servers`. Owner-only. Disabled by default; enable with `commands.mcp: true`.
...
- `/mcp` stores config in Crabfork config, not Pi-owned project settings.
...
- **Slack:** `channels.slack.slashCommand` is still supported for a single `/crabfork`-style command. If you enable `commands.native`, you must create one Slack slash command per built-in command (same names as `/help`). Command argument menus for Slack are delivered as ephemeral Block Kit buttons.
