...
read_when:
  ...
  - Debugging why crabfork is interfering with your own Chrome
  - Implementing browser settings + lifecycle in the macOS app
title: "Browser (Crabfork-managed)"
---

# Browser (crabfork-managed)

Crabfork can run a **dedicated Chrome/Brave/Edge/Chromium profile** that the agent controls.
...
- The `crabfork` profile does **not** touch your personal browser profile.
- The agent can **open tabs, read pages, click, and type** in a safe lane.
- The built-in `user` profile attaches to your real signed-in Chrome session via Chrome MCP.

## What you get

- A separate browser profile named **crabfork** (orange accent by default).
- Deterministic tab control (list/open/focus/close).
- Agent actions (click/type/drag/select), snapshots, screenshots, PDFs.
- Optional multi-profile support (`crabfork`, `work`, `remote`, ...).
...
```bash
crabfork browser --browser-profile crabfork status
...
If `crabfork browser` is missing entirely, or the agent says the browser tool
...
If you turn off only the plugin, the bundled browser CLI (`crabfork browser`),
...
If `crabfork browser` suddenly becomes an unknown command after an upgrade, or
...
Set `browser.defaultProfile: "crabfork"` if you want managed mode by default.

## Configuration

Browser settings live in `~/.crabfork/crabfork.json`.
...
- If you override the Gateway port (`gateway.port` or `CRABFORK_GATEWAY_PORT`),
...
- Default profile is `crabfork` (Crabfork-managed standalone browser). Use `defaultProfile: "user"` to opt into the signed-in user browser.
- Auto-detect order: system default browser if Chromium-based; otherwise Chrome → Brave → Edge → Chromium → Chrome Canary.
- Local `crabfork` profiles auto-assign `cdpPort`/`cdpUrl` — set those only for remote CDP.
...
Crabfork uses it automatically. Set `browser.executablePath` to override
auto-detection:
...
- **Remote CDP:** set `browser.profiles.<name>.cdpUrl` (or `browser.cdpUrl`) to
  attach to a remote Chromium-based browser. In this case, Crabfork will not launch a local browser.

Stopping behavior differs by profile mode:

- local managed profiles: `crabfork browser stop` stops the browser process that
  Crabfork launched
- attach-only and remote CDP profiles: `crabfork browser stop` closes the active
  ...
  though no browser process was launched by Crabfork
...
Crabfork preserves the auth when calling `/json/*` endpoints and when connecting
to the CDP WebSocket. Prefer environment variables or secrets managers for
tokens instead of committing them to config files.

## Node browser proxy (zero-config default)

If you run a **node host** on the machine that has your browser, Crabfork can
...
- If you set `nodeHost.browserProxy.allowProfiles`, Crabfork treats it as a least-privilege boundary: only allowlisted profiles can be targeted, and persistent profile create/delete routes are blocked on the proxy surface.
...
CDP connection URLs over HTTPS and WebSocket. Crabfork can use either form, but
...
- If Browserless gives you an HTTPS base URL, you can either convert it to
  `wss://` for a direct CDP connection or keep the HTTPS URL and let Crabfork
  discover `/json/version`.

## Direct WebSocket CDP providers

Some hosted browser services expose a **direct WebSocket** endpoint rather than
the standard HTTP-based CDP discovery (`/json/version`). Crabfork supports both:

- **HTTP(S) endpoints** — Crabfork calls `/json/version` to discover the
  WebSocket debugger URL, then connects.
- **WebSocket endpoints** (`ws://` / `wss://`) — Crabfork connects directly,
...
- The standalone loopback browser HTTP API uses **shared-secret auth only**:
  gateway token bearer auth, `x-crabfork-password`, or HTTP Basic auth with the
...
- If browser control is enabled and no shared-secret auth is configured, Crabfork
  auto-generates `gateway.auth.token` on startup and persists it to config.
- Crabfork does **not** auto-generate that token when `gateway.auth.mode` is
...
Crabfork supports multiple named profiles (routing configs). Profiles can be:

- **crabfork-managed**: a dedicated Chromium-based browser instance with its own user data directory + CDP port
- **remote**: an explicit CDP URL (Chromium-based browser running elsewhere)
- **existing session**: your existing Chrome profile via Chrome DevTools MCP auto-connect

Defaults:

- The `crabfork` profile is auto-created if missing.
...
Crabfork can also attach to a running Chromium-based browser profile through the
...
3. Keep the browser running and approve the connection prompt when Crabfork attaches.
...
```bash
crabfork browser --browser-profile user start
crabfork browser --browser-profile user status
crabfork browser --browser-profile user tabs
crabfork browser --browser-profile user snapshot --format ai
```
...
- `crabfork doctor` migrates old extension-based browser config and checks that
...
- This path is higher-risk than the isolated `crabfork` profile because it can
  act inside your signed-in browser session.
- Crabfork does not launch the browser for this driver; it attaches to an
  existing session only.
- Crabfork uses the official Chrome DevTools MCP `--autoConnect` flow here. If
  `userDataDir` is set, Crabfork passes it through to target that explicit
...
When launching locally, Crabfork picks the first available:
...
- `x-crabfork-password: <gateway password>` or HTTP Basic auth with that password
...
- ARIA snapshots
- Page screenshots for the managed `crabfork` browser when a per-tab CDP
...
```bash
docker compose run --rm crabfork-cli \
  node /app/node_modules/playwright-core/cli.js install chromium
```

To persist browser downloads, set `PLAYWRIGHT_BROWSERS_PATH` (for example,
`/home/node/.cache/ms-playwright`) and make sure `/home/node` is persisted via
`CRABFORK_HOME_VOLUME` or a bind mount. See [Docker](/install/docker).
...
- `crabfork browser snapshot --interactive --compact --depth 6`
- `crabfork browser snapshot --efficient`
- `crabfork browser snapshot --labels`
- `crabfork browser snapshot --selector "#main" --interactive`
- `crabfork browser snapshot --frame "iframe#main" --interactive`
- `crabfork browser console --level error`

Lifecycle note:

- For attach-only and remote CDP profiles, `crabfork browser stop` is still the
...
- `crabfork browser requests --filter api --clear`
- `crabfork browser pdf`
- `crabfork browser responsebody "**/api" --max-chars 5000`
...
- `crabfork browser storage local set theme dark`
- `crabfork browser storage session clear`
- `crabfork browser set offline on`
- `crabfork browser set headers --headers-json '{"X-Debug":"1"}'`
- `crabfork browser set credentials user pass`
- `crabfork browser set credentials --clear`
- `crabfork browser set geo 37.7749 -122.4194 --origin "https://example.com"`
- `crabfork browser set geo --clear`
- `crabfork browser set media dark`
- `crabfork browser set timezone America/New_York`
- `crabfork browser set locale en-US`
...
- Download and trace output paths are constrained to Crabfork temp roots:
  - traces: `/tmp/crabfork` (fallback: `${os.tmpdir()}/crabfork`)
  - downloads: `/tmp/crabfork/downloads` (fallback: `${os.tmpdir()}/crabfork/downloads`)
- Upload paths are constrained to an Crabfork temp uploads root:
  - uploads: `/tmp/crabfork/uploads` (fallback: `${os.tmpdir()}/crabfork/uploads`)
...
Crabfork supports two “snapshot” styles:

- **AI snapshot (numeric refs)**: `crabfork browser snapshot` (default; `--format ai`)
  - Output: a text snapshot that includes numeric refs.
  - Actions: `crabfork browser click 12`, `crabfork browser type 23 "hello"`.
  - Internally, the ref is resolved via Playwright’s `aria-ref`.

- **Role snapshot (role refs like `e12`)**: `crabfork browser snapshot --interactive` (or `--compact`, `--depth`, `--selector`, `--frame`)
  - Output: a role-based list/tree with `[ref=e12]` (and optional `[nth=1]`).
  - Actions: `crabfork browser click e12`, `crabfork browser highlight e12`.
...
- Wait for URL (globs supported by Playwright):
  - `crabfork browser wait --url "**/dash"`
- Wait for load state:
  - `crabfork browser wait --load networkidle`
- Wait for a JS predicate:
  - `crabfork browser wait --fn "window.ready===true"`
- Wait for a selector to become visible:
  - `crabfork browser wait "#main"`
...
1. `crabfork browser snapshot --interactive`
2. Use `click <ref>` / `type <ref>` (prefer role refs in interactive mode)
3. If it still fails: `crabfork browser highlight <ref>` to see what Playwright is targeting
...
5. For deep debugging: record a trace:
   - `crabfork browser trace start`
   - reproduce the issue
   - `crabfork browser trace stop` (prints `TRACE:<path>`)
...
```bash
crabfork browser status --json
crabfork browser snapshot --interactive --json
crabfork browser requests --filter api --json
crabfork browser cookies --json
```
...
- The crabfork browser profile may contain logged-in sessions; treat it as sensitive.
- `browser act kind=evaluate` / `crabfork browser evaluate` and `wait --fn`
...
- **CDP startup or readiness failure** means Crabfork cannot confirm that the browser control plane is healthy.
...
- CDP startup or readiness failure:
  - `Chrome CDP websocket for profile "crabfork" is not reachable after start`
...
- For the local loopback `crabfork` managed profile, CDP health checks intentionally skip browser SSRF reachability enforcement for Crabfork's own local control plane.
...
- `browser` accepts:
  - `profile` to choose a named browser profile (crabfork, chrome, or remote CDP).
