...
Crabfork supports Kimi as a `web_search` provider, using Moonshot web search
...
`https://api.moonshot.cn/v1`), Crabfork reuses that same host for Kimi
...
Gateway environment. For a gateway install, put it in `~/.crabfork/.env`.

If you omit `baseUrl`, Crabfork defaults to `https://api.moonshot.ai/v1`.
If you omit `model`, Crabfork defaults to `kimi-k2.5`.
