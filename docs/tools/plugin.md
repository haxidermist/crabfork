---
summary: "Install, configure, and manage Crabfork plugins"
read_when:
...
search, and more. Some plugins are **core** (shipped with Crabfork), others
...
<Steps>
  ...
  <Step title="Install a plugin">
    ...
    crabfork plugins install ./my-plugin
    crabfork plugins install ./my-plugin.tgz
    ```
...
```text
/plugin install clawhub:@crabfork/voice-call
...
`crabfork doctor --fix`. The only recovery exception is a narrow bundled-plugin
reinstall path for plugins that opt into
`crabfork.install.allowInvalidConfigRecovery`.

## Plugin types

Crabfork recognizes two plugin formats:

| Format     | How it works                                                       | Examples                                               |
| ---------- | ------------------------------------------------------------------ | ------------------------------------------------------ |
| **Native** | `crabfork.plugin.json` + runtime module; executes in-process       | Official plugins, community npm packages               |
| **Bundle** | Codex/Claude/Cursor-compatible layout; mapped to Crabfork features | `.codex-plugin/`, `.claude-plugin/`, `.cursor-plugin/` |

Both show up under `crabfork plugins list`. See [Plugin Bundles](/plugins/bundles) for bundle details.
...
| Plugin          | Package                | Docs                                 |
| --------------- | ---------------------- | ------------------------------------ |
| Matrix          | `@crabfork/matrix`     | [Matrix](/channels/matrix)           |
| Microsoft Teams | `@crabfork/msteams`    | [Microsoft Teams](/channels/msteams) |
| Nostr           | `@crabfork/nostr`      | [Nostr](/channels/nostr)             |
| Voice Call      | `@crabfork/voice-call` | [Voice Call](/plugins/voice-call)    |
| Zalo            | `@crabfork/zalo`       | [Zalo](/channels/zalo)               |
| Zalo Personal   | `@crabfork/zalouser`   | [Zalo Personal](/plugins/zalouser)   |

### Core (shipped with Crabfork)
...
<AccordionGroup>
  ...
  <Accordion title="Other">
    - `browser` — bundled browser plugin for the browser tool, `crabfork browser` CLI, `browser.request` gateway method, browser runtime, and default browser control service (enabled by default; disable before replacing it)
...
watch + in-process restart enabled (the default `crabfork gateway` path), that
...
Crabfork scans for plugins in this order (first match wins):
...
<Steps>
  ...
  <Step title="Workspace extensions">
    `\<workspace\>/.crabfork/<plugin-root>/*.ts` and `\<workspace\>/.crabfork/<plugin-root>/*/index.ts`.
  </Step>

  <Step title="Global extensions">
    `~/.crabfork/<plugin-root>/*.ts` and `~/.crabfork/<plugin-root>/*/index.ts`.
  </Step>

  <Step title="Bundled plugins">
    Shipped with Crabfork. Many are enabled by default (model providers, speech).
...
```bash
crabfork plugins list                       # compact inventory
crabfork plugins list --enabled            # only loaded plugins
crabfork plugins list --verbose            # per-plugin detail lines
crabfork plugins list --json               # machine-readable inventory
crabfork plugins inspect <id>              # deep detail
crabfork plugins inspect <id> --json       # machine-readable
crabfork plugins inspect --all             # fleet-wide table
crabfork plugins info <id>                 # inspect alias
crabfork plugins doctor                    # diagnostics

crabfork plugins install <package>         # install (ClawHub first, then npm)
crabfork plugins install clawhub:<pkg>     # install from ClawHub only
crabfork plugins install <spec> --force    # overwrite existing install
crabfork plugins install <path>            # install from local path
crabfork plugins install -l <path>         # link (no copy) for dev
crabfork plugins install <plugin> --marketplace <source>
crabfork plugins install <plugin> --marketplace https://github.com/<owner>/<repo>
crabfork plugins install <spec> --pin      # record exact resolved npm spec
crabfork plugins install <spec> --dangerously-force-unsafe-install
crabfork plugins update <id>             # update one plugin
crabfork plugins update <id> --dangerously-force-unsafe-install
crabfork plugins update --all            # update all
crabfork plugins uninstall <id>          # remove config/install records
crabfork plugins uninstall <id> --keep-files
crabfork plugins marketplace list <source>
crabfork plugins marketplace list <source> --json

crabfork plugins enable <id>
crabfork plugins disable <id>
```

Bundled plugins ship with Crabfork. Many are enabled by default (for example
bundled model providers, bundled speech providers, and the bundled browser
plugin). Other bundled plugins still need `crabfork plugins enable <id>`.
...
override instead, while `crabfork skills install` remains the separate ClawHub
...
`crabfork plugins inspect <id>` also reports detected bundle capabilities plus
...
See [`crabfork plugins` CLI reference](/cli/plugins) for full details.
...
Crabfork loads the entry object and calls `register(api)` during plugin
