...
Hooks are small scripts that run when something happens inside the Gateway. They are automatically discovered from directories and can be inspected with `crabfork hooks`.

There are two kinds of hooks in Crabfork:

- **Internal hooks** (this page): run inside the Gateway when agent events fire, like `/new`, `/reset`, `/stop`, or lifecycle events.
- **Webhooks**: external HTTP endpoints that let other systems trigger work in Crabfork. See [Webhooks](/automation/cron-jobs#webhooks).

Hooks can also be bundled inside plugins. `crabfork hooks list` shows both standalone hooks and plugin-managed hooks.
...
metadata:
  { "crabfork": { "emoji": "🔗", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
...
**Metadata fields** (`metadata.crabfork`):
...
1. **Bundled hooks**: shipped with Crabfork
2. **Plugin hooks**: hooks bundled inside installed plugins
3. **Managed hooks**: `~/.crabfork/hooks/` (user-installed, shared across workspaces). Extra directories from `hooks.internal.load.extraDirs` share this precedence.
...
Hook packs are npm packages that export hooks via `crabfork.hooks` in `package.json`. Install with:
...
| command-logger        | `command`                      | Logs all commands to `~/.crabfork/logs/commands.log`  |
| boot-md               | `gateway:startup`              | Runs `BOOT.md` when the gateway starts                |

Enable any bundled hook:

```bash
crabfork hooks enable <hook-name>
```
...
Logs every slash command to `~/.crabfork/logs/commands.log`.
...
# Enable/disable
crabfork hooks enable <hook-name>
crabfork hooks disable <hook-name>
```
...
1. Verify the hook is enabled: `crabfork hooks list`
