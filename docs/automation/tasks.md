...
- `crabfork tasks list` shows all tasks; `crabfork tasks audit` surfaces issues.
...
crabfork tasks list --runtime acp
crabfork tasks list --status running

# Show details for a specific task (by ID, run ID, or session key)
crabfork tasks show <lookup>

# Cancel a running task (kills the child session)
crabfork tasks cancel <lookup>

# Change notification policy for a task
crabfork tasks notify <lookup> state_changes

# Run a health audit
crabfork tasks audit

# Preview or apply maintenance
crabfork tasks maintenance
crabfork tasks maintenance --apply

# Inspect TaskFlow state
crabfork tasks flow list
...
| CLI operations         | `cli`        | `crabfork agent` commands that run through the gateway | `silent`              |
...
| `cancelled` | Stopped by the operator via `crabfork tasks cancel`                        |
...
When a task reaches a terminal state, Crabfork notifies you. There are two delivery paths:

**Direct delivery** — if the task has a channel target (the `requesterOrigin`), the completion message goes straight to that channel (Telegram, Discord, Slack, etc.). For subagent completions, Crabfork also preserves bound thread/topic routing when available and can fill a missing `to` / account from the requester session's stored route (`lastChannel` / `lastTo` / `lastAccountId`) before giving up on direct delivery.
...
```bash
crabfork tasks list [--runtime <acp|subagent|cron|cli>] [--status <status>] [--json]
```
...
```bash
crabfork tasks notify <lookup> <done_only|state_changes|silent>
```

### `tasks audit`

```bash
crabfork tasks audit [--json]
```

Surfaces operational issues. Findings also appear in `crabfork status` when issues are detected.
...
```bash
crabfork tasks maintenance [--json]
crabfork tasks maintenance --apply [--json]
```
...
```bash
crabfork tasks flow list [--status <status>] [--json]
crabfork tasks flow show <lookup> [--json]
crabfork tasks flow cancel <lookup>
```
...
For the full operator ledger, use the CLI: `crabfork tasks list`.

## Status integration (task pressure)

`crabfork status` includes an at-a-glance task summary:
...
```
$CRABFORK_STATE_DIR/tasks/runs.sqlite
```
...
[Task Flow](/automation/taskflow) is the flow orchestration layer above background tasks. A single flow may coordinate multiple tasks over its lifetime using managed or mirrored sync modes. Use `crabfork tasks` to inspect individual task records and `crabfork tasks flow` to inspect the orchestrating flow.

See [Task Flow](/automation/taskflow) for details.

### Tasks and cron

A cron job **definition** lives in `~/.crabfork/cron/jobs.json`. **Every** cron execution creates a task record — both main-session and isolated. Main-session cron tasks default to `silent` notify policy so they track without generating notifications.
