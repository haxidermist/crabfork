---
title: "Kilocode"
summary: "Use Kilo Gateway's unified API to access many models in Crabfork"
read_when:
  - You want a single API key for many LLMs
  - You want to run models via Kilo Gateway in Crabfork
---
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    crabfork onboard --auth-choice kilocode-api-key
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    crabfork models list --provider kilocode
    ```
  </Step>
</Steps>
...
<Note>
Crabfork treats `kilocode/kilo/auto` as the stable default ref, but does not
...
hard-coded in Crabfork.
</Note>

## Available models

Crabfork dynamically discovers available models from the Kilo Gateway at startup. Use
...
<Tip>
At startup, Crabfork queries `GET https://api.kilo.ai/api/gateway/models` and merges
...
<AccordionGroup>
  <Accordion title="Transport and compatibility">
    ...
    - Gemini-backed Kilo refs stay on the proxy-Gemini path, so Crabfork keeps
  ...
  <Accordion title="Troubleshooting">
    - If model discovery fails at startup, Crabfork falls back to the bundled static catalog containing `kilocode/kilo/auto`.
    - Confirm your API key is valid and that your Kilo account has the desired models enabled.
    - When the Gateway runs as a daemon, ensure `KILOCODE_API_KEY` is available to that process (for example in `~/.crabfork/.env` or via `env.shellEnv`).
  </Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
  ...
  <Card title="Configuration reference" href="/gateway/configuration" icon="gear">
    Full Crabfork configuration reference.
  </Card>
