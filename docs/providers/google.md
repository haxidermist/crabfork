...
read_when:
  - You want to use Google Gemini models with Crabfork
...
<Tabs>
    ...
    **Best for:** standard Gemini API access through Google AI Studio.
    ...
    <Steps>
      <Step title="Run onboarding">
        ```bash
        crabfork onboard --auth-choice gemini-api-key
        ```
      ...
      <Step title="Verify the model is available">
        ```bash
        crabfork models list --provider google
        ```
      </Step>
    </Steps>
  ...
  <Tab title="Gemini CLI (OAuth)">
    **Best for:** reusing an existing Gemini CLI login via PKCE OAuth instead of a separate API key.
    ...
    <Steps>
      <Step title="Install the Gemini CLI">
        ...
        Crabfork supports both Homebrew installs and global npm installs, including
        common Windows/npm layouts.
      </Step>
      <Step title="Log in via OAuth">
        ```bash
        crabfork models auth login --provider google-gemini-cli --set-default
        ```
      </Step>
      <Step title="Verify the model is available">
        ```bash
        crabfork models list --provider google-gemini-cli
        ```
      </Step>
    </Steps>
    ...
    **Environment variables:**
    ...
    - `CRABFORK_GEMINI_OAUTH_CLIENT_ID`
    - `CRABFORK_GEMINI_OAUTH_CLIENT_SECRET`
...
<Tip>
Gemma 4 models (for example `gemma-4-26b-a4b-it`) support thinking mode. Crabfork
...
<AccordionGroup>
  <Accordion title="Direct Gemini cache reuse">
    For direct Gemini API runs (`api: "google-generative-ai"`), Crabfork
    ...
    - Gemini cache-hit usage is normalized into Crabfork `cacheRead` from
  ...
  <Accordion title="Gemini CLI JSON usage notes">
    When using the `google-gemini-cli` OAuth provider, Crabfork normalizes
  ...
  <Accordion title="Environment and daemon setup">
    ...
    is available to that process (for example, in `~/.crabfork/.env` or via
    `env.shellEnv`).
  </Accordion>
</AccordionGroup>
