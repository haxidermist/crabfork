---
summary: "Use Synthetic's Anthropic-compatible API in Crabfork"
read_when:
...
Crabfork registers it as the `synthetic` provider and uses the Anthropic
Messages API.
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    crabfork onboard --auth-choice synthetic-api-key
    ```
  </Step>
...
<Warning>
Crabfork's Anthropic client appends `/v1` to the base URL automatically, so use
...
`crabfork models list --provider synthetic` to see all models available on your
account.
</Tip>
...
<AccordionGroup>
  ...
  <Accordion title="Base URL override">
    ...
    Remember that Crabfork appends `/v1` automatically.
