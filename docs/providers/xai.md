---
summary: "Use xAI Grok models in Crabfork"
read_when:
  - You want to use Grok models in Crabfork
...
Crabfork ships a bundled `xai` provider plugin for Grok models.
...
<Steps>
  ...
  <Step title="Set your API key">
    ...
    ```bash
    crabfork onboard --auth-choice xai-api-key
    ```
...
<Note>
Crabfork uses the xAI Responses API as the bundled xAI transport. The same
...
Crabfork includes these xAI model families out of the box:
...
<AccordionGroup>
  <Accordion title="Web search">
    ...
    ```bash
    crabfork config set tools.web.search.provider grok
    ```
  ...
  <Accordion title="x_search configuration">
    The bundled xAI plugin exposes `x_search` as an Crabfork tool for searching
  ...
  <Accordion title="Code execution configuration">
    The bundled xAI plugin exposes `code_execution` as an Crabfork tool for
  ...
  <Accordion title="Known limits">
    ...
    - `grok-4.20-multi-agent-experimental-beta-0304` is not supported on the
      ...
      surface than the standard Crabfork xAI transport.
  </Accordion>

  <Accordion title="Advanced notes">
    - Crabfork applies xAI-specific tool-schema and tool-call compatibility fixes
    ...
    - `web_search`, `x_search`, and `code_execution` are exposed as Crabfork
      tools. Crabfork enables the specific xAI built-in it needs inside each tool
