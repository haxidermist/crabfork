...
read_when:
  - You want to use Vercel AI Gateway with Crabfork
...
<Tip>
Crabfork auto-discovers the Gateway `/v1/models` catalog, so
...
<Steps>
  <Step title="Set the API key">
    ...
    ```bash
    crabfork onboard --auth-choice ai-gateway-api-key
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    crabfork models list --provider vercel-ai-gateway
    ```
  </Step>
</Steps>
...
Crabfork accepts Vercel Claude shorthand model refs and normalizes them at
runtime:
...
configuration. Crabfork resolves the canonical form automatically.
</Tip>
...
<AccordionGroup>
  <Accordion title="Environment variable for daemon processes">
    If the Crabfork Gateway runs as a daemon (launchd/systemd), make sure
    ...
    `~/.crabfork/.env` or via `env.shellEnv` to ensure the gateway process can
    read it.
    </Warning>
