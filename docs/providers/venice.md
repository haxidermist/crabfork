---
summary: "Use Venice AI privacy-focused models in Crabfork"
read_when:
  - You want privacy-focused inference in Crabfork
...
<Steps>
  ...
  <Step title="Configure Crabfork">
    ...
    <Tabs>
      <Tab title="Interactive (recommended)">
        ```bash
        crabfork onboard --auth-choice venice-api-key
        ```
  ...
  <Step title="Verify setup">
    ```bash
    crabfork agent --model venice/kimi-k2-5 --message "Hello, are you working?"
    ```
  </Step>
</Steps>
...
After setup, Crabfork shows all available Venice models. Pick based on your needs:
...
```bash
crabfork models set venice/kimi-k2-5
crabfork models set venice/claude-opus-4-6
```

List all available models:

```bash
crabfork models list | grep venice
```

You can also run `crabfork configure`, select **Model/auth**, and choose **Venice AI**.
...
Crabfork automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
crabfork agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
crabfork agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
crabfork agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
crabfork agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
crabfork agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
<AccordionGroup>
  ...
  <Accordion title="Model not available">
    The Venice model catalog updates dynamically. Run `crabfork models list` to see currently available models. Some models may be temporarily offline.
  </Accordion>
