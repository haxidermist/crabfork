---
summary: "Run Crabfork with SGLang (OpenAI-compatible self-hosted server)"
read_when:
  - You want to run Crabfork against a local SGLang server
...
Crabfork can connect to SGLang using the `openai-completions` API.

Crabfork can also **auto-discover** available models from SGLang when you opt
...
define `models.providers.sglang`, Crabfork will query:
