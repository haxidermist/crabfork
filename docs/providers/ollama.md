---
summary: "Run Crabfork with Ollama (cloud and local models)"
read_when:
  - You want to run Crabfork with cloud or local models via Ollama
...
Ollama is a local LLM runtime that makes it easy to run open-source models on your machine. Crabfork integrates with Ollama's native API (`/api/chat`), supports streaming and tool calling, and can auto-discover local Ollama models when you opt in with `OLLAMA_API_KEY` (or an auth profile) and do not define an explicit `models.providers.ollama` entry.

<Warning>
**Remote Ollama users**: Do not use the `/v1` OpenAI-compatible URL (`http://host:11434/v1`) with Crabfork. This breaks tool calling and models may output raw tool JSON as plain text. Use the native Ollama API URL instead: `baseUrl: "http://host:11434"` (no `/v1`).
</Warning>
    ...
    **Best for:** fastest path to a working Ollama setup with automatic model discovery.
    ...
    <Steps>
      ...
      <Step title="Verify the model is available">
        ```bash
        crabfork models list --provider ollama
        ```
      </Step>
    </Steps>
    ...
    ```bash
    crabfork onboard --non-interactive \
      --auth-choice ollama \
      --accept-risk
    ```
  ...
  <Tab title="Manual setup">
    **Best for:** full control over installation, model pulls, and config.
    ...
    <Steps>
      ...
      <Step title="Enable Ollama for Crabfork">
        ...
        crabfork config set models.providers.ollama.apiKey "ollama-local"
        ```
      </Step>
      <Step title="Inspect and set your model">
        ```bash
        crabfork models list
        crabfork models set ollama/gemma4
        ```
...
<Tabs>
  <Tab title="Cloud + Local">
    ...
    Crabfork currently suggests these cloud defaults: `kimi-k2.5:cloud`, `minimax-m2.7:cloud`, `glm-5.1:cloud`.

  </Tab>

  <Tab title="Local only">
    In local-only mode, Crabfork discovers models from the local Ollama instance. No cloud sign-in is needed.

    Crabfork currently suggests `gemma4` as the local default.
...
When you set `OLLAMA_API_KEY` (or an auth profile) and **do not** define `models.providers.ollama`, Crabfork discovers models from the local Ollama instance at `http://127.0.0.1:11434`.
...
| Vision models        | Models with a `vision` capability reported by `/api/show` are marked as image-capable (`input: ["text", "image"]`), so Crabfork auto-injects images into the prompt |
| Reasoning detection  | Marks `reasoning` with a model-name heuristic (`r1`, `reasoning`, `think`)                                                                                          |
| Token limits         | Sets `maxTokens` to the default Ollama max-token cap used by Crabfork                                                                                               |
...
<Tabs>
  <Tab title="Basic (implicit discovery)">
    ...
    <Tip>
    If `OLLAMA_API_KEY` is set, you can omit `apiKey` in the provider entry and Crabfork will fill it for availability checks.
    </Tip>
...
Crabfork supports **Ollama Web Search** as a bundled `web_search` provider.
...
Choose **Ollama Web Search** during `crabfork onboard` or `crabfork configure --section web`, or set:
...
<AccordionGroup>
  <Accordion title="Legacy OpenAI-compatible mode">
    ...
    **Tool calling is not reliable in OpenAI-compatible mode.** Use this mode only if you need OpenAI format for a proxy and do not depend on native tool calling behavior.
    ...
    When `api: "openai-completions"` is used with Ollama, Crabfork injects `options.num_ctx` by default so Ollama does not silently fall back to a 4096 context window. If your proxy/upstream rejects unknown `options` fields, disable this behavior:
  ...
  <Accordion title="Context windows">
    For auto-discovered models, Crabfork uses the context window reported by Ollama when available, otherwise it falls back to the default Ollama context window used by Crabfork.
  ...
  <Accordion title="Reasoning models">
    Crabfork treats models with names such as `deepseek-r1`, `reasoning`, or `think` as reasoning-capable by default.

    ```bash
    ollama pull deepseek-r1:32b
    ```

    No additional configuration is needed -- Crabfork marks them automatically.
  ...
  <Accordion title="Streaming configuration">
    Crabfork's Ollama integration uses the **native Ollama API** (`/api/chat`) by default, which fully supports streaming and tool calling simultaneously. No special configuration is needed.
