---
summary: "Run Crabfork through inferrs (OpenAI-compatible local server)"
read_when:
  - You want to run Crabfork against a local inferrs server
  - You are serving Gemma or another model through inferrs
  - You need the exact Crabfork compat flags for inferrs
title: "inferrs"
---
...
OpenAI-compatible `/v1` API. Crabfork works with `inferrs` through the generic
`openai-completions` path.

`inferrs` is currently best treated as a custom self-hosted OpenAI-compatible
backend, not a dedicated Crabfork provider plugin.
...
<Steps>
  ...
  <Step title="Add an Crabfork provider entry">
...
<AccordionGroup>
  <Accordion title="Why requiresStringContent matters">
    ...
    <Warning>
    If Crabfork runs fail with an error like:
    ...
    Crabfork will flatten pure text content parts into plain strings before sending
    the request.
  ...
  <Accordion title="Gemma and tool-schema caveat">
    ...
    `/v1/chat/completions` requests but still fail on full Crabfork agent-runtime
    turns.
    ...
    That disables Crabfork's tool schema surface for the model and can reduce prompt
    pressure on stricter local backends.

    If tiny direct requests still work but normal Crabfork agent turns continue to
    crash inside `inferrs`, the remaining issue is usually upstream model/server
    behavior rather than Crabfork's transport layer.
...
<AccordionGroup>
  ...
  <Accordion title="Direct /v1/chat/completions calls pass but crabfork infer model run fails">
  ...
  <Accordion title="inferrs still crashes on larger agent turns">
    If Crabfork no longer gets schema errors but `inferrs` still crashes on larger
...
<CardGroup cols={2}>
  <Card title="Local models" href="/gateway/local-models" icon="server">
    Running Crabfork against local model servers.
  </Card>
