---
summary: "GLM model family overview + how to use it in Crabfork"
read_when:
  - You want GLM models in Crabfork
...
GLM is a **model family** (not a company) available through the Z.AI platform. In Crabfork, GLM
...
<Steps>
  ...
  <Step title="Set GLM as the default model">
    ```bash
    crabfork config set agents.defaults.model.primary "zai/glm-5.1"
    ```
  </Step>
...
<Tip>
`zai-api-key` lets Crabfork detect the matching Z.AI endpoint from the key and
...
Crabfork currently seeds the bundled `zai` provider with these GLM refs:
...
<AccordionGroup>
  <Accordion title="Endpoint auto-detection">
    When you use the `zai-api-key` auth choice, Crabfork inspects the key format
