---
summary: "Use Mistral models and Voxtral transcription with Crabfork"
read_when:
  - You want to use Mistral models in Crabfork
...
Crabfork supports Mistral for both text/image model routing (`mistral/...`) and
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    crabfork onboard --auth-choice mistral-api-key
    ```

    Or pass the key directly:

    ```bash
    crabfork onboard --mistral-api-key "$MISTRAL_API_KEY"
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    crabfork models list --provider mistral
    ```
  </Step>
</Steps>
...
Crabfork currently ships this bundled Mistral catalog:
...
<AccordionGroup>
  <Accordion title="Adjustable reasoning (mistral-small-latest)">
    ...
    Crabfork maps the session **thinking** level to Mistral's API:

    | Crabfork thinking level                          | Mistral `reasoning_effort` |
    | ------------------------------------------------ | -------------------------- |
    | **off** / **minimal**                            | `none`                     |
