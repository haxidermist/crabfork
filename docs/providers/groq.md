...
read_when:
  - You want to use Groq with Crabfork
...
(Llama, Gemma, Mistral, and more) using custom LPU hardware. Crabfork connects
...
Groq's model catalog changes frequently. Run `crabfork models list | grep groq`
...
<Tip>
Use `crabfork models list --provider groq` for the most up-to-date list of
...
media-understanding provider, Crabfork uses Groq's `whisper-large-v3-turbo`
...
<AccordionGroup>
  ...
  <Accordion title="Environment note">
    ...
    gateway processes. Use `~/.crabfork/.env` or `env.shellEnv` config for
