---
summary: "Sign in to GitHub Copilot from Crabfork using the device flow"
read_when:
  - You want to use GitHub Copilot as a model provider
  - You need the `crabfork models auth login-github-copilot` flow
...
models for your GitHub account and plan. Crabfork can use Copilot as a model
provider in two different ways.

## Two ways to use Copilot in Crabfork

<Tabs>
  <Tab title="Built-in provider (github-copilot)">
    Use the native device-login flow to obtain a GitHub token, then exchange it for
    Copilot API tokens when Crabfork runs. This is the **default** and simplest path
    because it does not require VS Code.

    <Steps>
      <Step title="Run the login command">
        ```bash
        crabfork models auth login-github-copilot
        ```
      ...
      <Step title="Set a default model">
        ```bash
        crabfork models set github-copilot/gpt-4o
        ```
  ...
  <Tab title="Copilot Proxy plugin (copilot-proxy)">
    Use the **Copilot Proxy** VS Code extension as a local bridge. Crabfork talks to
...
```bash
# Skip confirmation
crabfork models auth login-github-copilot --yes

# Login and set the default model in one step
crabfork models auth login --provider github-copilot --method device --set-default
```
...
<AccordionGroup>
  ...
  <Accordion title="Transport selection">
    ...
    o-series, and Gemini models keep the OpenAI Responses transport. Crabfork
    selects the correct transport based on the model ref.
  </Accordion>

  <Accordion title="Environment variable resolution order">
    Crabfork resolves Copilot auth from environment variables in the following
    priority order:
    ...
    When multiple variables are set, Crabfork uses the highest-priority one.
    The device-login flow (`crabfork models auth login-github-copilot`) stores
  ...
  <Accordion title="Token storage">
    ...
    for a Copilot API token when Crabfork runs. You do not need to manage the
    token manually.
  </Accordion>
</AccordionGroup>
