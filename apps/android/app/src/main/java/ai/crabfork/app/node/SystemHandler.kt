...
private const val NOTIFICATION_CHANNEL_BASE_ID = "crabfork.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "Crabfork Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "Crabfork Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "Crabfork Active")
      }
