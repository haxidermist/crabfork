...
internal fun crabForkAndroidVersionLabel(): String {
...
internal fun buildGatewayDiagnosticsReport(
  ...
  return """
    Help diagnose this Crabfork Android gateway connection failure.
    ...
    - tell me whether `crabfork devices list` should show a pending pairing request
    - if more signal is needed, ask for `crabfork qr --json`, `crabfork devices list`, and `crabfork nodes status`
    - give the next exact command or tap

    Debug info:
    - screen: $screen
    - app version: ${crabForkAndroidVersionLabel()}
    - device: $device
...
internal fun copyGatewayDiagnosticsReport(
  ...
  clipboard.setPrimaryClip(ClipData.newPlainText("Crabfork gateway diagnostics", report))
