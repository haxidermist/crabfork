...
import "@a2ui/lit/ui";
import { themeContext } from "@crabfork/a2ui-theme-context";
...
class CrabforkA2UIHost extends LitElement {
  static properties = {
    surfaces: { state: true },
  ...
  connectedCallback() {
    ...
    globalThis.crabforkA2UI = api;
    this.addEventListener("a2uiaction", (evt) => this.#handleA2UIAction(evt));
    this.#statusListener = (evt) => this.#handleActionStatus(evt);
    for (const eventName of ["crabfork:a2ui-action-status"]) {
  ...
  #handleA2UIAction(evt) {
    ...
    const handler =
      globalThis.webkit?.messageHandlers?.crabforkCanvasA2UIAction ??
      globalThis.crabforkCanvasA2UIAction;
    if (handler?.postMessage) {
      try {
        // WebKit message handlers support structured objects; Android's JS interface expects strings.
        if (handler === globalThis.crabforkCanvasA2UIAction) {
...
if (!customElements.get("crabfork-a2ui-host")) {
  customElements.define("crabfork-a2ui-host", CrabforkA2UIHost);
}
