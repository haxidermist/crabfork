...
import Foundation
import Testing
@testable import CrabforkChatUI
...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> CrabforkChatHistoryPayload
{
    CrabforkChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> CrabforkChatSessionEntry {
    CrabforkChatSessionEntry(
        key: key,
        kind: nil,
        displayName: nil,
        surface: nil,
...
private func sessionEntry(
    ...
    updatedAt: Double,
    model: String?,
    modelProvider: String? = nil) -> CrabforkChatSessionEntry
{
    CrabforkChatSessionEntry(
        key: key,
        kind: nil,
...
private func modelChoice(id: String, name: String, provider: String = "anthropic") -> CrabforkChatModelChoice {
    CrabforkChatModelChoice(modelID: id, name: name, provider: provider, contextWindow: nil)
}

private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [CrabforkChatHistoryPayload],
    sessionsResponses: [CrabforkChatSessionsListResponse] = [],
    modelResponses: [[CrabforkChatModelChoice]] = [],
    ...
    -> (TestChatTransport, CrabforkChatViewModel)
{
...
private func sendUserMessage(_ vm: CrabforkChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, CrabforkChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [CrabforkChatHistoryPayload]
    private let sessionsResponses: [CrabforkChatSessionsListResponse]
    private let modelResponses: [[CrabforkChatModelChoice]]
    ...
    private let stream: AsyncStream<CrabforkChatTransportEvent>
    private let continuation: AsyncStream<CrabforkChatTransportEvent>.Continuation
        ...
        setSessionThinkingHook: (@Sendable (String) async throws -> Void)? = nil)
    {
        ...
        var cont: AsyncStream<CrabforkChatTransportEvent>.Continuation!
    ...
    func events() -> AsyncStream<CrabforkChatTransportEvent> {
        self.stream
    }

    func setActiveSessionKey(_: String) async throws {}

    func requestHistory(sessionKey: String) async throws -> CrabforkChatHistoryPayload {
        ...
        return self.historyResponses.last ?? CrabforkChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
    ...
    func sendMessage(
        ...
        attachments _: [CrabforkChatAttachmentPayload]) async throws -> CrabforkChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        await self.state.sentThinkingLevelsAppend(thinking)
        return CrabforkChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> CrabforkChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? CrabforkChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func listModels() async throws -> [CrabforkChatModelChoice] {
    ...
    func emit(_ evt: CrabforkChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let sessions = CrabforkChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesUseResolvedMainSessionKeyInsteadOfLiteralMain() async throws {
        ...
        let sessions = CrabforkChatSessionsListResponse(
            ...
            path: nil,
            count: 2,
            defaults: CrabforkChatSessionsDefaults(
                model: nil,
                contextTokens: nil,
    ...
    @Test func bootstrapsModelSelectionFromSessionAndDefaults() async throws {
        ...
        let sessions = CrabforkChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: CrabforkChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func selectingDefaultModelPatchesNilAndUpdatesSelection() async throws {
        ...
        await MainActor.run { vm.selectModel(CrabforkChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == CrabforkChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func selectingProviderQualifiedModelDisambiguatesDuplicateModelIDs() async throws {
        ...
        let sessions = CrabforkChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: CrabforkChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func lateModelCompletionDoesNotReplayCurrentSessionSelectionIntoPreviousSession() async throws {
        let now = Date().timeIntervalSince1970 * 1000
        let initialSessions = CrabforkChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
        ...
        let sessionsAfterOtherSelection = CrabforkChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
    ...
    @Test func explicitThinkingLevelWinsOverHistoryAndPersistsChanges() async throws {
        let history = CrabforkChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [],
            thinkingLevel: "off")
...
"""]],
        ...
        let vm = await MainActor.run { CrabforkChatViewModel(sessionKey: "main", transport: transport) }
