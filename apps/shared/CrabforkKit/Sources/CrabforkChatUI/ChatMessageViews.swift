...
struct ChatMessageBubble: View {
    let message: CrabforkChatMessage
    let style: CrabforkChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? CrabforkChatTheme.userText : CrabforkChatTheme.assistantText
    ...
    private var inlineAttachments: [CrabforkChatMessageContent] {
    ...
    private var toolCalls: [CrabforkChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [CrabforkChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? CrabforkChatTheme.userBubble
        }
        if self.style == .onboarding {
            return CrabforkChatTheme.onboardingAssistantBubble
        }
        return CrabforkChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return CrabforkChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: CrabforkChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? CrabforkChatTheme.userText : CrabforkChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: CrabforkChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(CrabforkChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [CrabforkChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: CrabforkChatTheme.assistantText)
            }
        }
    }
}
