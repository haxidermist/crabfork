...
private let chatUILogger = Logger(subsystem: "ai.crabfork", category: "CrabforkChatUI")

@MainActor
@Observable
public final class CrabforkChatViewModel {
    public static let defaultModelSelectionID = "__default__"

    public private(set) var messages: [CrabforkChatMessage] = []
    public var input: String = ""
    public private(set) var thinkingLevel: String
    public private(set) var modelSelectionID: String = "__default__"
    public private(set) var modelChoices: [CrabforkChatModelChoice] = []
    public private(set) var isLoading = false
    public private(set) var isSending = false
    public private(set) var isAborting = false
    public var errorText: String?
    public var attachments: [CrabforkPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [CrabforkChatPendingToolCall] = []
    public private(set) var sessions: [CrabforkChatSessionEntry] = []
    private let transport: any CrabforkChatTransport
    private var sessionDefaults: CrabforkChatSessionsDefaults?
    ...
    private var pendingToolCallsById: [String: CrabforkChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        sessionKey: String,
        transport: any CrabforkChatTransport,
    ...
    public var sessionChoices: [CrabforkChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        let mainSessionKey = self.resolvedMainSessionKey

        var result: [CrabforkChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: CrabforkPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [CrabforkChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: CrabforkChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: CrabforkChatMessage) -> CrabforkChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> CrabforkChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return CrabforkChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageContentFingerprint(for message: CrabforkChatMessage) -> String {
    ...
    private static func messageIdentityKey(for message: CrabforkChatMessage) -> String? {
    ...
    private static func userRefreshIdentityKey(for message: CrabforkChatMessage) -> String? {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    ...
    private static func reconcileMessageIDs(
        previous: [CrabforkChatMessage],
        incoming: [CrabforkChatMessage]) -> [CrabforkChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [CrabforkChatMessage]) -> [CrabforkChatMessage] {
        var result: [CrabforkChatMessage] = []
    ...
    private static func dedupeKey(for message: CrabforkChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [CrabforkChatMessageContent] = [
            CrabforkChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> CrabforkChatAttachmentPayload in
            CrabforkChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> CrabforkChatSessionEntry {
        ...
        syncSelection: Bool)
    {
        if let index = self.sessions.firstIndex(where: { $0.key == sessionKey }) {
            ...
            self.sessions[index] = CrabforkChatSessionEntry(
                key: current.key,
                kind: current.kind,
    ...
    private func handleTransportEvent(_ evt: CrabforkChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: CrabforkChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: CrabforkAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = CrabforkChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> CrabforkPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
