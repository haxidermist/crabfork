...
@MainActor
public struct CrabforkChatView: View {
    public enum Style {
        case standard
        case onboarding
    }

    @State private var viewModel: CrabforkChatViewModel
    ...
    public init(
        viewModel: CrabforkChatViewModel,
    ...
    private var visibleMessages: [CrabforkChatMessage] {
        let base: [CrabforkChatMessage]
    ...
    private func mergeToolResults(in messages: [CrabforkChatMessage]) -> [CrabforkChatMessage] {
        ...
        for message in messages {
            ...
            let merged = CrabforkChatMessage(
                id: last.id,
                role: last.role,
                content: content,
    ...
    private func isToolResultMessage(_ message: CrabforkChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: CrabforkChatMessage) -> Bool {
    ...
    private func primaryText(in message: CrabforkChatMessage) -> String {
    ...
    private func hasInlineAttachments(in message: CrabforkChatMessage) -> Bool {
    ...
    private func toolCalls(in message: CrabforkChatMessage) -> [CrabforkChatMessageContent] {
    ...
    private func inlineToolResults(in message: CrabforkChatMessage) -> [CrabforkChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: CrabforkChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: CrabforkChatMessage) -> String {
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 18, style: .continuous)
                .fill(CrabforkChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 18, style: .continuous)
