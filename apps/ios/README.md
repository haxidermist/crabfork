# Crabfork iOS (Super Alpha)

This iPhone app is super-alpha and internal-use only. It connects to an Crabfork Gateway as a `role: node`.
...
- Beta release uses canonical `ai.crabfork.client*` bundle IDs through a temporary generated xcconfig in `apps/ios/build/BetaRelease.xcconfig`.
- Beta release also switches the app to `CrabforkPushTransport=relay`, `CrabforkPushDistribution=official`, and `CrabforkPushAPNsEnvironment=production`.
...
- `CRABFORK_PUSH_RELAY_BASE_URL=https://relay.example.com`
...
```bash
export CRABFORK_PUSH_RELAY_BASE_URL=https://relay.example.com
```
...
7. Expected outputs after a successful run:
   - `apps/ios/build/beta/Crabfork-<version>.ipa`
   - `apps/ios/build/beta/Crabfork-<version>.app.dSYM.zip`
...
- `apps/ios/Sources/Crabfork.entitlements` sets `aps-environment` to `development`.
- APNs token registration to gateway happens only after gateway connection (`push.apns.register`).
- Local/manual builds default to `CrabforkPushTransport=direct` and `CrabforkPushDistribution=local`.
- Your selected team/profile must support Push Notifications for the app bundle ID you are signing.
- If push capability or provisioning is wrong, APNs registration fails at runtime (check Xcode logs for `APNs registration failed`).
- The gateway host also needs direct APNs auth configured separately with `CRABFORK_APNS_TEAM_ID`, `CRABFORK_APNS_KEY_ID`, and either `CRABFORK_APNS_PRIVATE_KEY_P8` or `CRABFORK_APNS_PRIVATE_KEY_PATH`.
- Recommended gateway-host storage for the APNs `.p8` file is `~/.crabfork/credentials/apns/AuthKey_<KEYID>.p8` with restrictive permissions, then point `CRABFORK_APNS_PRIVATE_KEY_PATH` at that file.
...
- Debug builds default to `CrabforkPushAPNsEnvironment=sandbox`; Release builds default to `production`.
...
- Gateway-side relay sending is configured through `gateway.push.apns.relay.baseUrl` in `crabfork.json`. `CRABFORK_APNS_RELAY_BASE_URL` remains a temporary env override only.
...
This exists to keep the hosted relay limited to genuine Crabfork official builds and to ensure a
