...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__CRABFORK_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<CrabforkTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.crabfork", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = CrabforkTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> CrabforkTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return CrabforkTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> CrabforkTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> CrabforkTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return CrabforkTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> CrabforkTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = CrabforkTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: CrabforkTalkPTTStopPayload) {
