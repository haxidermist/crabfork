...
final class RemindersService: RemindersServicing {
    func list(params: CrabforkRemindersListParams) async throws -> CrabforkRemindersListPayload {
        ...
        let payload: [CrabforkReminderPayload] = try await withCheckedThrowingContinuation { cont in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return CrabforkReminderPayload(
        ...
        return CrabforkRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: CrabforkRemindersAddParams) async throws -> CrabforkRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = CrabforkReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return CrabforkRemindersAddPayload(reminder: payload)
    }
