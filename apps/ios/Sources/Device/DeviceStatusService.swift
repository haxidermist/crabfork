...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> CrabforkDeviceStatusPayload {
        ...
        return CrabforkDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    func info() -> CrabforkDeviceInfoPayload {
        ...
        return CrabforkDeviceInfoPayload(
    ...
    private func batteryStatus() -> CrabforkBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: CrabforkBatteryState = switch device.batteryState {
        ...
        return CrabforkBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> CrabforkThermalStatusPayload {
        let state: CrabforkThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return CrabforkThermalStatusPayload(state: state)
    }

    private func storageStatus() -> CrabforkStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return CrabforkStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
