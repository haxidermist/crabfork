...
enum ExecApprovalNotificationBridge {
    ...
    static let categoryIdentifier = "crabfork.exec-approval"
    static let reviewActionIdentifier = "crabfork.exec-approval.review"
    ...
    static func approvalID(from userInfo: [AnyHashable: Any]) -> String? {
        let raw = self.crabForkPayload(userInfo: userInfo)?["approvalId"] as? String
    ...
    static func payloadKind(userInfo: [AnyHashable: Any]) -> String {
        let raw = self.crabForkPayload(userInfo: userInfo)?["kind"] as? String
        let trimmed = raw?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        return trimmed.isEmpty ? "unknown" : trimmed
    }

    private static func crabForkPayload(userInfo: [AnyHashable: Any]) -> [String: Any]? {
