...
final class ContactsService: ContactsServicing {
    ...
    func search(params: CrabforkContactsSearchParams) async throws -> CrabforkContactsSearchPayload {
        ...
        return CrabforkContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: CrabforkContactsAddParams) async throws -> CrabforkContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return CrabforkContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return CrabforkContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> CrabforkContactPayload {
        CrabforkContactPayload(
