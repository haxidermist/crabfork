...
final class MotionService: MotionServicing {
    func activities(params: CrabforkMotionActivityParams) async throws -> CrabforkMotionActivityPayload {
        ...
        let mapped: [CrabforkMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return CrabforkMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: CrabforkPedometerParams) async throws -> CrabforkPedometerPayload {
        ...
        let payload: CrabforkPedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = CrabforkPedometerPayload(
