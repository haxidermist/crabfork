...
@MainActor
enum CrabforkAppModelRegistry {
    static var appModel: NodeAppModel?
}

@MainActor
final class CrabforkAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.crabfork.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.crabfork.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.crabfork.ios.bgrefresh"
    ...
    private func resolvedAppModel() -> NodeAppModel? {
        self.appModel ?? CrabforkAppModelRegistry.appModel
    }
...
enum WatchPromptNotificationBridge {
    static let typeKey = "crabfork.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "crabfork.watch.promptId"
    static let sessionKeyKey = "crabfork.watch.sessionKey"
    static let actionPrimaryIDKey = "crabfork.watch.action.primary.id"
    static let actionPrimaryLabelKey = "crabfork.watch.action.primary.label"
    static let actionSecondaryIDKey = "crabfork.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "crabfork.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "crabfork.watch.action.primary"
    static let actionSecondaryIdentifier = "crabfork.watch.action.secondary"
    static let actionIdentifierPrefix = "crabfork.watch.action."
    static let actionIDKeyPrefix = "crabfork.watch.action.id."
    static let actionLabelKeyPrefix = "crabfork.watch.action.label."
    static let categoryPrefix = "crabfork.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: CrabforkWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> CrabforkWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return CrabforkWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "Crabfork" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [CrabforkWatchAction]) -> [UNNotificationAction] {
...
@main
struct CrabforkApp: App {
    @State private var appModel: NodeAppModel
    @State private var gatewayController: GatewayConnectionController
    @UIApplicationDelegateAdaptor(CrabforkAppDelegate.self) private var appDelegate
    ...
    init() {
        ...
        CrabforkAppModelRegistry.appModel = appModel
...
extension CrabforkApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("Crabfork: installing uncaught exception handler")
