...
extension NodeAppModel {
    static func normalizeWatchNotifyParams(_ params: CrabforkWatchNotifyParams) -> CrabforkWatchNotifyParams {
    ...
    static func normalizeWatchActions(
        _ actions: [CrabforkWatchAction]?,
        kind: String?,
        promptId: String?) -> [CrabforkWatchAction]
    {
        let provided = (actions ?? []).compactMap { action -> CrabforkWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return CrabforkWatchAction(
                id: id,
                label: label,
        ...
        if normalizedKind.contains("approval") || normalizedKind.contains("approve") {
            return [
                CrabforkWatchAction(id: "approve", label: "Approve"),
                CrabforkWatchAction(id: "decline", label: "Decline", style: "destructive"),
                CrabforkWatchAction(id: "open_phone", label: "Open iPhone"),
                CrabforkWatchAction(id: "escalate", label: "Escalate"),
            ]
        }

        return [
            CrabforkWatchAction(id: "done", label: "Done"),
            CrabforkWatchAction(id: "snooze_10m", label: "Snooze 10m"),
            CrabforkWatchAction(id: "open_phone", label: "Open iPhone"),
            CrabforkWatchAction(id: "escalate", label: "Escalate"),
        ]
    }
    ...
    static func normalizedWatchRisk(
        _ risk: CrabforkWatchRisk?,
        priority: CrabforkNotificationPriority?) -> CrabforkWatchRisk?
    {
    ...
    static func normalizedWatchPriority(
        _ priority: CrabforkNotificationPriority?,
        risk: CrabforkWatchRisk?) -> CrabforkNotificationPriority?
    {
