...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.crabfork/crabfork.json. Prefer CRABFORK_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.crabfork/crabfork.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.crabfork/crabfork.json. Restart the gateway to apply."
        }
