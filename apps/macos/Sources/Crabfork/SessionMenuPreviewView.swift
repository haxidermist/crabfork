...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.crabfork", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> CrabforkSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: CrabforkSessionPreviewEntry,
    ...
    private static func cache(payload: CrabforkSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: CrabforkChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [CrabforkKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [CrabforkKit.AnyCodable]) -> [CrabforkChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(CrabforkChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: CrabforkChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: CrabforkChatMessage) -> Bool {
    ...
    private static func toolNames(for message: CrabforkChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: CrabforkChatMessage) -> String? {
