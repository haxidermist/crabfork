...
private let webChatSwiftLogger = Logger(subsystem: "ai.crabfork", category: "WebChatSwiftUI")
private let webChatThinkingLevelDefaultsKey = "crabfork.webchat.thinkingLevel"
...
struct MacGatewayChatTransport: CrabforkChatTransport {
    ...
    func listSessions(limit: Int?) async throws -> CrabforkChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        let decoded = try JSONDecoder().decode(CrabforkChatSessionsListResponse.self, from: data)
        let mainSessionKey = await GatewayConnection.shared.cachedMainSessionKey()
        let defaults = decoded.defaults.map {
            CrabforkChatSessionsDefaults(
                model: $0.model,
            ...
            contextTokens: nil,
            mainSessionKey: mainSessionKey)
        return CrabforkChatSessionsListResponse(
            ts: decoded.ts,
            path: decoded.path,
            count: decoded.count,
            defaults: defaults,
    ...
    func sendMessage(
        ...
        attachments: [CrabforkChatAttachmentPayload]) async throws -> CrabforkChatSendResponse
    {
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> CrabforkChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                CrabforkGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    CrabforkAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
    ...
    private static func mapModelChoice(_ model: CrabforkProtocol.ModelChoice) -> CrabforkChatModelChoice {
        CrabforkChatModelChoice(
            modelID: model.id,
            name: model.name,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<CrabforkChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any CrabforkChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = CrabforkChatViewModel(
            sessionKey: sessionKey,
            transport: transport,
        ...
        self.hosting = NSHostingController(rootView: CrabforkChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<CrabforkChatView>) -> NSViewController
    {
