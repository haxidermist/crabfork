...
enum CrabforkLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = CrabforkOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = CrabforkFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        CrabforkLogging.bootstrapIfNeeded()
        let label = CrabforkLogging.makeLabel(subsystem: subsystem, category: category)
...
struct CrabforkOSLogHandler: AppLogLevelBackedHandler {
...
struct CrabforkFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = CrabforkLogging.parseLabel(self.label)
