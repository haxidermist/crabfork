...
enum DebugActions {
    private static let verboseDefaultsKey = "crabfork.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "crabfork.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = CrabforkPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "Crabfork", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = CrabforkPaths.configURL
        guard
