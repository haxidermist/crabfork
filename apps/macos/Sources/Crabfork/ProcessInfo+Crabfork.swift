...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.crabfork.mac`) even if the shipped
    ...
    {
        if environment["CRABFORK_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "crabfork.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "crabfork.nixMode") { return true }
