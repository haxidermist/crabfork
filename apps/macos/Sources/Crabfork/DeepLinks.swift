...
private let deepLinkLogger = Logger(subsystem: "ai.crabfork", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "Crabfork is paused", message: "Unpause Crabfork to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run Crabfork agent?", message: body) else { return }
        }
