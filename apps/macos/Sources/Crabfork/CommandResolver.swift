...
enum CommandResolver {
    private static let projectRootDefaultsKey = "crabfork.gatewayProjectRootPath"
    private static let helperName = "crabfork"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let crabforkEntry = root.appendingPathComponent("crabfork.mjs").path
        if FileManager().isReadableFile(atPath: crabforkEntry) { return crabforkEntry }
        let binEntry = root.appendingPathComponent("bin/crabfork.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/crabfork")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let crabforkPaths = self.crabforkManagedPaths(home: home)
        if !crabforkPaths.isEmpty {
            extras.insert(contentsOf: crabforkPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + crabforkPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func crabforkManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".crabfork"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func crabforkExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectCrabforkExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("crabfork.mjs").path,
            root.appendingPathComponent("bin/crabfork.js").path,
        ]
    ...
    static func hasAnyCrabforkInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.crabforkExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func crabforkNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        projectRoot: URL? = nil) -> [String]
    {
        ...
        let root = projectRoot ?? self.projectRoot()
        if let crabforkPath = self.projectCrabforkExecutable(projectRoot: root) {
            return [crabforkPath, subcommand] + extraArgs
        }
        if let crabforkPath = self.crabforkExecutable(searchPaths: searchPaths) {
            return [crabforkPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "crabfork", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            crabfork entrypoint missing (looked for dist/index.js or crabfork.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real crabfork CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/crabfork"
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/crabfork.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/crabfork.mjs"
            node "$PRJ/crabfork.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/crabfork.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/crabfork.js"
            node "$PRJ/bin/crabfork.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent crabfork"
          pnpm --silent crabfork \(quotedArgs);
        else
          echo "crabfork CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? CrabforkConfigFile.loadDict()
