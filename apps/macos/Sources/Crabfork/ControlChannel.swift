...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: CrabforkProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.crabfork", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: CrabforkKit.AnyCodable]()) {
                $0[$1.key] = CrabforkKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: CrabforkProtocol.AnyCodable?) -> [String: CrabforkProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: CrabforkProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: CrabforkKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: CrabforkProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("crabfork.control.heartbeat")
    static let controlAgentEvent = Notification.Name("crabfork.control.agent")
}
