...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: CrabforkProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: CrabforkProtocol.AnyCodable] = [
            "presence": .init([CrabforkProtocol.AnyCodable(entry)]),
        ]
        let payload = CrabforkProtocol.AnyCodable(payloadMap)
