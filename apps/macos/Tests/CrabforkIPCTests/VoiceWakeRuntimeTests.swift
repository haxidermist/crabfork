...
struct VoiceWakeRuntimeTests {
    ...
    @Test func `has content after trigger false when only trigger`() {
        let triggers = ["crabfork"]
        let text = "hey crabfork"
    ...
    @Test func `gate requires gap between trigger and command`() {
        let transcript = "hey crabfork do thing"
        ...
        let config = WakeWordGateConfig(triggers: ["crabfork"], minPostTriggerGap: 0.3)
    ...
    @Test func `gate command text handles foreign string ranges`() {
        ...
        let segments = [
            ...
            WakeWordSegment(text: "crabfork", start: 0.2, duration: 0.1, range: transcript.range(of: "crabfork")),
