...
@Suite(.serialized)
struct CrabforkConfigFileTests {
    private func makeConfigOverridePath() -> String {
        FileManager().temporaryDirectory
            .appendingPathComponent("crabfork-config-\(UUID().uuidString)")
            .appendingPathComponent("crabfork.json")
            .path
    }
    ...
    func `config path respects env override`() async {
        ...
        await TestIsolation.withEnvValues(["CRABFORK_CONFIG_PATH": override]) {
            #expect(CrabforkConfigFile.url().path == override)
        }
    }
    ...
    func `remote gateway port parses and matches host`() async {
        ...
        await TestIsolation.withEnvValues(["CRABFORK_CONFIG_PATH": override]) {
            ...
            #expect(CrabforkConfigFile.remoteGatewayPort() == 19999)
            #expect(CrabforkConfigFile.remoteGatewayPort(matchingHost: "gateway.ts.net") == 19999)
            #expect(CrabforkConfigFile.remoteGatewayPort(matchingHost: "gateway") == 19999)
            #expect(CrabforkConfigFile.remoteGatewayPort(matchingHost: "other.ts.net") == nil)
        }
    }
    ...
    func `set remote gateway url preserves scheme`() async {
        ...
        await TestIsolation.withEnvValues(["CRABFORK_CONFIG_PATH": override]) {
            ...
            CrabforkConfigFile.setRemoteGatewayUrl(host: "new-host", port: 2222)
            let root = CrabforkConfigFile.loadDict()
    ...
    func `clear remote gateway url removes only url field`() async {
        ...
        await TestIsolation.withEnvValues(["CRABFORK_CONFIG_PATH": override]) {
            ...
            CrabforkConfigFile.clearRemoteGatewayUrl()
            let root = CrabforkConfigFile.loadDict()
    ...
    func `state dir override sets config path`() async {
        let dir = FileManager().temporaryDirectory
            .appendingPathComponent("crabfork-state-\(UUID().uuidString)", isDirectory: true)
            .path

        await TestIsolation.withEnvValues([
            "CRABFORK_CONFIG_PATH": nil,
            "CRABFORK_STATE_DIR": dir,
        ]) {
            #expect(CrabforkConfigFile.stateDirURL().path == dir)
            #expect(CrabforkConfigFile.url().path == "\(dir)/crabfork.json")
        }
    }
    ...
    func `save dict appends config audit log`() async throws {
        let stateDir = FileManager().temporaryDirectory
            .appendingPathComponent("crabfork-state-\(UUID().uuidString)", isDirectory: true)
        let configPath = stateDir.appendingPathComponent("crabfork.json")
        let auditPath = stateDir.appendingPathComponent("logs/config-audit.jsonl")

        defer { try? FileManager().removeItem(at: stateDir) }

        try await TestIsolation.withEnvValues([
            "CRABFORK_STATE_DIR": stateDir.path,
            "CRABFORK_CONFIG_PATH": configPath.path,
        ]) {
            CrabforkConfigFile.saveDict([
                "gateway": ["mode": "local"],
            ])
            ...
            #expect(auditRoot?["source"] as? String == "macos-crabfork-config-file")
    ...
    func `load dict audits suspicious out-of-band clobbers`() async throws {
        ...
        try await TestIsolation.withEnvValues([
            ...
            let loaded = CrabforkConfigFile.loadDict()
