...
struct MacGatewayChatTransportMappingTests {
    @Test func `snapshot maps to health`() {
        let snapshot = Snapshot(
            presence: [],
            health: CrabforkProtocol.AnyCodable(["ok": CrabforkProtocol.AnyCodable(false)]),
    ...
    @Test func `health event maps to health`() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: CrabforkProtocol.AnyCodable(["ok": CrabforkProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func `chat event maps to chat`() {
        let payload = CrabforkProtocol.AnyCodable([
            "runId": CrabforkProtocol.AnyCodable("run-1"),
            "sessionKey": CrabforkProtocol.AnyCodable("main"),
            "state": CrabforkProtocol.AnyCodable("final"),
        ])
    ...
    @Test func `unknown event maps to nil`() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: CrabforkProtocol.AnyCodable(["a": CrabforkProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
