...
import type { CrabforkConfig } from "../api.js";
import type { CrabforkPluginApi, CrabforkPluginToolContext } from "../api.js";
...
describe("PlaywrightDiffScreenshotter", () => {
  ...
  beforeEach(async () => {
    vi.useFakeTimers();
    ({ rootDir, cleanup: cleanupRootDir } = await createTempDiffRoot("crabfork-diffs-browser-"));
...
describe("diffs plugin registration", () => {
  it("applies plugin-config defaults through registered tool and viewer handler", async () => {
    ...
    type RegisteredHttpRouteParams = Parameters<CrabforkPluginApi["registerHttpRoute"]>[0];

    let registeredToolFactory:
      | ((ctx: CrabforkPluginToolContext) => RegisteredTool | RegisteredTool[] | null | undefined)
      | undefined;
    ...
    const api = createTestPluginApi({
      ...
      runtime: {} as never,
      registerTool(tool: Parameters<CrabforkPluginApi["registerTool"]>[0]) {
    ...
    registerDiffsPlugin(api as unknown as CrabforkPluginApi);
