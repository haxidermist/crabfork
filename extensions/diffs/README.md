...
Read-only diff viewer plugin for **Crabfork** agents.
...
Set plugin-wide defaults in `~/.crabfork/crabfork.json`:
  ...
  plugins: {
    entries: {
      diffs: {
        enabled: true,
        config: {
          viewerBaseUrl: "https://gateway.example.com/crabfork",
        },
      },
    },
  },
...
Before:
Crabfork supports plugins.

After:
Crabfork supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/crabfork-diffs`).
