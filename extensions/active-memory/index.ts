...
import { definePluginEntry, type CrabforkPluginApi } from "crabfork/plugin-sdk/plugin-entry";
...
function resolvePersistentTranscriptBaseDir(api: CrabforkPluginApi, agentId: string): string {
  return path.join(
...
function resolveToggleStatePath(api: CrabforkPluginApi): string {
  return path.join(
    api.runtime.state.resolveStateDir(),
    "plugins",
    "active-memory",
...
function isActiveMemoryGloballyEnabled(cfg: CrabforkConfig): boolean {
...
function resolveActiveMemoryPluginConfigFromConfig(cfg: CrabforkConfig): unknown {
...
async function runRecallSubagent(params: {
  ...
  const tempDir = params.config.persistTranscripts
    ? undefined
    : await fs.mkdtemp(path.join(resolvePreferredCrabforkTmpDir(), "crabfork-active-memory-"));
