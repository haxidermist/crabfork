...
`crabfork.invoke --each`) without adding new Crabfork code per workflow.
...
This extension depends on Crabfork internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Crabfork extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.crabfork/extensions` as a standalone plugin directory.
