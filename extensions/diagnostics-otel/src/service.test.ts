...
import type { CrabforkPluginServiceContext } from "../api.js";
import { emitDiagnosticEvent } from "../api.js";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/crabfork-diagnostics-otel-test";
...
function createTraceOnlyContext(endpoint: string): CrabforkPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("crabfork.webhook.received")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("crabfork.webhook.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("crabfork.message.queued")?.add).toHaveBeenCalled();
    expect(telemetryState.counters.get("crabfork.message.processed")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("crabfork.message.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.histograms.get("crabfork.queue.wait_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("crabfork.session.stuck")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("crabfork.session.stuck_age_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("crabfork.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("crabfork.webhook.processed");
    expect(spanNames).toContain("crabfork.message.processed");
    expect(spanNames).toContain("crabfork.session.stuck");
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["crabfork.token"];
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionCounter = telemetryState.counters.get("crabfork.session.state");
    expect(sessionCounter?.add).toHaveBeenCalledWith(
      1,
      expect.objectContaining({
        "crabfork.reason": expect.stringContaining("…"),
      }),
    );
    const attrs = sessionCounter?.add.mock.calls[0]?.[1] as Record<string, unknown> | undefined;
    expect(typeof attrs?.["crabfork.reason"]).toBe("string");
    expect(String(attrs?.["crabfork.reason"])).not.toContain(
