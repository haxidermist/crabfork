...
import { optimizeImageToPng } from "crabfork/plugin-sdk/media-runtime";
...
import { captureEnv } from "crabfork/plugin-sdk/testing";
import { mockPinnedHostnameResolution } from "crabfork/plugin-sdk/testing";
...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(
    path.join(resolvePreferredCrabforkTmpDir(), "crabfork-media-test-"),
  );
...
describe("web media loading", () => {
  beforeAll(() => {
    // Ensure state dir is stable and not influenced by other tests that stub CRABFORK_STATE_DIR.
    // Also keep it outside the Crabfork temp root so default localRoots doesn't accidentally make all state readable.
    stateDirSnapshot = captureEnv(["CRABFORK_STATE_DIR"]);
    process.env.CRABFORK_STATE_DIR = path.join(
      path.parse(os.tmpdir()).root,
      "var",
      "lib",
      "crabfork-media-state-test",
    );
  });
...
describe("local media root guard", () => {
  ...
  it("allows local paths under an explicit root", async () => {
    const result = await loadWebMedia(tinyPngFile, 1024 * 1024, {
      localRoots: [resolvePreferredCrabforkTmpDir()],
    });
    expect(result.kind).toBe("image");
  });
  ...
  it("allows default Crabfork state workspace and sandbox roots", async () => {
  ...
  it("rejects default Crabfork state per-agent workspace-* roots without explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));
