import { resolveInboundDebounceMs } from "crabfork/plugin-sdk/channel-inbound";
import { formatCliCommand } from "crabfork/plugin-sdk/cli-runtime";
import { hasControlCommand } from "crabfork/plugin-sdk/command-detection";
import { drainPendingDeliveries } from "crabfork/plugin-sdk/infra-runtime";
import { enqueueSystemEvent } from "crabfork/plugin-sdk/infra-runtime";
import { DEFAULT_GROUP_HISTORY_LIMIT } from "crabfork/plugin-sdk/reply-history";
import { resolveAgentRoute } from "crabfork/plugin-sdk/routing";
import { logVerbose } from "crabfork/plugin-sdk/runtime-env";
import { registerUnhandledRejectionHandler } from "crabfork/plugin-sdk/runtime-env";
import { getChildLogger } from "crabfork/plugin-sdk/runtime-env";
import {
  defaultRuntime,
    ...
    while (true) {
      ...
      if (decision.action === "stop") {
        ...
        if (decision.healthState === "logged-out") {
          runtime.error(
            `WhatsApp session logged out. Run \`${formatCliCommand("crabfork channels login --channel web")}\` to relink.`,
          );
          ...
          runtime.error(
            `WhatsApp Web connection closed (status ${decision.normalized.statusLabel}: session conflict). Resolve conflicting WhatsApp Web sessions, then relink with \`${formatCliCommand("crabfork channels login --channel web")}\`. Stopping web monitoring.`,
          );
        } else {
          reconnectLogger.warn(
            {
