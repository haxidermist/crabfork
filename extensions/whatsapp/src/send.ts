...
import { loadConfig, type CrabforkConfig } from "crabfork/plugin-sdk/config-runtime";
import { resolveMarkdownTableMode } from "crabfork/plugin-sdk/config-runtime";
import { generateSecureUuid } from "crabfork/plugin-sdk/core";
import { normalizePollInput, type PollInput } from "crabfork/plugin-sdk/media-runtime";
import { createSubsystemLogger } from "crabfork/plugin-sdk/runtime-env";
import { getChildLogger } from "crabfork/plugin-sdk/text-runtime";
import { redactIdentifier } from "crabfork/plugin-sdk/text-runtime";
...
function requireOutboundActiveWebListener(params: { cfg: CrabforkConfig; accountId?: string }): {
  accountId: string;
  ...
  if (!listener) {
    throw new Error(
      `No active WhatsApp Web listener (account: ${resolvedAccountId}). Start the gateway, then link WhatsApp with: ${formatCliCommand(`crabfork channels login --channel whatsapp --account ${resolvedAccountId}`)}.`,
    );
  }
...
export async function sendPollWhatsApp(
  to: string,
  poll: PollInput,
  options: { verbose: boolean; accountId?: string; cfg?: CrabforkConfig },
): Promise<{ messageId: string; toJid: string }> {
  const correlationId = generateSecureUuid();
  const startedAt = Date.now();
  const cfg = options.cfg ?? loadConfig();
