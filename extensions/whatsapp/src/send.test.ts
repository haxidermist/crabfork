...
import { redactIdentifier } from "crabfork/plugin-sdk/logging-core";
...
let resetLogger: typeof import("crabfork/plugin-sdk/runtime-env").resetLogger;
let setLoggerOverride: typeof import("crabfork/plugin-sdk/runtime-env").setLoggerOverride;
...
describe("web outbound", () => {
  ...
  beforeAll(async () => {
    ...
    ({ resetLogger, setLoggerOverride } = await import("crabfork/plugin-sdk/runtime-env"));
  });
  ...
  it("redacts recipients and poll text in outbound logs", async () => {
    const logPath = path.join(os.tmpdir(), `crabfork-outbound-${crypto.randomUUID()}.log`);
