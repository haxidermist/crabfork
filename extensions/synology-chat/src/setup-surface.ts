...
function getChannelConfig(cfg: CrabforkConfig): SynologyChatChannelConfig {
  return (cfg.channels?.[channel] as SynologyChatChannelConfig | undefined) ?? {};
}

function getRawAccountConfig(cfg: CrabforkConfig, accountId: string): SynologyChatAccountRaw {
...
function isSynologyChatConfigured(cfg: CrabforkConfig, accountId: string): boolean {
...
function resolveExistingAllowedUserIds(cfg: CrabforkConfig, accountId: string): string[] {
  ...
  textInputs: [
    {
      ...
      helpLines: [
        ...
        "This is the URL Crabfork uses to send replies back to Chat.",
      ],
