...
const synologyChatSetupPlugin = {
  ...
  config: {
    ...
    resolveAllowFrom: ({ cfg, accountId }: { cfg: CrabforkConfig; accountId?: string }) =>
...
describe("synology-chat core", () => {
  beforeEach(() => {
    ...
    delete process.env.CRABFORK_BOT_NAME;
  });
...
describe("synology-chat account resolution", () => {
  ...
  it("returns full defaults for empty config", () => {
    ...
    expect(account.botName).toBe("Crabfork");
  });
  ...
  it("uses env var fallbacks", () => {
    ...
    process.env.CRABFORK_BOT_NAME = "TestBot";
