...
metadata: { "crabfork": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Crabfork `sessions_spawn`
- **File I/O** == Crabfork `read`/`write`
- **Remote fetch** == Crabfork `web_fetch` (or `exec` with curl when POST is required)
