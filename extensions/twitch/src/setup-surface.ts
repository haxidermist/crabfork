...
function resolveSetupAccountId(cfg: CrabforkConfig): string {
...
async function noteTwitchSetupHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Env vars supported: CRABFORK_TWITCH_ACCESS_TOKEN",
...
): Promise<{ cfg: CrabforkConfig } | null> {
  const useEnv = await prompter.confirm({
    message: "Twitch env var CRABFORK_TWITCH_ACCESS_TOKEN detected. Use env token?",
    initialValue: true,
  });
  if (!useEnv) {
    return null;
...
function resolveTwitchGroupPolicy(cfg: CrabforkConfig): "open" | "allowlist" | "disabled" {
...
export const twitchSetupWizard: ChannelSetupWizard = {
  ...
  finalize: async ({ cfg, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.CRABFORK_TWITCH_ACCESS_TOKEN?.trim();
