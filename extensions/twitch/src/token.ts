 ...
 * The CRABFORK_TWITCH_ACCESS_TOKEN env var is only used for the default account.
 *
 * Token resolution priority:
 * 1. Account access token from merged config (accounts.{id} or base-level for default)
 * 2. Environment variable: CRABFORK_TWITCH_ACCESS_TOKEN (default account only)
 */

import type { CrabforkConfig } from "crabfork/plugin-sdk/config-runtime";
import { DEFAULT_ACCOUNT_ID, normalizeAccountId } from "crabfork/plugin-sdk/core";
...
export function resolveTwitchToken(
  ...
  const envToken = allowEnv
    ? normalizeTwitchToken(opts.envToken ?? process.env.CRABFORK_TWITCH_ACCESS_TOKEN)
    : undefined;
  if (envToken) {
