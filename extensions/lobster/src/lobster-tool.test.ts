...
import type { CrabforkPluginApi, CrabforkPluginToolContext } from "../runtime-api.js";
import { createLobsterTool } from "./lobster-tool.js";
import { createFakeTaskFlow } from "./taskflow-test-helpers.js";

function fakeApi(overrides: Partial<CrabforkPluginApi> = {}): CrabforkPluginApi {
...
function fakeCtx(overrides: Partial<CrabforkPluginToolContext> = {}): CrabforkPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: CrabforkPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
