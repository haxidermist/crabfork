import { formatCliCommand, parseDurationMs } from "crabfork/plugin-sdk/cli-runtime";
import type {
  CrabforkPluginApi,
  ProviderAuthContext,
...
import {
  ...
  type CrabforkConfig as ProviderAuthConfig,
  type ProviderAuthResult,
  suggestOAuthProfileIdForLegacyDefault,
  upsertAuthProfile,
  validateAnthropicSetupToken,
} from "crabfork/plugin-sdk/provider-auth";
import { cloneFirstTemplateModel } from "crabfork/plugin-sdk/provider-model-shared";
import { fetchClaudeUsage } from "crabfork/plugin-sdk/provider-usage";
...
const ANTHROPIC_SETUP_TOKEN_NOTE_LINES = [
  ...
  `If you want a direct API billing path instead, use ${formatCliCommand("crabfork models auth login --provider anthropic --method api-key --set-default")} or ${formatCliCommand("crabfork models auth login --provider anthropic --method cli --set-default")}.`,
] as const;
...
function buildAnthropicAuthDoctorHint(params: {
  ...
  return [
    ...
    `Fix: run "${formatCliCommand("crabfork doctor --yes")}"`,
  ].join("\n");
}
...
export function registerAnthropicPlugin(api: CrabforkPluginApi): void {
