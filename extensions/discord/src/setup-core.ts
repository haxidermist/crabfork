...
import type { DiscordGuildEntry, CrabforkConfig } from "crabfork/plugin-sdk/config-runtime";
import type { ChannelSetupDmPolicy, ChannelSetupWizard } from "crabfork/plugin-sdk/setup-runtime";
import { createStandardChannelSetupStatus } from "crabfork/plugin-sdk/setup-runtime";
import { formatDocsLink } from "crabfork/plugin-sdk/setup-tools";
...
export function createDiscordSetupWizardBase(handlers: {
  ...
  return {
    ...
    credentials: [
      {
        ...
        inspect: ({ cfg, accountId }: { cfg: CrabforkConfig; accountId: string }) => {
    ...
    groupAccess: createAccountScopedGroupAccessSection({
      ...
      currentPolicy: ({ cfg, accountId }: { cfg: CrabforkConfig; accountId: string }) =>
        resolveDiscordSetupAccountConfig({ cfg, accountId }).config.groupPolicy ?? "allowlist",
      currentEntries: ({ cfg, accountId }: { cfg: CrabforkConfig; accountId: string }) =>
        Object.entries(
      ...
      updatePrompt: ({ cfg, accountId }: { cfg: CrabforkConfig; accountId: string }) =>
    ...
    disable: (cfg: CrabforkConfig) => setSetupChannelEnabled(cfg, channel, false),
