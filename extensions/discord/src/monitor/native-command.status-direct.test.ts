...
vi.mock("crabfork/plugin-sdk/reply-dispatch-runtime", async () => {
  const actual = await vi.importActual<typeof import("crabfork/plugin-sdk/reply-dispatch-runtime")>(
...
function createConfig(params?: { requireMention?: boolean }): CrabforkConfig {
  return {
    commands: {
...
describe("discord native /status", () => {
  ...
  beforeEach(() => {
    ...
    discordNativeCommandTesting.setDispatchReplyWithDispatcher(
      runtimeModuleMocks.dispatchReplyWithDispatcher as typeof import("crabfork/plugin-sdk/reply-dispatch-runtime").dispatchReplyWithDispatcher,
    );
