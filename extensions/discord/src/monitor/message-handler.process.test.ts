import { DEFAULT_EMOJIS } from "crabfork/plugin-sdk/channel-feedback";
...
const configSessionsMocks = vi.hoisted(() => ({
  ...
  resolveStorePath: vi.fn<(path?: unknown, opts?: unknown) => string>(
    () => "/tmp/crabfork-discord-process-test-sessions.json",
  ),
}));
...
const replyRuntimeModule = await import("crabfork/plugin-sdk/reply-runtime");
...
const conversationRuntimeModule = await import("crabfork/plugin-sdk/conversation-runtime");
...
const configRuntimeModule = await import("crabfork/plugin-sdk/config-runtime");
...
beforeEach(() => {
  ...
  resolveStorePath.mockReturnValue("/tmp/crabfork-discord-process-test-sessions.json");
  ...
  it("applies status reaction emoji/timing overrides from config", async () => {
    ...
    const ctx = await createBaseContext({
      cfg: {
        ...
        session: { store: "/tmp/crabfork-discord-process-test-sessions.json" },
      },
    });
