...
import { clearSessionStoreCacheForTest } from "crabfork/plugin-sdk/config-runtime";
import { afterEach, beforeAll, beforeEach, describe, expect, it, vi } from "vitest";
import { createNoopThreadBindingManager } from "./thread-bindings.js";

type ConversationRuntimeModule = typeof import("crabfork/plugin-sdk/conversation-binding-runtime");
...
vi.mock("crabfork/plugin-sdk/conversation-binding-runtime", async () => {
  ...
  return await createConfiguredBindingConversationRuntimeModuleMock<
    typeof import("crabfork/plugin-sdk/conversation-binding-runtime")
  >(
    {
    ...
    () =>
      vi.importActual<typeof import("crabfork/plugin-sdk/conversation-binding-runtime")>(
        "crabfork/plugin-sdk/conversation-binding-runtime",
      ),
  );
});

vi.mock("crabfork/plugin-sdk/agent-runtime", () => ({
...
vi.mock("crabfork/plugin-sdk/models-provider-runtime", () => ({
  buildModelsProviderData: buildModelsProviderDataMock,
}));

const STORE_PATH = path.join(
  os.tmpdir(),
  `crabfork-discord-think-autocomplete-${process.pid}.json`,
);
const SESSION_KEY = "agent:main:main";
let findCommandByNativeName: typeof import("crabfork/plugin-sdk/command-auth").findCommandByNativeName;
let resolveCommandArgChoices: typeof import("crabfork/plugin-sdk/command-auth").resolveCommandArgChoices;
...
async function loadDiscordThinkAutocompleteModulesForTest() {
  ...
  const commandAuth = await import("crabfork/plugin-sdk/command-auth");
