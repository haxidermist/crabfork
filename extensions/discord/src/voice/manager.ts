...
import { resolveAgentDir } from "crabfork/plugin-sdk/agent-runtime";
import { agentCommandFromIngress } from "crabfork/plugin-sdk/agent-runtime";
import { resolveTtsConfig, type ResolvedTtsConfig } from "crabfork/plugin-sdk/agent-runtime";
import type { CrabforkConfig } from "crabfork/plugin-sdk/config-runtime";
import type { DiscordAccountConfig, TtsConfig } from "crabfork/plugin-sdk/config-runtime";
...
import { parseTtsDirectives } from "crabfork/plugin-sdk/speech";
import { formatErrorMessage } from "crabfork/plugin-sdk/ssrf-runtime";
...
function resolveVoiceTtsConfig(params: { cfg: CrabforkConfig; override?: TtsConfig }): {
  cfg: CrabforkConfig;
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredCrabforkTmpDir(), "discord-voice-"));
