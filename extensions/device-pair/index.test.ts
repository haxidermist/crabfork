...
import type { CrabforkPluginApi } from "./api.js";
...
const pluginApiMocks = vi.hoisted(() => ({
  ...
  resolvePreferredCrabforkTmpDir: vi.fn(() => path.join(os.tmpdir(), "crabfork-device-pair-tests")),
}));
...
vi.mock("./api.js", () => {
  return {
    ...
    resolvePreferredCrabforkTmpDir: pluginApiMocks.resolvePreferredCrabforkTmpDir,
...
function createApi(params?: {
  runtime?: CrabforkPluginApi["runtime"];
  ...
  return createTestPluginApi({
    ...
    runtime: (params?.runtime ?? {}) as CrabforkPluginApi["runtime"],
...
function registerPairCommand(params?: {
  ...
  let command: CrabforkPluginCommandDefinition | undefined;
...
describe("device-pair /pair qr", () => {
  beforeEach(async () => {
    ...
    await fs.mkdir(pluginApiMocks.resolvePreferredCrabforkTmpDir(), { recursive: true });
  });

  afterEach(async () => {
    await fs.rm(pluginApiMocks.resolvePreferredCrabforkTmpDir(), { recursive: true, force: true });
  });
  ...
  it("returns an inline QR image for webchat surfaces", async () => {
    ...
    expect(text).toContain("Scan this QR code with the Crabfork iOS app:");
    expect(text).toContain("![Crabfork pairing QR](data:image/png;base64,ZmFrZXBuZw==)");
  ...
  it.each([
    ...
    expect(caption).toContain("Scan this QR code with the Crabfork iOS app:");
