...
import { runPluginCommandWithTimeout } from "crabfork/plugin-sdk/run-command";
import type { RuntimeEnv } from "crabfork/plugin-sdk/runtime-env";
import { CONFIG_DIR, extractArchive, resolveBrewExecutable } from "crabfork/plugin-sdk/setup-tools";
...
async function installSignalCliFromRelease(runtime: RuntimeEnv): Promise<SignalInstallResult> {
  ...
  const tmpDir = await fs.mkdtemp(path.join(resolvePreferredCrabforkTmpDir(), "crabfork-signal-"));
