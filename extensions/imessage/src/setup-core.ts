...
export const imessageDmPolicy = {
  ...
  resolveConfigKeys: (_cfg: CrabforkConfig, accountId?: string) => {
  ...
  getCurrent: (cfg: CrabforkConfig, accountId?: string) => {
...
function resolveIMessageCliPath(params: { cfg: CrabforkConfig; accountId: string }) {
...
export const imessageCompletionNote = {
  ...
  lines: [
    ...
    "Ensure Crabfork has Full Disk Access to Messages DB.",
...
export const imessageSetupStatusBase = {
  ...
  configuredScore: 1,
  unconfiguredScore: 0,
  resolveConfigured: ({ cfg, accountId }: { cfg: CrabforkConfig; accountId?: string }) =>
