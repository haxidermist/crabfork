...
vi.mock("crabfork/plugin-sdk/provider-web-search", () => ({
...
describe("tavily client X-Client-Source header", () => {
  ...
  it("runTavilySearch sends X-Client-Source: crabfork", async () => {
    ...
    expect(params.extraHeaders).toEqual({ "X-Client-Source": "crabfork" });
  });

  it("runTavilyExtract sends X-Client-Source: crabfork", async () => {
