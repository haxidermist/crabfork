...
import type { CrabforkConfig } from "crabfork/plugin-sdk/provider-onboard";
import {
  buildSearchCacheKey,
...
function resolveKimiBaseUrl(kimi?: KimiConfig, crabForkConfig?: CrabforkConfig): string {
  ...
  const moonshotBaseUrl = crabForkConfig?.models?.providers?.moonshot?.baseUrl;
...
function createKimiToolDefinition(
  ...
  crabForkConfig: CrabforkConfig | undefined,
  ...
  return {
    ...
    execute: async (args) => {
      ...
      const baseUrl = resolveKimiBaseUrl(kimiConfig, crabForkConfig);
...
export function createKimiWebSearchProvider(): WebSearchProviderPlugin {
  return {
    ...
    docsUrl: "https://docs.crabfork.ai/tools/web",
    autoDetectOrder: 40,
