...
import type { CrabforkConfig, PluginRuntime, RuntimeEnv } from "../runtime-api.js";
import {
  ...
  return {
    ...
    state: {
      resolveStateDir: (env?: NodeJS.ProcessEnv) => {
        ...
        return stateDir ?? path.join(os.homedir(), ".crabfork");
      },
    },
...
describe("msteams file consent invoke FS fallback", () => {
  ...
  beforeEach(async () => {
    ...
    tmpDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), "crabfork-msteams-invoke-"));
    process.env.CRABFORK_STATE_DIR = tmpDir;
