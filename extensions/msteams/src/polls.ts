...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["crabforkPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["crabfork", "pollId"]) ??
    readNestedString(value, ["crabfork", "poll", "id"]) ??
    readNestedString(value, ["data", "crabforkPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "crabfork", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          crabforkPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "crabfork poll vote",
          displayText: "Vote recorded",
          value: { crabforkPollId: pollId, pollId },
        },
      },
    ],
  };
