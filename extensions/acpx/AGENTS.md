...
The bundled ACPX extension is a thin Crabfork wrapper around the published `acpx` package. Keep reusable ACP runtime logic in `crabfork/acpx`, not in this extension.
...
Use this flow when Crabfork needs unreleased ACPX changes before the ACPX version is published.

1. Make the ACPX code change in the `crabfork/acpx` repo first.
2. In Crabfork, temporarily point `extensions/acpx/package.json` at the ACPX GitHub commit you need.
...
6. Rebuild Crabfork and restart the gateway before doing live ACP validation.
...
- Do not rely on a globally installed `acpx` binary for Crabfork ACP validation.
...
If a change feels like shared ACP runtime behavior instead of Crabfork-specific glue, move it to `crabfork/acpx` and consume it from here instead of re-implementing it inside `extensions/acpx`.
