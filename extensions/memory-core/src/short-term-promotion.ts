...
import { formatMemoryDreamingDay } from "crabfork/plugin-sdk/memory-core-host-status";
import { appendMemoryHostEvent } from "crabfork/plugin-sdk/memory-host-events";
...
const PROMOTION_MARKER_PREFIX = "crabfork-memory-promotion:";
...
function extractPromotionMarkers(memoryText: string): Set<string> {
  ...
  const matches = memoryText.matchAll(/<!--\s*crabfork-memory-promotion:([^\n]+?)\s*-->/gi);
