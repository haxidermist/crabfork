    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["crabfork memory status", "Show index and provider status."],
          [
            "crabfork memory status --fix",
            "Repair stale recall locks and normalize promotion metadata.",
          ],
          ["crabfork memory status --deep", "Probe embedding provider readiness."],
          ["crabfork memory index --force", "Force a full reindex."],
          ['crabfork memory search "meeting notes"', "Quick search using positional query."],
          [
            'crabfork memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          [
            `crabfork memory promote --limit 10 --min-score ${DEFAULT_PROMOTION_MIN_SCORE}`,
          ...
          [
            "crabfork memory rem-backfill --path ./memory",
          ...
          ["crabfork memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.crabfork.ai/cli/memory")}\n`,
    );
