...
describe("memory watcher config", () => {
  ...
  async function setupWatcherWorkspace(seedFile: { name: string; contents: string }) {
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "crabfork-memory-watch-"));
  ...
  function createWatcherConfig(overrides?: Partial<MemorySearchConfig>): CrabforkConfig {
    const defaults: NonNullable<NonNullable<CrabforkConfig["agents"]>["defaults"]> = {
  ...
  async function expectWatcherManager(cfg: CrabforkConfig) {
