...
const MCPORTER_STATE_KEY = Symbol.for("crabfork.mcporterState");
const QMD_EMBED_QUEUE_KEY = Symbol.for("crabfork.qmdEmbedQueueTail");
...
vi.mock("crabfork/plugin-sdk/memory-core-host-engine-foundation", async () => {
  const actual = await vi.importActual<
    typeof import("crabfork/plugin-sdk/memory-core-host-engine-foundation")
  >("crabfork/plugin-sdk/memory-core-host-engine-foundation");
  return {
    ...actual,
...
vi.mock("crabfork/plugin-sdk/file-lock", async () => {
  const actual = await vi.importActual<typeof import("crabfork/plugin-sdk/file-lock")>(
    "crabfork/plugin-sdk/file-lock",
  );
  return {
    ...actual,
...
import type { CrabforkConfig } from "crabfork/plugin-sdk/memory-core-host-engine-foundation";
import {
  requireNodeSqlite,
...
describe("QmdMemoryManager", () => {
  ...
  async function createManager(params?: { mode?: "full" | "status"; cfg?: CrabforkConfig }) {
