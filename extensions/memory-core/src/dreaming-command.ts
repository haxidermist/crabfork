...
import { resolveMemoryDreamingConfig } from "crabfork/plugin-sdk/memory-core-host-status";
import { normalizeLowercaseStringOrEmpty } from "crabfork/plugin-sdk/text-runtime";
import { asRecord } from "./dreaming-shared.js";
import { resolveShortTermPromotionDreamingConfig } from "./dreaming.js";

function resolveMemoryCorePluginConfig(cfg: CrabforkConfig): Record<string, unknown> {
  const entry = asRecord(cfg.plugins?.entries?.["memory-core"]);
  return asRecord(entry?.config) ?? {};
}

function updateDreamingEnabledInConfig(cfg: CrabforkConfig, enabled: boolean): CrabforkConfig {
...
function formatStatus(cfg: CrabforkConfig): string {
...
export function registerDreamingCommand(api: CrabforkPluginApi): void {
  api.registerCommand({
    name: "dreaming",
