...
import { resolveMemoryRemDreamingConfig } from "crabfork/plugin-sdk/memory-core-host-status";
...
function resolveMemoryPluginConfig(cfg: CrabforkConfig): Record<string, unknown> {
...
async function createHistoricalRemHarnessWorkspace(params: {
  ...
  const workspaceDir = await fs.mkdtemp(
    path.join(resolvePreferredCrabforkTmpDir(), "crabfork-rem-harness-"),
  );
...
function formatDreamingSummary(cfg: CrabforkConfig): string {
...
function resolveAgent(cfg: CrabforkConfig, agent?: string) {
...
function resolveAgentIds(cfg: CrabforkConfig, agent?: string): string[] {
  const trimmed = agent?.trim();
  if (trimmed) {
    return [trimmed];
  }
...
export async function runMemoryStatus(opts: MemoryCommandOptions) {
  ...
  for (const result of allResults) {
    ...
    if (audit?.issues.length) {
      ...
      if (!opts.fix) {
        lines.push(`  ${muted(`Fix: crabfork memory status --fix --agent ${agentId}`)}`);
      }
    }
  ...
  await withMemoryManagerForAgent({
    ...
    run: async (manager) => {
      ...
      const scratchDir = await fs.mkdtemp(
        path.join(resolvePreferredCrabforkTmpDir(), "crabfork-rem-backfill-"),
      );
      try {
