...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.CRABFORK_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "crabfork-nostr-"));
  process.env.CRABFORK_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const stateEnv = env ?? process.env;
        const override = stateEnv.CRABFORK_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        const resolveHome = homedir ?? os.homedir;
        return path.join(resolveHome(), ".crabfork");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.CRABFORK_STATE_DIR = previous;
    }
