...
- Extension production code should import from `crabfork/plugin-sdk/*` and its
...
- Keep plugin metadata accurate in `crabfork.plugin.json` and the package
  `crabfork` block so discovery and setup work without executing plugin code.
...
- Before adding a new provider-local `wrapStreamFn`, `buildReplayPolicy`,
  ...
  whether the same behavior already exists through `crabfork/plugin-sdk/*`.
