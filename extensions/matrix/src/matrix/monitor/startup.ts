...
export async function runMatrixStartupMaintenance(
  ...
  try {
    ...
    if (deviceHealth.staleCrabforkDevices.length > 0) {
      params.logger.warn(
        `matrix: stale Crabfork devices detected for ${params.auth.userId}: ${deviceHealth.staleCrabforkDevices.map((device) => device.deviceId).join(", ")}. Run 'crabfork matrix devices prune-stale --account ${params.effectiveAccountId}' to keep encrypted-room trust healthy.`,
      );
    }
  } catch (err) {
    if (isMatrixStartupAbortError(err)) {
