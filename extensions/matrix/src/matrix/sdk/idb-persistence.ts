...
import { withFileLock } from "crabfork/plugin-sdk/infra-runtime";
...
// `crabfork matrix verify bootstrap`) can corrupt each other's state.
...
function resolveDefaultIdbSnapshotPath(): string {
  const stateDir =
    process.env.CRABFORK_STATE_DIR || path.join(process.env.HOME || "/tmp", ".crabfork");
