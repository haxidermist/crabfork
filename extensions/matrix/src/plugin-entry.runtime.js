...
const CRABFORK_PLUGIN_SDK_PACKAGE_NAMES = [
  ["crabfork", "plugin-sdk"].join("/"),
  ["@crabfork", "plugin-sdk"].join("/"),
];
...
function hasTrustedCrabforkRootIndicator(packageRoot, packageJson) {
  ...
  const hasCrabforkBin =
    (typeof packageJson?.bin === "string" &&
      normalizeLowercaseStringOrEmpty(packageJson.bin).includes("crabfork")) ||
    (typeof packageJson?.bin === "object" &&
      packageJson.bin !== null &&
      typeof packageJson.bin.crabfork === "string");
  const hasCrabforkEntrypoint = fs.existsSync(path.join(packageRoot, "crabfork.mjs"));
  return hasCliEntryExport || hasCrabforkBin || hasCrabforkEntrypoint;
}

function findCrabforkPackageRoot(startDir) {
  let cursor = path.resolve(startDir);
  for (let i = 0; i < 12; i += 1) {
    const pkg = readPackageJson(cursor);
    if (pkg?.name === "crabfork" && hasTrustedCrabforkRootIndicator(cursor, pkg)) {
...
function buildPluginSdkAliasMap(moduleUrl) {
  const location = findCrabforkPackageRoot(path.dirname(fileURLToPath(moduleUrl)));
  if (!location) {
    return {};
  }
  ...
  if (rootAlias) {
    for (const packageName of CRABFORK_PLUGIN_SDK_PACKAGE_NAMES) {
  ...
  if (extensionApi) {
    aliasMap["crabfork/extension-api"] = extensionApi;
  }
...
function resolveBundledPluginRuntimeModulePath(moduleUrl, params) {
  ...
  const location = findCrabforkPackageRoot(moduleDir);
  if (location) {
