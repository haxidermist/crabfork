...
const matrixWrapperGlobal = globalThis as typeof globalThis & {
  __crabforkMatrixWrapperJitiOptions?: unknown;
};
const PLUGIN_SDK_ROOT = ["crabfork", "plugin-sdk"].join("/");
const SCOPED_PLUGIN_SDK_ROOT = ["@crabfork", "plugin-sdk"].join("/");
...
function writeCapturingJitiFixture(fixtureRoot: string) {
  writeFixtureFile(
    ...
    [
      ...
      "  globalThis.__crabforkMatrixWrapperJitiOptions = options;",
...
function writeCrabforkPackageFixture(fixtureRoot: string) {
  writeFixtureFile(
    fixtureRoot,
    "package.json",
    JSON.stringify(
  ...
  writeFixtureFile(fixtureRoot, "crabfork.mjs", "export {};\n");
  writeFixtureFile(fixtureRoot, "dist/plugin-sdk/index.js", "export {};\n");
}

function writeCrabforkAliasFixture(fixtureRoot: string, extraExports?: Record<string, string>) {
  writeFixtureFile(
    fixtureRoot,
    "package.json",
    JSON.stringify(
...
function writeTrustedCrabforkBinFixture(
  fixtureRoot: string,
...
it("loads the source-checkout runtime wrapper through native ESM import", async () => {
  ...
  writeCrabforkPackageFixture(fixtureRoot);
...
it("builds scoped and unscoped plugin-sdk aliases for the wrapper jiti loader", async () => {
  ...
  delete matrixWrapperGlobal.__crabforkMatrixWrapperJitiOptions;
  writeCrabforkAliasFixture(fixtureRoot);
  ...
  expect(matrixWrapperGlobal.__crabforkMatrixWrapperJitiOptions).toMatchObject({
    alias: {
...
it("resolves extension-api aliases through the same source extension family", async () => {
  ...
  expect(matrixWrapperGlobal.__crabforkMatrixWrapperJitiOptions).toMatchObject({
    alias: {
      "crabfork/extension-api": path.join(fixtureRoot, "src", "extensionAPI.mts"),
    },
  });
}, 240_000);
...
it("keeps wrapper plugin-sdk aliases deterministic and ignores unsafe subpaths", async () => {
  ...
  const aliasKeys = Object.keys(
    (
      (matrixWrapperGlobal.__crabforkMatrixWrapperJitiOptions ?? {}) as {
...
it("ignores nearby untrusted crabfork package stubs when resolving the wrapper root", async () => {
...
it("treats string bin hints case-insensitively when trusting wrapper package roots", async () => {
  ...
  delete matrixWrapperGlobal.__crabforkMatrixWrapperJitiOptions;
  writeTrustedCrabforkBinFixture(fixtureRoot, "Crabfork.MJS");
  writeCapturingJitiFixture(fixtureRoot);
  writeFixtureFile(fixtureRoot, "extensions/matrix/src/plugin-entry.runtime.js", wrapperSource);
  writeFixtureFile(
    fixtureRoot,
