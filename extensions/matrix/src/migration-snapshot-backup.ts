...
import { writeJsonFileAtomically } from "crabfork/plugin-sdk/json-store";
import { resolveRequiredHomeDir } from "crabfork/plugin-sdk/provider-auth";
import { resolveStateDir } from "crabfork/plugin-sdk/state-paths";

const MATRIX_MIGRATION_SNAPSHOT_DIRNAME = "crabfork-migrations";
...
export async function maybeCreateMatrixMigrationSnapshot(params: {
  ...
  outputDir?: string;
  createBackupArchive?: typeof import("crabfork/plugin-sdk/runtime").createBackupArchive;
  log?: { info?: (message: string) => void; warn?: (message: string) => void };
}): Promise<MatrixMigrationSnapshotResult> {
  const env = params.env ?? process.env;
  const createBackupArchive =
    params.createBackupArchive ?? (await import("crabfork/plugin-sdk/runtime")).createBackupArchive;
