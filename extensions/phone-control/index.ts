...
function normalizeDenyList(cfg: CrabforkPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: CrabforkPluginApi["config"]): string[] {
...
export default definePluginEntry({
  ...
  register(api: CrabforkPluginApi) {
    ...
    const timerService: CrabforkPluginService = {
