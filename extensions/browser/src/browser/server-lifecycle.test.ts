...
const { stopCrabforkChromeMock } = vi.hoisted(() => ({
  stopCrabforkChromeMock: vi.fn(async () => {}),
}));
...
vi.mock("./chrome.js", () => ({
  stopCrabforkChrome: stopCrabforkChromeMock,
}));
...
beforeEach(() => {
  ...
  stopCrabforkChromeMock.mockClear();
});
...
describe("stopKnownBrowserProfiles", () => {
  it("stops all known profiles and ignores per-profile failures", async () => {
    listKnownProfileNamesMock.mockReturnValue(["crabfork", "user"]);
    const stopMap: Record<string, ReturnType<typeof vi.fn>> = {
      crabfork: vi.fn(async () => {}),
      user: vi.fn(async () => {
    ...
    expect(stopMap.crabfork).toHaveBeenCalledTimes(1);
  ...
  it("stops tracked runtime browsers even when the profile no longer resolves", async () => {
    ...
    expect(stopCrabforkChromeMock).toHaveBeenCalledWith(launchedBrowser);
  ...
  it("warns when profile enumeration fails", async () => {
    ...
    expect(onWarn).toHaveBeenCalledWith("crabfork browser stop failed: Error: oops");
  });
});
