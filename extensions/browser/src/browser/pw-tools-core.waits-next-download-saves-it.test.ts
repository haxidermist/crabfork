...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredCrabforkTmpDir: vi.fn(() => "/tmp/crabfork"),
}));
...
let tmpDirModule: typeof import("../infra/tmp-crabfork-dir.js");

describe("pw-tools-core", () => {
  beforeAll(async () => {
    vi.doMock("./pw-session.js", () => sessionMocks);
    vi.doMock("./chrome.js", () => chromeMocks);
    tmpDirModule = await import("../infra/tmp-crabfork-dir.js");
    vi.spyOn(tmpDirModule, "resolvePreferredCrabforkTmpDir").mockImplementation(
      tmpDirMocks.resolvePreferredCrabforkTmpDir,
    );
  ...
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredCrabforkTmpDir.mockReturnValue("/tmp/crabfork");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "crabfork-browser-download-test-"));
    try {
  ...
  async function expectAtomicDownloadSave(params: {
    ...
    expect(path.basename(String(savedPath))).toContain(".crabfork-output-");
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredCrabforkTmpDir.mockReturnValue("/tmp/crabfork-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "crabfork-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "crabfork-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredCrabforkTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(outPath)).toBe(
      path.resolve(path.join(path.sep, "tmp", "crabfork-preferred", "downloads")),
    );
    expect(path.basename(outPath)).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "crabfork-preferred", "downloads")}${path.sep}`),
    );
  });
