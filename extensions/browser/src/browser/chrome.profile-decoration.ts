...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".crabfork-profile-decorated");
}
 ...
 */
export function decorateCrabforkProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_CRABFORK_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_CRABFORK_BROWSER_COLOR).toUpperCase();
