...
function localCrabforkProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "crabfork",
...
function createLocalCrabforkResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localCrabforkProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveCrabforkUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "crabfork-reset-"));
    const profileDir = path.join(tempRoot, "crabfork");
    ...
    const ops = createLocalCrabforkResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveCrabforkUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "crabfork-reset-no-own-"));
    const profileDir = path.join(tempRoot, "crabfork");
    fs.mkdirSync(profileDir, { recursive: true });
