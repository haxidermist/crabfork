...
export function resolveCrabforkUserDataDir(profileName = DEFAULT_CRABFORK_BROWSER_PROFILE_NAME) {
...
export function buildCrabforkChromeLaunchArgs(params: {
...
export async function launchCrabforkChrome(
  resolved: ResolvedBrowserConfig,
  profile: ResolvedBrowserProfile,
  ...
  const userDataDir = resolveCrabforkUserDataDir(profile.name);
  fs.mkdirSync(userDataDir, { recursive: true });

  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_CRABFORK_BROWSER_COLOR).toUpperCase(),
  );
  ...
  if (needsDecorate) {
    try {
      decorateCrabforkProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
      log.info(`🦞 crabfork browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`crabfork browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`crabfork browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  log.info(
    `🦞 crabfork browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
...
export async function stopCrabforkChrome(
