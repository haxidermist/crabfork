...
import { resolveCrabforkUserDataDir } from "./chrome.js";
...
vi.mock("./chrome.js", () => ({
  resolveCrabforkUserDataDir: vi.fn(() => "/tmp/crabfork-test/crabfork/user-data"),
}));
...
describe("BrowserProfilesService", () => {
  ...
  it("creates existing-session profiles with an explicit userDataDir", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "crabfork-profile-"));
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    vi.mocked(resolveCrabforkUserDataDir).mockReturnValue(userDataDir);
