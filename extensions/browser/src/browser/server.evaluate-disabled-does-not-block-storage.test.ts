  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          crabfork: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.CRABFORK_GATEWAY_PORT;
    process.env.CRABFORK_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.CRABFORK_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.CRABFORK_GATEWAY_PASSWORD;
