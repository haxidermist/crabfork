...
describe("browser server-context loopback direct WebSocket profiles", () => {
  it("uses an HTTP /json/list base when opening about:blank under strict SSRF", async () => {
    ...
    const state = makeState("crabfork");
    ...
    const crabfork = ctx.forProfile("crabfork");

    const opened = await crabfork.openTab("about:blank");
  ...
  it("uses an HTTP /json base for focus and close under strict SSRF", async () => {
    ...
    await crabfork.focusTab("T1");
    await crabfork.closeTab("T1");
  ...
  it("uses an HTTPS /json base for secure direct WebSocket profiles with a /cdp suffix", async () => {
    ...
    const tabs = await crabfork.listTabs();
    expect(tabs.map((tab) => tab.targetId)).toEqual(["T2"]);

    await crabfork.focusTab("T2");
    await crabfork.closeTab("T2");
  });
  ...
  it("blocks direct WebSocket tab operations when strict SSRF policy rejects the cdpUrl", async () => {
    ...
    await expect(crabfork.listTabs()).rejects.toBeInstanceOf(BrowserCdpEndpointBlockedError);
    await expect(crabfork.focusTab("T1")).rejects.toBeInstanceOf(BrowserCdpEndpointBlockedError);
    await expect(crabfork.closeTab("T1")).rejects.toBeInstanceOf(BrowserCdpEndpointBlockedError);
