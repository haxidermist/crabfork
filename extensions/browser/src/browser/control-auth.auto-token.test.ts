...
const mocks = vi.hoisted(() => ({
  loadConfig: vi.fn<() => CrabforkConfig>(),
  writeConfigFile: vi.fn<(cfg: CrabforkConfig) => Promise<void>>(async (_cfg) => {}),
  resolveGatewayAuth: vi.fn(
    ({
      authConfig,
    }: {
      authConfig?: NonNullable<NonNullable<CrabforkConfig["gateway"]>["auth"]>;
    }) => {
      const token =
  ...
  ensureGatewayStartupAuth: vi.fn(async ({ cfg }: { cfg: CrabforkConfig }) => ({
    cfg: {
      ...cfg,
      gateway: {
        ...cfg.gateway,
...
function readPersistedConfig(): CrabforkConfig {
...
async function expectUnresolvedBrowserSecretRefSkipsPersistence(cfg: CrabforkConfig) {
