import type { CrabforkPluginSecurityAuditContext } from "crabfork/plugin-sdk/plugin-entry";
import { hasConfiguredSecretInput } from "crabfork/plugin-sdk/secret-input";
import { formatCliCommand } from "crabfork/plugin-sdk/setup-tools";
import { isPrivateNetworkOptInEnabled, isPrivateIpAddress } from "crabfork/plugin-sdk/ssrf-policy";
...
export function collectBrowserSecurityAuditFindings(ctx: CrabforkPluginSecurityAuditContext) {
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.cdpUrl in ${ctx.configPath} and re-run "${formatCliCommand("crabfork security audit --deep")}".`,
    });
    return findings;
  }
  ...
  const tokenConfigured =
    ...
    hasNonEmptyString(ctx.env.CRABFORK_GATEWAY_TOKEN) ||
  ...
  const passwordConfigured =
    ...
    (passwordCanWin &&
      (hasNonEmptyString(ctx.env.CRABFORK_GATEWAY_PASSWORD) ||
