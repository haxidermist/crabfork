...
import { resolveDefaultSecretProviderAlias } from "crabfork/plugin-sdk/provider-auth";
import { resolveSecretInputString, normalizeSecretInput } from "crabfork/plugin-sdk/secret-input";
...
type WebFetchConfig = NonNullable<CrabforkConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: CrabforkConfig): WebSearchConfig {
...
function resolveFetchConfig(cfg?: CrabforkConfig): WebFetchConfig {
...
export function resolveFirecrawlSearchConfig(cfg?: CrabforkConfig): FirecrawlSearchConfig {
...
export function resolveFirecrawlFetchConfig(cfg?: CrabforkConfig): FirecrawlFetchConfig {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlApiKey(cfg?: CrabforkConfig): string | undefined {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlBaseUrl(cfg?: CrabforkConfig): string {
  const search = resolveFirecrawlSearchConfig(cfg);
  const fetch = resolveFirecrawlFetchConfig(cfg);
  const configured =
...
export function resolveFirecrawlOnlyMainContent(cfg?: CrabforkConfig, override?: boolean): boolean {
...
export function resolveFirecrawlMaxAgeMs(cfg?: CrabforkConfig, override?: number): number {
