...
import { normalizeChannelId, type ChannelId } from "crabfork/plugin-sdk/channel-targets";
import type {
  CrabforkConfig,
  TtsAutoMode,
  TtsConfig,
...
import { redactSensitiveText } from "crabfork/plugin-sdk/logging-core";
import {
  resolveSendableOutboundReplyParts,
  type ReplyPayload,
} from "crabfork/plugin-sdk/reply-payload";
import { isVerbose, logVerbose } from "crabfork/plugin-sdk/runtime-env";
import { resolvePreferredCrabforkTmpDir } from "crabfork/plugin-sdk/sandbox";
import {
...
function resolveTtsPrefsPathValue(prefsPath: string | undefined): string {
  ...
  const envPath = process.env.CRABFORK_TTS_PREFS?.trim();
  if (envPath) {
...
function sortSpeechProvidersForAutoSelection(cfg?: CrabforkConfig) {
...
function _resolveRegistryDefaultSpeechProviderId(cfg?: CrabforkConfig): TtsProvider {
...
export function resolveTtsConfig(cfg: CrabforkConfig): ResolvedTtsConfig {
...
function resolveEffectiveTtsAutoState(params: { cfg: CrabforkConfig; sessionAuto?: string }): {
...
export function buildTtsSystemPromptHint(cfg: CrabforkConfig): string | undefined {
...
export function resolveTtsProviderOrder(primary: TtsProvider, cfg?: CrabforkConfig): TtsProvider[] {
...
export async function textToSpeech(params: {
  ...
  const tempRoot = resolvePreferredCrabforkTmpDir();
