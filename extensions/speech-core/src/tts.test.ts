...
import type { ReplyPayload } from "crabfork/plugin-sdk/reply-payload";
import type { SpeechProviderPlugin, SpeechSynthesisRequest } from "crabfork/plugin-sdk/speech-core";
...
vi.mock("crabfork/plugin-sdk/channel-targets", () => ({
...
function createTtsConfig(prefsName: string): CrabforkConfig {
  return {
    messages: {
      tts: {
        enabled: true,
...
describe("speech-core native voice-note routing", () => {
  ...
  it("marks Discord auto TTS replies as native voice messages", async () => {
    const cfg = createTtsConfig("crabfork-speech-core-tts-test");
  ...
  it("keeps non-native voice-note channels as regular audio files", async () => {
    const cfg = createTtsConfig("crabfork-speech-core-tts-slack-test");
    const payload: ReplyPayload = {
