 ...
 * 3. the Crabfork temp directory as a last resort
 */
...
/**
 * Return a path under `~/.crabfork/qqbot`, creating it on demand.
 */
export function getQQBotDataDir(...subPaths: string[]): string {
  const dir = path.join(getHomeDir(), ".crabfork", "qqbot", ...subPaths);
...
/**
 * Return a path under `~/.crabfork/media/qqbot`, creating it on demand.
 *
 * Unlike `getQQBotDataDir`, this lives under Crabfork's core media allowlist so
...
export function getQQBotMediaDir(...subPaths: string[]): string {
  const dir = path.join(getHomeDir(), ".crabfork", "media", "qqbot", ...subPaths);
  if (!fs.existsSync(dir)) {
    fs.mkdirSync(dir, { recursive: true });
  }
  return dir;
...
/** Return the preferred Crabfork temp directory. */
...
export function resolveQQBotLocalMediaPath(p: string): string {
  ...
  const workspaceRoot = path.join(homeDir, ".crabfork", "workspace", "qqbot");
