...
export function createToolFactoryHarness(cfg: CrabforkPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<CrabforkPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
