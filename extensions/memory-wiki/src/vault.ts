...
function buildIndexMarkdown(): string {
  return withTrailingNewline(
    replaceManagedMarkdownBlock({
      original: "# Wiki Index\n",
      heading: "## Generated",
      startMarker: "<!-- crabfork:wiki:index:start -->",
      endMarker: "<!-- crabfork:wiki:index:end -->",
...
- Use \`.crabfork-wiki/cache/agent-digest.json\` and \`claims.jsonl\` for machine reads; markdown pages are the human view.
`);
}
...
This vault is maintained by the Crabfork memory-wiki plugin.
...
- \`.crabfork-wiki/cache/agent-digest.json\` is the agent-facing compiled digest.

## Notes
<!-- crabfork:human:start -->
<!-- crabfork:human:end -->
`);
}
...
export async function initializeMemoryWikiVault(
  ...
  await writeFileIfMissing(
    path.join(rootDir, ".crabfork-wiki", "state.json"),
    withTrailingNewline(
      JSON.stringify(
        {
          version: 1,
  ...
  await writeFileIfMissing(path.join(rootDir, ".crabfork-wiki", "log.jsonl"), "", createdFiles);
