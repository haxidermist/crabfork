...
export const legacyConfigRules: LegacyConfigRule[] = [
  {
    ...
    message:
      'plugins.entries.memory-wiki.config.bridge.readMemoryCore is legacy; use plugins.entries.memory-wiki.config.bridge.readMemoryArtifacts. Run "crabfork doctor --fix".',
    match: hasLegacyBridgeArtifactToggle,
  },
];

export function migrateMemoryWikiLegacyConfig(config: CrabforkConfig): {
...
export function normalizeCompatibilityConfig({ cfg }: { cfg: CrabforkConfig }): {
  config: CrabforkConfig;
  changes: string[];
} {
