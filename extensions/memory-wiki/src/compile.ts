...
const AGENT_DIGEST_PATH = ".crabfork-wiki/cache/agent-digest.json";
const CLAIMS_DIGEST_PATH = ".crabfork-wiki/cache/claims.jsonl";
...
async function writeDashboardPage(params: {
  ...
  const updatedBody = replaceManagedMarkdownBlock({
    ...
    heading: "## Generated",
    startMarker: `<!-- crabfork:wiki:${path.basename(params.definition.relativePath, ".md")}:start -->`,
    endMarker: `<!-- crabfork:wiki:${path.basename(params.definition.relativePath, ".md")}:end -->`,
  ...
  for (const group of COMPILE_PAGE_GROUPS) {
    ...
    if (
      await writeManagedMarkdownFile({
        filePath,
        title: group.heading,
        startMarker: `<!-- crabfork:wiki:${group.dir}:index:start -->`,
        endMarker: `<!-- crabfork:wiki:${group.dir}:index:end -->`,
