...
description: Maintain the Crabfork memory wiki vault with deterministic pages, managed blocks, and source-backed updates.
---
...
- Use `crabfork wiki ingest`, `crabfork wiki compile`, and `crabfork wiki lint` as the default maintenance loop.
- In `bridge` mode, run `crabfork wiki bridge import` before relying on search results if you need the latest public memory artifacts pulled in.
- In `unsafe-local` mode, use `crabfork wiki unsafe-local import` only when the user explicitly opted into private local path access.
