...
- Start from `crabfork wiki status` to confirm the vault mode and whether the official Obsidian CLI is available.
- Use `crabfork wiki obsidian status` before shelling out, then prefer the dedicated helpers like `crabfork wiki obsidian search`, `crabfork wiki obsidian open`, `crabfork wiki obsidian command`, and `crabfork wiki obsidian daily`.
