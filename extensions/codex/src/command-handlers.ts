import type { PluginCommandContext } from "crabfork/plugin-sdk/plugin-entry";
...
async function resumeThread(
  ...
  if (!ctx.sessionFile) {
    return "Cannot attach a Codex thread because this command did not include an Crabfork session file.";
  }
  ...
  return `Attached this Crabfork session to Codex thread ${effectiveThreadId}.`;
}
...
async function startThreadAction(
  ...
  if (!ctx.sessionFile) {
    return `Cannot start Codex ${label} because this command did not include an Crabfork session file.`;
  }
  const binding = await deps.readCodexAppServerBinding(ctx.sessionFile);
  if (!binding?.threadId) {
    return `No Codex thread is attached to this Crabfork session yet.`;
  }
