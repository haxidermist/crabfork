...
const CRABFORK_OPENSHELL_E2E = process.env.CRABFORK_E2E_OPENSHELL === "1";
const CRABFORK_OPENSHELL_E2E_TIMEOUT_MS = 12 * 60_000;
const CRABFORK_OPENSHELL_COMMAND =
  process.env.CRABFORK_E2E_OPENSHELL_COMMAND?.trim() || "openshell";
...
RUN echo "crabfork-openshell-e2e" > /opt/openshell-e2e-marker.txt
...
describe("openshell sandbox backend e2e", () => {
  it.runIf(process.platform !== "win32" && CRABFORK_OPENSHELL_E2E)(
    "creates a remote-canonical sandbox through OpenShell and executes over SSH",
    { timeout: CRABFORK_OPENSHELL_E2E_TIMEOUT_MS },
    async () => {
      if (!(await dockerReady())) {
        return;
      }
      if (!(await commandAvailable(CRABFORK_OPENSHELL_COMMAND))) {
        return;
      }
      if (!(await openshellGatewayAvailable(CRABFORK_OPENSHELL_COMMAND))) {
        return;
      }

      const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "crabfork-openshell-e2e-"));
      ...
      const gatewayName = `crabfork-e2e-${scopeSuffix}`;
      const scopeKey = `session:openshell-e2e-deny:${scopeSuffix}`;
      const allowSandboxName = `crabfork-policy-allow-${scopeSuffix}`;
      ...
      const sandboxCfg = {
        ...
        ssh: createSandboxSshConfig("/tmp/crabfork-sandboxes"),
      ...
      const pluginConfig = resolveOpenShellPluginConfig({
        command: CRABFORK_OPENSHELL_COMMAND,
        gateway: gatewayName,
        from: dockerfilePath,
        mode: "remote",
        autoProviders: false,
      ...
      try {
        ...
        expect(stdout).toContain("crabfork-openshell-e2e");
