...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): CrabforkConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as CrabforkConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): CrabforkConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as CrabforkConfig;
    await expect(
      handleTelegramAction(
        {
          action: "sendSticker",
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: CrabforkConfig, accountId = "media") {
    await handleTelegramAction(
