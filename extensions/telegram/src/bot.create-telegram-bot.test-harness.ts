...
type LoadConfigFn = typeof import("crabfork/plugin-sdk/config-runtime").loadConfig;
type LoadSessionStoreFn = typeof import("crabfork/plugin-sdk/config-runtime").loadSessionStore;
type ResolveStorePathFn = typeof import("crabfork/plugin-sdk/config-runtime").resolveStorePath;
...
type DispatchReplyWithBufferedBlockDispatcherFn =
  typeof import("crabfork/plugin-sdk/reply-dispatch-runtime").dispatchReplyWithBufferedBlockDispatcher;
...
const { sessionStorePath } = vi.hoisted(() => ({
  sessionStorePath: `/tmp/crabfork-telegram-${process.pid}-${process.env.VITEST_POOL_ID ?? "0"}.json`,
}));
...
function resolveDefaultModelForAgentForTest(params: { cfg: CrabforkConfig }): {
  provider: string;
  model: string;
} {
...
function createModelsProviderDataFromConfig(cfg: CrabforkConfig): {
...
const DEFAULT_TELEGRAM_TEST_CONFIG: CrabforkConfig = {
  agents: {
    defaults: {
...
beforeEach(() => {
  ...
  buildModelsProviderData.mockImplementation(async (cfg: CrabforkConfig) => {
