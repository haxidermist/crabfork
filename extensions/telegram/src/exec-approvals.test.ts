...
function createTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "crabfork-telegram-exec-approvals-"));
  tempDirs.push(dir);
  return dir;
}

function buildConfig(
  execApprovals?: NonNullable<NonNullable<CrabforkConfig["channels"]>["telegram"]>["execApprovals"],
  channelOverrides?: Partial<NonNullable<NonNullable<CrabforkConfig["channels"]>["telegram"]>>,
): CrabforkConfig {
  return {
    channels: {
      telegram: {
