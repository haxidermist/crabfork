...
function resolveQianfanPreset(cfg: CrabforkConfig): {
  api: ModelApi;
  baseUrl: string;
...
const qianfanPresetAppliers = createDefaultModelsPresetAppliers({
  ...
  resolveParams: (cfg: CrabforkConfig) => {
...
export function applyQianfanProviderConfig(cfg: CrabforkConfig): CrabforkConfig {
  return qianfanPresetAppliers.applyProviderConfig(cfg);
}

export function applyQianfanConfig(cfg: CrabforkConfig): CrabforkConfig {
