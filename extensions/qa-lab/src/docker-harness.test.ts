...
describe("qa docker harness", () => {
  it("writes compose, env, config, and workspace scaffold files", async () => {
    ...
    expect(result.files).toEqual(
      expect.arrayContaining([
        ...
        path.join(outputDir, "state", "crabfork.json"),
    ...
    expect(compose).toContain('      - "http://crabfork-qa-gateway:18789/"');
    expect(compose).toContain("      - --send-kickoff-on-start");
    expect(compose).toContain("      - --ui-dist-dir");
    expect(compose).toContain('      - "/opt/crabfork-qa-lab-ui"');
    expect(compose).toContain(":/opt/crabfork-repo:ro");
    expect(compose).toContain("./state:/opt/crabfork-scaffold:ro");
    expect(compose).toContain(
      "cp -R /opt/crabfork-scaffold/seed-workspace/. /tmp/crabfork/workspace/",
    );
    expect(compose).toContain("CRABFORK_CONFIG_PATH: /tmp/crabfork/crabfork.json");
    expect(compose).toContain("CRABFORK_STATE_DIR: /tmp/crabfork/state");
    expect(compose).toContain('CRABFORK_NO_RESPAWN: "1"');

    const envExample = await readFile(path.join(outputDir, ".env.example"), "utf8");
    expect(envExample).toContain("CRABFORK_GATEWAY_TOKEN=qa-token");
    ...
    const config = await readFile(path.join(outputDir, "state", "crabfork.json"), "utf8");
    ...
    expect(config).toContain('"/tmp/crabfork/workspace"');
  ...
  it("builds the reusable QA image with bundled QA extensions", async () => {
    ...
    const result = await buildQaDockerHarnessImage(
      {
        repoRoot: "/repo/crabfork",
        imageName: "crabfork:qa-local-prebaked",
      },
      {
    ...
    expect(result.imageName).toBe("crabfork:qa-local-prebaked");
    expect(calls).toEqual([
      expect.stringContaining(
        "docker build -t crabfork:qa-local-prebaked --build-arg CRABFORK_EXTENSIONS=qa-channel qa-lab -f Dockerfile . @/repo/crabfork",
      ),
    ]);
  });
});
