...
vi.mock("crabfork/plugin-sdk/ssrf-runtime", async () => {
  const actual = await vi.importActual<typeof import("crabfork/plugin-sdk/ssrf-runtime")>(
    "crabfork/plugin-sdk/ssrf-runtime",
  );
  return {
    ...actual,
...
describe("telegram live qa runtime", () => {
  ...
  it("resolves required Telegram QA env vars", () => {
    expect(
      __testing.resolveTelegramQaRuntimeEnv({
        CRABFORK_QA_TELEGRAM_GROUP_ID: "-100123",
        CRABFORK_QA_TELEGRAM_DRIVER_BOT_TOKEN: "driver",
        CRABFORK_QA_TELEGRAM_SUT_BOT_TOKEN: "sut",
      }),
    ).toEqual({
      groupId: "-100123",
      driverToken: "driver",
  ...
  it("fails when a required Telegram QA env var is missing", () => {
    ...
    ).toThrow("CRABFORK_QA_TELEGRAM_SUT_BOT_TOKEN");
  });

  it("fails when the Telegram group id is not numeric", () => {
    expect(() =>
      __testing.resolveTelegramQaRuntimeEnv({
        CRABFORK_QA_TELEGRAM_GROUP_ID: "qa-group",
        CRABFORK_QA_TELEGRAM_DRIVER_BOT_TOKEN: "driver",
        CRABFORK_QA_TELEGRAM_SUT_BOT_TOKEN: "sut",
      }),
    ).toThrow("CRABFORK_QA_TELEGRAM_GROUP_ID must be a numeric Telegram chat id.");
  });
