...
describe("qa suite failure reply handling", () => {
  ...
  it("normalizes suite concurrency to a bounded integer", () => {
    const previous = process.env.CRABFORK_QA_SUITE_CONCURRENCY;
    delete process.env.CRABFORK_QA_SUITE_CONCURRENCY;
    try {
      ...
      if (previous === undefined) {
        ...
        process.env.CRABFORK_QA_SUITE_CONCURRENCY = previous;
      }
    }
  });
  ...
  it("reads transport transcripts with generic helper names", () => {
    ...
    expect(formatted).toContain("ASSISTANT Crabfork QA: working on it");
  });
