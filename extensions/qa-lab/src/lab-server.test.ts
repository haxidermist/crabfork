...
describe("qa-lab server", () => {
  ...
  it("reports startup reachability for proxy and gateway", async () => {
    ...
    process.env.CRABFORK_DEBUG_PROXY_URL = `http://127.0.0.1:${proxyAddress.port}`;
    const lab = await startQaLabServer({
      host: "127.0.0.1",
      port: 0,
    ...
    cleanups.push(async () => {
      delete process.env.CRABFORK_DEBUG_PROXY_URL;
      await lab.stop();
    });
  ...
  it("aborts an in-flight runner model catalog when the lab stops", async () => {
    ...
    await writeFile(
      ...
      [
        'const fs = require("node:fs");',
        `fs.writeFileSync(${JSON.stringify(markerPath)}, process.env.CRABFORK_CODEX_DISCOVERY_LIVE || "", "utf8");`,
        "process.on('SIGTERM', () => {",
  ...
  it("serves proxy capture sessions, events, and query rows", async () => {
    ...
    cleanups.push(async () => {
      delete process.env.CRABFORK_DEBUG_PROXY_DB_PATH;
      delete process.env.CRABFORK_DEBUG_PROXY_BLOB_DIR;
      await lab.stop();
    });
