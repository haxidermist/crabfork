...
// Prefix for Crabfork-generated action IDs to scope our handler
const CRABFORK_ACTION_PREFIX = "crabfork:";
...
export function registerSlackInteractionEvents(params: { ctx: SlackMonitorContext }) {
  ...
  const modalMatcher = new RegExp(`^${CRABFORK_ACTION_PREFIX}`);

  // Handle Crabfork modal submissions with callback_ids scoped by our prefix.
  registerModalLifecycleHandler({
