import type { SlackSlashCommandConfig } from "crabfork/plugin-sdk/config-runtime";
...
export function resolveSlackSlashCommandConfig(
  ...
  const normalizedName = normalizeSlackSlashCommandName(
    normalizeOptionalString(raw?.name) ?? "crabfork",
  );
  const name = normalizedName || "crabfork";
  return {
