...
describe("handleSlackAction", () => {
  function slackConfig(overrides?: Record<string, unknown>): CrabforkConfig {
    return {
      channels: {
        slack: {
          botToken: "tok",
  ...
  function createReplyToFirstScenario() {
    const cfg = { channels: { slack: { botToken: "tok" } } } as CrabforkConfig;
  ...
  async function resolveReadToken(cfg: CrabforkConfig): Promise<string | undefined> {
  ...
  async function resolveSendToken(cfg: CrabforkConfig): Promise<string | undefined> {
    sendSlackMessage.mockClear();
