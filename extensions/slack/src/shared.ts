...
import { patchChannelConfigForAccount } from "crabfork/plugin-sdk/setup-runtime";
...
import { getChatChannelMeta, type ChannelPlugin, type CrabforkConfig } from "./channel-api.js";
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "Crabfork";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for Crabfork`,
    },
    features: {
      bot_user: {
...
export function buildSlackSetupLines(botName = "Crabfork"): string[] {
  return [
