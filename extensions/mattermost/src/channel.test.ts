...
vi.mock("crabfork/plugin-sdk/ssrf-runtime", async () => {
  const original = (await vi.importActual("crabfork/plugin-sdk/ssrf-runtime")) as Record<
    string,
    unknown
  >;
...
function getDescribedActions(cfg: CrabforkConfig, accountId?: string): string[] {
