...
surface: mcp
objective: Verify Crabfork can expose plugin tools over MCP and a real MCP client can call one successfully.
successCriteria:
...
execution:
  kind: flow
  summary: Verify Crabfork can expose plugin tools over MCP and a real MCP client can call one successfully.
  config:
