...
- `CRABFORK_QA_CONVEX_SECRET_MAINTAINER`
- `CRABFORK_QA_CONVEX_SECRET_CI`

Client URL policy:

- `CRABFORK_QA_CONVEX_SITE_URL` must use `https://` in normal use.
- Local development may use loopback `http://` only when `CRABFORK_QA_ALLOW_INSECURE_HTTP=1`.
...
```bash
pnpm crabfork qa credentials add \
  --kind telegram \
...
Admin endpoints require `CRABFORK_QA_CONVEX_SECRET_MAINTAINER`.
